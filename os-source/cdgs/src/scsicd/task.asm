...
CDTask:
                ...
                lea     db_Task(db),a1                                  ; Allocate our signals
                or.l    #SIGNALS,TC_SIGALLOC(a1)

                exec    CreateMsgPort                                   ; Create msg port Frm
                move.l  d0,db_TimerPort(db)
                move.l  d0,a0
                move.b  #SIGB_TESTUNITREADY,MP_SIGBIT(a0)

                move.l  db_TimerPort(db),a0                             ; Create I/O request Frm
                move.l  #IOTV_SIZE,d0
                exec    CreateIORequest
                move.l  d0,db_TimerIOR(db)

                lea     TimerDevName(pc),a0                             ; Open timer device with this request
                move.l  #UNIT_ECLOCK,d0
                move.l  db_TimerIOR(db),a1
                clr.l   d1
                exec    OpenDevice
                jsr     SendTimerReq
...
TaskLoop
                ...
                and.l   #TASK_SIGS,d2                                   ; Any set signals?
                bne     TestSignals
...
*************** Test Unit Ready ****************************************
                ...
                btst    #SIGB_TESTUNITREADY,d2                          ; Time to test for a disk?
                beq     CheckAbortSig
                save    d2

                clr.l   d0                                              ; Clear this signal
                move.l  #SIGF_TESTUNITREADY,d1
                exec    SetSignal

*============== Simulate SIGF_PLAYDONE =================================

                move.b  db_OpenState(db),d2                             ; If there is a disk change, signal ourselves
                clr.b   db_OpenState(db)
                jsr     TestUnitReady
                beq     2$
                save    d0                                              ; - If disk is present, set volume attenuator
                move.b  #1,db_OpenState(db)
                move.w  db_Attenuation(db),d0
                cmp.w   db_LastAttenuation(db),d0
                beq     1$
                bsr     Attenuate
1$              restore d0
2$
                cmp.b   #AUDIOSTAT_PLAYING,d0                           ; Is the drive playing audio?
                beq     SimDiskChange
...
*============== Simulate SIGF_DISKCHANGE ===============================

SimDiskChange
                cmp.b   db_OpenState(db),d2                             ; Change in door status?
                beq     SimSearch

                tst.w   db_Info+CDINFO_EjectReset(db)                   ; Reset on disk eject?
                beq     1$
                exec    ColdReboot
1$
                move.l  db_ClassDReq(db),d0                             ; Abort disk command if one is in progress
                beq     2$
                move.l  d0,a1
                move.b  #CDERR_NoDisk,IO_ERROR(a1)
                SIGNAL  SIGF_ABORTDRIVE
2$
                SIGNAL  SIGF_DISKCHANGE                                 ; Disk changed

*============== Simulate search modes ==================================

SimSearch
                move.w  db_Info+CDINFO_Status(db),d0                    ; Are we playing in a search mode?
                btst    #CDSTSB_PLAYING,d0
                beq     NextTimerSig
                btst    #CDSTSB_PAUSED,d0
                bne     NextTimerSig
                ...
                save    d2                                              ; Fast forward?
                move.w  db_Info+CDINFO_Status(db),d0
                btst    #CDSTSB_DIRECTION,d0
                bne     1$

                add.l   #10*75,d1                                       ; Check bounds
                move.l  db_PlayStop(db),d2
                cmp.l   d2,d1
                bhs     8$
                bra     2$
1$
                sub.l   #10*75+45,d1                                    ; Check bounds
                move.l  db_PlayStop(db),d2
                cmp.l   db_PlayStart(db),d1
                blo     8$
2$
                ...
                bra     9$
8$
                SIGNAL  SIGF_PLAYDONE                                   ; End of request

                lea     db_command(db),a0                               ; Stop playing audio
...
*============== Clear signal and send another timer request ============

NextTimerSig
                jsr     SendTimerReq                                    ; Clear signal and send another request
                restore d2
...
2$
                ...
                jsr     DoSCSI
3$
                move.w  #1,db_AbortTimer(db)                            ; Stop frame interrupts
...
ScanMsgPorts
                clr.l   d0                                              ; Clear this signal
                move.l  #SIGF_CMDPORT|SIGF_DISKCMDPORT,d1
                exec    SetSignal
...
************************************************************************
 ...
 FUNCTION SendTimerReq
                ...
                move.l  db_TimerPort(db),a0                             ; Make sure request is not in use
                exec    GetMsg
                tst.l   d0
                bne     SendTimerReq

                move.l  db_TimerIOR(db),a1                              ; Send a timer request
                clr.l   IOTV_TIME+TV_SECS(a1)
                move.l  #2000000,d1
                move.w  db_Info+CDINFO_Status(db),d0                    ; - Sample more frequently when playing
                btst    #CDSTSB_PLAYING,d0
                beq     1$
                btst    #CDSTSB_PAUSED,d0
                bne     1$
                move.l  #250000,d1
1$              move.l  d1,IOTV_TIME+TV_MICRO(a1)
...
CmdConfig:
                ...
                rts
1$
                move.w  db_Info+CDINFO_SectorSize(db),-(sp)                     ; TagList ok, assign values
                move.w  #1,d0
                jsr     SetTagList

                move.w  (sp)+,d0                                                ; If the sector size has been changed
                cmp.w   db_Info+CDINFO_SectorSize(db),d0                        ;   do a mode select
                beq     2$
                jsr     SetSectorSize
2$
                rts
...
ConfigTable:
                dc.l    TAGCD_PLAYSPEED,75,75,0                                 ; Valid tags and values
                dc.l    TAGCD_READSPEED,75,75,0
                dc.l    TAGCD_READXLSPEED,75,75,0
...
CmdQCode:
                ...
                move.w  db_Info+CDINFO_Status(db),d0                            ; If we are searching while playing, no packet
                btst    #CDSTSB_PLAYING,d0
                beq     1$
                btst    #CDSTSB_SEARCH,d0
                bne     2$
...
2$
                ...
                move.b  db_QCode+5(db),(a1)+                                    ; Store CTLADR byte

                move.b  db_QCode+6(db),d0                                       ; Store track
                move.b  d0,(a1)+

                move.b  db_QCode+7(db),d0                                       ; Store subindex
                move.b  d0,(a1)+

                move.b  db_QCode+12(db),(a1)+                                   ; Store zero byte

                move.l  db_QCode+12(db),d0                                      ; Get relative time - convert BCD to BIN format
                and.l   #$00ffffff,d0
                move.l  d0,(a1)+

                move.l  db_QCode+8(db),d0                                       ; Get absolute time - convert BCD to BIN format
                and.l   #$00ffffff,d0
                move.l  d0,(a1)+
