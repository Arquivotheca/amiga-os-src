...
1$
                ...
                move.l  #$00000000,d1                                   ; Set search mode and speed
                btst    #CDSTSB_SEARCH,d0
                beq     3$
                btst    #CDSTSB_DIRECTION,d0
                bne     2$
...
4$              move.l  d1,db_Packet+7(db)
                ...
                clr.b   db_Sampling(db)                                 ; Playing not sampling
                clr.b   db_QAvail(db)                                   ; Q-Code packet is not here yet
...
6$
                ...
                jsr     DoPacket
7$
                jsr     Attenuate                                       ; Set desired attenuation
...
SAMPLE:
                ...
                move.l  db_ARG1(db),db_SARG1(db)                        ; Save audio parameters
                move.l  db_ARG2(db),db_SARG2(db)
                move.l  db_ARG3(db),db_SARG3(db)
                move.l  db_ARG4(db),db_SARG4(db)

                clr.l   db_SectorIndex(db)                              ; Clear sector index
                clr.l   db_SampleDump(db)
                ...
                move.l  db_ARG3(db),db_AudioTarget(db)                  ; Save audio address
...
4$              move.l  d1,db_Packet+7(db)
                ...
                move.b  #1,db_Sampling(db)                              ; Sampling not playing
                clr.b   db_QAvail(db)                                   ; Q-Code packet is not here yet
...
7$
                ...
                jsr     ClearPrefetch                                   ; Clear out prefetch
                or.l    #CF_CDROM|CF_DATA,CDCONFIG(hb)                  ; Turn on AUDIO DATA DECODE hardware
                or.l    #INTF_PBX,CDINT2ENABLE(hb)                      ; Enable audio interrupts
...
PAUSE:
                ...
                and.l   #-1-(INTF_PBX|INTF_OVERFLOW),CDINT2ENABLE(hb)   ; Turn off decoder (in case it is on)
...
RESUME:
                ...
                tst.b   db_Sampling(db)                                 ; If we are sampling, turn on decoder
                beq     1$
                move.b  #0,db_BufferCount(db)                           ; Next expected buffer count
                jsr     ClearPrefetch                                   ; Clear out prefetch
                or.l    #CF_CDROM|CF_DATA,CDCONFIG(hb)                  ; Turn on AUDIO DATA DECODE hardware
...
1$              move.l  d0,db_Packet+3(db)
                ...
                move.l  #$00000000,d1                                                   ; Set search mode and direction
...
4$              move.l  d1,db_Packet+7(db)
                ...
                and.l   #-1-(INTF_PBX|INTF_OVERFLOW),CDINT2ENABLE(hb)                   ; If we are sampling, turn off decoder
                and.l   #-1-CF_CDROM,CDCONFIG(hb)
...
6$
                ...
                or.l    #INTF_PBX,CDINT2ENABLE(hb)                                      ; Enable audio interrupts
7$
                jsr     Attenuate                                                       ; Reset desired attenuation
8$
...
Q_CODE:
                ...
                btst    #CDSTSB_PAUSED,d0
                bne     Q_GotIt                                         ; - Then report the same packet again

                DISABLE                                                 ; Copy Q-Code packet
                move.b  #16,d0
                lea     db_QCodeI(db),a0
                lea     db_QCode(db),a1
0$              move.b  (a0)+,(a1)+
...
Q_GotIt
                ...
                tst.b   db_QAvail(db)                                   ; Have not received first Q-Code packet yet
                beq     4$
