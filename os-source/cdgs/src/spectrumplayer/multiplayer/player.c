...
UWORD Laser;
    ...
    if (V->PlayerDisk && (V->SABlank || ((V->PlayerState.PlayState >= PLS_PLAYING)              // Can head move now?
                                     &&  (V->PlayerState.PlayMode == PLM_NORMAL)))) {
...
UBYTE *Analysis, Analyzed = 0;
    ...
    if ((V->HeadX == V->LastHeadX) && (V->PlayerState.PlayState >= PLS_PLAYING)         // When do we analyze?
        ...
        if (V->SAAvailable) {                                                           // Does player.library have SA?
    ...
    for (i=10,x=41,V->SABlank=TRUE; i>=0; --i,x+=12) {                                  // Check all frequencies
        ...
        if (V->SALevels[i]) V->SABlank = FALSE;                                         // Is spectrum analyzer blank?
        }
    }
...
WORD i;
    ...
    while (!V->SABlank) { UpdateSA(V); WaitTOF(); }                                     // Wait for SA to go blank
