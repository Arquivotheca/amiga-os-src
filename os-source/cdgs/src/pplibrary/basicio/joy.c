...
register UWORD                  lastcode = ((UWORD) ~0);
...
struct Message                 *msg = NULL;
    ...
    if (!(inport = (struct MsgPort *)CreateMsgPort())) goto xit;        
    inport->mp_Node.ln_Name = JoyMousePortName;
    inport->mp_Node.ln_Pri = 0;
    AddPort( inport );
...
struct MsgPort *port,*rp;
    ...
    if (port = FindPort(JoyMousePortName)) {
        ...
        if (rp = (struct MsgPort *)CreateMsgPort()) {
