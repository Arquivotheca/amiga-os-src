/*
 *
 * cdgtest.c - V38 cdg.library example
 *
...
int track;
	...
	/* Make sure we are using V38 of the library, or a later version */
 ...
 ** the CDGPrefs structure with it.  The image data can be anywhere
 ** in RAM, or ROM, but must not be altered/deallocated after you start
 ** CD+G/+MIDI by calling CDGBegin().  The sprite data should be
 ** be deallocated after you call CDGEnd().
 **/
...
register int loop;
	...
	/* Sprites are 2 words per sprite line, plus 2 leading NULL words,
	 * and 2 ending NULL words.
	 */


	if(spritedata = (UWORD *)
		AllocMem( (ULONG)((SPRITEHEIGHT+2)*4),MEMF_PUBLIC|MEMF_CLEAR))
	{
		...
		cdp->cdgp_SpriteHeight = SPRITEHEIGHT;

		status = StartDevices(cdp, track);

		FreeMem(spritedata,(ULONG)((SPRITEHEIGHT+2)*4));
	}
	return(status);
}
...
BOOL loop;
	...
	/* Make sure we are running under 2.0 before using PeekQualifier() */
	...
	if(signal = AllocSignal(-1L))
	{
		...
		if(CDGBegin(cdp))
		{
			 ...
			 * for this example.
			 *
			 * Also aborts if LEFT AMIGA key is pressed.
			 *
			 ***************************************************/
			...
			while(loop)
			{
				...
				if(buttons & (IEQUALIFIER_RBUTTON|IEQUALIFIER_LCOMMAND)) loop = FALSE;

				/* process pending PACKETS -- +G PACKS only */
			...
			/* Force Intuition to make its own screens visible again */
