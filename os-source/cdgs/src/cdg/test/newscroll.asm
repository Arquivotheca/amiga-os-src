...
_TVScrollCopy:
	...
	; if combo scroll (sigh), must do separate scrolls to get
	; this right (argh!!)  The spec is really unclear on this
	; and there are no examples - only find this out through use
	; of the test disk
	...
	; tis a combo scroll, so hold off on doing COPH for now
...
TVSCC_DontCopyCol:
	...
	; check if combo scroll required

		move.w	d7,d5
		beq.s	TVSC_SwappedPlanes

		lsl.w	#5,d5			;*32 for lookup
		moveq	#00,d7			;clear for next loop

		bra	TVSC_SecondScroll
