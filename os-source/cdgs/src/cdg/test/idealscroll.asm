...
TVSCC_Blit4:
	...
	; Peform memory COPY of ROW while this blit is running
		...
		tst.w	d2			;copy left/right also?
		beq.s	TVSCC_DoCopyRow

	; special case - slow copy with shift required

		move.w	#(((FONT_HEIGHT*DISPLAY_BYTES)/2)-1),d1
		moveq	#6,d7
		
		cmp.b	#1,d2			;copy right?
		beq.s	TVSCC_SlowRowRight

	; slow row left - shift left 6 bits while copy
...
TVSCC_SlowRowLeft:
		...
		move.w	d0,(a1)+
		addq.w	#2,a0
		dbf	d1,TVSCC_SlowRowLeft
		bra.s	TVSCC_DontCopyRow

	; slow row right - shift right 6 bits while copy

TVSCC_SlowRowRight:
		move.w	#((FONT_HEIGHT*DISPLAY_BYTES)+4),d0
...
TVSCC_SlowRowRLoop:
		...
		move.w	d0,-(a1)
		addq.w	#2,a0
		dbf	d1,TVSCC_SlowRowRLoop

		bra.s	TVSCC_DontCopyRow

	; fast row copy - average case
...
TVSCC_DontCopyRow:
	...
	; Perform memory COPY of COLUMN while bit is running
		...
		tst.w	d3			;copv also true?
		beq.s	TCSCC_DoCopyRight

		move.w	#((FONT_HEIGHT*(DISPLAY_ROWS-1))-1),d1

		move.w	#(FONT_HEIGHT*DISPLAY_BYTES),d0
		move.w	#-(FONT_HEIGHT*DISPLAY_BYTES),d7

	; special case then


		cmp.b	#1,d3			;scroll down?
		beq.s	TVSCC_AdjustColRight
