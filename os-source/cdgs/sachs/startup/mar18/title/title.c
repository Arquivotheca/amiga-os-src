...
#define STDCYCLE_SPEED		(1000000/60)
...
ULONG __interrupt __saveds __asm FreeDeadSpace(
								register __a0 struct MemHandlerData *md,
								register __a1 struct TData *td,
								register __a2 struct Interrupt *intr )
{
	ULONG ans = MEM_DID_NOTHING;

	if ((md->memh_RequestFlags & MEMF_CHIP) && td->MHand.Data)
	{
		FreeMem( td->MHand.Data, td->MHand.DataSize );
		td->MHand.Data = NULL;
		ans = MEM_ALL_DONE;
	}
...
SetupMemHand( struct TData *td )
{
	td->MHand.MemIntr.is_Node.ln_Type 	= NT_INTERRUPT;
	td->MHand.MemIntr.is_Node.ln_Name 	= "DEAD SPACE";
	td->MHand.MemIntr.is_Node.ln_Pri  	= 50;	// Very soon.
	td->MHand.MemIntr.is_Data			= td;
	td->MHand.MemIntr.is_Code			= (void (*)())FreeDeadSpace;
	
	td->MHand.DataSize = AvailMem( MEMF_CHIP | MEMF_LARGEST) - (1024 * 1024);
	if (td->MHand.DataSize < 100) td->MHand.DataSize = 100;
	if (!(td->MHand.Data = AllocMem( td->MHand.DataSize, MEMF_CHIP | MEMF_LARGEST)))
		TERR( TERR_MEMORY );

	AddMemHandler( &td->MHand );
	td->MHand.Task 		= FindTask( NULL );	// For An Emergency Shutdown.

	return 0;
}

void RemoveMemHand( struct TData *td )
{
	if (td->MHand.Task)
	{
		RemMemHandler( &td->MHand );
	}
	if (td->MHand.Data) FreeMem( td->MHand.Data, td->MHand.DataSize );
...
static struct TagItem videoTags[] =
{
...
//				0xc0,0xff,NULL );
	...
	VideoControl( td->Vp.ColorMap, videoTags );
	...
	if (!CutCNode( td,&td->CNode[ CNODE_LOWWAVE],td->Bmi,td->Table,197,255 ))
		TERR( TERR_MEMORY );
	if (!CutCNode( td,&td->CNode[ CNODE_CD ],td->PalBmi,NULL,160,196 ))
		TERR( TERR_MEMORY );
...
void FreeBackground( struct TData *td )
{
	...
	CopClear( td );
	if (td->BYTable)	FreeYTable( td->BYTable,td->Bmi->bmi_Height );
...
static getExtSprite( struct TData *td, struct ExtSprite *s,ULONG tags,... )
{
	return GetExtSpriteA( s, (struct TagItem *)&tags );
}
...
InitSprites( register struct TData *td )
{
	...
	for(i=0,si = SAnim.SData;i<8;i++,si++)
	{
		...
		if (i==0)
		{
			td->Sprite[i].es_SimpleSprite.num =
				getExtSprite( td,&td->Sprite[i], GSTAG_SPRITE_NUM,1,0L );
		...
		{
			td->Sprite[i].es_SimpleSprite.num =
				getExtSprite( td,&td->Sprite[i], GSTAG_SPRITE_NUM,i,0L );
			if (td->Sprite[i].es_SimpleSprite.num == 0xffff)
				TERR( TERR_SPRITE );
		}
	}
	...
	SetSpriteGroup( td, SAnim.Number,2,235,305 );
							...
							LONG frmto, WORD type, UBYTE sn )
{
	...
	cn = &td->CNode[sn];
	StartCNode( td,&td->CnList,cn,wptr[0],wptr[1],STDCYCLE_SPEED,0,type );
							...
							LONG linkto, WORD type,UBYTE sn )
{
	...
	StartCNode( td,(fcn->Node.mln_Succ ? NULL:&td->CnList),
				cn,fcn->From,fcn->To,STDCYCLE_SPEED,0,type);
...
static BOOL cmdCNodeWait( register struct TData *td, LONG temp,WORD bottom,UBYTE sn )
{
	...
	if (sn == CNODE_ALL)
	{
		for(cn = (struct CNode *)td->CnList.mlh_Head;
			cn = (struct CNode *)cn->Node.mln_Succ;)
		{
			if (!(cn->Flags & CNF_CYCLE)) return FALSE;
		}
		return TRUE;
...
static BOOL cmdPlaySnd( register struct TData *td, LONG number )
{
	...
	if (ss = GetSmSound( td, SndArr[number] ))
		SendSMsg( td, ST_ONESOUND,(LONG)ss, 1, 0, 0, 0 ); 
	return TRUE;
}


static BOOL cmdSetDestVol( register struct TData *td, LONG vol,WORD ch )
{
	SendSMsg( td, ST_SETDESTVOL,vol,ch);
	return TRUE;
}
...
static BOOL cmdSetCNodeSpeed( register struct TData *td, 
								LONG speed, WORD state, UBYTE sn  )
{
	struct CNode	*cn;
...
struct Command	StartupCmd[] =
{
	...
	{ CMD_WAITCDSTATE,  0,0,0 },
	{ CMD_STARTCNODE,	CNODE_CD,CNTYPE_CYCLE,(22<<16)+31 },
	{ CMD_SETCNODESPEED,CNODE_CD,0,STDCYCLE_SPEED*4 },
...
//	{ CMD_SETCNODESPEED,CNODE_CD,0,STDCYCLE_SPEED*4 },
	...
	{ CMD_SETCNODESPEED,CNODE_TEMP3,0,STDCYCLE_SPEED*4 },
	{ CMD_NUMBER_ONOFF, 0,0,1 },
	
	{ CMD_SETCNODE,		CNODE_TEMP4,0,(128<<16)+159 },
	{ CMD_STARTCNODE,	CNODE_TEMP4,CNTYPE_ONECOLOR,(21<<16)+21 },
	{ CMD_SETCNODESPEED,CNODE_TEMP4,0,STDCYCLE_SPEED*4 },
	{ CMD_SETCNODE,		CNODE_TEMP5,0,(128<<16)+159 },
	{ CMD_STARTCNODE,	CNODE_TEMP5,CNTYPE_ONECOLOR,(17<<16)+17 },
	{ CMD_SETCNODESPEED,CNODE_TEMP5,0,STDCYCLE_SPEED*4 },
...
struct Command WaitSetup[] =
{
	...
	{ CMD_SETCNODE,		CNODE_TEMP1,0,(160<<16)+196 },
	{ CMD_STARTCNODE,	CNODE_TEMP1,CNTYPE_MOVEUP,(160<<16)+196 },
	{ CMD_SETCNODESPEED,CNODE_TEMP1,0,STDCYCLE_SPEED*2 },
	{ CMD_SETCNODECYCLE,CNODE_TEMP1,1,1000000 },
...
struct Command	ReadySpin[] =
{
	...
	{ CMD_SETCNODECYCLE,CNODE_TEMP1,0,0 },
	{ CMD_CNODEWAIT,	CNODE_ALL,0,0 },
...
SpinCD( register struct TData *td, WORD speed )
{
	struct Message 	*msg;
...
StopCD( register struct TData *td )
{
	struct BMInfo 	*bmi;
	
	if (!(td->Flags & TD_SPINNING)) return 0;
...
//		td->RBm3= AllocBitMap( bmi->bmi_Width,bmi->bmi_Height,3,BMF_DISPLAYABLE,NULL );
//		
		if (!td->Bm || !td->RBm || !td->RBm3) TERR( TERR_MEMORY );
	}
	...
	// MOVE SPRITE BACK BACK TO THE ORIGINAL COLORS.
	//
	SetCopList( td, CD_SPRITEBANKODD, CD_SPRITEBANKEVEN );
	LoadRGB32( &td->Vp, td->Table ); 
	MakeVPort( &td->View,&td->Vp ); 
...
struct Command	FinalCmd[] =
{
	...
	{ CMD_WAITFADE,		0,0,0 },
	{ CMD_NUMBER_ONOFF, 0,0,0 },
	{ CMD_WAITCDSTATE,  0,0,0 },
	{ CMD_SETCDSTATE,   0,0,CDSTATE_STARTWAIT },
	{ CMD_WAITCDSTATE,  0,0,0 },
	{ CMD_SETCNODECYCLE,CNODE_CD,0,0 },
	{ CMD_END,0,0,0 }
};
...
//	ExecLoop( td, FinalCmd );
	...
	while( ExecCommands( td ) )
	{
		AnimCD(td);
		Fade(td);
		FadeColor( td, td->Table,td->FadeLv, 17 );
		FadeColor( td, td->Table,td->FadeLv, 21 );
		LoadTitleRGB32(td);
		WaitTOF();
	}
}
...
struct Message *ActionSpinCD( register struct TData *td )
{
	struct Message 	*msg;

	SpinCD( td, 4 ); // The disk should be spinning.,Screens sould be down.
...
/*------------------------------------------------------------------------*/
struct Command	holdOffCMD[] =
{
	{ CMD_SETSTARS,		0,0,0 },
	{ CMD_END,0,0,0 }
};

struct Command  holdOffEndCMD[] =
{
	{ CMD_SETSTARS,		0,0,STARS_WAVELV },
	{ CMD_END,0,0,0 }
};
...
struct Message *WaitAction( struct TData *td )
{	
	WORD			holdOff = 0;
	...
	while( 1 )
	{
		while( msg = GetMsg(td->MsgPort ) )
		{
			switch (msg->mn_Length)
			{			
				case ANIMMSG_HOLDOFFANIM:
					holdOff = 1;
					ExecLoop( td, holdOffCMD );
					break;
				case ANIMMSG_BOOTING:
					ReplyMsg( msg ); msg = NULL;
					if (msg = ActionSpinCD(td)) 
						return msg;	// SpinCD has exited.
					break;
				case ANIMMSG_FALSEALARM:
					ExecLoop( td, holdOffEndCMD );
					holdOff = 0;
					break;
				case ANIMMSG_SHUTDOWN: 
					EndScreen( td );
