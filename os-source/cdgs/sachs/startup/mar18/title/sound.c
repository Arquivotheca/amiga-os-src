...
void sendWaiting( struct TData *td )
{
	UBYTE 							*tc;
	register struct SoundCommand 	*sc;

	while (tc = (UBYTE *)RemHead( &td->readycmd )) 
	{
...
void queSoundCommand( struct TData *td, register struct SoundCommand *sc )
{
	if (sc->s) sc->s->Lock++;
	td->SQue++;

	AddTail( &td->readycmd, (struct Node *)&sc->n );
}
...
//	}
	...
	if (sc = AllocMem( sizeof(struct SoundCommand),(MEMF_REVERSE | MEMF_PUBLIC)))
	{
...
queNextSound( struct TData *td,UBYTE ch, int next )
{
	register struct SoundCommand *sc = NULL;
			 ...
			 int period)
{
	...
	while( 1 ) 
	{
		queNextSound( td, ch, TRUE ); 
		for(i=1;i<0x10;i += i )
		{
			if (ch & i) 
			{
				if (!(sc = AllocSC(td))) goto xit;
				sc->s = ss;
...
{
	register struct SoundCommand		*sa;
	...
	{
		ch = 0x03;
		custom.aud[0].ac_vol = custom.aud[1].ac_vol = volume;
	}
...
{
	struct SmSound	*ss;
	LONG			size = data->ci_Size;

	if (!(ss = AllocMem( size,(MEMF_REVERSE | MEMF_CHIP) )))
		{ td->Error = TERR_MEMORY; return NULL; }
	
	if (DecompData( data,NULL,ss) != size) 
	{ 
		FreeMem( ss,size );
		td->Error = TERR_DATA; 
		return NULL; 
