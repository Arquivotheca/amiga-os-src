...
void DumpTransaction(struct Transaction *trans);
							...
							else
							{
								...
								if (ABORT == FALSE)
								{
									...
									if (newbroker =	AllocMem(sizeof(struct NewBroker), MEMF_CLEAR))
									{
										...
										{
											newbroker->nb_Pri =	(BYTE) ArgInt(tooltypes, "CX_PRIORITY",	50);
																	...
																	switch (cxmsg_id)
																	{
																		...
																		case 3:
																			if (tracker	!= cxmsg_id)
																			{
																				...
																				if(server = FindEntity(host[cxmsg_id],"RemoteControl", entity,NULL))
																				{
																					ActivateCxObj(ibroker, 0);
																		...
																		if(server)
																		{
																			...
																			key_trans->trans_Timeout = 5;
																			DoTransaction(server, entity, key_trans);
																			if(key_trans->trans_Error)
																			{
																				ActivateCxObj(ibroker, 0);
																				RemoveCxObj(keyfilter);
																				RemoveCxObj(mousefilter);
...
/*
void DumpTransaction(trans)
struct Transaction *trans;
{
	kprintf("trans_Msg.mn_Node.ln_Succ: %lx\n",(ULONG)trans->trans_Msg.mn_Node.ln_Succ);
	kprintf("trans_Msg.mn_Node.ln_Pred: %lx\n",(ULONG)trans->trans_Msg.mn_Node.ln_Pred);
	kprintf("trans_Msg.mn_ReplyPort:	%lx\n",(ULONG)trans->trans_Msg.mn_ReplyPort);
	kprintf("trans_SourceEntity:		%lx\n",(ULONG)trans->trans_SourceEntity);
	kprintf("trans_DestinationEntity:	%lx\n",(ULONG)trans->trans_DestinationEntity);
	kprintf("trans_Command:				%ld\n",(ULONG)trans->trans_Command);
	kprintf("trans_Type:				%ld\n",(ULONG)trans->trans_Type);
	kprintf("trans_Error:				%ld\n",(ULONG)trans->trans_Error);
	kprintf("trans_Flags:				%lx\n",(ULONG)trans->trans_Flags);
	kprintf("trans_Sequence:			%ld\n",(ULONG)trans->trans_Sequence);
	kprintf("trans_RequestData:			%lx\n",(ULONG)trans->trans_RequestData);
	kprintf("trans_ReqDataLength:		%ld\n",(ULONG)trans->trans_ReqDataLength);
	kprintf("trans_ReqDataActual:		%ld\n",(ULONG)trans->trans_ReqDataActual);
	kprintf("trans_ResponseData:		%lx\n",(ULONG)trans->trans_ResponseData);
	kprintf("trans_RespDataLength:		%ld\n",(ULONG)trans->trans_RespDataLength);
	kprintf("trans_RespDataActual:		%ld\n",(ULONG)trans->trans_RespDataActual);
	kprintf("trans_Timeout:				%ld\n",(ULONG)trans->trans_Timeout);
} */