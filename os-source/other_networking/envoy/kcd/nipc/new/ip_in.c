...
**
** $Id: ip_in.c,v 1.25 92/12/16 14:42:40 kcd Exp $
**
***************************************************************************
...
//#define SHOW_IP_HEADERS Yeppers
		...
		kprintf("IP SrcAddr: %ld.%ld.%ld.%ld\n",(ipkt->iph_SrcAddr >>24),
							(ipkt->iph_SrcAddr >>16) & 0xff,
							(ipkt->iph_SrcAddr >>8) & 0xff,
							ipkt->iph_SrcAddr & 0xff);
		kprintf("IP DstAddr: %ld.%ld.%ld.%ld\n",(ipkt->iph_DestAddr >>24),
							(ipkt->iph_DestAddr >>16) & 0xff,
							(ipkt->iph_DestAddr >>8) & 0xff,
							ipkt->iph_DestAddr & 0xff);
		kprintf("IP HLength: %ld\n",ipkt->iph_IHL << 2);
		kprintf("IP DLength: %ld\n",ipkt->iph_Length);
		kprintf("IP Ident:   %ld\n",ipkt->iph_Ident);
		kprintf("IP Checksum is ");
		if(CalcChecksum((UBYTE *) ipkt, (ipkt->iph_IHL << 2), 0L) == 0xFFFF)
		    kprintf("Good\n");
		else
		    kprintf("Bad\n");
		kprintf("IP TTL:     %ld\n",ipkt->iph_TimeToLive);
		kprintf("IP FragOff: %ld\n",ipkt->iph_FragmentOffset);
		kprintf("IP Flags:   ");
		if(ipkt->iph_Flags & IPFLAG_MOREFRAGMENTS)
