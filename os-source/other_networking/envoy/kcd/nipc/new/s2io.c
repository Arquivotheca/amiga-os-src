...
**
** $Id: s2io.c,v 1.42 93/04/05 11:16:32 gregm Exp $
**
***************************************************************************
...
BOOL __asm MyPacketHook(register __a0 struct Hook *hook,
			register __a1 APTR packetdata,
			register __a2 struct IOSana2Req *ios2);

#define TAG_WANTPACKET TAG_USER+0xB0000+3
...
BOOL InitSana2()
{
    ...
    copytags[0]=S2_CopyToBuff;
    copytags[1]=(LONG) &ACTB;
    copytags[2]=S2_CopyFromBuff;
    copytags[3]=(LONG) &ACFB;
    copytags[4]=TAG_WANTPACKET;
    copytags[5]=(LONG) &gb->PacketHook;
    copytags[6]=TAG_DONE;

    gb->PacketHook.h_Entry = (HOOK_FUNC)&MyPacketHook;
    gb->PacketHook.h_Data = &gb->DeviceList;
                ...
                if(nb->DataPort = (struct MsgPort *) CreateMsgPort())
                {
                    ...
                    while (ci)
                    {
                        ...
                        if(ndp->ndp_Flags & NDPFLAGF_ONLINE)
                        {
                            ...
                            ioreq.ios2_BufferManagement = &copytags;
...

{
	...
	/* Most likely we'll want to see broadcasts */
	if(ios2->ios2_Req.io_Flags & (SANA2IOF_BCAST|SANA2IOF_MCAST))
		return TRUE;

//	/* If it's short enough, we won't save much time */
//	if(ios2->ios2_DataLength < 64)
//		return TRUE;

	/* Find the matching device pointer (skip the first node) */

	dev = (struct sanadev *)((struct List *)hook->h_Data)->lh_Head->ln_Succ;
	while(dev->sanadev_Node.mln_Succ)
	{
	    if (dev->sanadev_Device == ios2->ios2_Req.io_Device)
	    {
	    	if(ios2->ios2_PacketType == dev->sanadev_IPType)
	    	{
	    	    iph = (struct iphdr *)packetdata;

//#define SHOW_IP_HEADERS Yeppers
#ifdef SHOW_IP_HEADERS

		kprintf("IP Version: %ld\n",iph->iph_Version);
		kprintf("IP SrcAddr: %ld.%ld.%ld.%ld\n",(iph->iph_SrcAddr >>24),
							(iph->iph_SrcAddr >>16) & 0xff,
							(iph->iph_SrcAddr >>8) & 0xff,
							iph->iph_SrcAddr & 0xff);
		kprintf("IP DstAddr: %ld.%ld.%ld.%ld\n",(iph->iph_DestAddr >>24),
							(iph->iph_DestAddr >>16) & 0xff,
							(iph->iph_DestAddr >>8) & 0xff,
							iph->iph_DestAddr & 0xff);
		kprintf("IP HLength: %ld\n",iph->iph_IHL << 2);
		kprintf("IP DLength: %ld\n",iph->iph_Length);
		kprintf("IP Ident:   %ld\n",iph->iph_Ident);
		kprintf("IP Checksum is ");
		if(CalcChecksum((UBYTE *) iph, (iph->iph_IHL << 2), 0L) == 0xFFFF)
		    kprintf("Good\n");
		else
		    kprintf("Bad\n");
		kprintf("IP TTL:     %ld\n",iph->iph_TimeToLive);
		kprintf("IP FragOff: %ld\n",iph->iph_FragmentOffset);
		kprintf("IP Flags:   ");
		if(iph->iph_Flags & IPFLAG_MOREFRAGMENTS)
			kprintf("MoreFragments");
		kprintf("\nIP Protocl: %ld\n",iph->iph_Protocol);
#endif
