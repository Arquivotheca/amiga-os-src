...
BOOL CreateReqGads(struct UserReqData *urd)
{
    ...
    if((urd->urd_Window->Width < urd->urd_OptimalWidth) ||
       (urd->urd_Window->Height < urd->urd_OptimalHeight))
    {
    	urd->urd_ReqFont = ENVOYBASE->eb_TopazFont;
    	urd->urd_ReqFontAttr = (struct TextAttr *) &topaz8;
    }
    else
    {
    	urd->urd_ReqFont = urd->urd_SysFont;
    	urd->urd_ReqFontAttr = urd->urd_SysFontAttr;
    }
    urd->urd_FontHeight = urd->urd_ReqFont->tf_YSize;

    SetFont(urd->urd_Window->RPort,urd->urd_ReqFont);
    ...
    urd->urd_IText.ITextFont = urd->urd_ReqFontAttr;
    urd->urd_IText.IText="Password";
    ...
    newgads[0].ng_Height = InnerHeight - (urd->urd_FontHeight+6) - 4;
    ...
    newgads[1].ng_Height = newgads[2].ng_Height = urd->urd_FontHeight + 6;

    for(index=0;index<3;index++)
    {
    	newgads[index].ng_TextAttr = urd->urd_ReqFontAttr;
    ...
    urd->urd_LastGadget = urd->urd_ListView = CreateGad(LISTVIEW_KIND,urd->urd_LastGadget,&newgads[1],
    				    GTLV_ShowSelected,0,
    				    GTLV_ScrollWidth, 18,
    				    TAG_DONE);

    newgads[1].ng_TopEdge = InnerHeight + TopOffset - newgads[1].ng_Height - 2;

    newgads[2].ng_TopEdge = newgads[1].ng_TopEdge;


    urd->urd_LastGadget = urd->urd_OK = CreateGad(BUTTON_KIND,urd->urd_LastGadget,&newgads[2],
    				    TAG_DONE);

    urd->urd_LastGadget = urd->urd_Cancel =CreateGad(BUTTON_KIND,urd->urd_LastGadget,&newgads[3],
    				    TAG_DONE);
...
VOID DeleteReqGads(struct UserReqData *urd)
{
    if(urd->urd_IsUP)
    {
...
VOID FixGroupNames(struct UserReqData *urd)
{
    ...
    NumTotChars = LVWidth / urd->urd_Screen->RastPort.Font->tf_XSize;
    NumDrwChars = strlen("<Group>");
    NumGrpChars = NumTotChars - NumDrwChars - 1;

    sprintf(urd->urd_LVFmt,"%%%ld.%ldld <Group>",(ULONG)NumGrpChars);
    ...
    while(gdat->Node.ln_Succ)
    {
    	if(gdat->Node.ln_Type == 253)
    	{
    	    ...
    	    sprintf(gdat->Node.ln_Name,urd->urd_LVFmt,gdat->GroupName);
    	}
    ...
    if(ABase = OpenLibrary("accounts.library",37L))
    {
        if(urd = AllocMem(sizeof(struct UserReqData),MEMF_CLEAR|MEMF_PUBLIC))
        {
            ...
            if(urd->urd_SysFont = OpenFont(urd->urd_SysFontAttr))
            {
                ...
                urd->urd_IText.IText = "N";
                urd->urd_BigSpace = IntuiTextLength(&urd->urd_IText);
                urd->urd_BigHeight = urd->urd_SysFont->tf_YSize;
                if(wintags[10].ti_Tag == WA_DragBar)
                    urd->urd_TopOffset = urd->urd_BigHeight + 1;
                else
		...
		AskFont(&urd->urd_Screen->RastPort,&urd->urd_LVTextAttr);


                urd->urd_OptimalWidth = (urd->urd_BigSpace)*15 + 20 + urd->urd_Screen->WBorLeft + urd->urd_Screen->WBorRight;
                urd->urd_OptimalHeight = (urd->urd_BigHeight) * 4 + 1 + 2 + 8 + 18 + 8 + urd->urd_BottomBorder;

                urd->urd_IText.IText = "  Cancel  ";
                urd->urd_IText.ITextFont = (struct TextAttr *) &topaz8;

                urd->urd_MinWidth = (IntuiTextLength(&urd->urd_IText) + 8)*2 + 12 + urd->urd_Screen->WBorLeft + urd->urd_Screen->WBorRight;
                urd->urd_MinHeight = (urd->urd_BigHeight) + 1 + 24 + + 2 + 8 + 18 + 8 + urd->urd_BottomBorder;

                wintags[12].ti_Data = urd->urd_MinWidth;
                wintags[13].ti_Data = urd->urd_MinHeight;

                if(!wintags[4].ti_Data)
                    wintags[4].ti_Data = urd->urd_OptimalWidth + (urd->urd_BigSpace * 15);

                if(!wintags[5].ti_Data)
                    wintags[5].ti_Data = urd->urd_OptimalHeight;
    ...
    //                              BusyWindow(urd,urd->urd_BlockWindow,TRUE);
                    ...
                    if(urd->urd_Window->Width >= urd->urd_MinWidth)
                    {
                        if(urd->urd_Window->Height >= urd->urd_MinHeight)
                        {
                            ...
                            urd->urd_VisualInfo = (APTR) GetVisInfo(urd->urd_Screen,TAG_DONE);
                            if(urd->urd_IsUP = CreateReqGads(urd))
                            {
