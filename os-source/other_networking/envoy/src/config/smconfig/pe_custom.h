...
#define PORT_NAME   "ServicesConfig_Prefs"
#define ENV_NAME    "ENV:Envoy/services.prefs"
#define ARC_NAME    "ENVARC:Envoy/services.prefs"
...
{
    struct Node         sp_Node;
    struct IFFService   sp_Prefs;
    struct DiskObject  *sp_DiskObject;
    UBYTE               sp_Name[256];
};
...
typedef struct EdData
{
    ...
    struct FileRequester *ed_SvcReq;
    struct ServicePrefs  *ed_CurrentService;
    STRPTR                ed_StatusLabels[3];
    struct Gadget        *ed_ServiceStatus;
    struct Gadget        *ed_ServiceList;
    struct Gadget	 *ed_AddSvc;
    struct Gadget        *ed_RemoveSvc;
    BOOL                  ed_Refresh;
    BOOL                  ed_Write;
...
#define RenderGadgets(ed)       RenderDisplay(ed)
