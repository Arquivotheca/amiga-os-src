...
*/
ULONG __saveds ASM StartService(REG(a0) struct TagItem *tagList)
{
    ULONG cptags[5];
    ...
    if(!LPDBase->LPD_Entity)
    {
        ...
        if(sigbit = AllocSignal(-1L))
        {
            ...
            */
            ...
            LPDUser = (STRPTR)GetTagData(SSVC_UserName,0,tagList);
            LPDPassword = (STRPTR)GetTagData(SSVC_Password,0,tagList);
            LPDEntityName = (STRPTR)GetTagData(SSVC_EntityName,0,tagList);
    ...
    */
    ...
    else
    {
    	...
    	Permit();
        LPDError = 0;
        strcpy((STRPTR)GetTagData(SSVC_EntityName,0,tagList),"LPD_Service");
    }
    return LPDError;
}
