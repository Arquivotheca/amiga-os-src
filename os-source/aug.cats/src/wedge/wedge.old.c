...
/* Wedge.c v 36.19 --- Carolyn Scheppner  CBM  11/90
 *
 ...
 * 36_19 changes: knock % signs out of ASCII dumps
 */
...
extern VOID  myFunc();     /* The assembler entry */
...
char *hline[HELPCNT] =
 {
 "Wedge  v36.19 --- Carolyn Scheppner  CBM  --- Copyright 1990  CBM\n",
 "\nCLI Usage:\n",
...
BOOL  Zap = FALSE, KillAll = FALSE, ListAll = FALSE, Help = FALSE;
...
ULONG
MyFunc(d0,d1,d2,d3,d4,d5,d6,d7,a0,a1,a2,a3,a4,a5,a6,a7)
         ...
         if(regmask & (0x0001 << k))
            ...
            if(ptrmask & (0x0001 << k))
               ...
               if(reg[k])
                  ...
                  for(j=0; j<16; j++)
                     {
                     if((ub2[j]<0x20)||(ub2[j]==0x25)||(ub2[j]>0x7e))  ub2[j]='.';
                     }
                  ub2[j] = NULL;
 ...
 */
VOID
MyFuncRet(d0,d1,d2,d3,d4,d5,d6,d7,a0,a1,a2,a3,a4,a5,a6,a7,id)
...
void main(int argc, char **argv)
   ...
   if(wm=(struct WedgeMaster *)FindPort(wmNameText))
      ...
      RealForbid = (ULONG)SetFunction(SysBase,FORBIDOFFS,dummy);
      SetFunction(SysBase,FORBIDOFFS,RealForbid);
      wm->wm_RealForbid = RealForbid;

      RealPermit = (ULONG)SetFunction(SysBase,PERMITOFFS,dummy);
      SetFunction(SysBase,PERMITOFFS,RealPermit);
      wm->wm_RealPermit = RealPermit;

      RealWait = (ULONG)SetFunction(SysBase,WAITOFFS,dummy);
      SetFunction(SysBase,WAITOFFS,RealWait);
      wm->wm_RealWait = RealWait;

      RealDebug = (ULONG)SetFunction(SysBase,DEBUGOFFS,dummy);
      SetFunction(SysBase,DEBUGOFFS,RealDebug);
   ...
   if(!BcplDosLib)
      {
      RealFunc    = SetFunction(LibBase, offs, (void *)myFunc);
      }
