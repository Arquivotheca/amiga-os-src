...
int
encode_stored_crc (FILE *ifp, FILE *ofp, long size, long *original_size_var,
		   long *write_size_var)
{
	int save_quiet;
...
/* i_rename - change the name of file
...
int i_rename(char *from, char *to)
{
	struct	stat s1,s2;
	extern	int errno;
...
static int my_toupper(register int n)
{
  if(n >= 'a' && n <= 'z') return n & (~('a'-'A'));
  return n;
}

int strucmp(register char *s, register char *t)
{
