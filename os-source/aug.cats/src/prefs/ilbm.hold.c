...
extern struct Library *SysBase, *IFFParseBase, *GfxBase;
...
ILBM *GetILBM (struct IFFHandle * iff)
{
    ...
    if (ilbm = (ILBM *) AllocMem (msize, MEMF_CLEAR))
    {
    ...
    PREF *MP;
{
    ...
    if ((bmhd = (struct BitMapHeader *) AllocMem (sizeof (struct BitMapHeader),
						  MEMF_CLEAR)) == NULL)
	return (FALSE);
    ...
    FreeMem (bmhd, sizeof (struct BitMapHeader));
    return (status);
}
...
WORD GetColors (struct IFFHandle *iff, struct ColorRegister *cmap)
{
	...
	ncolors = MIN (MAXCOLORS, (sp->sp_Size / 3));
 ...
 * ==									 == *
 * ==	Peter's AllocBMRasters() (slightly modified...)			 == *
 * ==									 == *
 * ======================================================================== */
