...
*
* 04/91 fixed a memoration problem (marked with !!!)
...
*	XDEF	_wputch		; for RawDoFmt in wmain.c
*	XDEF	_wgetscdata	; for wmain.c
...
TESTCODE	EQU 0
	IFGT TESTCODE
* for RawDoFmt in wmain.c
_wputch
	move.b	d0,(a3)+
	rts

_wgetscdata
	move.l	4(sp),d0	; pointer to screen data buffer on stack
	movem.l d1/a0-a2/a6,-(sp)
	move.l	d0,a2		; save buffer in a2
	lea	IntuiName(PC),A1
	move.l  AbsExecBase.W,a6
	moveq.l	#0,D0
	CALLSYS	OpenLibrary	; open intuition
	beq.s	wgetscexit	; d0 = 0 = failure
	move.l	d0,a6		; intlib in a6
	move.l 	a2,a0		; buffer to a0
	move.l	#sc_SIZEOF,d0	; size to d0
	move.l	#WBENCHSCREEN,d1 ; type to d1
	suba.l	a1,a1		 ; clear a1
	CALLSYS GetScreenData
	move.l	d0,a2		; save success
	move.l	a6,a1
	move.l  AbsExecBase.W,a6
	CALLSYS CloseLibrary	; close intuition
	move.l	a2,d0		; success to d0
wgetscexit
