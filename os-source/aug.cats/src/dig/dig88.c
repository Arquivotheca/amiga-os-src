...
/* dig.c --- version of protodig.c
 * modified again 10/87 to accomodate removed line in BIX headers
 *  with no comments
 */
...
if ((in = Open(title, MODE_OLDFILE)) == NULL)
        {
        printf("couldn't read %s, sorry\n", title);
        exit(0);
        }
outtitle = argv[2];
if ((out = Open(outtitle, MODE_NEWFILE)) == NULL)
        {
        printf("couldn't open %s to write, sorry\n", outtitle);
        exit(0);
        }

if (argc > 3)  OneLine = TRUE;
        {
        for(l=3; l<argc; l++)
           {
...
if (!skip_past(separator) )
        {
        printf("%s doesn't seem to have any BIX messages... \n", title);
        cleanup();
        exit(0);
        }
while (b = one_bix())
