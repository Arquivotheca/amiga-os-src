;/* munglist.c - Execute me to compile me with SAS C 6.3
SC DATA=NEAR NMINC STRMERGE STREQ NOSTKCHK IGNORE=73 munglist.c
slink FROM LIB:c.o,munglist.o,munge.o TO munglist LIBRARY LIB:SC.lib,LIB:Amiga.lib,LIB:debug.lib
quit
Based on MemList so there's probably a lot of excess baggage

$Id: munglist.c,v 37.12 1993/10/01 22:09:38 Vic rel $

$Log: munglist.c,v $
 * Revision 37.12  1993/10/01  22:09:38  Vic
 * Changed - the script at the beginning to run with SAS C 6.x
 *         - the version (natch)
 * Added   - text in the "Usage" to reflect MARK and SINCE MARK (missing from
 *           previous version)
 *         - an ONLY option to list only those blocks allocated by a specific
 *           task.  The ONLY option forces the TOTAL option.
 *         - RCS Id and Log entries for future tracking
 *
...
UBYTE *vers = "\0$VER: munglist 37.12";
UBYTE *Copyright = 
...
void main(int argc,char **argv)
   ...
   for(k=1; k<argc; k++)
	...
	else if((!(stricmp(argv[k],"TOTAL"))) || (!(stricmp(argv[k],"ONLY"))))
	    {
	    if(k<(argc-1))
		{
		Total=TRUE;
		ShowList=(!(stricmp(argv[k],"ONLY")));
		k++;
		taskname = argv[k];
		}
	    }
...
VOID ASM munglist(REG(a0) ULONG *ustart, REG(d0) ULONG usize)
	...
	if((wp[0] == 0xC0DE)&&(wp[1]==0xF00D)&&(up[UKEEPI]==KeeperKey)&&(up[1] <= usize))
		...
		{
	    	name = (UBYTE *)(&up[6]);
		if(!(stricmp(name,taskname)))	
                  {
		    tasktotal += up[1];
		    if (ShowList) 
		      {
	                goto ShowAnyway;   // about 6 lines further down
		      }
                  }
		}
	    else if(ShowList)
		{
		ShowAnyway:                // This is where we 'goto'
