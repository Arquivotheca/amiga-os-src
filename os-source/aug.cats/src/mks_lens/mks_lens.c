 ...
 * V36.2 change - add version string
 */
...
register		SHORT		JumpFlag=FALSE;
	...
	while (!ExitFlag)
	{
		...
		x=(x1=sc->MouseX) - ((MyData->NormalX) >> 1);
		if (x<0) x=0;
		else if ((x+MyData->NormalX) > sc->Width) x=sc->Width-MyData->NormalX;

		y=(y1=sc->MouseY) - ((MyData->NormalY) >> 1);
		if (y<0) y=0;
		...
		if (GfxBase=(struct GfxBase *)OpenLibrary("graphics.library",33L))
		{
			...
			short	JumpFlag=TRUE;
				...
				while (JumpFlag)
				{
					...
					nw.MinWidth=98+sc.WBorRight+sc.WBorLeft;
