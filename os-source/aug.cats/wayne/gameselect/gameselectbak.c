...
// argument parsing 
#define TEMPLATE    "FROM/A,SECONDS/K/N,MINUTES/K/N,REBOOTLOOPS/K/N"
#define	OPT_FROM	0
...
Config2Gadgets( struct VisualInfo * vi, struct Gadget **glist )
{
    ...
    SHORT		  i = 0,len,cnt,rows,columns = COLUMNS;
...
#define	SHINE_OFF 2 /* was 2 for Topaz */
...
//	int	middle = GAD_TOP + (room >> 1);


//	ht = PanelScreen->Height;
    ...
    if ( gadtop < 80 ) {
	gadtop = ng.ng_TopEdge = 80;
    }

    ng.ng_Height = GAD_HT;
//    ng.ng_TextAttr = it.ITextFont = &Topaz;
    ng.ng_TextAttr = it.ITextFont = &GameSelectAttr;
    ng.ng_VisualInfo = vi;
    ng.ng_Flags = NULL;
...
//		prev->GadgetText->ITextFont = &Topaz;
		prev->GadgetText->ITextFont = &GameSelectAttr;
		...
		prev->GadgetText->LeftEdge = (prev->Width >> 1) - (len >> 1)+1;
//		prev->GadgetText->TopEdge = (prev->Height >> 1) - (Topaz.ta_YSize >>1)+1;
		prev->GadgetText->TopEdge = (prev->Height >> 1) - (GameSelectAttr.ta_YSize >>1)+1;
...
//		    prev->GadgetText->TopEdge = (prev->Height >> 1) - (Topaz.ta_YSize >>1) + SHINE_OFF;
		    prev->GadgetText->TopEdge = (prev->Height >> 1) - (GameSelectAttr.ta_YSize >>1) + SHINE_OFF;
...
{
    int			  ret,checkplay = 0,i=0,j,rebootloops;
    struct DisplayInfo	  disp;
    BOOL		  DoAudio,DoReboot;
