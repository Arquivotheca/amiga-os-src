...
// argument parsing 
#define TEMPLATE    "FROM/A,SECONDS/K/N,MINUTES/K/N,REBOOTLOOPS/K/N,ANBR/K"
#define	OPT_FROM	0
	...
	if ( prev = GadCreate( &ng, vi, prev, BUTTON_KIND, TAG_DONE ) ) {
		...
		if ( git = (struct IntuiText *)AllocMem( sizeof (it), MEMF_CLEAR )) {
		    *git = *prev->GadgetText;
		    ...
		    prev->GadgetText->LeftEdge = (prev->Width >> 1) - (len >> 1) + 2;
		    prev->GadgetText->TopEdge = (prev->Height >> 1) - (Topaz.ta_YSize >>1) + 2;
...
ReadPort( ULONG port )
{
	...
	switch ( JP_TYPE_MASK & state ) {
		...
		if (S(JPF_BTN6) && S(JPF_BTN4) && S(JPF_BTN1)) {
		    D(PRINTF(" left ear, Blue, Red... LoadWB\n");)
