...
FreeXlEdit( XLEDIT * xledit )
{
    ...
    D(PRINTF("FreeXlEdit() 1 xledit= 0x%lx\n",xledit);)
...
CDXLProc( VOID )
{
    ...
    D(PRINTF("CDXLProc CDXL_OB= 0x%lx, CDXL_OB->FrameNum= %ld\n",
...
//			    ChooseScreenMode( window, &scdef );
			...
			case QUIT_ID:
			    if ( XlControl.XlCur ) {
				D(PRINTF("QUIT_ID 1 XlControl.XlCur= 0x%lx, Size= %ld\n",XlControl.XlCur,XlControl.XlCur->dnode.Size);)
				KillCDXL( XlControl.XlCur->cdxlob );
				D(PRINTF("QUIT_ID 2 XlControl.XlCur= 0x%lx, Size= %ld\n",XlControl.XlCur,XlControl.XlCur->dnode.Size);)
			    }
			    quit = TRUE;
			    D(PRINTF("QUIT_ID\n");)
			    break;
    ...
    D(PRINTF("HandlePanel() W\n");)

    while ( XlControl.XlCur ) {
	D(PRINTF("HandlePanel() X XlControl.XlCur= 0x%lx, Size= %ld\n",XlControl.XlCur,XlControl.XlCur->dnode.Size);)
	...
	D(PRINTF("HandlePanel() Y XlControl.XlCur= 0x%lx, Size= %ld\n",XlControl.XlCur,XlControl.XlCur->dnode.Size);)

	CDXLTerm( XlControl.XlCur->cdxlob );

	D(PRINTF("HandlePanel() Z XlControl.XlCur= 0x%lx, Size= %ld\n",XlControl.XlCur,XlControl.XlCur->dnode.Size);)
	FreeXlEdit( XlControl.XlCur );
    }

    D(PRINTF("HandlePanel() END\n");)
