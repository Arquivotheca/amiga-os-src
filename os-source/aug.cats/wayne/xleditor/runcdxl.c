...
ASYNCXLFILE		* XLfile;
IMPORT struct Task	* CDXLTask,* parent;
IMPORT ULONG		  ParentSig;
...
//    disp_def->ModeID = NULL;
    ...
    D(PRINTF("Pan2Display disp_def->ModeID= 0x%lx\n",disp_def->ModeID);)
...
CDXLFreeOb( CDXLOB * CDXL_ob )
{
	...
	if ( CDXL_ob->xlfile ) {
	    kprintf("CDXLFreeOb() 1\n");
	    CloseAsyncXL( CDXL_ob->xlfile, CDXL_ob );
	    CDXL_ob->xlfile = NULL;
	}
	kprintf("CDXLFreeOb() 2\n");

	for ( i = 0; i < 2; i++ ) {
	    if( CDXL_ob->Buffer[i] ) {
	    	kprintf("CDXLFreeOb() 3\n");
		FreeMem( CDXL_ob->Buffer[i], CDXL_ob->BufSize );
	    }
	}
	kprintf("CDXLFreeOb() 4\n");

	if ( (CDXL_ob->flags & CDXL_FLIK) && CDXL_ob->colortable32 ) {
	    FreeMem( CDXL_ob->colortable32, CDXL_ob->CMapSize );
	}

	kprintf("CDXLFreeOb() 4.1\n");
	FreeMem( CDXL_ob, sizeof( CDXLOB ) );
	kprintf("CDXLFreeOb() 5\n");
...
CDXLObtain( UBYTE * CDXLFile, DISP_DEF * disp_def, CDXLOB **CDXLob, ULONG flags, struct TagItem * inti )
{
    ...
    *CDXLob = CDXL_ob = NULL;
	...
	Close( File );
	File = NULL;
	return( FlikObtain( CDXLFile, disp_def, CDXLob, flags, inti ) );
...
//    CDXL_ob->FrameSize = FRAME_SIZE( &pan );
    ...
    if ( (flags & CDXL_DOSXL) && !(CDXL_ob->xlfile = OpenAsyncXL( CDXLFile, CDXL_ob, 0 ) ) ) {
	kprintf("Could NOT open AsyncXL FILE\n");
	ret = RC_NO_MEM; // ??
	goto error;
    }
...
CDXLCallBack( register __a1 APTR intdata, register __a2 struct CDXL * xl )
{
    ...
    Signal( (struct Task *)CDXLTask, XLSignal );
...
VOID
ctable8TO32( COLOR8 c8[], COLOR32 c32[], int numcolors )
{
    int   i;
    UBYTE c;
	...
	c = c8[i].r;
	D(PRINTF("0x%lx ",c);)
	c32[i].r = c | (c<<8) | (c << 16) | (c<<24);

	c = c8[i].g;
	D(PRINTF("0x%lx ",c);)
	c32[i].g = c | (c<<8) | (c << 16) | (c<<24);

	c = c8[i].g;
	D(PRINTF("0x%lx\n",c);)
	c32[i].b = c | (c<<8) | (c << 16) | (c<<24);
    }
...
DumpRGB4( UWORD * cmap, int num )
{
	...
	D(PRINTF("0x%lx 0x%lx 0x%lx\n",
	    (cmap[i] >> 8) & 0xF,(cmap[i] >> 4) & 0xF,cmap[i] & 0xF);)
    }
...
//	    SendIOR(CDDeviceMReq,CD_READXL,CDXL_ob->StartSector * cdinfo.SectorSize,CDXL_ob->NumSectors * cdinfo.SectorSize,XList.mlh_Head);
	...
	while ( Count < LastFrame ) {
	    ...
	    if ( signals & (CDXL_ob->KillSig|CDPortSignal) ) {
		// Got a kill signal.
		CDXL_ob->loops = 1;
		break;
	    }
		...
		CDXL_ob->FrameNum = ((PAN *)CDXL_ob->Buffer[curvid])->Frame;

		Signal( parent, ParentSig );
	    }

	    /*
	     * Its time to call ChangeVPBitMap() 
	     */
	    if( (signals & DBufSignal) || Safe2Change ) {
		if ( Need2Change ) {
		    WaitBlit();
			...
			if ( CDXL_ob->flags & CDXL_FLIK ) {
			    LoadRGB32(  disp_def->vp, (ULONG *)CDXL_ob->CMap[curvid^1] );
...
//			    LoadRGB4( disp_def->vp, CDXL_ob->CMap[CDXL_ob->curVideo^1],NumColors );
    ...
    } while ( (CDXL_ob->loops == -1) || (CDXL_ob->loops && --CDXL_ob->loops) );
...
PlayAsyncXL( DISP_DEF * disp_def, CDXLOB * CDXL_ob, ASYNCXLFILE * xlfile )
{
    ...
    if ( CDXL_ob->flags & CDXL_FLIK ) {
	// If this is a FLIK file, then the colortable32 is in LoadRGB32() ready
	// format, with the first ULONG containing:
	// - 1 Word with the number of colors to load and
	// - 1 Word with the first color to be loaded.

	NumColors = min( (*CDXL_ob->colortable32)>>16,disp_def->vp->ColorMap->Count);

	if ( (CDXL_ob->flags & CDXL_MULTI_PALETTE) && 
	  !(CDXL_ob->flags & CDXL_RGB32) )
	{
	    if ( !(colortable = AllocVec( CDXL_ob->CMapSize, MEMF_CLEAR )) ) {
		return( RC_NO_MEM );
	    }
	    CopyMem( CDXL_ob->colortable32, colortable, CDXL_ob->CMapSize );
	}
    } else {
	NumColors = min(CDXL_ob->CMapSize >> 1,disp_def->vp->ColorMap->Count);
    }
...
//    oldpri = SetTaskPri( FindTask( NULL ), 50L );
    ...
    D(PRINTF("XLSignal= 0x%lx, KillSig= 0x%lx, CDPortSignal= 0x%lx, DBufSignal= 0x%lx\n",
	XLSignal,CDXL_ob->KillSig,CDPortSignal,DBufSignal);)
     ...
     */
 	...
 	kprintf("FrameNum= %ld, LastFrame= %ld\n",CDXL_ob->FrameNum,LastFrame);
	kprintf("%ld - %ld\n",((PAN *)CDXL_ob->Buffer[0])->Frame,((PAN *)CDXL_ob->Buffer[1])->Frame);

	while ( /*Count*/CDXL_ob->FrameNum < LastFrame ) {
//	    xlfile->af_PacketPort.mp_Flags = PA_SIGNAL;

	    signals = Wait( SignalMask );

	    if ( signals & CDXL_ob->KillSig ) {
		kprintf("// Got a kill signal Signal= 0x%lx, CDXL_ob->KillSig= 0x%lx, CDPortSignal= 0x%lx\n",Signal,CDXL_ob->KillSig,CDPortSignal);
		CDXL_ob->loops = 1;
		break;
	    }
	     ...
	     * has finished with one of our transfer lists.
	     */
	    if ( !(CDXL_ob->AudioSize) || (signals & XLSignal) || !Count ) {
		WaitPacket( xlfile );
#ifndef	OUTT	// [
		    CDXL_ob->FrameNum++;
#else		// ][
		if ( CDXL_ob->flags & CDXL_FLIK ) {
		    CDXL_ob->FrameNum = ((FLIKFRAME *)CDXL_ob->Buffer[curvid])->framenum;
		    D(PRINTF("FLIK. FrameNum= %ld, curvid= %ld, Buffer= 0x%lx\n\n",CDXL_ob->FrameNum,curvid,CDXL_ob->Buffer[curvid]);)
		} else {
		    CDXL_ob->FrameNum = ((PAN *)CDXL_ob->Buffer[curvid])->Frame;
		    D(PRINTF("FrameNum= %ld, curvid= %ld, Buffer= 0x%lx\n\n",CDXL_ob->FrameNum,curvid,CDXL_ob->Buffer[curvid]);)
		}
#endif		// ]
		...
		if (bytesArrived <= 0) {
			...
			ret = RC_READ_ERROR;
			//D(
			 kprintf("Read Error, CDXL_ob= 0x%lx, FrameNum= %ld, IoErr()= %ld, Res2= %ld\n",
			    CDXL_ob,CDXL_ob->FrameNum,IoErr(),xlfile->af_Packet.sp_Pkt.dp_Res2);
			 PrintFault(xlfile->af_Packet.sp_Pkt.dp_Res2, NULL);
			//)
			break;
		    }
		}
		...
		if ( Need2Change ) {
		    ...
		    if ( CDXL_ob->flags & CDXL_MULTI_PALETTE ) {
			if ( CDXL_ob->flags & CDXL_FLIK ) {
			    if ( !(CDXL_ob->flags & CDXL_RGB32) ) {
//				Delay(50);
				D(PRINTF("----\n");)
				ctable8TO32( (COLOR8 *)CDXL_ob->CMap[curvid^1], (COLOR32 *)&colortable[1], NumColors );
				LoadRGB32( disp_def->vp, colortable );
			    } else {
				LoadRGB32( disp_def->vp, (ULONG *)CDXL_ob->CMap[curvid^1] );
			    }
			} else {
			     ...
			     * 4 bits per gun, might as well just use LoadRGB4.
			     */
//			    DumpRGB4( CDXL_ob->CMap[curvid^1], NumColors );
//			    LoadRGB4( disp_def->vp, CDXL_ob->CMap[CDXL_ob->curVideo^1],NumColors );
			    LoadRGB4( disp_def->vp, CDXL_ob->CMap[curvid^1],NumColors );
			}
		    }
...
//	ChangeVPBitMap(disp_def->vp, disp_def->bm[curbm], disp_def->dbuf);
	...
	if ( (pos = Seek(xlfile->af_File,0L,OFFSET_CURRENT)) != -1L ) {
	    kprintf("Old FrameNum= %ld\n",CDXL_ob->FrameNum);

//	    CDXL_ob->FrameNum = ((CDXL_ob->StartSector*DEFAULT_SECTOR_SIZE) + pos) / CDXL_ob->FrameSize;
	    CDXL_ob->FrameNum = pos / CDXL_ob->FrameSize;

	    kprintf("New FrameNum= %ld\nStartSector= %ld, *DEFAULT_SECTOR_SIZE= %ld, pos= %ld, FrameSize= %ld\n",
		CDXL_ob->FrameNum,CDXL_ob->StartSector,
		(CDXL_ob->StartSector*DEFAULT_SECTOR_SIZE),pos,CDXL_ob->FrameSize);
	}
#endif		// ]
	if ( Count ) {
	    if ( CDXL_ob->AudioSize )
...
//    SetTaskPri( FindTask( NULL ), oldpri );
    ...
    if ( colortable ) {
	*CDXL_ob->CMap[0] = colortable[0] << 16;
	*(CDXL_ob->CMap[0]+1) = colortable[0] && 0xFFFF;
	FreeVec( colortable );
    }
...
/*
 * Get ready to start playing the ASyncXL.
 */
...
//    parent = (struct Process *)FindTask( NULL );
     ...
     */
    ret = PlayAsyncXL( disp_def, CDXL_ob, CDXL_ob->xlfile );
...
StartCDXL( DISP_DEF * disp_def, CDXLOB * CDXL_ob )
{
    ...
    D(PRINTF("StartCDXL ENTERED with disp_def= 0x%lx, CDXL_ob= 0x%lx\n",
	disp_def,CDXL_ob);)
...
RunCDXL( ULONG tag, ... )
{
	...
	if ( !(ret = OpenDisplay( &disp_def, ilbmfile ) ) ) {
	    ...
	    kprintf("\nGot Display\n");
	    if ( disp_def.Flags & DISP_BACKGROUND )
		kprintf("DISP_BACKGROUND\n");

	    if ( disp_def.Flags & DISP_XLPALETTE )
		kprintf("DISP_XLPALETTE\n");
