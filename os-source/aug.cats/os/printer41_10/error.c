...
/* CAS */
	if (PInfo->pi_ColorTmp != NULL) { /* if we allocated this */
#if FREEDEBUG
		kprintf("BO: freeing %ld bytes for ColorTmp @ %lx\n",
			PInfo->pi_ColorIntSize, PInfo->pi_ColorTmp);
#endif
		FreeMem(PInfo->pi_ColorTmp, PInfo->pi_ColorIntSize); /* release the mem */
	}
