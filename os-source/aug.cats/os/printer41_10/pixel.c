...
struct PrtInfo *PInfo;
{
	...
	UWORD *HamBuf;
	union colorEntry *cm, *ArrayInt, *IHamInt, oldcolor;
	struct RastPort *rp;
	...
	if(PInfo->pi_flags & PRT_COLORFUNC)	return;
	...
	/* CAS
	 * will shift modify values left 4 for HAM6 and use for both nibbles in 8 bit color
	 * will shift modify values left 2 for HAM8 and replace upper 6 bits in 8 bit color
	 */
	...
	if (xend) { /* if not starting at left edge */
#if IHDEBUG
		kprintf("InitHam: not left edge, yend=%ld\n",yend);
#endif
		for (; y != yend; y += yinc) { /* for each row */
			...
			if(IHamInt) {	/* different usage of HamInt for inverted HAM only */
				IHamInt->colorLong = ArrayInt->colorLong; /* save pixel color */
				IHamInt++;
				}
			ArrayInt++;
		}
...
#if IHDEBUG
		kprintf("InitHam: Is left edge, yend=%ld\n",yend);
#endif
		for (; y != yend; y += yinc) { /* for each row */
...
UWORD count;
{
	...
	extern struct PrinterExtendedData *PED;
	ULONG cavg;
	UWORD *RowBuf; /* row of color indices */
	union colorEntry *cm, *ColorInt, *IHamInt, oldcolor, *HamInt, *ColorTmp;
	...
	if ((PInfo->pi_flags & PRT_INVERT|PRT_HAM) == (PRT_INVERT|PRT_HAM)) IHamInt = HamInt;
	else								    IHamInt = NULL;
	...
	if(PInfo->pi_flags & PRT_COLORFUNC) {
		while (width--) {
			if (sum) {
				...
				oldcolor.colorLong = ColorTmp->colorLong ^ (~additive);
				for(u=0; u<4; u++) {
				...
				ColorInt->colorLong = ColorTmp->colorLong ^ (~additive);
			}
			GetBlack(ColorInt, PInfo->pi_flags);
			ColorInt++;
			ColorTmp++;
		}
	}
...
#if CPDEBUG
	kprintf("CPA: PInfo=$%lx, ColorInt=$%lx, y=%ld, flags=$%lx, width=%ld",
		PInfo, ColorInt, y, PInfo->pi_flags, width);
#endif
	else if (PInfo->pi_flags & PRT_HAM) { /* ham picture */
#if CPDEBUG
		kprintf("HAM (sum=%ld)\n",sum);
#endif
		...
		if (sum) {
			do {
				if (PInfo->pi_flags & PRT_INVERT) {
					oldcolor.colorLong = IHamInt->colorLong; /* CAS - was ColorInt */
				}
				color = *RowBuf++; /* get color lookup code */
#if DEBUGCAS
				kprintf(" %08lx-%04lx-",oldcolor.colorLong,color);
#endif
...
#endif
				if (IHamInt) {
					IHamInt->colorLong = ColorInt->colorLong;
					IHamInt++;
					}
...
#if CMDEBUG
	kprintf("CM: width=%ld, sytotal=%ld, ColorInt=$%lx, sx %ld %ld %ld %ld %ld %ld %ld %ld...",
		width, sytotal, ColorInt1, 
		sxptr[0],sxptr[1],sxptr[2],sxptr[3],sxptr[4],sxptr[5],sxptr[6],sxptr[7]);
#endif /* CMDEBUG */
