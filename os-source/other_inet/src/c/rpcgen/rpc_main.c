 ...
 * $Locker:  $
 *
 * $Id: rpc_main.c,v 1.1 90/11/09 13:43:58 bj Exp $
 *
 * $Revision: 1.1 $
 *
 * $Header: Hog:Other/inet/src/c/rpcgen/RCS/rpc_main.c,v 1.1 90/11/09 13:43:58 bj Exp $
 *
 *-----------------------------------------------------------------------------------
 */
...
#include "rpcgen_rev.h"
static char *compiler = "MANX36 MANX36" ;
static char *rpcgenvers = VERSTAG ;
	...
	char *define;
{
	...
	rtn = fexecl("gnucpp", "gnucpp", define, infile, tmpfname, 0L);
	if(rtn != 0){
		fprintf(stderr, "could not pre-process file %s\n", infile);
		crash();
	}
	...
	/*
	 * What follows is very MANX
	 */
	tmpbuf = malloc(strlen(tmpfname)+1);
	if(tmpbuf){
		strcpy(tmpbuf, tmpfname);
		fin->_flags |= _TEMP;
		fin->_tmpname = tmpbuf;
	}
}
