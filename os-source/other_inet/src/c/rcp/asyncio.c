...
struct MinList videolist;
struct MinList audiolist;
...
static VOID SendPacket(struct GlobalData *g, struct AsyncFile *file, APTR arg2)
{
 ...
 */
static LONG WaitPacket(struct GlobalData *g, struct AsyncFile *file)
{
LONG bytes;
    ...
    if (file->af_PacketPending)
    {
        ...
        while (TRUE)
        {
            ...
            SendPacket(g, file,file->af_Buffers[file->af_CurrentBuf]);
        }
    }
 ...
 * message port.
 */
static VOID RequeuePacket(struct GlobalData *g, struct AsyncFile *file)
{
 ...
 */
VOID RecordSyncFailure(struct GlobalData *g, struct AsyncFile *file)
{
...
D_S(struct InfoData,infoData);
	...
	NewList((struct List *)&videolist);
	NewList((struct List *)&audiolist);
    ...
    if (handle)
    {
	     ...
	     * helps performance on 68040 systems with copyback cache. Aligning
            ...
            if (accessMode == MODE_READ)
            {
                ...
                file->af_BytesLeft             = 0;
                if (file->af_Handler)
                    SendPacket(g, file,file->af_Buffers[0]);
            }
            else
            {
...
LONG bytesArrived;
    ...
    while (numBytes > file->af_BytesLeft)
    {
        ...
        bytesArrived = WaitPacket(g, file);
        if (bytesArrived <= 0)
        {
            if (bytesArrived == 0)
                return(totalBytes);
        ...
        SendPacket(g, file,file->af_Buffers[1-file->af_CurrentBuf]);
...
LONG SeekAsync(struct GlobalData *g, struct AsyncFile *file, LONG position, BYTE mode)
{
LONG  current, target;
LONG  minBuf, maxBuf;
LONG  bytesArrived;
