/*
 *  $Header: AS225:src/c/telnet_hansen/RCS/sys_amiga.c,v 1.1 92/10/12 14:23:27 kcd Exp Locker: kcd $
 */
/*
 ...
 * netex	- if set, include net exceptions in select
	...
	int	poll;
{
	...
	event |= SIGBREAKF_CTRL_C;
    	...
    	/*
     	 ...
     	 */
	if( (returnValue==0) && (event & cons_kybd_sigF) ){
		...
		if (c < 0){
			...
			if(*ttyiring.supply == 0x9b){
/*				*ttyiring.supply++ = 0x1b;
				*ttyiring.supply-- = '['; */
				ttyiring.supply[0] = 0x1b;
				ttyiring.supply[1] = '[';
				c = 2;
			}
...
TerminalNewMode(mode)
	int	mode;
{	/* This is where we need to check for compatibility of
	   hisopts and myopts to resolve contention for unsupported modes */
globalmode = mode;
}
