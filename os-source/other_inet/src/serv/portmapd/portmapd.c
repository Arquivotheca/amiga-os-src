...
#include "portmapd_rev.h"
static char *compiler = "manx36" ;
static char *pmdvers = VERSTAG ;

#define PORTNAME  "portmapd_rendezvous"

struct MsgPort *rendezvous_port = NULL, *CreatePort() ;
...
main()
{
	...
	struct MsgPort *rendezvous_port = NULL  ;
	
	if(  FindPort( PORTNAME ))
		exit(1) ;
	
    rendezvous_port = (struct MsgPort *)CreatePort(PORTNAME, 0L) ;
	if( rendezvous_port == NULL )
		exit(1) ;
...
#ifdef DEBUG
	fprintf(stderr, "server: about to do a switch, proc=%d\n",
			rqstp->rq_proc);
#endif
