...
{
    Debug1("pc_drive(%s)\n", string);
    if(stricmp(string, pc_a) == 0) {
        pc_number = PC_A;
        pc_name = pc_a;
    }
    else if(stricmp(string, pc_b) == 0) {
        pc_number = PC_B;
        pc_name = pc_b;
    }
    else {
        Debug1("pc_drive: don't grok %s\n", string);
        return 0;
    }
    return 1;
}

int amiga_drive(string)
char *string;
{
    Debug1("amiga_drive(%s)\n", string);
    if(stricmp(string, amiga_df0) == 0) {
        amiga_name = &amiga_df0[0];
        amiga_number = AMIGA_DF0;
    }
    else if(stricmp(string, amiga_df1) == 0) {
        amiga_name = &amiga_df1[0];
        amiga_number = AMIGA_DF1;
    }
    else {
        Debug1("amiga_drive: don't grok %s\n", string);
        return 0;
    }
    return 1;
}
