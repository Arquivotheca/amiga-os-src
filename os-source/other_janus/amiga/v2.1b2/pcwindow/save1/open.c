 ...
 */
{
   ...
   {
      ((struct ScreenExt *)superwindow->DisplayScreen->UserData)->UserCount += 1;
   ...
   if (FlagIsClear(superwindow->Flags, HEIGHT_INITED))
   {
      ...
      if (FlagIsSet(display->Modes, PAL_PRESENCE))
      {
               ...
               && FlagIsSet(display->Modes, MEDIUM_RES) )
         {
            ...
            if ( FlagIsClear(superwindow->Flags, WINDOW_WAS_HIRES)
...
/*   i = GlobalScreenHeight - (newwindow->TopEdge + newwindow->Height); */

   {
      ...
      GetScreenData(&TScreen,sizeof(struct Screen),newwindow->Type,newwindow->Screen);
      kprintf("TScreen.Height=%ld\n",TScreen.Height);
      i = TScreen.Height - (newwindow->TopEdge + newwindow->Height);
   }
...
#if (DEBUG3 & DBG_OPEN_DISPLAY_WINDOW_TOP_EDGE_CALC)
   ...
   kprintf("Open.c:       OpenDisplayWindow: GSHeight = %ld\n",GlobalScreenHeight);
   kprintf("Open.c:       OpenDisplayWindow: i        = %ld\n",i);
#endif
