...
* ---------  -------------  ---------------------------------------------------
* 3-Aug-89   -BK            ;;;FreeSDSemaphore from ProcessDelete for PC
* 3-Aug-89   -BK            Initialize sd_Semaphore for PC added services
* 01-may-89  -BK            Fixed memclear in service task to do 1 less
...
* =============================================================================
   ...
   PUTMSG   1,<'%s/Services.asm:   InitService()'>
...
;NOSEMS
   ...
   PUTMSG   1,<'%s/services.asm: InitService: sb_ServiceSemaphore Initialized'>
;NOSEMS
...
* *****************************************************************************
   PUTMSG   1,<'%s/services.asm:   ServiceTask()'>
...
* - if we're the Amiga lock the ServiceBase semaphore
   ...
   PUTMSG   1,<'%s/services.asm: ProcessPCToAmiga: Obtain sb_ServiceSemaphore'>
;SEMS      CALLSYS Forbid
      EXG.L    A2,A6
...
;SEMS
   ...
   PUTMSG   1,<'%s/services.asm: ProcessPCToAmiga: Release sb_ServiceSemaphore'>
;SEMS      CALLSYS   Permit
      EXG.L   A6,A2
...
* - if PC service then init the sd_semaphore for the PC.
      MOVEA.L  A3,A0
      JSR      MakeSDSemaphore
...
* - if we're the Amiga, lock the ServiceData semaphore
      ...
      EXG   A2,A6
      CALLSYS   ObtainSemaphore
   PUTMSG   1,<'%s/ProcessAdd: sd_Semaphore Obtained'>
;SEMS      CALLSYS   Forbid
      EXG   A2,A6

* - lock the ServiceData lock
      lea.l    sd_ServiceDataLock(A4),A0
      CALLSYS  JanusLock
   PUTMSG   1,<'%s/ProcessAdd: sd_ServiceDataLock locked'>
...
* - InformNewCustomers()
   ...
   PUTMSG   1,<'%s/ProcessAdd: new customers informed'>

* - unlock the ServiceData lock
      lea      sd_ServiceDataLock(a4),a0
      CALLSYS  JanusUnlock
   PUTMSG   1,<'%s/ProcessAdd: sd_serviceDataLock unlocked'>

* - if we're the Amiga unlock the ServiceData semaphore
      MOVEA.L   sd_Semaphore(A3),A0
      EXG   A2,A6
      CALLSYS   ReleaseSemaphore
   PUTMSG   1,<'%s/ProcessAdd: sd_Semaphore Released'>
;SEMS      CALLSYS   Permit
      EXG   A2,A6
...
;      MOVEA.L  A3,A0
;      JSR      FreeSDSemaphore
   PUTMSG   1,<'%s/ProcessDELETE: FreeSDSemaphore'>
...
;;; we never die anyway, and freeing this is *very* questionable
;      MOVEA.L   A2,A1
;      MOVE.L   #16,D0
;      MOVEA.L   A3,A6
;      CALLSYS   FreeMem
