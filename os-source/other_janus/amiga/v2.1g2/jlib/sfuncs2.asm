...
CALLS_REGS   REG   A2-A6/D2
   ...
   ENDC
      ...
      ;***************************************************
      ;* Throughout this routine, except for local changes
      ;* (which must be repaired afterwards), these
      ;* registers will be constant:
      ;*    D2   = ServiceData.WA
      ;*    A2   = ExecBase
      ;*    A4   = ServiceParam.BA
      ;*    A5   = ServiceParam.WA
   ...
   PUTMSG   2,<'%s/CallService about to LockSD()'>
      ...
      ;*    - if the UserCount on this side of the Bridge is
      ;*      greater than zero, 
   ...
   PUTMSG   2,<'%s/$%lx: CallService about to SignalCustomers()'>
...
customersDone:
   ...
   PUTMSG   2,<'%s/CallService about to UnlockSD'>

      ;*    - UnlockServiceData()
      MOVEA.L  D2,A0
      CALLSYS  UnlockServiceData

   PUTMSG   2,<'%s/CallService about to ObtainSemaphore()'>
   ...
   PUTMSG   2,<'%s/CallService about to find unused AToPC'>

      ;*    - call FindUnusedAToPC() if we're the Amiga or 
      ;*      call FindUnusedPCToA() if we're the PC
      JSR      FindUnusedAToPC

   PUTMSG   2,<'%s/CallService about to write field'>
   ...
   PUTMSG   1,<'%s/CallService ReleaseSemaphore(sb_ServiceSemaphore)'>
   ...
   PUTMSG   1,<'%s/CallService ReleaseSemaphore(sb_ServiceSemaphore) done'>
                                  ;SEMS      CALLSYS   Permit
      EXG.L    A6,A2
...
*      the ServiceData semaphore must still be out there so unlock it
      TST.L   sd_FirstAmigaCustomer(A4)
      BEQ   REL_UNLOCK_DONE
   PUTMSG   1,<'%s/ReleaseService() ReleaseSemaphore(sd_Semaphore)'>
...
* - if we're the Amiga unlock the ServiceBase semaphore
;SEMS
   PUTMSG   1,<'%s/ReleaseService() ReleaseSemaphore(sb_ServiceSemaphore)'>
      LEA.L   sb_ServiceSemaphore(A5),A0
      EXG.L   A6,A2
      CALLSYS   ReleaseSemaphore
;SEMS      CALLSYS   Permit
...
;SEMS      CALLSYS   Forbid
	...
	move.l	d0,a0
      ...
      MOVEA.L  A4,A0
      CALLSYS  LockServiceData
   PUTMSG   1,<'%s/DeleteService() After LockServiceData()'>

      ;------  KillServiceCustomer() for this customer
      MOVEA.L  A4,A0
      JSR      KillServiceCustomer
   PUTMSG   1,<'%s/DeleteService() After KillServiceCustomer()'>
	...
	move.l	d0,a0
      ...
      BNE      DEL_OTHERS
      MOVEA.L  A4,A0
   PUTMSG   1,<'%s/DeleteService() Before ExpungeService()'>
      JSR      ExpungeService

   PUTMSG   1,<'%s/DeleteService() Before UnLockServiceParamLock()'>
      ;------  unlock the ServiceParam lock
      MOVEA.L  A5,A0
      CALLSYS  MakeBytePtr
	move.l	d0,a0
      ...
      ;------  if we're the Amiga unlock the ServiceBase semaphore
   PUTMSG   1,<'%s/DeleteService() ReleaseSemaphore(sb_ServiceSemaphore)'>
      MOVEA.L  jb_ServiceBase(A6),A0
      LEA.L    sb_ServiceSemaphore(A0),A0
      EXG.L    A6,A2
      CALLSYS  ReleaseSemaphore
...
DEL_OTHERS:
      ;------  else as there's still some customers out there, we're going
      ;------  to tell them
      ;------  about the delete:
      ...
      ;------  side of the Bridge
      MOVEA.L  A4,A0
   PUTMSG   1,<'%s/DeleteService() Before InformDeletedService()'>
      JSR      InformDeletedService
   PUTMSG   1,<'%s/DeleteService() After InformDeletedService()'>

      ;------  UnlockServiceData()
      MOVEA.L  A4,A0
      CALLSYS  UnlockServiceData
   PUTMSG   1,<'%s/DeleteService() After UnlockServiceData()'>
      ...
      TST.B    sd_AmigaUserCount(A0)
      BNE      ds5
   PUTMSG   1,<'%s/DeleteService() Before FreeSDSemaphore()'>
      JSR      FreeSDSemaphore
   PUTMSG   1,<'%s/DeleteService() After FreeSDSemaphore()'>
ds5:
...
;SEMS      CALLSYS   Permit
      ...
      ;------  signal the other processor with a DeletesService
