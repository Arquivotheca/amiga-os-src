...
*     declaring an RPTR in Janus memory initializing that pointer to
*     NULL. This pointer is called the JRememberKey, and is used by calls
...
*
*   INPUTS
*     JRememberKey = Pointer to an RPTR that is to be used as
*                    the JRememberKey. Before your very first call to
*                    AllocJRemember(), the RPTR should be set to NULL.
*                    After that, the Janus library will manage the contents
*                    of the pointer for you.
...
;      ;------  A4 = Pointer to RPTR
      ...
      ;------  A2 = Pointer to allocated JanusRemember struct
      ...
      ;------  A3 = Pointer to allocated memory
      ...
      MOVE.W   (A4),D0                 ; get value that is stored in the RPTR
...
ajrError:
      ;------  We failed, so deallocate anything we allocated
      CMPA.L   #0,A2
      BEQ      10$
      MOVE.L   A2,A1
      MOVE.L   #JanusRemember_SIZEOF,D0
...
*
*   INPUTS
*     ToKey   = Pointer to an RPTR that is the JRememberKey
*               that is going to receive the list pointed to by FromKey
*               The RPTR MUST be in Janus memory!
*     FromKey = Pointer to an RPTR that is the JRememberKey
*               that has the list that's going to be attached ToKey,
...
;      MOVEA.L  A1,A3
      ...
      MOVE.W   (A2),A0        ; contents of ToKey RPTR
      CMPA.W   #0,A0
      BNE      atrFindEnde
      MOVE.W   (A3),(A2)
      BRA      atrDone
...
atrEndFound
      ...
      MOVE.W   (A3),D0  ; Contents of FromKey RPTR
      MOVE.W   D0,jrm_NextRemember(A4)
...
*
*   INPUTS
*     JRememberKey = Pointer to an RPTR that is to be used as the
*                    JRememberKey. Before your very first call to
*                    AllocJRemember(), the RPTR should be set to NULL.
*                    After that, the Janus library will manage the contents
*                    of the pointer for you.
*                    The RPTR MUST reside in Janus memory!
...
FJR_REGS   REG   D2-D3/A2-A3
   ...
   ENDC
      ...
      MOVE.L   #0,A2
      MOVE.W   (A0),A2     ; Contents of RPTR into A2.w
      MOVE.W   #0,(A0)
...
fjrLoop:
      ;------  Grab a copy of the offset to the next structure
      ;------  before we free up the memory of this structure
      MOVE.W   A2,D0       ; offset of first JRemember struct
      MOVE.W   #MEMF_PARAMETER+MEM_WORDACCESS,D1
      CALLSYS  JanusOffsetToMem
      MOVE.L   D0,A3
      MOVE.W   jrm_NextRemember(A3),D3
      ...
      MOVE.W   jrm_Offset(A3),D0 ; Get the memory offset
      MOVE.W   jrm_Type(A3),D1   ; and memory type
      CALLSYS  JanusOffsetToMem  ; and turn it into a pointer
      MOVE.L   D0,A1             ; which
      MOVEQ.L  #0,D0             ; (after preparing D0 for a word)
      MOVE.W   jrm_Size(A3),D0   ; when combined with the size
...
*
*   NAME   
*     AllocServiceMem -- Allocate Janus memory linked to a ServiceData
*                        structure
*
*   SYNOPSIS
...
;      MOVEM.L  ASM_REGS,-(SP)
      ...
      CALLSYS  MakeWordPtr
      MOVE.L   D0,A0
      LEA.L    sd_JRememberKey(A0),A0 ;Get pointer to sd_JRememberKey
      MOVEM.L  (SP)+,D0-D1
...
FSM_REGS   REG   D2-D3/A2-A4
   ...
   ENDC
      ...
      ;------  turn the ServiceData pointer into word-access
      CALLSYS  MakeWordPtr
      MOVE.L   D0,A3      ; A3 has the address of ServiceData.WA

      ;------  A3 = ServiceData.wa
      CMP.W    #0,sd_JRememberKey(A3)
      BEQ      FSM_DONE                ; nothing to free

      ;------  A2 = MemPtr

      CMP.L    #0,A2
      BNE      5$
      LEA      sd_JRememberKey(A3),A0
      MOVE.L   #-1,D0
      CALLSYS  FreeJRemember
      BRA      FSM_DONE
5$:
      ;------  below, the phrase "masked-type" refers to taking a Type of
      ;------  Janus memory and masking it with the NOT of the constant
      ;------  MEM_ACCESSMASK, like this:
      ...
      MOVE.L   A2,D0
      CALLSYS  JanusMemToOffset
      MOVE.W   D0,D2                   ; D2 = Memptr offset.w
      MOVE.L   A2,D0
      CALLSYS  JanusMemType
      MOVE.W   D0,D3                   ; D3 = Memptr type.w + ACCESS
      AND.W    #~MEM_ACCESSMASK,D3     ; D3 = Memptr type.w
      MOVE.W   sd_JRememberKey(A3),D0  ; D0 = offset of first rememberkey
...
FSM_LOOP:
      ...
      ;------  else turn the offset into an actual address of a
      ;------  JanusRemember struct
      MOVE.W   #MEMF_PARAMETER+MEM_WORDACCESS,D1
      CALLSYS  JanusOffsetToMem

      ;------  D0 = Pointer.l.wa to remember struct

      ;------  if we're freeing all memory or offset and masked-type equal
      ;------  ours then
