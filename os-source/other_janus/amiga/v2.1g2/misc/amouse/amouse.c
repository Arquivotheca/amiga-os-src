...
static   char              VersionId[]   = "AMouse v1.6";
...
void main()
{
   ...
   AllSignals   = KeySignalMask | GameSignal;
   ...
   while( NotClose )
   {
      ...
      if ( MsgBits & GameSignal )
      {
         ...
         /*D( printf("amouse.c: main() after SendIO\n"); )*/
      }  /* end of if GameMsg */
...
void SetMousePort()
{
   ...
   if((ReqPort=CreatePort(NULL,NULL))==0)
   {
      ...
      DoExit("can´t get port for mouse change");
      return;
   }
   if((IOReq=CreateStdIO(ReqPort))==0)
   {
      ...
      DoExit("can´t CreateStdIO for mouse change");
      return;
   }
...
void SetMouseXY(short x,short y)
{
   struct AMouseParamMem *ap;
   struct AMouseBufMem   *ab;

   ap = (struct AMouseParamMem *)DualPortParams;
   ab = (struct AMouseBufMem   *)BufMemPtr;
...
*/
   ...
   JanusLock(&ap->WriteLock);
   ab->AmigaPCX += x;
   ab->AmigaPCY += y;
   JanusUnlock(&ap->WriteLock);
...
void SetMousePressed(short LeftP,short RightP,short LeftR,short RightR,short Status)
{
   ...
   JanusLock(&ap->WriteLock);
   ab->AmigaPCLeftP  += LeftP;
   ab->AmigaPCRightP += RightP;
   ab->AmigaPCLeftR  += LeftR;
   ab->AmigaPCRightR += RightR;
   ab->AmigaPCStatus  = Status;
   JanusUnlock(&ap->WriteLock);
