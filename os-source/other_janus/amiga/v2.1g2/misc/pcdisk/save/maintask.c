...
struct AmigaDiskReq *adr;
{
   ...
   *((int *)&adr->adr_Count_h) = 0;
   ...
   D( printf("File=%d ",adr->adr_File); )

   error=Read(ft.File[adr->adr_File].fh,buffer,length);

   D( printf("Actual=%ld ",error); )
...
struct AmigaDiskReq *adr;
{
   ...
   D( printf("Off=%ld ",*((int *)&adr->adr_Offset_h)); )
...
{
   D( printf("Close: "); )
   D( printf("File=%ld ",(int)adr->adr_File); )
