...
#define  DBG_GET_NEXT_NUMBER_ENTER        0
#define  DBG_SHORT_GET_NEXT_NUMBER_ENTER  0
#define  DBG_READ_KEY_TABLE_ENTER         1
#define  DBG_READ_SCAN_CODE_TABLE_ENTER   1
#define  DBG_SHORT_WRITE_ENTER            0
#define  DBG_S_PRINTF_SHORT_WRITE_ENTER   0
#define  DBG_READ_SETTINGS_GRUNT_ENTER    1
#define  DBG_READ_SETTINGS_GRUNT_SIZES    1
#define  DBG_WRITE_SETTINGS_FILE_ENTER    1
#define  DBG_READ_SETTINGS_FILE_ENTER     1
#define  DBG_READ_SETTINGS_FILE_SETTINGS  1
...
#if (DEBUG1 & DBG_GET_NEXT_NUMBER_ENTER)
   kprintf("FileIO.c:     GetNextNumber(numaddress=0x%lx,bufptr=0x%lx,indexaddress=0x%lx,filenumber=0x%lx)\n",numaddress,bufptr,indexaddress,filenumber);
#endif
...
SHORT *num;
{
#if (DEBUG1 & DBG_SHORT_GET_NEXT_NUMBER_ENTER)
   kprintf("FileIO.c:     ShortGetNextNumber(num=0x%lx)\n",num);
#endif

   return(GetNextNumber(num, &FileBuffer[0], &FileIndex, FileNumber));
}

/****i* PCWindow/ReadKeyTable ******************************************
*
*   NAME   
*
*   SYNOPSIS
...
VOID ReadKeyTable()
{
...
#if (DEBUG1 & DBG_READ_KEY_TABLE_ENTER)
   kprintf("FileIO.c:     ReadKeyTable(VOID)\n");
#endif
   ...
   {
      MyAlert(ALERT_BAD_KEYS_FILE, NULL);
      return;
   }
   ...
   if (ShortGetNextNumber(&num) == FALSE)
   {
      MyAlert(ALERT_OLD_KEYSFILE, NULL);
      goto CLOSE_RETURN;
   }
   PCKeypad0Code = num;
...
VOID ReadScanCodeTable()
{
...
#if (DEBUG1 & DBG_READ_SCAN_CODE_TABLE_ENTER)
   kprintf("FileIO.c:     ReadScanCodeTable(VOID)\n");
#endif
   ...
   {
      MyAlert(ALERT_BAD_SCANCODE_FILE, NULL);
      return;
   }
...
{
#if (DEBUG1 & DBG_SHORT_WRITE_ENTER)
   kprintf("FileIO.c:     ShortWrite(text=0x%lx,display=0x%lx)\n",text,display);
#endif
...
struct   Display *display;
{
#if (DEBUG1 & DBG_S_PRINTF_SHORT_WRITE_ENTER)
   kprintf("FileIO.c:     SPrintfShortWrite(text=0x%lx,data=0x%lx,display=0x%lx)\n",text,data,display);
#endif
...
#if (DEBUG1 & DBG_READ_SETTINGS_GRUNT_ENTER)
   kprintf("FileIO.c:     ReadSettingsGrunt(display=0x%lx,reporterrors=0x%lx)\n",display,reporterrors);
#endif

   returnval = FALSE;

   if (display)
      if (NOT MyLock(&display->DisplayLock)) goto RETURN;
...
#if (DEBUG3 & DBG_READ_SETTINGS_GRUNT_SIZES)
   kprintf("FileIO.c:     PresetMonoWidth    =%ld\n",Settings.PresetMonoWidth);
   kprintf("FileIO.c:     PresetMonoHeight   =%ld\n",Settings.PresetMonoHeight);
   kprintf("FileIO.c:     PresetMonoTopEdge  =%ld\n",Settings.PresetMonoTopEdge);
   kprintf("FileIO.c:     PresetMonoLeftEdge =%ld\n",Settings.PresetMonoLeftEdge);
   kprintf("FileIO.c:     PresetMonoDepth    =%ld\n",Settings.PresetMonoDepth);
   kprintf("FileIO.c:     PresetColorWidth   =%ld\n",Settings.PresetColorWidth);
   kprintf("FileIO.c:     PresetColorHeight  =%ld\n",Settings.PresetColorHeight);
   kprintf("FileIO.c:     PresetColorTopEdge =%ld\n",Settings.PresetColorTopEdge);
   kprintf("FileIO.c:     PresetColorLeftEdge=%ld\n",Settings.PresetColorLeftEdge);
#endif
...
struct   Display *display;
{
...
#if (DEBUG1 & DBG_WRITE_SETTINGS_FILE_ENTER)
   kprintf("FileIO.c:     WriteSettingsFile(display=0x%lx)\n",display);
#endif
   ...
   /* Read Current File */
   /*********************/
   if(!ReadSettingsGrunt(display, FALSE))
   {
      Settings.DefaultColorTextDepth = 4;
	  ...
	  */
      ...
      Settings.PresetMonoWidth       = 640;
      Settings.PresetMonoHeight      = 200;
      Settings.PresetMonoTopEdge     = 0;
      Settings.PresetMonoLeftEdge    = 0;
      Settings.PresetMonoDepth       = 1;
      Settings.PresetColorWidth      = 640;
      Settings.PresetColorHeight     = 200;
      Settings.PresetColorTopEdge    = 0;
      Settings.PresetColorLeftEdge   = 0;
      Settings.PresetMonoFlags       = 0;
      Settings.PresetColorFlags      = 0;
   }
   ...
   for (i = 0; i < 2; i++)
   {
      SPrintfShortWrite("$%04lx ",TextOnePlaneColors[i], display);
   }

   ShortWrite("\n; Text Two Plane:\n   ", display);
   for (i = 0; i < 4; i++)
   {
      SPrintfShortWrite("$%04lx ",TextTwoPlaneColors[i], display);
   }

   ShortWrite("\n; Text Three Plane:\n   ", display);
   for (i = 0; i < 8; i++)
   {
      SPrintfShortWrite("$%04lx ",TextThreePlaneColors[i], display);
   }

   ShortWrite("\n; Text Four Plane:\n   ", display);
   for (i = 0; i < 8; i++)
   {
      SPrintfShortWrite("$%04lx ",TextFourPlaneColors[i], display);
   }
   ...
   for (i = 0; i < 2; i++)
   {
      SPrintfShortWrite("$%04lx ",HighGraphicsColors[i], display);
   }
   ...
   for (i = 0; i < 2; i++)
      for (i2 = 0; i2 < 4; i2++)
      {
         SPrintfShortWrite("$%04lx ",LowGraphicsColors[0][i][i2], display);
      }
   ...
   for (i = 0; i < 2; i++)
      for (i2 = 0; i2 < 4; i2++)
      {
         SPrintfShortWrite("$%04lx ",LowGraphicsColors[1][i][i2], display);
      }
...
/* The display argument can be NULL if there's no display for this fetch */
{
...
#if (DEBUG1 & DBG_READ_SETTINGS_FILE_ENTER)
   kprintf("FileIO.c:     ReadSettingsFile(display=0x%lx,reporterrors=0x%lx)\n",display,reporterrors);
#endif
   ...
   for (i = 0; i < 2; i++)
      for (i2 = 0; i2 < 4; i2++)
         LowGraphicsColors[0][i][i2] = Settings.LowGraphicsColors[0][i][i2];

   /* Low Graphics Intensified */
   for (i = 0; i < 2; i++)
      for (i2 = 0; i2 < 4; i2++)
         LowGraphicsColors[1][i][i2] = Settings.LowGraphicsColors[1][i][i2];
...
#if (DEBUG3 & DBG_READ_SETTINGS_FILE_SETTINGS)
   PrintSettings();
#endif
...
VOID PrintSettings()
{
   kprintf("Settings.DefaultColorTextDepth=%ld\n",Settings.DefaultColorTextDepth);
   kprintf("Settings.PresetMonoWidth      =%ld\n",Settings.PresetMonoWidth);
   kprintf("Settings.PresetMonoHeight     =%ld\n",Settings.PresetMonoHeight);
   kprintf("Settings.PresetMonoTopEdge    =%ld\n",Settings.PresetMonoTopEdge);
   kprintf("Settings.PresetMonoLeftEdge   =%ld\n",Settings.PresetMonoLeftEdge);
   kprintf("Settings.PresetMonoDepth      =%ld\n",Settings.PresetMonoDepth);
   kprintf("Settings.PresetColorWidth     =%ld\n",Settings.PresetColorWidth);
   kprintf("Settings.PresetColorHeight    =%ld\n",Settings.PresetColorHeight);
   kprintf("Settings.PresetColorTopEdge   =%ld\n",Settings.PresetColorTopEdge);
   kprintf("Settings.PresetColorLeftEdge  =%ld\n",Settings.PresetColorLeftEdge);
}
#endif
