...
;------ allocate the parameter memory and track buffer for jdisk.device ----
                moveq   #HardDskReq_SIZEOF,d0  ; get a diskrequest
                ...
                move.w  d0,hdr_BufferAddr(a2)   ; set up diskreq buffer
...
;------ find out how many partitions there are on the PC hard disk(s) ------
                ...
                move.w  #ADR_FNCTN_INIT,hdr_Fnctn(a4)
                moveq   #JSERV_HARDDISK,D0
...
;------ now get info for each partition on the hard disk(s) ----------------
                ...
                moveq   #HDskPartition_SIZEOF,d0 ; get a partition struct
                ...
                move.w  d0,hdr_BufferAddr(a4)   ; link into disk part struct
                move.w  #ADR_FNCTN_INFO,hdr_Fnctn(A4)
                clr.l   jd_Units(a5)            ; nothing found yet

;------ loop for each partition (d2 holds the number of partitions - 1) -----
SizeLoop        move.l  #HDskPartition_SIZEOF,hdr_Count(a4)
                move.w  d2,hdr_Part(a4)         ; get info on this partition
                moveq   #JSERV_HARDDISK,d0
                CALLLVO SendJanusInt
                move.l  ss_SigMask(a3),d0
                move.l  jd_ExecBase(a5),a6
                CALLLVO Wait                    ; wait for PC to reply
                tst.w   hdr_Err(a4)             ; did it work OK ?
...
30$             movea.l d0,a0
                ...
                move.w  hdp_EndCyl(a2),d0        ; calculate where ...
                sub.w   hdp_BaseCyl(a2),d0       ; ... bad block map may be
                subq.w  #1,d0                   ; it takes up 2 cylinders
                move.w  hdp_NumSecs(a2),d1       ; get offset to bad blocks
                mulu.w  hdp_NumHeads(a2),d1      ; d0 contains #cylinders
                ...
                move.w  hdp_NumSecs(a2),d1       ; how many blocks in bad area
                mulu.w  hdp_NumHeads(a2),d1
...
;------ we don't need partition info any more ------------------------------
                ...
                moveq.l #HDskPartition_SIZEOF,d0
...
;------ get bad block info from each partition (if it's there) -------------
                ...
                CALLLVO JanusMemToOffset        ; convert physical to offset
                move.w  d0,hdr_BufferAddr(a4)   ; set up diskreq buffer
...
GetBadBlocks    move.w  BB_Unit(a2),hdr_Part(a4) ; this partition
                moveq.l #16-1,d2                ; retry count
GetUnitBad      move.w  #ADR_FNCTN_READ,hdr_Fnctn(a4)
                move.l  #1024,hdr_Count(a4)     ; read 2 sectors
                move.l  BB_RealMapOffset(a2),hdr_Offset(a4)
