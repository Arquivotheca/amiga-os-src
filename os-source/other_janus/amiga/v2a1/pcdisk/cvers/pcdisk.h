 ...
 * not contain any information used exclusively by one side. Also long
 * declarations should not be used as the PC side will have a hard time
 * using them (PC is 16 bit ints and longs are stored low word|high word).
 ************************************************************************/

#define PCDISK_BUFFER_SIZE 8192

/* Disk request structure for higher level Amiga file request from 8086 */

struct AmigaDiskReq {
   short adr_Fnctn;              /* Function code (see below)           */
   short adr_File;               /* File number (filehandle for PC side)*/
   short adr_Offset_h;           /* High byte of offset into file       */
   short adr_Offset_l;           /* Low  byte of offset into file       */
   short adr_Count_h;            /* High byte of # of bytes to transfer */
                                 /* Actual number transfered on return  */
   short adr_Count_l;            /* Low byte as above                   */
   short adr_BufferOffset;       /* Offset into MEMF_BUFFER memory      */
   short adr_Err;                /* Return code (see below) 0 if all OK */
};
