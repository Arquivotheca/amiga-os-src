...
AJR_REGS   REG   D3-D4/A2-A4
   ...
   ENDC
      ...
      MOVE.W   D0,jr_Offset(A2)
      MOVE.W   D3,jr_Size(A2)      ; Get the user's size
      MOVE.W   D4,jr_Type(A2)      ; Get the user's type 
      ...
      BRA   20$
10$      MOVE.W   #-1,D0
20$      MOVE.W   D0,jr_NextRemember(A2)
      MOVE.L   A2,(A4)
...
*    - if we're freeing all memory or offset and masked-type equal ours then
      ...
      CMP.W   jr_Offset(A0),D2
      BNE   FSM_NEXT      ; If offsets not equal, branch
      MOVE.W   jr_Type(A0),D0
...
*          - goto FSM_DONE
      CMP.L   #0,A4
      BNE   10$
      MOVE.W   jr_NextRemember(A0),sd_JRememberKey(A3)
      BRA   20$
10$      MOVE.W   jr_NextRemember(A0),jr_NextRemember(A4)
20$

      MOVE.L   A0,-(SP)      ; Stash JanusRemember.WA

      ;------   Free the memory first
      MOVE.W   jr_Offset(A0),D0
      MOVE.W   jr_Type(A0),D1
      CALLSYS   JanusOffsetToMem
      MOVE.L   D0,A1
      MOVEQ.L   #0,D0
      MOVE.L   (SP),A0         ; Get copy of addr of JRem.WA
      MOVE.W   jr_Size(A0),D0
