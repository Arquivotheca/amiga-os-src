...
SERVICE_IO_FIELDS	EQU	4	; The number of channel IO fields 
 ...
 IFD	CHANNELING
	...
	STRUCT 	sp_PCToAmiga,SERVICE_IO_FIELDS	; Set by PC, cleared by Amiga 
	STRUCT 	sp_AmigaToPC,SERVICE_IO_FIELDS	; Set by Amiga, cleared by PC 
	...
	; You don't need to lock before accessing these fields.
	UBYTE 	sp_PCAddsService
	UBYTE 	sp_AmigaAddsService
 ...
 IFND	CHANNELING
	...
	; to send before the other can get around to receiving them.
	; You don't need to lock before accessing these fields.
	STRUCT 	sp_PCToAmiga,SERVICE_IO_FIELDS*2 ; Set by PC, unset by Amiga 
	STRUCT 	sp_AmigaToPC,SERVICE_IO_FIELDS*2 ; Set by Amiga, unset by PC 
	...
	; You don't need to lock before accessing these fields.
	STRUCT	sp_PCAddsService,2*2
	STRUCT	sp_PCDeletesService,2*2
	STRUCT	sp_AmigaAddsService,2*2
	STRUCT	sp_AmigaDeletesService,2*2
 ENDC	; of IFD CHANNELING
	...
	RPTR	sp_FirstServiceData


	STRUCT	sp_CyclopsReserved,4*4
...
* channel
* STRUCTURE PREFIX sc_

 STRUCTURE ServiceCustomer,0

	APTR	sc_NextCustomer

	USHORT 	sc_Flags

	APTR	sc_Task		; This points to the task of the customer 
	ULONG 	sc_SignalBit	; Signal the customer with this bit 

	STRUCT 	sc_JazzReserved,4*4
