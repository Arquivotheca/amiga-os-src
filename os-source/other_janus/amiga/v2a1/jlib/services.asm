...
ClearParam:   MOVE.B   #0,(A0)+
      ...
      MOVEA.L   D0,A0
      MOVEQ.L   #-1,D1
      MOVE.W   D1,sp_FirstServiceData(A0) ; No service data
 IFD   CHANNELING
      ;------   The following presumes that the first 16 channels 
      ;------   are reserved, which indeed they are
      MOVE.W   D1,sp_ChannelMasks(A0)
 ENDC   ; of IFD CHANNELING
 IFND   CHANNELING
      ;------   The following presumes that the AmigaToPC array is 
      ;------   comprised of 4 contiguous 16-bit fields
      MOVE.L   D1,sp_AmigaToPC(A0)
      MOVE.L   D1,sp_AmigaToPC+4(A0)
      ;------   The following presumes that the PCToAmiga array is 
      ;------   comprised of 4 contiguous 16-bit fields
      MOVE.L   D1,sp_PCToAmiga(A0)
      MOVE.L   D1,sp_PCToAmiga+4(A0)
      ;------   The following presumes that the Adds and Deletes arrays 
      ;------   are comprised of 2 contiguous 16-bit fields each
      MOVE.L   D1,sp_AmigaAddsService(A0)
      MOVE.L   D1,sp_AmigaDeletesService(A0)
      MOVE.L   D1,sp_PCAddsService(A0)
      MOVE.L   D1,sp_PCDeletesService(A0)
 ENDC   ; of IFND CHANNELING
      ;------   Initialize the Lock
      CALLSYS   MakeBytePtr
      MOVE.B   #$7F,sp_Lock(A0)
...
PC_CHANNELCHECK:
 IFD   CHANNELING
* - find a non-zero channel number in PCToAmiga[]
      JSR   FindPCToA
      CMPA.L   #0,A0
      BEQ   PC_DONE
      MOVE.B   (A0),D2
...
* - if ADD_SERVICE
*    - Do ADD_SERVICE routine
      CMP.B   #ADD_SERVICE,D2
      BNE   10$
      JSR   ProcessADD
      BRA   PC_CHANNELCHECK
10$
...
* - else if DELETE_SERVICE
*    - Do DELETE_SERVICE routine
      CMP.B   #DELETE_SERVICE,D2
      BNE   20$
      JSR   ProcessDELETE
      BRA   PC_CHANNELCHECK
20$
...
*   task isn't the current one, send a signal
 ...
 IFND   CHANNELING   ; On the other hand, if CHANNELING is not defined
...
* - find a non-zero ServiceData offset (if any) in PCToAmiga[]
   ...
   MOVEM.L D0-D3,-(SP)
   MOVEQ.L #0,D0
   MOVE.W   sp_PCToAmiga(A5),D0
   MOVEQ.L #0,D1
   MOVE.W   sp_PCToAmiga+2(A5),D1
   MOVEQ.L #0,D2
   MOVE.W   sp_PCToAmiga+4(A5),D2
   MOVEQ.L #0,D3
   MOVE.W   sp_PCToAmiga+6(A5),D3
   MOVEM.L D0-D3,-(SP)
   MOVE.L   A0,-(SP)
...
*    - InformDeletedService(ServiceData)
      MOVE.W   sp_FirstServiceData(A5),D0
...
* === =========================================================================
* ===  Here end the Channel-Specific  Functions ===============================
* === =========================================================================
 ENDC   ; of IFD CHANNELING
...
* === =========================================================================
* ===  Here end the non-Channel-Specific  Functions ===========================
* === =========================================================================
 ENDC   ; of IFND CHANNELING
