...
*   if it isn't then checking for a match with ApplicationID and LocalID
      MOVE.W   sp_FirstServiceData(A3),A3
      BRA   CHECK_END
...
10$      MOVE.L   #JSERV_DUPSERVICE,A_D0_OFFSET(SP)
...
*       - if user's ADDS_LOCKDATA flag is set, lock the 
*         ServiceDataLock now
...
; NOPE:  don't do this here, as AllocServiceCustomer() takes care of it for us
; NOPE      MOVEA.L   A4,A0
; NOPE      CALLSYS   MakeBytePtr
; NOPE      MOVE.B   #1,sd_UserCount(A0)
...
*    - initialize and add the new ServiceCustomer to the ServiceData list
      MOVE.L   sd_FirstAmigaCustomer(A4),sc_NextCustomer(A0)
...
ADDS_add:
      MOVE.L   A4,A0
      CALLSYS   MakeBytePtr
      MOVE.B   sd_ChannelNumber(A0),sp_AmigaAddsService(A3)
      JSR   FindUnusedAToPC
 ...
 MOVE.L D0,-(SP)
 MOVE.L   A0,-(SP)
 PUTMSG   59,<'%s/Unused Amiga To PC = $%lx = $%02lx'>
 LEA   2*4(SP),SP
 ENDC

      MOVE.B   #ADD_SERVICE,(A0)
...
ADDS_wait:
      CMP.W   #-1,sp_AmigaAddsService(A3)
      BEQ   ADDS_add0
      CMP.W   #-1,sp_AmigaAddsService+2(A3)
      BEQ   ADDS_add1
   PUTMSG   2,<'%s/still waiting to ADD_SERVICE...'>
      ;------   Call WaitTOF() to let the system run while we're 
      ;------   waiting to check again
...
ADDS_add0:
      MOVE.L   A4,D0
      CALLSYS   JanusMemToOffset
      MOVE.W   D0,sp_AmigaAddsService(A3)
...
* - if janus memory allocated, free
   PUTMSG   2,<'AddService error'>
      ...
      ;------   set ServiceData's UserCount back to 0
      MOVEA.L   A4,A0
      CALLSYS   MakeBytePtr
      MOVE.B   #0,sd_UserCount(A0)
...
;NOSEMS      MOVEA.L   ja_ServiceBase(A6),A3
;NOSEMS      LEA   sb_ServiceSemaphore(A3),A0
      EXG.L   A6,A2
...
*    - increment ServiceData UserCount
      ADDQ.B   #1,sd_UserCount(A3)

*    - add ServiceCustomer to ServiceData list of customers
      MOVE.L   sd_FirstAmigaCustomer(A5),sc_NextCustomer(A4)
