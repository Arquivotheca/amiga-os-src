...
RELTASK_LOOP:   CMPA.L   #0,A1      ; Did we reach the end of the Customers?
      ...
      CMP.L   sc_Task(A1),D0   ; Compare customer's task with caller's
      BEQ   RELTASK_FOUND   ; If equal, this is the one we want!
      MOVEA.L   A1,A3      ; else save this struct address 
      MOVEA.L   sc_NextCustomer(A1),A1 ; get the next struct 
...
*    - unlink the ServiceCustomer structure
      ...
      MOVE.L   sc_NextCustomer(A1),sd_FirstAmigaCustomer(A4)
      BRA   20$
10$      MOVE.L   sc_NextCustomer(A1),sc_NextCustomer(A3)
20$
...
* - if we're the Amiga
*    - lock the ServiceBase semaphore
;NOSEMS      MOVEA.L   ja_ServiceBase(A6),A0
;NOSEMS      LEA   sb_ServiceSemaphore(A0),A0
      EXG.L   A6,A2
;NOSEMS      CALLSYS   ObtainSemaphore
      CALLSYS   Forbid
...
* If the UserCount is 0, we are going to 
*     o  expunge the service
*     o  unlock the ServiceParam
*     o  continue
* else if the UserCount != 0
*     o  unlock the ServiceParam
*     o  signal the PC
*     o  continue
...
*    - unlock the ServiceParam lock
 ...
 IFD   CHANNELING
* - signal the other processor with DELETED_SERVICE
      JSR   FindUnusedAToPC
      MOVE.B   #DELETE_SERVICE,(A0)
 ENDC   ; of IFD CHANNELING
 IFND   CHANNELING
...
DELETES_wait:
      CMP.W   #-1,sp_AmigaDeletesService(A3)
      BEQ   DELETES_del0
      CMP.W   #-1,sp_AmigaDeletesService+2(A3)
      BEQ   DELETES_del1
...
DELETES_del0:
      MOVE.L   A4,D0
      CALLSYS   JanusMemToOffset
      MOVE.W   D0,sp_AmigaDeletesService(A3)
 ENDC   ; of IFND CHANNELING
