...
struct Task *MyCreateTask( UBYTE *name, UBYTE priority, APTR initialPC, ULONG stacksize)
{
   struct Task *taskptr;
   ULONG realsize;
   UBYTE *stackptr;
...
VOID DrainPort(struct MsgPort *port)
...
VOID FixPropPots(struct Gadget *propgadget)
{
   ...
   freeplay -= (((struct PropInfo *)propgadget->SpecialInfo)->LeftBorder*2L);
    ...
    */
   if ((start == freeplay) || (freeplay == 0L)) 
      pot = MAXPOT;
   else 
      pot = (((LONG)MAXPOT * start) + (freeplay >> 1L)) / freeplay;
   ...
   freeplay -= (((struct PropInfo *)propgadget->SpecialInfo)->TopBorder * 2L);
...
SHORT CreateSignalNumber(ULONG mask)
{
   SHORT i;
...
VOID GetPCDisplay(struct Display *display)
{
...
VOID SetTimer(ULONG seconds,ULONG  micros, struct IOStdReq *timermessage)
{
...
VOID DrawBox(struct RastPort *rport,SHORT x,SHORT y,SHORT width,SHORT height,SHORT pen,SHORT hthick,SHORT vthick)
{
...
VOID PALBottomBorder( struct Window *w,struct Display *display)
{
...
VOID ModifyDisplayProps(struct Display *display)
{
   ULONG hbody, vbody;
...
#endif
         hbody = (((ULONG)(window->Width-window->BorderRight-window->BorderLeft))*65535L)/
		         (FlagIsSet(display->Modes,MEDIUM_RES)?320L:640L);
         vbody = (((ULONG)(window->Height-window->BorderTop-window->BorderBottom))*65535L)/ZAPHOD_HEIGHT;
...
SHORT TextXPos(struct Display *display,SHORT x)
{
   SHORT i;
   struct Window *window;
...
SHORT TextYPos(struct Display *display,SHORT y)
{
   SHORT i;
   struct Window *window;
...
VOID XAddGadget(struct Window *window,struct Gadget *gadget, SHORT pos)
