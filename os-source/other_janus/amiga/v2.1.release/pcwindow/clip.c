...
static struct Remember *ClipKey = NULL;
static UBYTE NullString = '\0';
static BOOL CBOpened = FALSE;
 ...
 */
{
   ...
   length = strlen(text) + 1; /* plus one for the trailing NULL */

   if (AllocRemember(&ClipKey, length, MEMF_CLEAR))
      {
      /*CopyString(ClipKey->Memory, text);*/
      strcpy(ClipKey->Memory, text);
      }
