...
struct ReqSupport
	 ...
	 */
	LONG (*StartRequest)(VOID);		/* Called after the requester is created */
	LONG (*GadgetHandler)(struct Gadget *g,SHORT x,SHORT y,LONG s,LONG m);	/* Called with each GADGETUP event */
	LONG (*NewDiskHandler)(VOID);	/* Called with each DISKINSERTED event */
	LONG (*KeyHandler)(VOID);		/* Called with each VANILLAKEY event */
	LONG (*MouseMoveHandler)(VOID); /* Called when the mouse is seen to move */
