...
struct Display *display;
{
	...
	if ((displaymodes & TEXT_MODE) && (width == 640) && (depth == 2)
		...
		{
		screen = GetExtraScreen(display, superwindow);
...
OpenDisplayWindow(display, port, idcmp)
struct Display *display;
struct MsgPort *port;
ULONG idcmp;
 ...
 */
{
	...
	if (FlagIsClear(superwindow->Flags, HEIGHT_INITED))
		...
		if (FlagIsClear(display->Modes, PAL_PRESENCE))
			newwindow->MaxHeight = newwindow->Height = ZAPHOD_HEIGHT;
		else newwindow->MaxHeight = newwindow->Height
