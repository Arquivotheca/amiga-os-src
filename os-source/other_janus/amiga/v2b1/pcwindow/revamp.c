...
RevampDisplay(display, newmodes, forceAll, borderForceNewWindow)
 ...
 */
{
	...
	if (forceAll) revamp = TRUE;
	else
		{
		revamp = FALSE;
/*BBB		if (superwindow && (display->Modes & TEXT_MODE))*/
		if (superwindow && FlagIsSet(newmodes, TEXT_MODE))
			...
			{
			if (FlagIsSet(newmodes, MEDIUM_RES)) i = 2;
			else i = 1;
			if (i != superwindow->DisplayDepth) revamp = TRUE;
			}
	...
	if (superwindow)
		...
		if (revamp) OpenDisplay(display);

		if (redowindow)
			OpenDisplayWindow(display, saveport, saveidcmp);
		}	
	else
		{
