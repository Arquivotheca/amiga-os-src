...
InformNewCustomers:
* === InformNewCustomers() ====================================================
* VOID InformNewCustomers(ServiceData.WA, OneIfAdd.BYTE)
*                         A0              D0.B
* Assembly language expects JanusBase in A6
* 
* 
...
SC_LOOP:	CMPA.L	#0,A3
	...
	ENDC
		...
		ADDQ.B	#1,D2
SC_SIGNALLED:	MOVEA.L	scs_NextCustomer(A3),A3
		BRA	SC_LOOP
...
*    - D0 has 8 bits of a non-zero channel number, or 0 if failure
		...
		CALLSYS	MakeBytePtr
		LEA	spm_ChannelMasks(A0),A0	; Get address of channel bytes
...
* 
		...
		MOVEA.L	jb_ServiceBase(A6),A0	; A0 points to ServiceBase
		MOVE.L	sb_ServiceParam(A0),A0	; Get address of ServiceParam
		;------	sb_ServiceParam is a 
		;------	word-access pointer, but 
		;------	we want bytes below, so switch
...
*   if it isn't then checking for a match with ChannelNumber
		...
		MOVE.W	spm_FirstServiceData(A5),A3
