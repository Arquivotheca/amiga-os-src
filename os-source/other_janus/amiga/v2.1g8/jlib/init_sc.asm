...
Init:
      ...
      IFGE     INFOLEVEL-1
       MOVE.L   MY_VERSION,-(SP)
       MOVE.L   MY_REVISION,-(SP)
...
;      move.w   #VERNUM,LIB_VERSION(a2)
      move.w   MY_VERSION,LIB_VERSION(a2)
	  move.w   MY_REVISION,LIB_REVISION(a2)
	...
	lea	prodcode(pc),a1
	move.b	cd_Rom+er_Product(a0),(a1)
      unlk     a5
...
*************************************************************************
; set up the A2386 stuff (if this is a 2386)

	movem.l	a0-a1,-(sp)

	lea.l	prodcode(pc),a0
	cmp.b	#103,(a0)
	bne	end2386

	; it is a 2386 - deal with it.

	lea	$1f9f(a1),a1		;point to magic 2386 config reg

; **** set up speaker

	move.b	prefsdata+1(pc),d0
	and.b	#$80,d0
	beq	spkr_on
	move.b	#$06,(a1)
	bra	end_spkr
...
; **** set up drive configuration

	move.b	prefsdata+1(pc),d0
	and.b	#$38,d0

;	cmp.b	#$00,d0			;int a, int b
	bne	try_drive1
...
try_drive6:
	cmp.b	#$30,d0			;shared a, ext b
	bne	do_drive0	;illegal!
...
;      MOVE.W   #REVNUM,ja_JLibRev(A3)
      move.w   MY_VERSION,ja_JLibVer(a3)
	  move.w   MY_REVISION,ja_JLibRev(a3)
...
; fix for PC/AT.  Load a different boot file if it's an AT
	...
	move.b	prodcode(pc),d0		;is it a 386?
	cmp.b	#103,d0
	bne	loadat			;no.
	move.b	prefsdata+1(pc),d0	;shadow enabled? 
	btst.b	#0,d0			;(bit 0==1 means yes)
	beq.s	loadat			;no.

	lea.l	BootNameShadow(pc),a0	;yes - load shadowed verison
      move.l   #MODE_OLDFILE,d2		;does shadowed version exist?
      move.l   jb_DOSBase(a2),a6                ; pointer to DOS library
      CALLSYS  Open
      move.l   d0,d1                            ; save open handle
      beq.s    loadat			;no.  load the at version.
...
loadit:
      ...
      cmpi.w   #$d400,jb_ATOffset(a2)           ; maybe we need offset
...
;;;;
	move.b	prefsdata(pc),d0		;user have load disabled?
	and.b	#$60,d0				;(bits 6,5 == 0)
	beq	readNothing			;yes.
;;;;
...
GetPrefs
	...
	lea	prefsdata(pc),a0
      move.w   #$fe00,(a0)                     ; put default value for XT
                                                ; on the stack
	...
	lea	prefsdata(pc),a0
      move.w   #$de00,(a0)                      ; AT, use AT default
...
XTPrefs
	...
	lea	prefsdata(pc),a0
      move.l   a0,d2                            ; read to stack
      moveq.l  #2,d3                            ; only 1 byte to read
      move.l   d4,d1
      CALLSYS  Read
...
;;; 1060
	and.b	#$fe,(a0)	;disable the evil serial port
;;;

      move.b   (a0),d0                          ; return correct value
...
BootName:      dc.b  'sys:pc/system/pc.boot',0
      CNOP     0,2
BootNameShadow:      dc.b  'sys:pc/system/pc.boot.2386sx',0
      CNOP     0,2
