...
struct Screen wbscr;
    ...
    if (GetScreenData((APTR)&wbscr, sizeof(struct Screen), WBENCHSCREEN, NULL))	{
        FlipperWindow.Height += wbscr.BarHeight - 10;
	ManGadget.TopEdge += wbscr.BarHeight - 10;
	AmigaGadget->TopEdge += wbscr.BarHeight - 10;
	PCGadget->TopEdge += wbscr.BarHeight - 10;
	AutoGadget.TopEdge += wbscr.BarHeight - 10;
    }

	if (FlipperWindow.LeftEdge + FlipperWindow.Width > wbscr.Width)
		FlipperWindow.LeftEdge = wbscr.Width - FlipperWindow.Width;

	if (FlipperWindow.TopEdge + FlipperWindow.Height > wbscr.Height)
		FlipperWindow.TopEdge = wbscr.Height - FlipperWindow.Height;
}
