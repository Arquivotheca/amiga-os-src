...
{ 
    ...
    bodyText2.IText = string2;
    	SetWindowTitles(w, STITLE, (char *) -1);
    AutoRequest(w, &bodyText1, 0, &buttonText, 0, 0, RWIDTH, RHEIGHT);
	if (wt)
		SetWindowTitles(w, wt, (char *) -1);
}
...
static char abuf[58] = "  File Blocks Selected To Install[     ]                 ";
	...
	SetWindowTitles(w, (char *) -1, wt);
}


LONG UnpackNode(struct file_node *node, UBYTE *buf, int del)
{
LONG len, check;
...
LONG check,y;
	...
	check=UnpackNode(node, buf, del)*8;
