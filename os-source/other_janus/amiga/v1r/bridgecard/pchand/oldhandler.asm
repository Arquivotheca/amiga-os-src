...
getPkt:
	...
	ENDC
		...
		beq	closePacket
		cmpi.w	#ACTION_CREATE_DIR,d0
		beq	mkdirPacket
		cmpi.w	#ACTION_RENAME_OBJECT,d0
		beq	renamePacket
...
;---------- mkdirPacket ----------
mkdirPacket:
		move.l	dp_Arg2(a2),a0		name parameter
		bsr	GetName
		move.l	d0,d2
		beq	endPacket		badPacket
		move.l	d1,d3

		bsr	GetSegAddress

		move.l	l_PCCallData-L(a5),a0
		move.w	#$3900,s86_AX(a0)
		move.w	d0,s86_DS(a0)
		swap	d0
		move.w	d0,s86_DX(a0)
...
;---------- renamePacket ----------
renamePacket:
		move.l	dp_Arg2(a2),a0		from name parameter
		bsr	GetName
		move.l	d0,d2
		beq	endPacket		badPacket
		move.l	d1,d3
		...
		move.l	l_PCCallData-L(a5),a0
		move.w	#$5600,s86_AX(a0)
		move.w	d0,s86_DS(a0)
		swap	d0
		move.w	d0,s86_DX(a0)

		move.l	dp_Arg4(a2),a0		to name parameter
		bsr	GetName
		move.l	d0,d4
		beq.s	badRename		badPacket
		move.l	d1,d5
...
;
;   INPUT
;	a0	BPTR/BSTR w/ JF: prefix
;
;   RETURNS
;	d0	pointer to name, if zero then error
;	d1	length of name, or ERROR_NO_FREE_STORE if error
;
GetName:
		movem.l d2/a2,-(a7)
		...
		adda.l	a2,a2
		moveq	#0,d2
		move.b	(a2),d2 	; length of JF:<JFileName>
		subq	#2,d2		; length of \0 terminated remainder

		move.l	d2,d0
		moveq	#MEMF_PARAMETER+MEM_BYTEACCESS,d1
		move.l	l_JanusBase-L(a5),a6
		CALLLVO AllocJanusMem
		tst.l	d0
		beq	getNameFail

		addq.l	#4,a2		; skip the (count),"JF:" part
		move.l	d0,a0
		move.l	d2,d1
		subq	#1,d2		; don't copy \0
		bra.s	fnameDBF
fnameCopy:
		move.b	(a2)+,(a0)+
fnameDBF:
		dbf	d2,fnameCopy
		clr.b	(a0)		; insert null terminator
...
;
;   INPUT
;	d0	Amiga pointer to PARAMETER memory
;
;   RETURNS
;	d0	Segment/Offset in low/high word
;
GetSegAddress:
	IFGE	INFOLEVEL-5
	move.l	d0,-(sp)
	INFOMSG	5,<'GetSegAddress: Amiga Ptr = $%lx'>
	addq.l	#4,sp
	ENDC
		move.l	l_JanusBase-L(a5),a6
		move.l	ja_ParamMem(a6),a0
		add.l	#WordAccessOffset,a0
		sub.l	jb_ParamMem+jmh_68000Base(a0),d0
		swap	d0
		move.w	jb_ParamMem+jmh_8088Segment(a0),d0
	IFGE	INFOLEVEL-5
	move.l	d0,-(sp)
	INFOMSG	5,<'               PC Seg|Offset = $%lx'>
	addq.l	#4,sp
	ENDC
		rts
...
Do86Function:
	...
	IFGE	INFOLEVEL-5
	INFOMSG	5,<'====Start of Do86Function===='>
	move.w	s86_DX(a0),-(sp)
	move.w	s86_CX(a0),-(sp)
	move.w	s86_BX(a0),-(sp)
	move.w	s86_AX(a0),-(sp)
	...
	ENDC
		...
		move.l	ss_SigMask(a0),d0
		move.l	_AbsExecBase,a6
	INFOMSG 20,<'Wait  (Do86Function)'>
		CALLLVO Wait
	INFOMSG 20,<'Awake (Do86Function)'>
	INFOMSG	5,<'==== PC returned the following ===='>
		move.l	l_PCCallData-L(a5),a0
...
ReturnPkt:
		cmpa.w	#0,a2
		beq.s	rpRts		; no packet, no work

	IFGE	INFOLEVEL-50
	move.l	d1,-(a7)
	move.l	d0,-(a7)
	INFOMSG 1,<'ReturnPkt res1: %ld, res2: %ld'>
	addq.l	#8,a7
	ENDC
		move.l	d0,dp_Res1(a2)	; cache results
		move.l	d1,dp_Res2(a2)	;

		move.l	(a2),d1 	; check message address
		ble.s	rpErr

		move.l	4(a2),a0	; get port for PutMsg
		;------ get process/task message port
		lea	pr_MsgPort(a4),a1
		move.l	a1,4(a2)	; cache this port
		move.l	d1,a1
		move.l	a2,LN_NAME(a1)	; cache packet in message name
		clr.l	LN_SUCC(a1)
		clr.l	LN_PRED(a1)
		move.l	_AbsExecBase,a6
		CALLLVO PutMsg
