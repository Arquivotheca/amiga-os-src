...
ReDoRefresh:
		...
		move.b  #$fe,jio_PCconfiguration(a1)
...
;****		clr.b	jr_Reset(a2)	    ; **** NEVER USED ****

refreshGoing:
		add.l	#IoRegOffset+IoAccessOffset,a0
		move.l	#250000,d1		; timeout
		move.b	jio_Lpt1Data(a0),d0	; make sure PC is really
10$		cmp.b	jio_Lpt1Data(a0),d0	; going for sure
		bne.s	ReallyGoing
		subq.l	#1,d1
		bne.s	10$
		suba.l	#IoRegOffset+IoAccessOffset,a0
		bra.s	ReDoRefresh		; something wrong
...
ReBoot8088:
	INFOMSG	1,<'Trying to boot pc.boot'>
		...
		lea	janusBootName(pc),a0	; get filename
		move.l	a0,d1
	...
	INFOMSG	1,<'File opened OK'>
		...
		move.l	d0,d5			; stash number of bytes
readError:
