...
* - initialize the ServiceDataLock to $7F
...
10$		DBRA	D1,5$
	...
	PUTMSG	59,<'%s/ExpungeService done'>
		MOVEA.L	(SP)+,A2
		RTS
...
* - if the PC_READY flag isn't set yet
	...
	PUTMSG	2,<'%s/PC not ready yet, so enter wait logic'>
...
WFPC_WAITLOAD:
	...
	PUTMSG	2,<'%s/PC not loaded yet, so wait 1 second'>
...
WFPC_LOADED:
	PUTMSG	2,<'%s/PC handler loaded, so wait 15 second'>

* 	- PC handler is loaded, so wait 15 seconds
		MOVE.L	#15*50,D1
		JSR	DelayWait

	PUTMSG	2,<'%s/Sure hope PC is ready!  Set AMIGA_PC_READY flag'>
