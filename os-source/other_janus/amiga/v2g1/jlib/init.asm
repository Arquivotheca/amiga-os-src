...
Init:		; ( d0: lib node, a0: seg list )
	...
	PUTMSG	1,<'%s / %ld.%ld'>
	LEA.L	2*4(SP),SP
	PUTMSG	1,<'%s / At or later than 10:31p California time, 24 Oct 1988'>
	PUTMSG	1,<'%s/Init: called'>
	ENDC

		;------ do some low level initialization
		move.l	d0,a2
		move.l	a0,jb_SegList(a2)
		...
		move.l	a6,jb_ExecBase(a2)

		;------ open the DOS
		lea	dosLibName(pc),a1
		moveq	#0,d0
		CALLSYS OpenLibrary
		move.l	d0,jb_DOSBase(a2)
		beq	errInit1
...
refreshGoing:
	INFOMSG	90,<'OK, AT/XT has woken up at last!'>
		;------ set up the various base pointers
ReallyGoing	move.l	d2,jb_ExpanBase(a2)
...
; START OF BILL'S 28-Mar-88 FIX
		...
		add.l    #WordAccessOffset,a0       ; access!
...
XTPrefs		movea.l	jb_DOSBase(a2),a6	get DOS library
...
XTBootName:	dc.b	'sys:pc/system/pc.boot',0
		CNOP	0,2
; ATBootName:	dc.b	'sys:sidecar/at.boot',0
; ATBootName:	dc.b	'sys:sidecar/pc.boot',0
ATBootName:	dc.b	'sys:pc/system/pc.boot',0
		CNOP	0,2
