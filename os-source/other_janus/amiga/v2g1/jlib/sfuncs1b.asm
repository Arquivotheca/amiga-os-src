...
*    D6 = internal flags
*    A2 = ExecBase
*    A3 = if A5 has a ServiceData.WA, A3 has address of ServiceData.BA
...
*   though it may already be requested (as described by whether or not 
*   a ServiceData was found).  Either no ServiceData was found, or a 
*   ServiceData was found but both UserCounts were zero
* - if the caller hasn't specified GETS_WAIT, then the caller is unwilling 
*   to wait around for the service so goto GETSERVICE_ERROR 
*   to return the error JSERV_NOSERVICE
...
20$      ;------   GETS_WAIT set
* - else since the caller has specified GETS_WAIT, the caller is willing 
*   to wait around for a pending service 
* - if ServiceData not found, create a ServiceData structure at this time 
...
*    - for simplicity, lock the ServiceData here which will allow us to 
*      always unlock it below whether error return or not
*       - LockServiceData()
      MOVEA.L   A5,A0
      CALLSYS   LockServiceData
...
ERROR_END:
* - forget we ever knew anything about any ServiceData structure (set 
*   the pointer to NULL, or whatever.  it's important below)
      MOVEA.L   #0,A5
...
* - if we're returning successfully with a valid ServiceData pointer
      CMPA.L   #0,A5
      BEQ   nullSD
...
* - if we're returning successfully with a ServiceData that has its 
*   SERVICE_ADDED flag set, go triumphantly to GETSERVICE_EXIT
...
* - else we didn't find an existing service.  we found either a pending 
*   service (hasn't been added yet) and we are a customer, or we 
*   have no ServiceData at all.  let's see if the caller wants us to take 
...
*      if Timeout hits zero, we give up
      ...
      MOVE.W   D3,D0
      ANDI.W   #GETS_AUTOWAITmask,D0
      BNE   10$
      MOVEQ.L   #0,D7
      BRA   40$
10$      CMP.W   #GETS_AUTOWAIT_15,D0
      BNE   20$
      MOVE.W   #15,D7
      BRA   40$
20$      CMP.W   #GETS_AUTOWAIT_30,D0
      BNE   30$
      MOVE.W   #30,D7
      BRA   40$
30$      MOVE.W   #120,D7
...
*    - if the caller has requested amiga autoload (GETS_ALOAD_A flag) 
*      then let's try to ask the autoloader to do the job for us.  
*      we start by trying to GetService() the autoloader
...
aloadWait:
      SUBQ.W   #1,D7      ; Any time left
      BLE   GETSERVICE_EXIT   ; Branch if not
