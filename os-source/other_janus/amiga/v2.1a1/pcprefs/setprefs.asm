...
_main
      suba.w   #mydata_SIZEOF,sp   allocate stack data area
...
; It can be closed immediately after because we don't need it any more.
      ...
      move.l   jb_IoBase(a1),JIOBase(a5)
      jsr      _LVOCloseLibrary(a6)   finished with janus now

; The libraries we need all the time are intuition,DOS and graphics
...
; If the file doesn't exist then fill in default values in the state vars.
      ...
      beq      SetDefaults      no file, use defaults
...
; prefs were saved as one byte so expand them into four separate fields
      clr.w    d0
      move.b   SerialState(a5),d0
      andi.w   #%01111001,d0      mask out unwanted stuff
      clr.b    d1
      lsr.b    #1,d0
      roxl.b   #1,d1
      move.b   d1,SerialState(a5)   serial flag
      clr.b    d1
      lsr.b    #3,d0
      roxl.b   #1,d1
      move.b   d1,MonoState(a5)   mono flag
      clr.b    d1
      lsr.b    #1,d0
      roxl.b   #1,d1
      move.b   d1,ColorState(a5)   color flag
      move.b   d0,RamBank(a5)      ram bank in d0
...
SetDefaults
      move.b   #DEFSER,SerialState(a5)   default serial preference
...
CheckPrefs
      move.b   SerialState(a5),d0   first 3 must be 0 or 1
...
PrefsOK
      tst.b    SerialState(a5)      serial on or off
      beq.s    1$         it's off
...
10$
      lea.l    SerialGadget,a0      turn off serial option
      movea.l  PrefWindow(a5),a1
      suba.l   a2,a2
      jsr      _LVOOffGadget(a6)

      movea.l  PrefWindow(a5),a0   get msgport address
...
; 2 = save out the preferences and use because user selected SAVE
      ...
      beq      CloseIt         exit time

      ; Combine all values into one control byte for configuration register

UseIt
      move.b   RamBank(a5),d0      make RAM bits (5and6)
      asl.b    #5,d0
      ...
      move.b   d0,SerialState(a5)      in mem for writing
      cmpi.w   #2,FinishCode(a5)      should we save
      bne.s    CloseIt            no, just use

      ; user wants to save these preferences as well as using them

SaveIt
      movea.l  Dlib(a5),a6      DOS library pointer
      lea.l    Pname(pc),a0
      move.l   a0,d1         open prefs file
      move.l   #MODE_NEWFILE,d2   replace if existing
      jsr      _LVOOpen(a6)
      move.l   d0,PrefsFile(a5)   save the handle
      beq      UseIt         couldn't open it???
...
Dname
      ...
      CNOP  0,2
Pname
      DC.B  'sys:PC/System/2500prefs',0
      CNOP  0,2
preftitle
      DC.B  'PC Configuration V1.3                    ',0
      CNOP  0,2

;debugtext1
;      DC.B  '###########debug text1',13,10,0
;      CNOP  0,2
;debugtext2
;      DC.B  '###########debug text2',13,10,0
;      CNOP  0,2

prefwindow
      DC.W  170,50,300,83      left,top,width,height
      DC.B  2,1         detailpen,blockpen
      DC.L  REFRESHWINDOW!GADGETUP!GADGETDOWN
      DC.L  WINDOWDRAG!WINDOWDEPTH!SMART_REFRESH
