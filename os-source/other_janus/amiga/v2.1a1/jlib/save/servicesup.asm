...
USD_REGS   REG   D2/A2-A3
   ...
   IFGE   INFOLEVEL-1
   MOVE.L   A0,-(SP)
   PUTMSG   1,<'%s/UnmakeServiceData( $%lx )'>
   LEA   1*4(SP),SP
   ENDC
...
*   notice that the SERVICE_DELETED flag is now set
   ...
   IFGE   INFOLEVEL-1
   MOVE.L   A0,-(SP)
   PUTMSG   1,<'%s/InformDeletedService( $%lx )'>
   LEA   1*4(SP),SP
   ENDC
...
* 
   ...
   MOVE.L   A0,-(SP)
   MOVE.L   D0,-(SP)
   PUTMSG   1,<'%s/AllocServiceCustomer( $%lx $%lx $%lx )'>
   LEA.L   3*4(SP),SP
...
SC_REGS      REG   D2-D3/A2-A3/A6
   ...
   IFGE   INFOLEVEL-1
   MOVEM.L   A0/A6,-(SP)
   PUTMSG   1,<'%s/SignalCustomers( $%lx ) with $%lx'>
   LEA.L   2*4(SP),SP
   ENDC   ; of IFGE   INFOLEVEL-2
...
WFPC_REGS   REG   A0-A3/D0-D1
   ...
   PUTMSG   1,<'%s/Do we need to WaitForPC()?'>
...
WFPC_LOADED:
   PUTMSG   3,<'%s/PC handler loaded, so wait 15 second'>

*    - PC handler is loaded, so wait 15 seconds
      MOVE.L   #15*50,D1
      JSR      DelayWait
...
* validate the service data in A0
   ...
   IFGE  INFOLEVEL-1   ; this includes the body of validate routine
      ...
      MOVE.L   A0,-(SP)
      PUTMSG   1,<'%s/validating ServiceData at %lx'>
      LEA.L    4(SP),SP
