...
;
;SUBROUTINE TO MASK OF INTERRUPT
;
F4	proc	near
...
;DISPLAY INTERNAL ERROR
        ...
        JZ      B0                     ;NO, JUMP
        MOV     SI,OFFSET ERROR_MSG_1
        CALL    DISPLAY
        JMP     B1

;DISPLAY EXTERNAL ERROR

B0:     MOV     SI,OFFSET ERROR_MSG_2
        CALL    DISPLAY
...
;	mov	si,offset readFD_MSG
;	call	display	
	CALL    BT10                   	;READ THE FLOPPY
BT3:    	POP     CX                     	     
        	JNC     BT4                    	;JUMP IF NO ERROR      
...
BT4:    	mov	si,0
	cmp	byte ptr es:[si],0		;check validity
	je	BT9			;invalid boot block
	mov	ax,word ptr es:[si]		;check first 4 bytes
	cmp	ax,word ptr es:[si+2]  	; if all equal, then not 
	je	bt9		   	; a system disk, otherwise 
					; jump to boot blk
...
;original routines
;	TEST    HF_CNTRL,1             ;HARD DISK PRESENT ?
;	JZ	BT9			;NO, JUMP 
;end of original routines
...
BT7:    POP     CX
        JNC     BT8                    ;JUMP IF NO ERROR
        LOOP    BT6
        JMP     SHORT BT9

;	CHECK TO SEE IF DATA IN MEMORY IS CORRECT

BT8:    MOV     SI,offset 7DFEH
        CMP     es:[SI],0AA55H            ;HARD DISK READ OK ?
        JNE     BT9                    ;NO, JUMP

	db	0eah			;make a far jump to location 0:7C00h
	dw	07C00h
	dw	0
...
readFD_MSG     label byte
	       db "Read FD",cr,lf,-1

readHD_MSG     label byte
	       db "Reset HD",cr,lf,-1
...
ERROR_MSG_2    LABEL BYTE     
	...
	DB "Parity error on expansion board", CR, LF, -1

ERROR_MSG_3    LABEL BYTE     

	DB "Fatal Error --  Computer stopped", -1
...
;
;	The size of this module must be 500h
;

empty_space	db	76-14 dup (0)

end_of_boot	label	byte
