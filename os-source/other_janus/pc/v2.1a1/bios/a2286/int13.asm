...
rdy_wait		equ	600h		;wait for drive ready
drq_wait		equ	100h		;wait for data request line
...
wd_setup		proc	near		
	...
	call	Init_Int1		;let Interrupts pass through
				; the first 8259
...
wd_reset_cmd	proc	near
	...
	out	dx,al			;
	push	bx
	mov	bl,3			;give some time to controller to reset
reset_cmd1:
	call	delay			;
	dec	bl
	jnz	reset_cmd1		;
...
wait_1:
	...
	jnz	wait_4_intr2		;yes,tehn exit from loop
	loop	wait_1
	dec	al
...
empty_space	db	15 dup (0)

end_of_int13	label	byte
