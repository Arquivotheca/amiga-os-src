...
;		01-mar-91 rsd	changed getservice to allow GETS_WAIT
;				recursion.  now checks for SERVICE_ADDED in
;				wait phase instead of WaitS flag.
;
;******************************************************************************
...
GS_ServiceFound:				;* matching SDstrucure found
	...
	jne	GS_WaitForService		;** set => update SDstructure
...
;	INFO	AutoLoadService			; AutoLoad
	...
	mov	DI,0
;;;
	mov	al,GETS_WAIT	;we wanna wait!
;;;
...
;	jne	GS_WaitEnd			;  comes available
	...
	test	es:ServiceData.sd_Flags[si],SERVICE_ADDED	;chk pcw flag
	jnz	GS_WaitEnd			;o-tay!

;	test	es:ServiceData.sd_Flags[si],SERVICE_PCWAIT	;chk pcw flag
;	jz	GS_WaitEnd			;o-tay!
...
FreeFound:					;* write pointer into free slot
	...
	push	ax
	mov	ax,es:ServiceData.sd_Flags[si]
	or	ax,SERVICE_PCWAIT
	mov	es:ServiceData.sd_Flags[si],ax
	pop	ax
...
RemoveService	proc	near
	...
	mov	es:ServiceParam.FirstServiceData[di],ax
	jmp	rs_freestuff

rs_findloop:
	mov	ax,es:ServiceData.sd_NextServiceData[bx]
	cmp	ax,si
	jz	rs_foundit
	mov	bx,ax
rs_findit:
	...
	mov	al,JSERV_OK		;didn't find the service
	ret
