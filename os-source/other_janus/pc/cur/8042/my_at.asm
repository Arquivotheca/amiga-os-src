...
;	08-apr-91 rsd
;		- fix TIMER routine to not destroy reg A.
;		- fixes to PCMODE routine to use proper signal polarity for
;		  pc keyboard holdoff.
;	
;
...
;08-apr-91 rsd - swap MOV and CALL to really preserve A
...
;kb sends at 9600 bits per second.  this means:
; 1 bit time = 104 microseconds
; 2 bit times = 208 microseconds
;
;the 8042 is running at 8 mhz.  this means 1 cycle = 1.88 microseconds.

PCMODE:
	ANL	P2,#0BFH		;kbclk high
	ORL	P2,#080H		;kbdata high (enable kb)
...
;we haven't started getting a key from the kbd, but we have received a
;command from the 286.  disable the keyboard, but hang around for a few
;bit times to see if it starts to send something anyways.

	anl	p2,#07fh		;kbdata low (disable kb)
	mov	a,#23			;208 microseconds (2 bit times)
dloop:	jnt0	pcmode		;2	;aha!  a clock!
	dec	a		;1	;no clock.  dec count
	jnz	dloop		;2	;until times out.
