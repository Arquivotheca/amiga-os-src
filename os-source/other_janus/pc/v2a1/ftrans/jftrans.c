...
#include <stdio.h>
#include "janus.h"
#include "jint.h"                         /* Janus support              */
#include "jftrans.h" 
                                               
#define   BUFFSIZE 8192                   /* Maximum of 8K buffer       */
#define   TRUE      (-1)
#define   FALSE      0

extern unsigned char * buffer;            /* Initialised as a far ptr   */
                                          /* by amigainit in jint.c     */
extern struct AmigaDiskReq *paramptr;

static	short File,Off_h,Off_l; 
static unsigned char tbuff[BUFFSIZE];
...
static unsigned char amiga_pc_table[256] = {
   ...
   if(direction==JFT_AMIGA_PC)         /* open appropriate PC file      */
   {
      ...
      rc = amiga(ADR_FNCTN_OPEN_OLD,0);/* Request to PCDisk to open     */
                                       /* Amiga file                    */
   } else {
      strcpy(buffer,outfile);
      rc = amiga(ADR_FNCTN_OPEN_NEW,0);/* Request to PCDisk to open     */
                                       /* Amiga file                    */
     }
   ...
   if ( rc != J_OK)
   {
	...
	File  = paramptr->adr_Part;
	Off_h = paramptr->adr_Offset_h;
	Off_l = paramptr->adr_Offset_l;

   /* Copy from infile to outfile */

   while((rc=jft_readblock(tbuff,BUFFSIZE,localfile,direction,&status))!=0)
   {
	...
	paramptr->adr_Part = File;   
   amiga(ADR_FNCTN_CLOSE,0);              /* Request to PCDisk, close file   */
   fclose(localfile);
   return(status);
}                                       /* End main                        */
...
int    *status;
{
   ...
   *status=JFT_NOERROR;                  /* assume OK                     */
   ...
   writecount=amiga(ADR_FNCTN_WRITE,size);
   if(writecount<0)
   {
...
int    *status;
{
   ...
   *status=JFT_NOERROR;                  /* assume OK                     */
   ...
   readcount=amiga(ADR_FNCTN_READ,size);
   if(readcount<0)
   {
