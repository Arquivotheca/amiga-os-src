...
;rsd - *DON'T*		- Attempt boot off drive B:
...
;test for valid floppy boot code

	mov	ax,es:[di]	;1st byte must be >= 06
	cmp	al,06h
	jc	bootfail

	push	cx		;1st 8 words must be different
	mov	cx,8
check:
	add	di,2
	cmp	ax,es:[di]
...
hdtest:
;test for valid hd boot block

	CMP	word ptr ES:[di+510],0AA55H	; Valid boot block?
	JNE	BOOTFAIL
...
BOOTDK1:
	...
	CMP	DL,80H			; if we just tried C:, we're done
	JE	BOOTFAIL
