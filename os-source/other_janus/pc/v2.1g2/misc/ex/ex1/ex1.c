/****************************************************************************
 * Ex1.c - Ex1 C Binding & services example #1
 *  			
 * This program finds the Amiga Ex1 service if available and
 * calls the Ex1 service once then exits.
 *
...
char *argv[];
{
   ...
   /************************************************************************/
   error=GetService(&SData,(ULONG)EX1_APPLICATION_ID,EX1_LOCAL_ID,0,Flags);
   if(DBG_GETSERVICE) printf("Error from  GetService = "); PError(error);
   if(error!=JSERV_OK) {
      printf("Error! Ex1 service not available. Run Ex1 on Amiga\n");	
      return(1);
   }
