...
HEADING		DB	'C O M M O D O R E   S E T U P   U T I L I T Y',-1,0

CONFIG_MSG	DB	' Date',-1
		DB	' Time',-1
		DB	' Diskette 1',-1
		DB	' Diskette 2',-1
		DB	' Hard Disk 1',-1
		...
		DB	' Coprocessor',-1
		DB	' Base Memory',-1
		DB	' Extended Memory',-1,0
     		DB	'Base memory found:     ',-1
		DB	'Extended memory found: ',-1,0

HELP_MSG	DB	' , : select items     <Esc>: abort SETUP      <PgDn>: view hard disk types',-1
		DB	' , : select values    <End>: exit and update  F1: secondary SETUP window',-1,0
SET2_TOP:
...
WAIT_KEY_2:
	...
	XOR	AH,AH				;   CMOS image table and
	ADD	BX,AX				;   restore main setup screen 
	CALL	CS:[BX]				;
	MOV	DX,MAIN_CURSOR
	MOV	AH,2
	XOR	BH,BH
...
DISP_DATE	PROC	NEAR
	...
	MOV	DX,0323H			;Set cursor position
	XOR	BH,BH
	MOV	AH,02
	INT	10H
	...
	MOV	AH,04				;Get date
	INT	1AH
	MOV	WORD PTR DATE,DX		;Save date & month
	MOV	YEAR,CL				;Save year

	MOV	BH,3				;3 fields (dd.mm.yy)
	MOV	AL,DATE				;Get date
...
DISP_TIME	PROC	NEAR
	...
	MOV	DX,0423H			;Set cursor position
	XOR	BH,BH
	MOV	AH,02
	INT	10H
...
DISP_HD	PROC	NEAR
	...
	MOV	DX,0727H			;Set cursor position for HD1
	OR	AL,AL				;See if it's right
	JZ	HD_0				;Yes
	MOV	DX,0827H			;2nd drive, set cursor for it
HD_0:	PUSH	AX
	MOV	AH,02
	XOR	BH,BH
	INT	10H
...
DISP_VIDEO	PROC	NEAR
	...
	MOV	DX,0923H			;Set cursor position
	MOV	AH,02
	XOR	BH,BH
	INT	10H
...
DISP_BASE	PROC	NEAR
	...
	MOV	DX,0B23H			;Set cursor position
	MOV	AH,02
	XOR	BH,BH
	INT	10H
...
DISP_EXT	PROC	NEAR
	...
	MOV	DX,0C23H			;Set cursor position
	MOV	AH,02
	XOR	BH,BH
	INT	10H
