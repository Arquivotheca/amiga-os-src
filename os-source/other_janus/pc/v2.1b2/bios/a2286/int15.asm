...
do_91:	cmp	ah,91h			;Check if this is int complete call
	jne	do_rest			;  if so, no need to save registers.
	jmp	int_comp		;  This is where CP/M run out of stack 
do_rest:				;  space. (v 2.01  W. Chessen) 
	push	ax
	call	save_all		;save all registers
	cmp	ah,80h			;check for valid function code
	jb	intr15_err		;error return if between 00-7f
;03/23/1987 jwy, new func 0c0h
...
;	if al = 0:	es:bx	address of semaphore byte in caller's
...
;	use CX:DX as a loop counter to eleminate infinite loop
