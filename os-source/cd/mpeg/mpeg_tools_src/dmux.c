...
#define read  b = ReadCharAsync(bs); bcount++;
#define vread b = ReadCharAsync(bs); bcount++; vbcount++; vb<<=8; vb+=b;
#define aread b = ReadCharAsync(bs); bcount++; abcount++; ab<<=8; ab+=b;
...
float t, vt, dvt, dat;
float dat_min=0, dat_max=0;
float dvt_min=0, dvt_max=0;
unsigned int pts_val=0;
...
/*unsigned char b, t1, t2, t3;
int pic_count=0, gop_count=0, slice_count=0;
...
*/
    ...
    {
        if(rdargs=ReadArgs("FileName/A",args,NULL))
        {
            if(bs = OpenAsync((STRPTR)args[0], MODE_READ, 65536))
            {
                while (ReadAsync(bs, &b, 1) == 1) {
                ...
                //              printf("%02x\t", b);
                                if(b == 0xb4)
                                        ...
                                        if((b&0xF0)==0x20) {
                                                ...
                                                pts_val = (pts_val | b) << 7;
                                                read
                                                pts_val = (pts_val | ((b >> 1) &0x7f)) << 8;
                                                read
                                                pts_val = (pts_val | b) << 7;
                                                read
                                                pts_val = pts_val | ((b >> 1) & 0x7f);

                                                printf("pts=%f\t",(float)(pts_val / 90000.0));
...
//                                                printf("pts=%f\t",(float)(pts_val / 90000.0));
						...
						PutStr(buff);
                                                ...
                                                read
                                                read
                                                printf("pts=%f DTS\t",(float)(pts_val / 90000.0));
                                        }
                                        else if (b==0x0F) {}
                                        else
                                                printf("!!!!! Packet header error!!!\n");
        ...
        sprintf(buff,"dat_min=%f \t dat_max=%f\t dt=%f\n", dat_min, dat_max,dat_max-dat_min);
        PutStr(buff);
        sprintf(buff,"dvt_min=%f \t dvt_max=%f\t dt=%f\n\n", dvt_min, dvt_max,dvt_max-dvt_min);
        PutStr(buff);
        sprintf(buff,"%d bytes audio\n", abcount);
        PutStr(buff);
        sprintf(buff,"%d bytes video\n", vbcount);
        PutStr(buff);
        sprintf(buff,"%d bytes read\n", bcount);
        PutStr(buff);
...
float pic_rate(unsigned char a)
{
	if (a == 1)
		return(23.976);
	else if(a == 2)
