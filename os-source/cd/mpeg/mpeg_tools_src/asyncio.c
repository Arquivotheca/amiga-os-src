...
LONG ReadLongAsync(struct AsyncFile *file,ULONG *val)
{
    if (file->af_BytesLeft)
    {
        ...
        *val = *(ULONG *)file->af_Offset;
        file->af_BytesLeft-=4;
        file->af_Offset = (APTR)((ULONG)file->af_Offset + 4);
...
LONG WriteLongAsync(struct AsyncFile *file, ULONG ch)
{
    if (file->af_BytesLeft)
    {
