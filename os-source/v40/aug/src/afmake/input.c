...
# endif
	...
	if (q != (char *) 0) {
		...
		*p = '\0';
	    ...
	    p = str1;
	    if ((a = gettok(&p)) == (char *) 0) error("No macro name");

	    if(!strcmp(a,".INCLUDE")) {
		if(++fcount > 15) {
			fatal("Includes nested >16\n");
		}
		ifiles[fcount]=fd;	/* save old file handle */
	...
	*((q = str1) + strlen(str1) + 1) = '\0';
	...
	cp = (struct cmd *) 0;
	if (getl(str2, &fd) == FALSE) {	/* Get commands  */
#ifdef os9
	    while (*str2 == ' ')
#else
	    while ((*str2 == '\t') || (*str2 == ' '))
...
FILE **fd;
{
    ...
    while(TRUE) {
	if (getline(string,*fd)) {
	    if(fcount < 0)return(TRUE); /* EOF for all files */
	    fclose(*fd);
	    *fd=ifiles[fcount--];
    	    }
	else {
