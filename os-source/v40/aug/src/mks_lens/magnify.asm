...
*
*	For standard C-like stacked parameters:
*
*		void MagnifyBitMapCStub(struct BitMap *from,
*					struct BitMap *to,
*					long xMag,		(0:7)
*					long yMag,		(0:7)
*					long xSize,		(0:15)
...
* checking is done on these and stange values will cause even
* stranger results...
*
...
*
	...
	xdef	_MagnifyBitMapCStub	; The external entries (stack)
	xdef	_MagnifyBitMap		; Register version
*
*************************************************************************
*
* This entry point is for C functions:  Stacked parameters
*
* void MagnifyBitMap(struct BitMap *from,struct BitMap *to,
*			short xMag,short yMag,short xSize,short grid);
*
* This C stub assumes all arguments are pushed onto the stack
* as 32-bit values.  (As per standard Amiga system calls...)
*
_MagnifyBitMapCStub:
	move.l	$04(sp),a0	; Get *from
	move.l	$08(sp),a1	; Get *to
	move.w	$0E(sp),d0	; Get xMag
	swap.w	d0		; Swap the words...
	move.w	$12(sp),d0	; Get yMag
	move.w	$16(sp),d1	; Get xSize...
	swap.w	d1		; Put it in the high word...
	move.w	$1A(sp),d1	; Get the grid flag...
	bra.s	DoMagnify	; Do it...
*
*************************************************************************
*
...
*
	...
	add.w	d4,d4		; Shift the from bit...
