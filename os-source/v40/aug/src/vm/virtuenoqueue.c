...
void AddQueue(PhysicalPage element) /* Make sure the queue is not full before calling */
{
...
PhysicalPage GetQueue()	/* Make sure the queue is not empty before calling */
{
...
/* Returns garbage if vpage is invalid */
...
char *argv[];
{
	...
	CleanUp(12);
    if( !(Age = AllocMem(ppages*2,MEMF_PUBLIC)) )
	CleanUp(13);
