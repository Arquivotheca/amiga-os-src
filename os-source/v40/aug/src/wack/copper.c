 ...
 *
 * $Id: copper.c,v 39.2 93/05/21 17:32:00 peter Exp $
 *
 * 
 */
...
doDumpCops( int listtype )
{
	...
	if ( listtype  == 2 )
	{
	    ...
	    dumpCprList( (struct cprlist * )GBase.copinit, listtype );
	}
	else
	{
	    ShowAddress ("ActiView", GBase.ActiView);
	    ReadBlock(GBase.ActiView, &view, sizeof view);
 ...
 */
void
dumpCprList( struct cprlist *list, int type )
{
    ...
    {
	ReadBlock( list, &coplist, sizeof coplist);
	count =  coplist.MaxCount;
	PPrintf("copper count: %ld\n", count);
	p = coplist.start;
	all = type;
    }
...
int
copDisasm( APTR addr, UWORD op, UWORD w, int all )
{
	...
	sprintf(buff, "MOVE\t%04lx,%s", w, regname(da));
	strcat(linebuff,  buff);
