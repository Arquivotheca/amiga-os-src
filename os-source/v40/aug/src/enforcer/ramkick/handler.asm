...
*
		include	"enforcer.ramkick_rev.i"
*
		xref	_serdatr
		xref	_serdat
		xref	_intreq
...
*
 STRUCTURE	MMU_Frame,0
	...
	ULONG	mmu_TC
	;
	ULONG	mmu_CRP_OLD	; URP for 68040
	ULONG	mmu_CRP_OLD_1	; SRP for 68040
	ULONG	mmu_TC_OLD
	;
	APTR	mmu_LevelA
	APTR	mmu_LevelB
	...
	APTR	mmu_Magic
	;
	APTR	mmu_Page0	; 68040 optimization ;^)
	;
	...
	;
	STRUCT	mmu_TT,4*4	; Storage for TT regs (68040 only)
	;
...
PrintDone:	movem.l	(sp)+,d2/d3	; Restore...
		rts			; Else done...
*
DoString2:	move.l	(sp)+,a0	; Get a0 back...
		bra.s	PrintIt1
DoString:	move.l	a0,-(sp)	; Save on stack...
...
*
		...
		bmi.s	usp_NotDisable		; If negative, not forbid...
		moveq.l	#'D',d0			; Set Forbid
usp_NotDisable:	move.b	d0,5(a0)		; Store Forbid flag...
*
...
*
* Format: SegT: 075483D2 - "mem"  Hunk 0000 Offset 00000272
*
...
NameDisp:	move.l	a0,d3			; Store fault address...
		...
		move.l	sp,a1			; Point at data...
		bsr	PrintIt			; Display task name...
...
nd_NoName:	move.l	a1,(sp)			; Save on stack...
		move.l	sp,a1			; Get pointer to data
		lea	TaskName(pc),a0		; Get name string
		bsr	PrintIt			; Display task name...
...
nd_PrintIt:	bsr	PrintIt			; Display it...
*
...
nd_PrintIt1:	bsr	PrintIt			; Output...
*
...
be_NonData:	bchg.b	#1,$bfe001		; Toggle the power LED
...
be_NotPhy:	move.l	a1,-(sp)		; PC onto argument list
		move.l	sp,a1			; Get argument list
		bsr	PrintIt			; Print it...
...
*
MMU_On_040:	tst.l	Quiet_Flag(pc)		; Check if quiet...
		bne	skip_hacks		; If quiet, skip page-0 hacks
*
****************
*
...
no_gfx_hack:
*
* The above is all hacks to make 68040 Enforcer run a bit faster...
*
****************
*
skip_hacks:
...
*
BusError040:	movem.l	d0-d7/a0-a6,-(sp)	; Save the register frame
		move.l	NewExecBase(pc),a6	; Get ExecBase
...
b4_NotPhy:	move.l	a1,-(sp)		; Move PC onto argument list...
		move.l	sp,a1			; Point at arguments...
		bsr	PrintIt			; Print it...
		addq.l	#8,sp			; Restore stack...
*
************************
...
NewReboot:	move.l	OldReboot(pc),-(sp)	; Where we go to...
		move.l	mmu_Frame(pc),a0	; Get Frame...
		clr.l	mmu_Frame		; Signal for Reboot...
		bsr	_MMU_Off		; Turn MMU off
		bset.b	#7,$00DE0002		; Mark as fake cold boot...
		rts				; Reboot!
*
*******************************************************************************
...
* of ZeroPage and InvalidPage for the 68040 MMU setup.
*
...
_Copyright:	xdef	_Copyright
		dc.b	10
		VERS
		dc.b	' by Michael Sinz - Copyright © 1992-1993'
