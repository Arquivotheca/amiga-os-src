...
#define TEMPLATE	"SHOW/S,DUMP/S,FIND/M" VERSTAG
#define	SEG_SEM		"SegTracker"
...
struct	SegSem		*segSem=NULL;
	...
	if (DOSBase = OpenLibrary("dos.library",37))
	{
		...
		if (segSem == &MySem)
		{
			...
			Forbid();
			OldLoadSeg   =(ULONG)SetFunction(DOSBase,(long)&LVOLoadSeg,MyLoadSeg);
			OldNewLoadSeg=(ULONG)SetFunction(DOSBase,(long)&LVONewLoadSeg,MyNewLoadSeg);
			OldUnLoadSeg =(ULONG)SetFunction(DOSBase,(long)&LVOUnLoadSeg,MyUnLoadSeg);
		//	OldRemSegment=(ULONG)SetFunction(DOSBase,(long)&LVORemSegment,MyRemSegment);
			Permit();
