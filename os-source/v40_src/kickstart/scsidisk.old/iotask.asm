...
******************************************************************************
		...
		INCLUDE	"exec/io.i"
		INCLUDE	"devices/trackdisk.i"	just for TDERR errors
...
;		DEBUGENABLE
		...
		XREF	_LVOOpenDevice,_LVOSendIO,_LVOCause
...
;==============================================================================
IOTask		movea.l	4(sp),a6		get exec library
		moveq.l	#iot_SIZEOF,d0		get the global memory area
		bsr	GetPubMem		assume this never fails
		movea.l	d0,a5			a5 always points to globals
		move.l	a6,iot_SysLib(a5)	stash Exec base
		move.l	8(sp),iot_BoardAddress(a5)
		move.l	16(sp),iot_ReqPort(a5)	stash msgport for IORequests

		moveq.l	#MP_SIZE,d0		get a port for returning cmds
		bsr	GetPubMem
		move.l	d0,iot_CmdRetPort(a5)	assume allocation works

		moveq.l	#MP_SIZE,d0		get a msg port for SCSITask
		bsr	GetPubMem
		move.l	d0,iot_SCSITaskPort(a5)	SCSITask will initialise it

		moveq.l	#MP_SIZE,d0		get a msg port timer reqs
		bsr	GetPubMem
		move.l	d0,iot_TimerPort(a5)
...
; now we'll start up the SCSITask with some arguments passed on its stack
		...
		move.l	iot_SysLib(a5),-(a0)	    and exec base pointer
...
; now it's safe for us to signal init that all initialisation was done.
		movea.l	12(sp),a1		get task to signal
		jsr	_LVOSignal(a6)		d0 = SIGF_SINGLE
...
Dispatch	DC.W	BadCmd-Dispatch		0  invalid	q
		...
		DC.W	BadCmd-Dispatch		20 addchangeint	q
		DC.W	BadCmd-Dispatch		21 remchangeint q
		DC.W	BadCmd-Dispatch		22 lastcomm	q
		DC.W	BadCmd-Dispatch		23
...
; the error handlers (for no disk and media changed) to do thier stuff!
		...
		move.l	#1,iot_TimeOut(a5)	out, check every second
...
PollFinished	movea.l	iot_TimerReq(a5),a1
...
; if there was a check condition and SCSIF_AUTOSENSE is 1, do request sense now
		...
		cmpi.b	#$02,scsi_Status(a1)	did we get check condition ?
		bne.s	DirectComplete		nope, return the command
		btst.b	#SCSIB_SENSE,scsi_Flags(a1)
...
ReserveDone	moveq.l	#-1,d0			assume an error occured
		...
		cmpi.b	#$08,d0			busy (this shouldn't happen)
...
ReleaseDone	moveq.l	#-1,d0			assume an error occured
		...
		cmpi.b	#$08,d0			unit was busy (can't happen)
...
CheckCondition	cmpi.b	#$02,d0			was it Check Condition status
...
CheckDone	tst.b	cb_ErrorCode(a0)	any SCSI errors
		...
		cmpi.b	#$08,d0			was the unit busy ?
...
ec05	printf <'drive not selected\n'>
		moveq.l	#-1,d0
		bra	ErrorQuit
...
ec0a	printf <'no disk\n'>
		...
		tst.l	hu_ChangeState(a1)	is disk marked as in ?
		bne.s	OutAlready		nope, so no Cause
		move.l	#-1,hu_ChangeState(a1)	mark disk as out now
		move.l	hu_RemoveAddress(a1),d1	should we wake someone ?
		beq.s	OutAlready		nope, finish up
...
ec0b		printf <'load/unload failure\n'>
		bra	ErrorRetry
...
ec0d		printf <'focus failure\n'>
		bra	ErrorRetry

; SONY tracking failure
ec0e		printf <'tracking failure\n'>
		bra	ErrorRetry

; ID CRC or ECC error, just retry using retry count
ec10	printf <'ID CRC or ECC error\n'>
		bra	MaybeRetryAll

; unrecoverable read error, retry using retry count
ec11	printf <'unrecoverable read error\n'>
		bra	MaybeRetryAll

; no ID address mark
ec12	printf <'no ID address mark\n'>
		bra	MaybeRetryAll

; no data address mark
ec13	printf <'no data address mark\n'>
		bra	MaybeRetryAll

; no record found
ec14	printf <'no record found\n'>
		bra	MaybeRetryAll

; seek error
ec15	printf <'seek error\n'>
		bra	MaybeRetryAll

; data sync mark error
ec16	printf <'data sync mark error\n'>
		bra	MaybeRetryAll

; recovered read with retries
ec17	printf <'recovered read with retries\n'>
		bra	ErrorRetry

; recovered read with ECC
ec18	printf <'recovered read with ECC\n'>
		bra	ErrorRetry

; defect list error
ec19	printf <'defect list error\n'>
		bra	MaybeRetryAll

; parameter overrrun
ec1a	printf <'parameter overrun\n'>
		moveq.l	#-1,d0
		bra	ErrorQuit

; synchronous request error
ec1b	printf <'synchronous request error\n'>
		moveq.l	#-1,d0
		bra	ErrorQuit

; defect list not found
ec1c	printf <'defect list not found\n'>
		moveq.l	#-1,d0
		bra	ErrorQuit

; compare error
ec1d	printf <'compare error\n'>
		bra	MaybeRetryAll

; recovered ID
ec1e	printf <'recovered ID\n'>
		bra	ErrorRetry
...
20$		cmpi.b	#REASSIGN_BLOCKS,(a1)	did we attempt a reassign ?
		beq.s	ec32			yep, do software block mapping
...
ec21	printf <'invalid block address\n'>
		moveq.l	#-1,d0
		bra	ErrorQuit

; illegal function for device
ec22	printf <'illegal function for device\n'>
		moveq.l	#-1,d0
		bra	ErrorQuit

; illegal field in CDB
ec24	printf <'illegal field in CDB\n'>
		moveq.l	#-1,d0
		bra	ErrorQuit
...
ec26	printf <'bad field in parameter list\n'>
		moveq.l	#-1,d0
		bra	ErrorQuit
...
ec28	printf <'media changed\n'>
		movea.l	cb_IORequest(a0),a1
		movea.l	IO_UNIT(a1),a1		get unit pointer
		tst.l	hu_ChangeState(a1)	is disk marked as out ?
		beq.s	InAlready		nope, so no Cause
		clr.l	hu_ChangeState(a1)	mark disk as in now
		move.l	hu_RemoveAddress(a1),d1	should we wake someone ?
		beq.s	InAlready		nope, finish up
		move.l	a0,-(sp)		save command block
		movea.l	d1,a1
	printf <'sending ChangeInt\n'>
		jsr	_LVOCause(a6)		wake 'em up
...
ec30	printf <'incompatible cartridge\n'>
		moveq.l	#-1,d0
		bra.s	ErrorQuit

; medium format corrupted
ec31	printf <'medium format corrupted\n'>
		moveq.l	#-1,d0
		bra.s	ErrorQuit

; no more spares available
ec32	printf <'no more spares\n'>
		moveq.l	#-1,d0
		bra.s	ErrorQuit

; RAM failure
ec40	printf <'RAM failure\n'>
		moveq.l	#-1,d0
		bra.s	ErrorQuit

; ECC diagnostic failure
ec41	printf <'ECC diagnostic failure\n'>
		moveq.l	#-1,d0
		bra.s	ErrorQuit

; power on diagnostic failure
ec42	printf <'power on diagnostic failure\n'>
		moveq.l	#-1,d0
		bra.s	ErrorQuit

; message reject error ?????
ec43	printf <'message reject error\n'>
		bra.s	MaybeRetryAll

; internal controller error
ec44	printf <'internal controller error\n'>
		moveq.l	#-1,d0
		bra.s	ErrorQuit

; reselect timeout ????
ec45	printf <'reselection timeout\n'>
		bra.s	MaybeRetryAll

; unsuccsesful soft reset
ec46	printf <'unsuccessful soft reset\n'>
		bra.s	MaybeRetryAll

; SCSI interface parity error
ec47	printf <'SCSI parity error\n'>
		bra.s	MaybeRetryAll
...
MaybeRetry	addq.w	#1,cb_Retries(a0)	bump the retry count
...
ErrorRetry	move.l	cb_OldReturnTo(a0),cb_ReturnTo(a0)
...
ErrorQuit	movea.l	cb_IORequest(a0),a1	stash the error
		move.b	d0,IO_ERROR(a1)
		bra.s	CmdComplete		reply IOReq and free CmdBlock
...
;
; New addition for disk inserted/removed poller.  If cb_IORequest is 0 then
...
SCSITaskName	DC.B	'SCSI bus handler',0
		CNOP	0,2
