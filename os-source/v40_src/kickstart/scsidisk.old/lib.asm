...
;		DEBUGENABLE
		...
		XDEF	SendInternalCmd,WaitASecond,TimerName
...
WaitNotBusy	movea.l	IO_DATA(a3),a0		point to SCSICmd
...
; or a busy status (most likely from the stupid Seagate drives)
		...
		bra.s	WaitNotBusy		unlimited wait for not busy
...
GetWriteProtect	movea.l	IO_DATA(a3),a0		get SCSICmd
		...
		move.b	#4,4(a1)		so just need MS header
		move.w	#6,scsi_CmdLength(a0)
		move.l	a4,scsi_Data(a0)	read data to here
		move.l	#4,scsi_Length(a0)	returns 4 bytes of info
		movea.l	a3,a0
		...
		bne.s	UnitReady		no, assume defaults are OK
...
IOTable		bra.s	BadIO		0  invalid	q
		...
		bra.s	BadIO		20 addchangeint	q
		bra.s	BadIO		21 remchangeint q
		bra.s	BadIO		22 lastcomm	q
		bra.s	BadIO		23
		bra.s	BadIO		24
		bra.s	BadIO		25
		bra.s	BadIO		26
