...
void   KillUnixParts(struct World *world);
...
BOOL isUnix(struct DosEnvec *ev)
{
   ...
   p=(UBYTE *)(&ev->de_DosType);
   if(p[0]!='U') return(FALSE);
   if(p[1]!='N') return(FALSE);
   if(p[2]!='I') return(FALSE);
   if((p[3]<0)||(p[3]>9)) return(FALSE);
   D( kprintf("isUnix() MATCH") );
   return(TRUE);
}
...
struct World *world;
{
   ...
   D( kprintf("Begin KillUnix MountList Search\n") );
   for(bn=(struct BootNode *)ExpansionBase->MountList.lh_Head;
       bn->bn_Node.ln_Succ;
       bn=(struct BootNode *)bn->bn_Node.ln_Succ)
   {
