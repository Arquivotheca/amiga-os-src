...
/* Defines ------------------------------------------------------------------- */
#define TEMPLATE "FILE/A,MEM=MEMSIZE/N/A,MT=MEMTYPE/K,REPS/K/N,PAT=MAKEPAT/K/N,CREATE/S,VER=VERIFY/S"
#define OPT_FILE    0
#define OPT_MEM     1
...
/* Where:
    ...
    MAKEPAT   - use particular pattern (given)
    CREATE    - whether to create file
    VER       - whether to verify info
*/
...
VOID main(int argc, UBYTE *argv[]) {
    ...
    BOOL   do_pat = FALSE;
    BOOL   verify = FALSE;
    LONG oldpos, curpos;            /* file positions for Seek() */
    ...
    /* Memory allocation
     */
    if (!servAlloc(mr->memSize, OK_WRITE, mr, &g)) {
    ...
    /* Perform Test .................................... */
    /* Think about the design here.  Are we writing a RAM buffer to disk, and
       verifying it?  If so, we need only read access to RAM, and we can run
       a LOT of these tests.
       Are we writing a RAM buffer to disk, then verifying it?
       Are we writing RAM to disk, then writing disk to RAM, all continuously?
       Are we reading disk-to-RAM continuously?
