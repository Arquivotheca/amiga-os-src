/* WriteMF.c
   This thingie just writes from a block of RAM to the given file,
...
/* CAUTION:  THIS PROGRAM IS QUITE CAPABLE OF TRASHING DISK DRIVES.
    ...
    12 Mar 92  Added better error checking/handling, in compliance with
               Memserver V2.x changes.  Fixed irritating little bug 
               which caused the file to be created with a size of
               max(memory_used,filesize).  Created memSet, which allows
               a LONG pattern such as 0x11223344 to be written out.

    16 Mar 92  Added BUF2_PAT to clear the read/verify buffer, matching
               ReadFM's behavior.
...
#define PORT_TEMPLATE "WriteMF_"          /* a number is inserted after the underscore */
#define PORT_TEMP_LEN 11                  /* length of template + # of digits of retries + null */
#define PROGNAME "WriteMF"                /* used with PrintFault... */
#define DEF_MEMTYPE LOC_ANY               /* also default for verify buffer */
#define DEFAULT_MEMSIZE 256*1024
#define DEFAULT_PRIORITY 2
#define DEFAULT_PATTERN  0
#define MAX_MEM_RETRIES  10
#define MEM_RETRY_DELAY  30 * TICKS_PER_SECOND
#define BUF2_PAT         0xEE             /* pattern to fill read buffer with for verifies */

/* ReadArgs Template Defines --------------------------------------------------- */
#define TEMPLATE "FILE/A,MS=MEMSIZE/K/N,MT=MEMTYPE/K,REPS/K/N,FS=FILESIZE/K/N,PRI=PRIORITY/K/N,PAT/K/N,VER=VERIFY/S,RM=DELETE/S"
#define OPT_FILE    0
#define OPT_MEM     1
...
/* Where:
    ...
    PRI       - priority program should run at
    PAT       - pattern to write (0..255)
...
void memSet(ULONG *ptr, ULONG pat, ULONG size);  /* stuff ULONG in arr of ulong */
...
VOID main(int argc, UBYTE *argv[]) {
    ...
    LONG protection = NC_READ;
    LONG i, fileSize, amtRead=0,amtWrote=0;
    LONG bytesLeft, pri=DEFAULT_PRIORITY;
    LONG pat = DEFAULT_PATTERN;
    SHORT retries = 0;
    ...
    /* Memory allocation
       Get block of RAM to write file from.
     */
    retries = 0;
    while (!(mr->memPtr)) {
        servAlloc(mr->memSize, protection, mr, g);
        if (mr->rc2 == MEM_NOT_VALID) {
            aprintf("%s: Nonexistent board specified: %ld\n",(LONG)(g->portName),mr->board);
            aprintf("%s: Memory allocation failed, exiting...\n",(LONG)(g->portName));
            GoodBye(g);
        }
        if (mr->rc2 == MEM_READ_ONLY) {
            aprintf("%s: Cannot write pattern to read-only location %ld\n",(LONG)(g->portName),mr->board);
            aprintf("%s: Memory allocation failed, exiting...\n",(LONG)(g->portName));
            GoodBye(g);
        }
        if (!(mr->memPtr)) {
            servLog(mr,g,"Waiting for memory to become available. (size %ld, loc %ld)", mr->memSize, (LONG)(mr->board));
            retries++;
            if (retries > MAX_MEM_RETRIES) {
                aprintf("%s: Maximum memory retries reached.\n",(LONG)(g->portName));
                aprintf("%s: Memory allocation failed (rc %ld), exiting...\n",(LONG)(g->portName),(LONG)(mr->rc2));
                GoodBye(g);
            }
            else
                Delay(MEM_RETRY_DELAY);
        }
    }
    if (Opts[OPT_PAT]) {
        /* write our pattern and then change to read access */
        memSet(mr->memPtr, pat, mr->memSize);
        servAccess(OK_READ, mr, g);
    }
    ...
    if (Opts[OPT_FSIZE]) {
        ...
        if (!(g->fh)) {
            ...
            aprintf("%s: Can't create file '%s'!\n", (LONG)(g->portName),(LONG)fileName);
            GoodBye(g);
        }
    }
    ...
    if (Opts[OPT_VER]) {
        if (!servAlloc(mr->memSize, OK_WRITE, mr2, g)) {
            if (mr->rc2 == MEM_NOT_VALID)
                aprintf("%s:  Nonexistent board specified: %ld\n",(LONG)(g->portName),mr->board);
            aprintf("%s: Memory allocation (size %ld) for Verify failed (rc %ld), exiting...\n",(LONG)(g->portName),(ULONG)(mr->memSize),(LONG)(mr2->rc2));
            GoodBye(g);
        }
    }
    if (Opts[OPT_PRI]) {
    ...
    servLog(mr,g,"Begin FileWrites of 0x%08lx (%ld) to '%s' (%ld)",
            (LONG)(mr->memPtr), mr->memSize, (LONG)fileName, fileSize);
    for (i = 0; i < reps; i++) {
        servLog(mr,g,"FileWrite %ld of %ld: 0x%08lx (%ld) to '%s' (%ld)",
                (LONG)i+1L, reps, mr->memPtr, mr->memSize, (LONG)fileName, fileSize);
        D(PutStr("Writing file memSize at a time...\n"));
        bytesLeft = fileSize;
        while (bytesLeft > 0) {
            oldpos = Seek(g->fh, 0, OFFSET_CURRENT);
            oldpos = Seek(g->fh, 0, OFFSET_CURRENT);
            amtWrote = Write(g->fh, mr->memPtr, min(mr->memSize,(LONG)fileSize));
            if (amtWrote < 0) {
                mr->status = STATUS_SEVERE_ERR;
                servLog(mr, g, "Error writing '%s' (%ld) - IoErr %ld", 
                         (LONG)fileName, amtWrote, IoErr());
            ...
            curpos = Seek(g->fh, 0, OFFSET_CURRENT);
            D(aprintf("%s: Wrote %ld, %ld left, cur %ld, old %ld\n", (LONG)(g->portName), amtWrote, bytesLeft, curpos, oldpos));
            if (verify) {
                UBYTE *src, *dest;
                LONG j, firstErr=0L,errCount=0L,tmppos;
                
                src  = (UBYTE *)(mr->memPtr);
                dest = (UBYTE *)(mr2->memPtr);

                /* wipe out previous contents of buffer */
                memset((UBYTE *)(mr2->memPtr),BUF2_PAT,mr2->memSize);

                tmppos = Seek(g->fh, oldpos, OFFSET_BEGINNING);
                amtRead = Read(g->fh, mr2->memPtr, min(mr2->memSize,(ULONG)fileSize));
                D(aprintf("%s: tmppos %ld, curpos %ld, oldpos %ld, amtRead %ld\n", (LONG)(g->portName),tmppos,curpos,oldpos,amtRead));
                if (amtRead < 0) {
                    mr->status = STATUS_SEVERE_ERR;
                ...
                for (j = 0; j < amtRead; j++) {
                    if (src[j] != dest[j]) {
                        ...
                        servLog(mr,g,"Verify Error, offset %ld: 0x%lx != 0x%lx (rep %ld)",fileSize-amtWrote-bytesLeft+j,src[j],dest[j],i+1L);
                        if (!firstErr)
                            firstErr = j;
                    }
                }
                if (errCount) {
                    servLog(mr, g, "%ld Errors in verify %ld of %ld on %s, First = %ld, Range: %ld to %ld",
                            errCount, i+1L, reps, (LONG)fileName, firstErr, oldpos, oldpos+amtRead);
                }
                errCount = 0; 
                firstErr = 0;
            }
    ...
    /* Tell server we finished tests, then exit */
    servLog(mr, g, "Completed %ld File Writes %s of '%s'", 
            reps, ((verify) ? "and verifies" : ""), fileName);
...
/* memSet =============== ulong version of memset() ==================================
   remember that size is in bytes, *NOT* in ULONGs 
 */
void memSet(ULONG *ptr, ULONG pat, ULONG size) {
    UBYTE *ptr2, rem = size % 4;

    size /= 4UL;
    while (size--) {
        *ptr++ = pat;
    }
    /* fill in remaining bytes from pattern */
    ptr2 = (UBYTE *)ptr;
    while (rem--) {
        *ptr2++ = (UBYTE)(pat>>((rem+1)*8));
    }
}
