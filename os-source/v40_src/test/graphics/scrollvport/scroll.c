...
/*			scr->ViewPort.RasInfo->RxOffset = RangeRand(scr->Width/2);
			...
			printf("vp=%08lx ri=%08lx",&scr->ViewPort,scr->ViewPort.RasInfo);
			if (scr->ViewPort.RasInfo->Next)
				printf("rinext=%08lx rinext->bm=%08lx\n",scr->ViewPort.RasInfo->Next,
						scr->ViewPort.RasInfo->Next->BitMap);
