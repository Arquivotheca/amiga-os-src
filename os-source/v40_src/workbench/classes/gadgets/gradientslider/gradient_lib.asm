...
LibName DC.B 'gradientslider.gadget',0
LibId   VSTRING

        CNOP    0,4

LibInitTable:
        DC.L    GradientLib_SIZEOF
        DC.L    LibFuncTable
        DC.L    0
        DC.L    LibInit
...
LibInit:
        ...
        lea     GfxName(pc),a1
        bsr.s	OpenLib
        move.l  d0,gb_GfxBase(a5)

        move.l  #AO_Intuition,d7
        lea     IntuitionName(pc),a1
        bsr.s	OpenLib
        move.l  d0,gb_IntuitionBase(a5)

        move.l  #AO_UtilityLib,d7
        lea     UtilityName(pc),a1
        bsr.s	OpenLib
        move.l  d0,gb_UtilityBase(a5)
...
; now initialize the four math function pointers
	...
	lea	gb_SMult32(a5),a0	; load target address of first vector
	moveq	#3,d0
1$:
	move.l	a1,(a0)+
	subq.l	#6,a1
...
; On entry, A6 has GradientBase, task switching is disabled
; Returns 0 for failure, or GradientBase for success.
LibOpen:
...
CloseLibs:
        move.l  gb_GfxBase(a5),a1
        CALL    CloseLibrary

        move.l  gb_IntuitionBase(a5),a1
        CALL    CloseLibrary

        move.l  gb_UtilityBase(a5),a1
        GO	CloseLibrary
