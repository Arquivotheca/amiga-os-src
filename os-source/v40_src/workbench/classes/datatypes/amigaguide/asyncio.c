...
static VOID SendPacket (struct AGLib * ag, struct AsyncFile * file, APTR arg2)
{
 ...
 */
static LONG WaitPacket (struct AGLib * ag, struct AsyncFile * file)
{
    LONG bytes;
	...
	while (TRUE)
	{
	    ...
	    /*
	     * Wait for the packet to come back, and remove it from the message list. Since we know no other packets can come in to the port, we can safely use Remove() instead of GetMsg(). If other packets could come in, we would have to
	     * use GetMsg(), which correctly arbitrates access in such a case
	     */
	    ...
	    SendPacket (ag, file, file->af_Buffers[file->af_CurrentBuf]);
	}
    }
 ...
 * message port.
 */
static VOID RequeuePacket (struct AGLib * ag, struct AsyncFile * file)
{
...
struct AsyncFile *OpenAsync (struct AGLib * ag, const STRPTR fileName, UBYTE accessMode, LONG bufferSize)
{
    ...
    {
	if (handle = LVOOpenE (ag, NULL, fileName, MODE_OLDFILE))
	...
	{
	    /* in append mode, we open for writing, and then seek to the end of the file.
	     * That way, the initial write will happen at the end of the file, thus extending it */
	...
	/* we want a lock on the same device as where the file is.
	 * We can't use DupLockFromFH() for a write-mode file though.
	 * So we get sneaky and get a lock on the parent of the file */
	if (handle)
    ...
    if (handle)
    {
	/* if it was possible to obtain a lock on the same device as the file
	 * we're working on, get the block size of that device and round up our
	 * buffer size to be a multiple of the block size. This maximizes DMA efficiency. */
	...
	/*
	 * now allocate the ASyncFile structure, as well as the read buffers. Add 15 bytes to the total size in order to allow for later quad-longword alignement of the buffers
	 */
	...
	if (file = AllocVec (sizeof (struct AsyncFile) + bufferSize + 15, MEMF_ANY))
	{
	    ...
	    /*
	     * initialize the ASyncFile structure. We do as much as we can here, in order to avoid doing it in more critical sections
	     *
	     * Note how the two buffers used are quad-longword aligned. This helps performance on 68040 systems with copyback cache. Aligning the data avoids a nasty side-effect of the 040 caches on DMA. Not aligning the data causes the
	     * device driver to have to do some magic to avoid the cache problem. This magic will generally involve flushing the CPU caches. This is very costly on an 040. Aligning things avoids the need for magic, at the cost of at most
	     * 15 bytes of ram.
	     */
	    ...
	    /*
	     * this is the port used to get the packets we send out back. It is initialized to PA_IGNORE, which means that no signal is generated when a message comes in to the port. The signal bit number is initialized to SIGB_SINGLE,
	     * which is the special bit that can be used for one-shot signalling. The signal will never be set, since the port is of type PA_IGNORE. We'll change the type of the port later on to PA_SIGNAL whenever we need to wait for a
	     * message to come in.
	     *
	     * The trick used here avoids the need to allocate an extra signal bit for the port. It is quite efficient.
	     */
		...
		/*
		 * if we are in read mode, send out the first read packet to the file system. While the application is getting ready to read data, the file system will happily fill in this buffer with DMA transfers, so that by the time the
		 * application needs the data, it will be in the buffer waiting
		 */

		file->af_Packet.sp_Pkt.dp_Type = ACTION_READ;
		file->af_BytesLeft = 0;
		if (file->af_Handler)
		    SendPacket (ag, file, file->af_Buffers[0]);
	    }
	    else
	    {
...
LONG ReadAsync (struct AGLib * ag, struct AsyncFile * file, APTR buffer, LONG numBytes)
{
	...
	bytesArrived = WaitPacket (ag, file);
	if (bytesArrived <= 0)
	{
	    if (bytesArrived == 0)
		return (totalBytes);
	...
	SendPacket (ag, file, file->af_Buffers[1 - file->af_CurrentBuf]);
...
LONG ReadCharAsync (struct AGLib * ag, struct AsyncFile * file)
{
    ...
    {
	/* if there is at least a byte left in the current buffer, get it directly. Also update all counters */
    ...
    /* there were no characters in the current buffer, so call the main read routine. This has the effect of sending a request to the file system to have the current buffer refilled. After that request is done, the character is extracted
     * for the alternate buffer, which at that point becomes the "current" buffer */

    if (ReadAsync (ag, file, &ch, 1) > 0)
	return ((LONG) ch);
...
LONG WriteCharAsync (struct AGLib * ag, struct AsyncFile * file, UBYTE ch)
{
    if (file->af_BytesLeft)
    {
	/* if there's any room left in the current buffer, directly write the byte into it,
	 * updating counters and stuff. */
	*(UBYTE *) file->af_Offset = ch;
    ...
    /* there was no room in the current buffer, so call the main write routine.
     * This will effectively send the current buffer out to disk, wait for the other buffer
     * to come back, and then put the byte into it. */
    return (WriteAsync (ag, file, &ch, 1));
}
...
LONG SeekAsync (struct AGLib * ag, struct AsyncFile * file, LONG position, BYTE mode)
{
	...
	{
	    /* the target seek location isn't currently in our buffers,
	     * so move the actual file pointer to the desired location,
	     * and then restart the async read thing... */
	...
	else if ((target < current) || (diff <= file->af_BytesLeft))
	{
	     ...
	     *
	     * 1. The target seek location is within the current read buffer, but before the current location within the buffer. Move back within the buffer and pretend we never got the pending packet, just to make life easier, and faster, in
	     * the read routine.
	     *
	     * 2. The target seek location is ahead within the current read buffer. Advance to that location. As above, pretend to have never received the pending packet.
	     */

	    RequeuePacket (ag, file);
	...
	else
	{
	    ...
	    /*
	     * at this point, we know the target seek location is within the buffer filled in by the packet that we just received at the start of this function. Throw away all the bytes in the current buffer, send a packet out to get the
	     * async thing going again, readjust buffer pointers to the seek location, and return with a grin on your face... :-)
	     */
	...
	/*
	 * this will unfortunately generally result in non block-aligned file access. We could be sneaky and try to resync our file pos at a later time, but we won't bother. Seeking in write-only files is relatively rare (except when
	 * writing IFF files with unknown chunk sizes, where the chunk size has to be written after the chunk data)
	 */
