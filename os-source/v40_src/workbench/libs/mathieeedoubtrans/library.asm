	TTL    '$Id: library.asm,v 37.1 91/01/21 15:18:26 mks Exp $'
**********************************************************************
*								     *
...
**********************************************************************
*
*   $Id: library.asm,v 37.1 91/01/21 15:18:26 mks Exp $
*
**********************************************************************
...
USE_020_881 EQU		1		* enable 020/881 optimizations
...
_MathIeeeDoubTransBase ds.l 1	; used by Pow function
_SysBase ds.l 1
	code
...
io68881	dc.b	'MathIEEE.resource',0
	cnop	0,2
mathbase dc.b	'mathieeedoubbas.library',0
	CNOP 0,2
MIInit:
...
*	find Basic library
	...
	move.l	d0,_MathIeeeDoubBasBase	; store here
	if =				; null?
		movem.l	(sp)+,d2/d3/a2
...
* retest 68881 bit for 68020/68881 and reflect state in local flag
	...
	if <>		; check for alternate functions
		...
		move.l	a1,MathIEEEBase_TaskOpenLib(a2)
		move.l	d3,a1
		btst	#MATHIEEERESOURCEB_DBLTRANS,MathIEEEResource_Flags+1(a1)
		if <>
			move.l	MathIEEEResource_DblTransInit(a1),a0
			jsr (a0)
		endif
	endif
