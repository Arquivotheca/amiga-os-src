...
static LONG NextState(struct IFFHandleP *);
static LONG ReadGenericType(struct IFFHandleP *);
static LONG PurgeLocalContextItem(struct IFFHandleP *,
                                            struct LocalContextItemP *);
static LONG InstallHandler(struct IFFHandleP *,
                           LONG ,
                           LONG ,
                           LONG ,
                           LONG ,
                           struct Hook *,
                           APTR);
static LONG BufferChunk(struct IFFHandleP *,
                        LONG ,
                        void *);
static LONG __asm HandleProperty(register __a0 struct Hook * ,
                                 register __a2 struct IFFHandleP * ,
                                 register __a1 LONG * );
static LONG __asm HandleCollectionChunk(register __a0 struct Hook * ,
                                        register __a2 struct IFFHandleP * ,
                                        register __a1 LONG * );
static LONG __asm PurgeCollectionList(register __a0 struct Hook * ,
                                      register __a2 struct LocalContextItem * ,
                                      register __a1 LONG * );
...
struct IFFHandle * ASM AllocIFFLVO(VOID)
{
...
VOID ASM FreeIFFLVO(REG(a0) struct IFFHandleP *iffp)
{
	 ...
	 */
	FreeContextNode (iffp, (struct ContextNodeP *)RemHead ((struct List *)&iffp -> iffp_Stack));
	FreeMem (iffp, (long) sizeof (*iffp));
    }
}
...
struct IFFStreamCmd	sc;
    ...
    return (CallAHook(iffp->iffp_StreamHook, iffp, (LONG *) &sc));
}
...
struct IFFStreamCmd		sc;
	...
	if ((iffp -> iffp_Flags & IFFF_RWBITS) == IFFF_WRITE)
		FreeBufferedStream (iffp);

	sc.sc_Command	= IFFSCC_CLEANUP;
	sc.sc_Buf	= NULL;
	sc.sc_NBytes	= 0;

	CallAHook(iffp -> iffp_StreamHook, iffp, (LONG *) &sc);
    }
}
                     ...
                     REG(d0) LONG control)
{
	...
	register struct ChunkHandler	 *ch;
	register long			 error, eoc;
 ...
 */
static LONG NextState(struct IFFHandleP *iffp)
{
	register struct ContextNodeP	*top;
	register long			error, topid;

	/*
	 * Deal with the case of the first chunk
	 */
	if (iffp -> iffp_Flags & IFFFP_NEWIO) {
		error = PushChunkR (iffp, 1);
...
static LONG ReadGenericType(struct IFFHandleP *iffp)
{
...
{
    if ((iffp -> iffp_Flags & IFFF_RWBITS) == IFFF_WRITE)
	return (PushChunkW (iffp, type, id, size));

    return (PushChunkR (iffp, 0));
}
...
{
    if ((iffp -> iffp_Flags & IFFF_RWBITS) == IFFF_WRITE)
	return (PopChunkW (iffp));

    return (PopChunkR (iffp));
}
...
struct LocalContextItemP *lip;
     ...
     */
    while (lip = (struct LocaleContextItemP *)RemHead ((struct List *)&cnp -> cnp_LocalItems))
        PurgeLocalContextItem (iffp, lip);
 ...
 */
static LONG InstallHandler(iffp, type, id, ident, pos, hook, object)
struct IFFHandleP	*iffp;
struct Hook		*hook;
APTR			object;
LONG			type, id, ident, pos;
{
	register struct LocalContextItemP	*lci;
	register struct ChunkHandler		*ch;
	register LONG				error;
...
LONG ASM EntryHandlerLVO(REG(a0) struct IFFHandleP *iffp,
                         ...
                         REG(a2) APTR object)
{
    return (InstallHandler(iffp,type,id,IFFLCI_ENTRYHANDLER,pos,hook,object));
}
...
LONG ASM ExitHandlerLVO(REG(a0) struct IFFHandleP *iffp,
                        ...
                        REG(a2) APTR object)
{
    return (InstallHandler(iffp,type,id,IFFLCI_EXITHANDLER,pos,hook,object));
}
...
LONG ASM PropChunkLVO(REG(a0) struct IFFHandleP	*iffp,
                      ...
                      REG(d1) LONG id)
{
	static struct Hook	InternalPropHook = {
		{ NULL },
		HandleProperty,
		NULL,
		NULL
...
LONG ASM PropChunksLVO(REG(a0) struct IFFHandleP	*iffp,
                       ...
                       REG(d0) LONG n)
{
    return (ListAction(iffp,list,n,PropChunkLVO));
}
...
LONG ASM StopChunkLVO(REG(a0) struct IFFHandleP	*iffp,
                      ...
                      REG(d1) LONG id)
{
	static struct Hook	InternalStopHook = {
		{ NULL },
		HandleStopChunk,
		NULL,
		NULL
...
LONG ASM StopChunksLVO(REG(a0) struct IFFHandleP *iffp,
                       ...
                       REG(d0) LONG n)
{
   return (ListAction(iffp,list,n,StopChunkLVO));
}
...
LONG ASM CollectionChunkLVO(REG(a0) struct IFFHandleP *iffp,
                            ...
                            REG(d1) LONG id)
{
	static struct Hook	InternalCollHook = {
		{ NULL },
...
LONG ASM CollectionChunksLVO(REG(a0) struct IFFHandleP *iffp,
                             ...
                             REG(d0) LONG n)
{
    return (ListAction(iffp,list,n,CollectionChunkLVO));
}
...
LONG ASM StopOnExitLVO(REG(a0) struct IFFHandleP *iffp,
                       ...
                       REG(d1) LONG id)
{
	static struct Hook	InternalExitHook = {
		{ NULL },
		HandleExitStop,
		NULL,
		NULL
                               ...
                               REG(a1) LONG *cmd)
{
	 ...
	 */
	if (error = BufferChunk (iffp, size, sp + 1)) {
		FreeLocalItem (lci);
		return (error);
	}
...
static LONG ASM HandleCollectionChunk(REG(a0) struct Hook *hook,
                                      ...
                                      REG(a1) LONG *cmd)
{
	register struct CollectionList	*cltop;
	...
	static struct Hook		InternalPurgeHook = {
		{ NULL },
		PurgeCollectionList,
		NULL,
		NULL
	...
	if (error = BufferChunk (iffp, size, ci + 1))
		goto ackphft;
...
static LONG ASM PurgeCollectionList(REG(a0) struct Hook *hook,
                                    ...
                                    REG(a1) LONG *cmd)
{
	register struct CollectionList	*cl;
	register struct CollectionItem	*ci, *nextci;
...
{
	register struct LocalContextItemP	*next;
	register LONG				ident, ID, type;
	...
	for (item = (struct LocalContextItemP *)NEXT (item);
		    ...
		    item->lip_Type == type)
		{
			...
			PurgeLocalContextItem (iffp, item);
			break;
		}
	}
}
...
{
	register struct ContextNodeP		*cnp;
	register struct LocalContextItem	*li;
