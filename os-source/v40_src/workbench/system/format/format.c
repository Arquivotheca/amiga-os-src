...
char                       str[80];
    ...
    if (DOSBase && IntuitionBase && GfxBase && UtilityBase && GadToolsBase && IconBase)
    {
        ...
        if (WBenchMsg)
        {
            ...
            if (diskObj)
            {
                if (arg = FindToolType(diskObj->do_ToolTypes,"TRASHCAN"))
                    global->gd_Trashcan = !MatchToolValue(arg,"NO");

                if (arg = FindToolType(diskObj->do_ToolTypes,"VOLUMENAME"))
                    stccpy(global->gd_VolumeName,arg,sizeof(global->gd_VolumeName));
...
ULONG                      sectorsPerBlock;
    ...
    sectorsPerBlock = GetSectorsPerBlock(env);

    global->gd_BlocksUsed      = infoData.id_NumBlocksUsed;
    global->gd_BytesPerBlock   = env->de_SectorSize*4 * sectorsPerBlock;
    global->gd_NumBlocks       = (env->de_SectorsPerTrack * env->de_Surfaces * (env->de_HighCyl-env->de_LowCyl+1)) / sectorsPerBlock;
...
struct Hook          nameHook;
    ...
    nameHook.h_Entry = (HOOKFUNC)NameHook;
 ...
 */
    ...
    if (global->gd_LastAdded)
    {
        ...
        if (result)
        {
            ...
            /* This should not be done. We should instead refuse to format
             * and ask the user for a real name.
             */
            if (global->gd_VolumeName[0] == 0)
                strcpy(global->gd_VolumeName,GetStr(MSG_FMT_EMPTY_DISK));
        ...
        if (infoData.id_DiskType != ID_NO_DISK_PRESENT)
        {
            ...
            if (infoData.id_DiskType != ID_BUSY_DISK)
            {
                ...
                if (infoData.id_DiskState != ID_WRITE_PROTECTED)
                {
                    ...
                    wordsPerSector = env->de_SectorSize;
                    bytesPerSector = wordsPerSector * 4;
                    sectorsPerBlk  = GetSectorsPerBlock(env);
                    sectorsPerCyl  = env->de_SectorsPerTrack * env->de_Surfaces;
                    sectorsPerDisk = sectorsPerCyl * (highCyl - lowCyl + 1);
                    bytesPerBlk    = bytesPerSector * sectorsPerBlk;
                    wordsPerBlk    = bytesPerBlk / 4;
                    blocksPerDisk  = sectorsPerDisk / sectorsPerBlk;
                    bytesPerCyl    = bytesPerSector * sectorsPerCyl;
                    rootBlk        = (blocksPerDisk - 1 + env->de_Reserved) >> 1;

                    if (env->de_TableSize >= DE_MAXTRANSFER)
                    {
                        /* if the Envec tells us what the max is, use it */
                        maxTrans = env->de_MaxTransfer;

                        if (maxTrans > bytesPerCyl)
                        {
                            /* we don't want than one cylinder's worth... */
                            maxTrans = bytesPerCyl;
                        }
                    }
                    else
                    {
                        /* otherwise, assume a max of cylimder size */
                        maxTrans = bytesPerCyl;
                    }

                    if (maxTrans > 65536)
                    {
                        /* We don't want more than 64K at a pop. We'd just
                         * waste memory...
                         */

                        if (bytesPerBlk < 65536)
                        {
                            /* set maxTrans to the largest number of blocks
                             * that fit in 64K
                             */
                        ...
                        {
                            /* We need to transfer at least one block at a
                             * time. BTW, this means a block on this device is
                             * more than 64K!
                             */
                            ...
                            if (DoPkt1(fsPort,ACTION_INHIBIT,DOSTRUE))
                            {
                                ...
                                if (!OpenDevice(BTOCSTR(fssm->fssm_Device),fssm->fssm_Unit,devIO,fssm->fssm_Flags))
                                {
                                    ...
                                    if (!quick)
                                    {
                                        ...
                                        error = ERROR_NO_FREE_STORE;
                                        if (buffer = AllocVec(maxTrans,memType))
                                        {
                                            error = 0;
                                            cyl   = lowCyl;
                                            ...
                                            while ((cyl <= highCyl) && !error)
                                            {
                                                ...
                                                for (i = 0; i < maxTrans / 4; i++)
                                                    buffer[i] = cyl - lowCyl << 16 | i | dosType;

                                                if (cyl == lowCyl)
                                                    buffer[0] = ID_BAD_DISK;
