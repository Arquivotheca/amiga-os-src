**********************************************************************
*
* Transfer routine for Okidata_293I
*
...
; ULONG BufOffset		20-23
	...
	cmpi.w	#288,ped_YDotsInch(a4)	;interleaving?
	bne.s	10$			;no, clear BufOffset
	asr.w	#1,d0			;y /= 2
	btst.b	#0,d3			;odd pass?
	bne.s	20$			;no, dont clear BufOffset

10$:	moveq.l	#0,d1			;BufOffset = 0

20$:	move.w	d0,d6
	not.b	d6			;d6 = bit to set
	divu.w	#16,d0			;d0 = y / 16
	swap.w	d0			;d0 = y % 16 (ymod)
...
gsx_loop:
	...
	bset.b	d6,0(a1,d0.w)		;*(ptr + x2) |= bit
