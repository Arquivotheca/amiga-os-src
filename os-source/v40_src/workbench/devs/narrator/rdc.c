...
while(*bytptr != 0xff)
{   ff = *(bytptr+6);		/* get flags */
    f1 = (ff & NASAL) ? (WORD)(*bytptr - 31) : f1t[(WORD)*bytptr];
    bytptr++;
    f2 = (ff & FRIC) ? (WORD)(1 + (*bytptr >> 3)) : f2t[(WORD)*bytptr];
    bytptr++;
    f3 = (ff & ASPIR) ? (WORD)(1 + (*bytptr >> 3)) : f3t[(WORD)*bytptr];
    bytptr++;
    a1 = (WORD)*bytptr++;
    a2 = (WORD)*bytptr++;
    a3 = (WORD)*bytptr++;  bytptr++; /* skip flags byte */
    f0 = (WORD)(0.5 + 11100.0 / (float)*bytptr++);
    for(k=0; k<6; k++)  flags[k] = ' ';		/* clear flags string */
    if(ff & VOICED)	flags[0] = 'v';
    if(ff & FRIC)	flags[1] = 'f';
    if(ff & ASPIR)	flags[2] = 'a';
    if(ff & NASAL)	flags[3] = 'n';
    if(ff & WORDBND)	flags[4] = 'w';
    if(ff & PHONBND)
    {	flags[5] = 'p';
	printf("%s\n",dashes);
    }
    printf("%5ld     %5d  %5d  %5d  %3d  %3d  %3d  %s  %3d\n",
	     j++,    f1,  f2,  f3,  a1,  a2,  a3,flags,f0);
}
printf("\n\n");
