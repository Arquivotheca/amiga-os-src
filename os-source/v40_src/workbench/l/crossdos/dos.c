...
struct DateStamp *date;
{
    ...
    {
          mfib->mfib_DiskCluster   = lock->ml_KeyPtr->kp_dir_clust;   /* point to disk cluster */
          mfib->mfib_DiskDirEntry  = lock->ml_KeyPtr->kp_dir_ent;   /* point to previous entry */
...
/* Abort rename if newlock == Disk name */
    if( (newlock = GetLock(fromlock, fromname, SHARED_LOCK))
        && !((newlock->ml_KeyPtr->kp_dir_clust == ROOTDIR_CLUST) && (newlock->ml_KeyPtr->kp_dir_ent == -1)))
    {
    /* copy old dirent into memory and remember where it is currently stored */ 
          mfib->mfib_DiskCluster    = (fromclust  = newlock->ml_KeyPtr->kp_dir_clust);
          mfib->mfib_DiskDirEntry   = (fromdirent = newlock->ml_KeyPtr->kp_dir_ent)-1;
          mfib->mfib_DirEntStatus   = MLDE_DIR;
        ...
        {       /* continue, no error occurred yet */
            ...
            {       /* Renaming across directories */
                ...
                /* move a directory.  Update directory parent pointer */
                      mfib->mfib_DiskCluster    = newlock->ml_KeyPtr->kp_start_clust;
                ...
                while( chklock = (struct MLock *)BADDR(chklock->ml_Link))
                {
                    if( (chklock->ml_KeyPtr->kp_dir_ent == fromdirent)
                        && (chklock->ml_KeyPtr->kp_dir_clust == fromclust))
                    {
                        chklock->ml_KeyPtr->kp_dir_ent = todirent;
                        chklock->ml_KeyPtr->kp_dir_clust = toclust;
                    }
                }
            }
             else
...
UCHAR *name;     /* BADDR(BSTR) */
{
    ...
    if( (dirlock = GetLock(lock, name, EXCLUSIVE_LOCK) )
        && (dirlock->ml_KeyPtr->kp_dir_ent > -1) )
    {
                /* get dirent for dirlock */
          mfib->mfib_DiskCluster   = (dirlock->ml_KeyPtr->kp_dir_clust);   /* point to disk clust */
          mfib->mfib_DiskDirEntry  = (dirlock->ml_KeyPtr->kp_dir_ent)-1;   /* point to previous entry */
          mfib->mfib_DirEntStatus  = MLDE_DIR;
        dirent = GetNextDirEnt(mfib);
