...
struct ConfigDev *TestProduct(struct ConfigDev *cd, STRPTR s, LONG len);
struct ConfigDev *TestManufacturer(STRPTR s);
struct Resident *FindRomTag(BPTR seg);
char *Index(STRPTR s, char c);
char *Indexn(STRPTR s, char c, LONG len);
BOOL NonAlpha(STRPTR s, LONG len);
struct ConfigDev *MarkConfigDev(struct ConfigDev *cd, LONG manufacturer,
                                LONG product);
...
*/
BOOL CheckLink(struct ConfigDev *cd, struct ConfigDev *newcd)
{
    while (cd)
    {
	if (cd == newcd)
