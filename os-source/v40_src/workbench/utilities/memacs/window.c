...
/*
 * Goto the beginning of the window.
 ...
 char *wname;
{
	...
	wp = wheadp;
	while (wp != NULL) {
		if (strcmp(wname, wp->w_bufp->b_bname) == 0) return (wp);
		wp = wp -> w_wndp;
	}
	return(NULL);
}			
    ...
    int f,n;
{
	...
	if ((wp = (EWINDOW *) malloc(sizeof(EWINDOW))) == NULL) {
		mlwrite("Cannot allocate EWINDOW block");
		return (FALSE);
	}
...
/* jump the window forward a page */
int windforwpage(f, n)
    int f,n;
{
    nextwind(0,0);
    forwpage(FALSE,0);
...
/* move the window back a page */
int windbackpage(f, n)
    int f,n;
{
    nextwind(0,0);
    backpage(FALSE,0);
