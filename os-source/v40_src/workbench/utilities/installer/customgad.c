...
#ifdef AZTEC_C
static long buttonhook(struct Hook *ihook,long *msg,struct Gadget *g);
#pragma regcall( buttonhook(a0,a1,a2) )
static long arrowhook(struct Hook *ihook,long *msg,struct Gadget *g);
#pragma regcall( arrowhook(a0,a1,a2) )
#else
...
{	WORD		text_width,
	...
	Move(rp,gad->LeftEdge + text_offset,
		gad->TopEdge + (gad->Height - rp->TxHeight)/2 +
		rp->TxBaseline + 1);
...
static long arrowhandle(struct gpInput *msg,struct Gadget *g)
{
	...
	if (msg->gpi_IEvent->ie_Class == IECLASS_RAWMOUSE)
	{
		...
		if (msg->gpi_IEvent->ie_Code == (UBYTE)(IECODE_LBUTTON+IECODE_UP_PREFIX))
		{
			if (inside) return GMR_NOREUSE | GMR_VERIFY;
			return GMR_REUSE;
		}
