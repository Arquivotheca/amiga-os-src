...
extern struct Library *IntuitionBase, *IconBase, *GfxBase;
...
#pragma libcall RexxSysBase CreateRexxMsg 90 89003
#pragma libcall RexxSysBase CreateArgstring 7e 8002
...
static struct DrawerData def_drawer_data = {
	{	100,50, 320,70, -1,-1, 0,
		...
		SIMPLE_REFRESH | SIZEBRIGHT | SIZEBBOTTOM | WBENCHWINDOW | 0x00040000,
		NULL, NULL, NULL, NULL, NULL,
		90,40, -1,-1, WBENCHSCREEN
	},
	0, 0
};
...
{	extern struct Library *IconBase;
	...
	if (IconBase->lib_Version >= 36) return GetDiskObject(NULL);
...
{	extern struct Library *IconBase;
	...
	if (IconBase->lib_Version >= 36) FreeDiskObject(dobj);
	else FreeMem(dobj,sizeof *dobj);
}
...
#else
void swap_mem(void *a,void *b,LONG size);
#endif
...
BOOL modify_icon(char *name,struct ToolType *ntt,char *deftool,int stack,
	WORD nopos,WORD swap)
{
	struct DiskObject	*dobj;
	struct ToolType		*att;
	char				*c,*ttbuf,**tt,**ftt,*tv;
	WORD				i,k,count,final;
	BOOL				result = FALSE;

	if (dobj = GetDiskObject(name))
	{
		count = 1;
		if (tt = dobj->do_ToolTypes) while (*tt++) count++;
		att = ntt;
		while (att) { count++; att = att->next; };
		...
		if (ftt = MemAlloc(count * (MAX_TOOLSTRING + sizeof (char *)),MEMF_CLEAR))
		{
			...
			if (tt = dobj->do_ToolTypes) while (*tt)
			{	i = strlen(*tt) + 1;
				if (i + ttbuf >= (MAX_TOOLSTRING * count) + (char *)(ftt + count))
				{
					/* ERROR */
				}
				ftt[final++] = ttbuf;
			...
			{	if (c = (char *)FindToolType(ftt,STR_VAL(att->tooltype)))
				{
					for (k=0;ftt[k];k++)
						if (c >= ftt[k] && c < ftt[k] + strlen(ftt[k])) break;

					if (att->toolval)
					{
change_it:				i = strlen(STR_VAL(att->tooltype)) + strlen(STR_VAL(att->toolval)) + 2;
						if (i + ttbuf >= (MAX_TOOLSTRING * count) + (char *)(ftt + count))
						{
							/* ERROR */
						}
			...
			if (deftool) dobj->do_DefaultTool = deftool;
			dobj->do_ToolTypes = ftt;

			if (stack) dobj->do_StackSize = stack;

			if (nopos) dobj->do_CurrentX = dobj->do_CurrentY =
				NO_ICON_POSITION;

#ifndef ONLY2_0
			if (swap && IntuitionBase->lib_Version < 36)
			{
			...
			if (deftool) dobj->do_DefaultTool = c;
...
{	BPTR			lock;
	...
	if (lock = LockDir(path,ACCESS_READ)) goto ok1;

	if (lock = CreateDir(path))
	{
ok1:	UnLock(lock);

		if (icon == FALSE || create_drawer_icon(path)) return TRUE;

		DeleteFile(path);
	}
	err_msg(ehead,ERR_DOS,FUNC_MAKEDIR,err_cannot_create);
	return FALSE;
}
...
/* this also needs to copy protection, filedate & filenote */
	...
	/* I figure that 16K is a good buffer size. */
/* #define COPYBUFSIZE		(1024 * 16) */
...
WORD CopyFileWithStatus(BPTR sfh,BPTR dfh,char *sname,BPTR dlock,long size)						/* support for copy progress box */
{
	...
	if (bufsize > COPYBUFSIZE) bufsize = COPYBUFSIZE;

	unless (buffer = AllocMem(bufsize,MEMF_PUBLIC|MEMF_CLEAR)) return -1;
	...
	while (TRUE)
	{
		...
		if (readsize < 0) break;
		if (readsize > 0 &&
			Write(dfh,buffer,readsize) != readsize) break;
		if (readsize < bufsize) { result = 1; break; }

		read += bufsize;
		if (sname && (i = ((int)fgad->Width * read) / size))
		{
			if (i == 0) i = 1;		/* no negative rectfills, please */

			SetAPen(rp,highlight_color);
			RectFill(rp,fgad->LeftEdge,fgad->TopEdge,
				fgad->LeftEdge + i - 1,fgad->TopEdge+fgad->Height-1);
...
{	BPTR	lock;
	...
	{
		unless (lock = CreateDir(name)) return NULL;

		UnLock(lock);
		
		unless (lock = Lock(name,ACCESS_READ)) return NULL;
	}
	...
	*error = 0;
	...
	if (protection & (FIBF_WRITE | FIBF_DELETE))
	{
		*error = (protection & FIBF_DELETE ?
		...
		if ( !(local_delopts & (DELOPT_ASKUSER | DELOPT_FORCE)) ) return NULL;
	...
	if (file = Open(name,MODE_NEWFILE)) return file;
	*error = IoErr();
	return NULL;
}
...
{	struct FileInfoBlock *fib;
	...
	if (newname == NULL) newname = FileOnly(filename);
	...
	{
	 	if (local_delopts & DELOPT_OKNOSRC) { result = 1; goto err2; }
		goto err1;
	}

	if (Examine(slock,fib) == 0) goto err1;

	unless (dlock = LockOrCreateDir(dirname)) goto err1;

	olddir = CurrentDir(dlock);

	unless (dfile = OpenNew(newname,&error))
	{
		if (error == 0) { result = 1; goto err2; }
		if ((error == ERROR_DELETE_PROTECTED ||
			error == ERROR_WRITE_PROTECTED ||
			error == ERROR_DISK_WRITE_PROTECTED) &&
			(local_delopts & DELOPT_OKNODELETE)) { result = 1; goto err2; }
#ifdef ONLY2_0
		SetIoErr(error);
#else
		mySetIoErr(error);
	...
	cp = CopyFileWithStatus(sfile,dfile,show_gauge ? filename : NULL,
		dlock,fib->fib_Size);
	if (ok) abortcopy_clear(counts);
	...
	if (cp == -1) { memerr(); goto err2; }

	Close(dfile);
	dfile = NULL;

	SetProtection(newname,fib->fib_Protection & ~FIBF_ARCHIVE);
	if (fib->fib_Comment[0]) SetComment(newname,fib->fib_Comment);
...
err1:
	...
	{
		err_msg(ehead,ERR_DOS,FUNC_COPYFILES,err_cannot_copy);
	}
...
int copy_tree_file(BPTR src,BPTR dest,struct FileInfoBlock *fib)
{
	...
	if (!strcmp(fib->fib_FileName,".info")) return 1;

	olddir = CurrentDir(src);

	unless (sfile = Open(fib->fib_FileName,MODE_OLDFILE)) goto err;
	...
	cp = CopyFileWithStatus(sfile,dfile,show_gauge ? fib->fib_FileName : NULL,
		dest,fib->fib_Size);
	...
	if (fib->fib_Comment[0]) SetComment(fib->fib_FileName,fib->fib_Comment);
	...
	if (fib = AllocMem(sizeof *fib,MEMF_PUBLIC|MEMF_CLEAR))
	{
		if (Examine(src,fib))
		{
			while (ExNext(src,fib))
			{
				...
				{
					if (!copy_tree_file(src,dest,fib)) goto err;
				}
				else if (fib->fib_EntryType > 0) /* dir */
				{
					olddir = CurrentDir(src);
...
#if 0
							unless (lock = CreateDir(fib->fib_FileName)) goto err;

							UnLock(lock);

							unless (lock = Lock(fib->fib_FileName,ACCESS_READ))
								goto err;
#endif
						}

						if (!copy_tree_dir(slock,lock)) goto err;
...
{	BPTR				lock;
	...
	{
		if ( !(local_delopts & DELOPT_NOFAIL) )
			err_msg(ehead,ERR_DOS,FUNC_COPYFILES,err_cannot_create);
...
err:	if ( !(local_delopts & DELOPT_OKNOSRC) )
	...
	if (name == NULL) name = FileOnly(src);
...
{	BPTR	lock;
	...
	if (*execute_dir_name == NULL) return 0;
...
int execute_command(char *command)
{
	...
	if (lock == (BPTR)-1) { Close(nil); return -1; }
...
bye:
	if (rxmsg)
	{	if (rxmsg->rm_Args[0]) DeleteArgstring(rxmsg->rm_Args[0]);
		DeleteRexxMsg(rxmsg);
	}
	if (nil) Close(nil);
...
{	long						length = strlen(name),
	...
	buf = AllocMem(sizeof *dnode + 4,MEMF_PUBLIC|MEMF_CLEAR);
	if (buf == NULL)
	{
bad:	memerr();
		return FALSE;
...
BOOL SetAssign(char *name,BPTR flock)
{
	...
	else
	{
		...
		if (dnode != NULL || (dnode == NULL && flock == NULL)) return 1;
...
long MatchAssign(char *new,char *old,long opts)
{
	...
	if (FindAssign(old,opts,&lock))
	{
		...
		{	struct RootNode	  	  		*rnode; 
			...
			if (!add_assign(dinfo,new,lock)) UnLock(lock);
			else result = TRUE;
		}
#endif
	}
...
long VolumeMounted(char *name, long types)
{
...
#endif
	{
		...
		{
			if (dnode = (struct DeviceNode *)FindDosEntry((void *)dnode,(UBYTE *)name,types))
			{
				if (dnode->dn_Task != NULL) result = TRUE;
			}
	...
	else
	{
		if (dnode = (struct DeviceNode *)LockDevList())
		{
			if (dnode = (struct DeviceNode *)FindDevEntry((void *)dnode,(UBYTE *)name,types))
			{
				if (dnode->dn_Task != NULL) result = TRUE;
			}
...
{	struct StandardPacket 	*packet;
	...
	{	lock = Lock(name,ACCESS_READ);
		if (lock) { parent = ParentDir(lock); UnLock(lock); }

		if (!MakeClearStruct(packet)) goto bye;

		if (ds == NULL) DateStamp((void *)(ds = &curds));
...
bye:
		if (parent) UnLock(parent);				/* free lock on parent */
