...
extern struct Library	*LayersBase, *GfxBase;
...
{	WORD		text_width,
	...
	if (underscore != NULL) text_width -= TextLength(rp,"_",1);
	...
	if (underscore != NULL)
	{
		if (underscore > text) Text(rp,text,underscore - text);
		x = rp->cp_x;
...
void new_border_button(struct Button *gad,UWORD id,char *text,struct Gadget **list)
{	struct Border	*b;
	WORD			*v;
...
#ifndef ONLY2_0
	if (text) write_text(rp,(struct Gadget *)gad,text);
#endif
...
void slider_gadget(struct Gadget **list, UWORD id, WORD x, WORD y,
...
void list_gadget(struct Gadget **list, UWORD id)
{
	...
	DrawBevel(rp,left_edge,place_top,window_width-r-left_edge,
		place_bottom-place_top-2,0L);

	gad->Gadget.NextGadget = *list;
	*list = (struct Gadget *)gad;
}

void layout_button(struct Gadget **list, char *text, UWORD id,
	WORD lower_flag, WORD width_type)
{
	struct Button		*gad;
	WORD				temp;
	...
	if (lower_flag & PUT_LEFT) gad->Gadget.LeftEdge = left_edge;
	else if (lower_flag & PUT_RIGHT)
		gad->Gadget.LeftEdge = window_width - right_edge - gad->Gadget.Width;
	else gad->Gadget.LeftEdge = (window_width - gad->Gadget.Width) / 2;
	...
	{	temp = place_bottom - gad->Gadget.Height - 1;
		...
		if ( !(lower_flag & PUT_STAY) ) place_bottom = temp;
	}
	else
	{	gad->Gadget.TopEdge = place_top;
		if ( !(lower_flag & PUT_STAY) ) place_top += gad->Gadget.Height + 1;
	}

	new_border_button(gad,id,text,list);
}

void layout_2_buttons(struct Gadget **list, char *text1, UWORD id1,
	char *text2, UWORD id2, WORD lower_flag)
{
...
void layout_3_buttons(struct Gadget **list, char *text1, UWORD id1,
	char *text2, UWORD id2, char *text3, UWORD id3, WORD lower_flag)
{
...
void layout_side_buttons(struct Gadget **list, char *text1, UWORD id1,
	char *text2, UWORD id2, WORD lower_flag)
{
	...
	{	gad1->Gadget.TopEdge = gad2->Gadget.TopEdge = place_top;
		if ( !(lower_flag & PUT_STAY) ) place_top += gad1->Gadget.Height + 1;
	}
...
void finish_textzone(struct TextZone *gad, char *text, UWORD id)
{
...
#endif
	{
		...
		else gad->Gadget.Flags |= GFLG_STRINGEXTEND;
	...
	/* common code isolated from two subroutines below... --DJ */
...
void std_text_gad(struct TextZone *gad, WORD lower_flag)
{
	...
	DrawBevel(rp,gad->Gadget.LeftEdge-4,gad->Gadget.TopEdge-2,
		gad->Gadget.Width+8,gad->Gadget.Height+4,0);
	DrawBevel(rp,gad->Gadget.LeftEdge-2,gad->Gadget.TopEdge-1,
		gad->Gadget.Width+4,gad->Gadget.Height+2,1);
}
...
{	UBYTE	*text = (UBYTE *)*textptr;
	...
	WORD	pixel_length = 0;
	WORD	last_size = 0, current_size = 0;
	UBYTE	*last_text = text;
	WORD	c, i;
	UBYTE	squote, quote;

		/* clear special condition flags */
	...
	for (;;)
	{
		/* has the allowed space been exceeded? */

		if (pixel_length > pixels)
		{
			/* space exceeded... was no break found? */

			if (last_size == 0)
			{
				/* no break... were we left inside a quoted string? */

				if (quote != 0 && !(wrap_flags & WRAP_NOQUOTE))
				{
					/* redo whole thing ignoring quoted strings */

					text = last_text = *textptr;
					current_size = pixel_length = 0;
					wrap_flags |= WRAP_NOQUOTE;
		...
		if ((quote == 0 || (wrap_flags & WRAP_NOQUOTE)) && c == ' ')
		...
		if ((UFont->tf_Flags & FPF_PROPORTIONAL) &&
			(c <= UFont->tf_HiChar && c >= UFont->tf_LoChar) )
		{	c -= UFont->tf_LoChar;
			pixel_length += (space[c] + kern[c]);
		}
		else pixel_length += xsize;		/* character position		*/

		current_size++;
	}

	if (last_quote = quote) wrap_flags |= WRAP_INQUOTE;
	else wrap_flags = 0;
	return current_size;
}
...
#else
extern LONG mySetSoftStyle(struct RastPort *, LONG, LONG);
#endif

void layout_text_length(char *text, WORD length, WORD lower_flag,
	WORD style_type, ULONG special)
{
	WORD				temp,
						text_width,
						text_offset,
	...
	{
		temp = TextLength(rp,"-",1);		
		if (text_width + temp <= text_space)
		{
			text_width += temp;
			hyphen = TRUE;
		}
	...
	if (style_type & TEXT_CENTER) text_offset = (window_width - text_width) / 2;
	else text_offset = left_edge + 3;

	if (special != GTEXT_SKIP)
	{
		if (special == GTEXT_PREWIPE || special == GTEXT_PATH)
		{
			SetAPen(rp,0);
			RectFill(rp,left_edge,text_top,
				window_width - right_edge,text_top + rp->TxHeight);
		}

			/* Set Style based on flags... */
		if (style_type & TEXT_BOLD) mySetSoftStyle(rp,FSF_BOLD,FSF_BOLD);

		SetAPen(rp,text_color);
		Move(rp,text_offset,text_top + rp->TxBaseline);
		if (ellipsis) render_text(rp,"...",3);
		render_text(rp,rtext,length);
		if (hyphen) render_text(rp,"-",1);

		if (style_type & TEXT_BOLD) mySetSoftStyle(rp,0,FSF_BOLD);
	}
}

void layout_text(char *text, WORD lower_flag, WORD style_type, UWORD special)
{
	layout_text_length(text,strlen(text),lower_flag,style_type,special);
}
...
{	char 				*textptr;
	...
	while (text)
	{
		...
		layout_text_length(text,length,lower_flag,style_type,
...
WORD layout_box_gads(struct Gadget **list, struct Rectangle *box,
	struct String **text, WORD count, WORD id, WORD type)
{
	WORD				gap = 1;
	WORD				line_height = clamp((type & 0x7f) != IS_CHECKMARK ?
							9 : 11,rp->TxHeight,100);
...
again:					/* in case a second (or third) run though is needed */
	...
	{	total_width = 0;
		...
		{	WORD	col_width = 0;
			...
			{	WORD index = line + col*lines;
				...
				{	pixels = TextPixels((char *)(text[index] + 1)) +
					...
					if (col_width < pixels) col_width = pixels;
				}
			}
		...
		{	if (last_good_lines) break;			/* if we had it before, go with it */
...
try_topaz8:				/* not so fast... lets try it with topaz 8 first */
				...
				{	extern struct TextAttr Topaz8;
					...
					/*	Well, that's a fine kettle of fish...
						Should try making kettle wider...					*/
		...
		files_displayed = columns * lines;		/* max files displayable		*/

			/* if we can't display all the files, then give up now. */

		last_good_lines = lines;
		last_good_cols = columns;
		last_good_width = total_width;

		if (files_displayed < count)
		{
			if (special) break;

			/* ok, lets try a smaller font... */

			if (efont == NULL) goto try_topaz8;
			...
			/* at this point we know that the files will fit on the screen */
			/* if columns are too uneven, then give it another try with less lines */

		if (files_displayed - count < columns) break;
		lines--;
	}
	...
	{	WORD	col_width = 0;
		...
		{	WORD index = line + col*lines;
			...
			{	WORD y = top_position + line * (line_height + gap);
				...
				SetAPen(rp,text_color);
				Move(rp,x + (type == IS_TEXT ? 0 : CHECK_SIZE),
				...
				{	gad->LeftEdge = x;
					...
					if (type == IS_CHECKMARK) build_checkmark(gad,id++);
					else build_radio(gad,id++);
...
{	struct Gadget *gad = FindGadget(window,id);
	...
	if (gad)
	{
		if (gad->Activation & (GADGIMMEDIATE|RELVERIFY))
			gad->Flags &= ~GADGDISABLED;
		SetAPen(rp,0L);
		RectFill(rp,gad->LeftEdge,gad->TopEdge,gad->LeftEdge+gad->Width-1,
...
{	UWORD	b;
	...
	while (--total != sel) gad = gad->NextGadget;
	gad->Flags |= GADGDISABLED;
...
{
	while (total--)
	{	if (gad->Flags & SELECTED) return total;
		gad = gad->NextGadget;
	}
	return -1;
}
...
{	LONG	bits = 1 << total;
	...
	while (total--)
	{	bits >>= 1;
		if (bits & mask) gad->Flags |= SELECTED;
		else gad->Flags &= ~SELECTED;
		gad = gad->NextGadget;
	}
}

LONG final_checkboxes(struct Gadget *gad,WORD total)
{	LONG	mask = 0,
			bits = 1 << total;

	while (total--)
	{	bits >>= 1;
		if (gad->Flags & SELECTED) mask |= bits;
		gad = gad->NextGadget;
	}
...
void hilite_file(struct Gadget *gad, struct ListControl *ctrl)
{
	...
	RectFill(rp,gad->LeftEdge+4,j,gad->LeftEdge+gad->Width-5,
		j + rp->TxHeight - 1);
...
void show_files(struct Gadget *gad, struct ListControl *ctrl)
{
	...
	if (scroll && (delta >= lines || delta <= -lines)) scroll = FALSE;
	...
	else
	{
		...
		SetDrMd(rp,JAM1);
		SetBPen(rp,0L);
		ScrollRaster(rp, 0, delta * ty, gad->LeftEdge, gad->TopEdge + off,
			gad->LeftEdge+gad->Width-1, gad->TopEdge+off+ctrl->Visible*ty-1);
	}
...
LONG which_file(struct Gadget *gad, struct ListControl *list, WORD my)
{
	...
	if (j >= list->Visible || j < 0) return list->Count;
 	j += list->Start;
	if (j >= list->Count) return list->Count;
	return j;
}
...
{	char	*at;
	...
	{	at++;
		ctrl->Count++;
		if (textloc == at - textbuf) ctrl->Hilite = ctrl->Count;
	}
	...
	if (ctrl->Start < 0) ctrl->Start = 0;
}

void show_text(struct Gadget *gad, struct ListControl *ctrl,WORD mode)
{	extern UWORD *RtArrow,*InsertMask,*InsertText;
	...
	for (i=0;i<start;i++) at = strchr(at,'\n') + 1;
	...
	SetAPen(rp,highlight_color);
	BltTemplate((void *)InsertText,0,14,rp,gad->LeftEdge + gad->Width - 102,j-4,99,9);
...
{	char	*text = def->Text;
	...
	if (text != NULL && def->Flags & GFLAG_PTR) text = *(char **)text;
	if (def->Flags & GFLAG_LOCAL) text = GetLocalText((long)text);

	return text;
}

LONG make_gadgets(struct Gadget **gad,struct GadgetDef *def,
	struct InstallationRecord *ir,UWORD mode)
{
	static LONG			u;
	...
	if (mode & GCLEAR_BUSY) clear_busy();

	if (gad == NULL)
	{
		if (mode & GSTART_LAYOUT) start_layout();
	}
	else if (*gad == NULL)
	{
		if (mode & GSTART_LAYOUT) start_layout();
	...
	if (mode & GERASE_PAGE) erase_page();
	...
	do {
		...
		{
			if (ir != NULL) longint = (LONG)ir->deftool_val.v;
		}
		...
		switch (type = def->Flags & GTYPE_MASK)
		{
			...
			DrawBevel(rp,left_edge,place_top,
				window_width-right_edge-28-left_edge,place_bottom-place_top-2,0);
			break;

			case GTYPE_SLIDER:
			slider_gadget(gad,SLIDER_ID,window_width-right_edge-24,place_top,
				24,place_bottom-24-place_top);
			break;

			case GTYPE_ARROW:
			arrow_gadget(gad,UP_ID,window_width-right_edge-24,place_bottom-24,UP_ARROW);
			arrow_gadget(gad,DN_ID,window_width-right_edge-24,place_bottom-13,DN_ARROW);
			break;

			case GTYPE_TEXT:
			if (text)
				layout_text(text,def->Flags >> 8 & 0x0f,def->Flags >> 12 & 0x07,
					def->Flags & GWIDTH_MASK);
			break;

			case GTYPE_WRAP:
			if (text)
				layout_wrap_text(text,def->Flags >> 8 & 0x0f,def->Flags >> 12 & 0x07);
			break;

			case GTYPE_STRGAD:
			layout_strgad(gad,text,def->ID,def->Flags >> 8 & 0x0f);
			if ((def->Flags & GWIDTH_MASK) == GSTRING_ACTIVE)
				tz_activate = (struct TextZone *)*gad;
			break;

			case GTYPE_INTGAD:
			layout_intgad(gad,longint,def->ID,def->Flags >> 8 & 0x0f);
			if ((def->Flags & GWIDTH_MASK) == GSTRING_ACTIVE)
				tz_activate = (struct TextZone *)*gad;
			break;
			...
			else if ((def->Flags & GWIDTH_MASK) == GRADIO_SPECIAL2)
			{
			...
			val = layout_box_gads(gad,&box,data->Choices,data->NumChoices,def->ID,
				(type == GTYPE_RADIO ? IS_RADIO : IS_CHECKMARK) |
				((def->Flags & GWIDTH_MASK) == GCHECK_SPECIAL1 ? 0x80 : 0));

			if (type == GTYPE_RADIO)
				default_radio(*gad,data->DefChoice,data->NumChoices);
			else if (data->DefChoice != -1)
				init_checkboxes(*gad,data->DefChoice,data->NumChoices);

			if (radio_special) place_bottom = box.MinY;
			break;
			...
			switch (def->Flags & GWIDTH_MASK)
			{
				case GBEVEL_STD:
				DrawBevel(rp,left_edge,place_top,
					window_width-right_edge-left_edge,place_bottom-place_top-2,
					def->ID);
				break;
				...
				erase_page();
				DrawBevel(rp,left_edge,top_edge,window_width-right_edge-left_edge,
					window_height-bottom_edge-top_edge,def->ID);
			...
			case GTYPE_BUTTON:
			if (text) switch (def->Flags & GWIDTH_MASK)
			{
				case GWIDTH_HALF:
				layout_button(gad,text,def->ID,def->Flags >> 8 & 0x0f,HALF_WIDTH);
				break;

				case GWIDTH_SIDES:
				text1 = make_gad_text(&def[1]);
				layout_side_buttons(gad,text,def->ID,
					text1,def[1].ID,def->Flags >> 8 & 0x0f);
				def++;
				break;

				case GWIDTH_2:
				text1 = make_gad_text(&def[1]);
				layout_2_buttons(gad,text,def->ID,
					text1,def[1].ID,def->Flags >> 8 & 0x0f);
				def++;
				break;
				...
				layout_3_buttons(gad,text,def->ID,text1,def[1].ID,
					text2,def[2].ID,def->Flags >> 8 & 0x0f);
				def += 2;
				break;
			}
			break;
...
void clip_on(struct RastPort *rp,WORD x1,WORD y1,WORD x2,WORD y2)
{	crect.MinX = x1;
