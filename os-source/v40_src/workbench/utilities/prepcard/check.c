...
/* Clear window */
	...
	RectFill(cv->cv_win->RPort,
		...
		(ULONG)(cv->cv_win->Height) - (cv->cv_win->BorderBottom + 18)
		);
...
/* Some reserved type? */
	...
	InfoBox( cv, GetString(cv,MSG_PREP_DEVICE_LABEL), 6, 10, 20, &info[0] );
...
/* check if pseudo disk format */
	...
	InfoBox(cv, GetString(cv,MSG_PREP_FORMAT_LABEL), 188, 10, 52, &info[0] );
...
/* disk-like, so we at least know block size, and total blocks */
	...
	InfoBox(cv,GetString(cv,MSG_PREP_GEO_LABEL),188,36,52,&info[0]);
 ...
 */
	...
	InfoBox(cv,GetString(cv,MSG_PREP_BATTERY_STATUS),6,72,20,&info[0]);
}
	...
	if(link >= 3)
	{
		...
		if(manu = IsString((UBYTE *)&tpv->TPLLV1_INFO[0], max))
		{
			if(name = IsString(manu + strlen(manu) + 1, max))
			{
				...
				InfoBox(cv,GetString(cv,MSG_PREP_PROD_LABEL),6,98,75,&info[0]);
			}
...
BOOL report;
	...
	InfoBox(cv,GetString(cv,MSG_PREP_INFO_LABEL),6,98,75,&info[0]);
}
