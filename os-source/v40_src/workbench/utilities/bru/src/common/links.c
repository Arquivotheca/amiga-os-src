...
static struct link *make_link PROTO((struct finfo *fip));
static struct link *find_link PROTO((struct finfo *fip));
...
struct finfo *fip;
{
    ...
    if (new == NULL) {
	bru_message (MSG_LALLOC, fip -> fname);
    } else {
	...
	if (new -> l_name == NULL) {
	    ...
	    new -> l_dev = fip -> bstatp -> bst_dev;
	    new -> l_ino = fip -> bstatp -> bst_ino;
	    new -> l_nlink = fip -> bstatp -> bst_nlink;
	    new -> l_unres = fip -> bstatp -> bst_nlink;
	    new -> l_next = NULL;
	}
    }
    DBUG_RETURN (new);
...
struct finfo *fip;
{
	...
	if (lp -> l_ino == fip -> bstatp -> bst_ino) {
	    if (lp -> l_dev == fip -> bstatp -> bst_dev) {
		break;
	    }
	}
    }
...
VOID unresolved ()
{
    ...
    for (lp = links; lp != NULL; lp = lp -> l_next) {
	if (lp -> l_unres > 1) {
	    bru_message (MSG_URLINKS, lp -> l_name, lp -> l_unres - 1);
	} else if (lp -> l_unres < 1) {
	    bru_message (MSG_ALINKS, lp -> l_name, 1 - lp -> l_unres);
	}
    }
    DBUG_VOID_RETURN;
}
