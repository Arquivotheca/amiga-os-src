...
(set #move-old
(cat "\n\nPode-se proseguir com a mudança dos ficheiros para a gaveta \"%s\"?\n\n%s"
))

(set #move-old-help
(cat "\nA instalação do Workbench 3.1 vai causar a substituição de ficheiros, "
     "incluindo os ficheiros: Startup-Sequence e Shell-Startup. "
     "Para conservar as mudanças feitas a estes ficheiros, o processo de "
     "instalação vai mudá-los para a gaveta \"%s\" "
...
(cat "\n\nMuitos dos ficheiros usados pelo sistema no Workbench 1.3 "
     "e Workbench 2.0 já não serão precisos pelo Workbench 3.1. "
     "Estes ficheiros poderam agora ser apagados.\n\n"
     "Quer que o sistema lhe peça autorização para apagar cada ficheiro "
     "ou poder-se-á proceder a essa acção automáticamente?"
))

(set #delete-old-help
(cat "\nO Workbench 1.3 e o Workbench 2.0 têm numerosos ficheiros que não "
     "são úteis ao Workbench 3.1. Estes têm de ser apagados para evitar "
     "conflitos e não ocuparem espaço no seu disco rígido.\n\n"
     "O botão \"Confirmar Apagamento\" vai forçar o processo de instalção "
     "a pedir a sua confirmação antes de apagar cada ficheiro. Isto "
     "permite-lhe conservar algum ficheiro que não deseja apagar.\n\n"
     "O botão \"Apagamento Automático\" vai fazer com que todos os ficheiros "
     "antigos sejam apagados sem que para isso seja pedida a sua confirmação."
))

(set #delete-old-1
(cat "Apagamento Automático"
))
...
(set #confirm-delete-help
(cat "\nO programa de instalação determinou que o ficheiro \"%s\" já não "
     "é util para o Workbench 3.1. Pode apagá-lo selecionando o botão "
     "\"Sim\", se quizer conservá-lo escolha o botão \"Não\". "
))

(set #confirm-delete-reason-1
(cat "Este ficheiro foi incorporado na ROM Versão 3.1"
))

(set #confirm-delete-reason-2
(cat "As funções deste programa foram integradas em outros programas do "
     "Workbench."
))

(set #confirm-delete-reason-3
(cat "Este programa foi movido para outro sítio e será mudado aí."
))

(set #confirm-delete-reason-4
(cat "Este programa está desactualizado e por isso não deverá ser "
     "utilizado com a Versão 3.1"
))
...
(set #keeping-old
(cat "\n\nFazendo cópias dos ficheiros mais importantes..."
))

(set #ask-workbench
(cat "\n\nInsira por favor a diskette \"Amiga Workbench\" Versão 3.1 em qualquer drive."
))

(set #ask-extras
(cat "\n\nInsira por favor a diskette \"Amiga Extras\" Versão 3.1 em qualquer drive."
))

(set #ask-storage
(cat "\n\nInsira por favor a diskette \"Amiga Storage\" Versão 3.1 em qualquer drive."
))
...
(cat "\nO Amiga pode controlar muitas impressoras diferentes. "
     "Os Ficheiros de Controlo de Impressora são ficheiros que "
     "fazem com que o Amiga se adapte a uma determinada impressora. "
     "Tem que se copiar um destes ficheiros para o disco rígido "
     "por cada impressora que queira que o Amiga suporte.\n\n"
     "Para reduzir o consumo de espaço por estes ficheiros "
     "poderá apenas escolher aqueles que lhe serão úteis.\n\n"
...
(cat "\nO Amiga pode ser operado com muitos tipos diferentes de teclados. "
     "Um ficheiro de Mapa de Teclado descreve o seu teclado específico ao "
     ...
     "ter á sua disposição no sistema.\n\n"
))
...
(set #ask-locale
(cat "\n\nInsira por favor a diskette \"Amiga Locale\" Versão 3.1 em qualquer drive."
))

(set #no-font-space
(cat "\nNeste momento não há espaço no disco \"%s\" para pôr as fontes de "
     "texto standard do Amiga.\n\n"
     "As fontes de texto do Amiga não serão copiadas.\n\n"
     "Poderá fazer isto mais tarde puxando o icone da diskette  "
     "\"Amiga Fonts\" Versão 3.1 para cima do icone do disco \"%s\"."
))

(set #ask-fonts
(cat "\n\nInsira por favor a diskette \"Amiga Fonts\" Versão 3.1 em qualquer drive."
))

(set #ask-install
(cat "\n\nInsira por favor a diskette \"Amiga Install\" Versão 3.1 em qualquer drive."
))

(set #reorganizing
(cat "\n\nModificando os ficheiros para ficarem de acordo com a sua "
     "consfiguração anterior..."
))

(set #a2090
(cat "Se está a usar uma controladora A2090 com um disco rígido SCSI "
     "e está a arrancar a partir de diskette, deverá instalar o ficheiro "
     "Startup-A2090SCSI, que se encontra na gaveta Update desta diskette, "
     "como a sua Startup-Sequence na sua diskette de arranque. "
     "Se está a usar uma controladora A2090 com um disco rígido ST-506"
     "e está a arrancar a partir de diskette, deverá instalar o ficheiro "
...
(set #move-old
(cat "\n\n¿Pueden ser movidos estos ficheros al cajón \"%s\"?\n\n%s"
))

(set #move-old-help
(cat "\nLa instalación del Workbench 3.1 reemplazará la mayor parte de los "
     "ficheros, incluido el Startup-Sequence. "
     "Para conservar los cambios que haya hecho a estos ficheros antes, el "
     "proceso de instalación los copiará al cajón \"%s\" de donde los podrá "
     "recuperar más tarde."
))
...
(cat "\n\nAlgunos de los ficheros utilizados en versiones anteriores "
     ...
     "¿Quiere confirmación individual antes de borrar cada fichero o que "
     "se borren todos automáticamente?"
))
...
(cat "\nEl Workbench 1.3 y el Workbench 2.0 tiene algunos ficheros que no "
     ...
     "El gadget \"Preguntar antes de borrar\" forzará a que el proceso de "
     "instalación pregunte antes de borrar cada fichero. Esto permite que "
     "guarde determinados ficheros que no quiere borrar.\n\n"
     "El gadget \"Borrado automático\" hará que se borren los ficheros "
     "del sistema antiguo sin pedir ninguna confirmación."
))

(set #delete-old-1
(cat "Borrado automático"
))
...
(set #confirm-delete-help
(cat "\nEl instalador ha determinado que el fichero \"%s\" no es válido "
     "en el Workbench 3.1. Puede borrar el fichero si pulsa en el gadget "
     "\"Sí\" o conservarlo si elige el gadget \"No\"."
))

(set #confirm-delete-reason-1
(cat "Ahora, este fichero está incluído en ROM."
))

(set #confirm-delete-reason-2
(cat "La utilidad de este programa está integrada en otros programas "
     "del Workbench."
))

(set #confirm-delete-reason-3
(cat "Este prgrama se a transaldado a otro lugar y se actualizará allí."
))

(set #confirm-delete-reason-4
(cat "Este programa es antiguo y no debería usarse con la Versión 3.1"
))
...
(set #ask-workbench
(cat "\n\nPor favor, inserte el disco \"Amiga Workbench\" Versión 3.1 en cualquier unidad."
))

(set #ask-extras
(cat "\n\nPor favor, inserte el disco \"Amiga Extras\" Versión 3.1 en cualquier unidad."
))

(set #ask-storage
(cat "\n\nPor favor, inserte el disco \"Amiga Storage\" Versión 3.1 en cualquier unidad."
))

(set #which-printer
(cat "¿Qué manejadores de impresora tienen que instalarse?"
))
...
(cat "\nAmiga puede controlar muchos tipos de impresoras. "
     ...
     "de impresora, puede seleccionar sólo el que necesite.\n\n"
     "Sólo tiene que pulsar en el modelo de impresora que tenga conectada "
     "a su sistema.\n\n"
     @askoptions-help
))

(set #which-keymap
(cat "¿Qué mapa de teclado hay que instalar?"
))
...
(cat "\nAmiga puede utilizar distintos tipos de teclado. Un fichero de "
     ...
     "cómo manejarlo. Elija el tipo de teclado de los que aparecen en "
     "la lista.\n\n"
...
(set #ask-locale
(cat "\n\nPor favor, inserte el dico \"Amiga Locale\" Versión 3.1 en cualquier unidad."
))

(set #no-font-space
(cat "\nNo hay suficiente espacio en  \"%s\" para el juego de caracteres "
     "de Amiga.\n\n"
     "No se copiará el juego de caracteres Amiga.\n\n"
     "Puede copiar los tipos de letra más tarde moviendo el icono del "
     "disco \"Amiga Fonts\" Versión 3.1 sobre el icono \"%s\"."
))

(set #ask-fonts
(cat "\n\nPor favor, inserte el disco \"Amiga Fonts\" Versión 3.1 en cualquier unidad."
))

(set #ask-install
(cat "\n\nPor favor, inserte el disco \"Amiga Install\" Versión 3.1 en cualquier unidad."
))

(set #reorganizing
(cat "\n\nAjustando ficheros para que coincidan con su configuración..."
))

(set #a2090
(cat "Si tiene una controladora A2090 con un disco duro SCSI y está "
     "arrancando desde disquete, debería instalar el fichero "
     "Startup-A2090SCSI del cajón Update de éste disco como su "
     "Startup-sequence en el disco de arranque. "
     "Si tiene una controladora A2090 con un disco duro ST-506 y está "
     "arrancando desde disquete, debería instalar el fichero "
     "Startup-A2090ST506 del cajón Update de éste disco como su "
     "startup-Sequence en el disco de arranque."
))
...
(set #move-old
(cat "\n\nLes fichiers suivants peuvent-ils être déplacés dans le "
     "tiroir \"%s\" ?\n\n%s"
))
...
(cat "\nL'installation du Workbench 3.1 remplacera la plupart des fichiers "
     ...
     "Pour préserver toutes les modifications que vous auriez pu "
     "y faire, ces fichiers seront copiés dans le tiroir \"%s\" où vous "
     "pourrez les récupérer ultérieurement."
))
...
(set #delete-old
(cat "\n\nDe nombreux fichiers utilisés par le Workbench 1.3 et le "
     "Workbench 2.0 sont inutiles avec le Workbench 3.1. "
     "Ces fichiers peuvent être effacés de votre système.\n\n"
     "Voulez-vous avoir à donner une autorisation avant que chaque "
     "fichier soit effacé, ou peuvent-ils tous être effacés automatiquement ?"
))

(set #delete-old-help
(cat "\n\nDe nombreux fichiers du Workbench 1.3 et du Workbench 2.0 "
     "sont inutiles avec le Workbench 3.1. Ces fichiers devraient être "
     "effacés pour éviter d'encombrer le disque dur et y récupérer de la "
     "place. La cellule \"Demander avant d'effacer\" obligera le processus "
     "d'installation à obtenir votre autorisation avant d'effacer chacun de "
     "ces fichiers. Ceci vous permettra de conserver les fichiers de votre "
     "choix.\n\nLa cellule \"Effacer automatiquement\" autorisera "
     "l'effacement de ces fichiers sans qu'il vous soit rien demandé. "
))
...
(set #confirm-delete-reason-1
(cat "Le fichier est maintenant intégré dans les routines des mémoires mortes "
))

(set #confirm-delete-reason-2
(cat "Les fonctionnalités de ce programme sont reprises par d'autres "
     "programmes du Workbench."
))

(set #confirm-delete-reason-3
(cat "Ce programme a changé de place, et y sera mis à jour."
))

(set #confirm-delete-reason-4
(cat "Ce programme est obsolète et ne devra pas être utilisé avec "
     "le Workbench 3.1."
))
...
(set #keeping-old
(cat "Copie des fichiers à conserver..."
))
...
(set #which-printer
(cat "Quels sont les pilotes d'imprimantes à installer ?"
))

(set #which-printer-help
(cat "\nL'Amiga peut gérer de nombreuses imprimantes. "
     "Les pilotes d'imprimantes sont des fichiers qui "
     "permettent à l'Amiga de s'adapter à une imprimante donnée. "
     "Il doit y avoir un pilote d'imprimante copié sur le disque dur "
     "pour chaque imprimante utilisée.\n\n"
     "Pour réduire la quantité d'espace utilisé par les pilotes "
     "d'imprimantes, vous pouvez sélectionner seulement ceux qui "
     "vous seront utiles.\n\n"
...
(set #which-keymap
(cat "Quel sont les claviers à installer ?"
))

(set #which-keymap-help
(cat "\nVotre ordinateur Amiga peut être utilisé avec des claviers différents. "
     "Un fichier \"Keymap\" décrit le clavier afin que l'ordinateur puisse le "
     "reconnaître. Choississez le clavier parmi la liste des claviers "
     "disponibles.\n\n"
...
(cat "\nIl n'y a, pour l'instant, pas assez de place sur \"%s\" pour "
     ...
     "Le jeu de fontes Amiga ne sera pas copié.\n\n"
     "Plus tard, après avoir fait de la place sur \"%s\", "
     "vous pourrez copier ces fontes en déplaçant l'icône de "
     "la disquette \"Amiga Fonts\" version 3.1 sur l'icône \"%s\"."
))
...
(set #a2090
(cat "Si vous utilisez un contrôlleur de disque dur A2090 avec un "
     "disque dur SCSI, et que vous démarrez depuis une disquette, "
     "vous devez copier le fichier Startup-A2090SCSI du tiroir Update "
     "de cette disquette sur votre disquette de démarrage en tant que "
     "fichier Startup-Sequence.\n"
     "Si vous utilisez un contrôlleur de disque dur A2090 avec un "
     "disque dur ST-506, et que vous démarrez depuis une disquette, "
...
(cat "\nDie Installation der Version 3.1 ersetzt die meisten bisherigen "
     ...
     "Damit alle Änderungen erhalten bleiben, die Sie früher an diesen "
     "Dateien vorgenommen haben, werden sie bei der Installation in die "
     "Schublade \"%s\" kopiert, woher Sie sie später zurückholen können."
))
...
(cat "\n\nViele System-Dateien, die unter früheren Versionen "
     "benötigt wurden, werden unter Version 3.1 nicht länger gebraucht. "
...
(cat "\nÄltere Systemversionen umfassen mehrere Dateien, die "
     "unter Version 3.1 nutzlos sind. Diese Dateien sollten gelöscht "
...
(set #confirm-delete-help
(cat "\nDie Datei "
     "\"%s\" ist unter Version 3.1 überholt. Sie können die Datei "
...
(set #confirm-delete-reason-4
(cat "Dieses Programm ist überholt und sollte nicht unter Version 3.1 "
     "benutzt werden."
))
...
(cat "\nIm Augenblick reicht der Platz auf \"%s\" nicht aus, um alle "
     "standardmäßigen Amiga-Zeichensätze (Fonts) zu installieren.\n\n"
     "Die Zeichensätze werden vorerst nicht kopiert.\n\n"
     "Sie können die Zeichensätze aber später kopieren, indem Sie das "
     "Piktogramm der Diskette \"Amiga Fonts\" Version 3.1 über das "
     "Piktogramm \"%s\" ziehen."
))
...
(cat "\nDe installatie van Release 3.1 zal de meeste huidige Workbench "
     ...
     "kopiëren, zodat ze daar alsnog door u opgevraagd kunnen worden."
))
...
(set #which-printer
(cat "Welke printer-drivers moeten geïnstalleerd worden?"
))
...
(set #which-keymap
(cat "Welke Keymap moet geïnstalleerd worden?"
))
...
(cat "\nEr is niet genoeg ruimte op \"%s\" voor de standaard "
     ...
     "U kunt de fonts later kopiëren door het icoon van de diskette "
...
(cat "\nInstallando la Versione 3 molti file del Workbench attuale "
     ...
     "questi file, il processo di installazione li copierà nel cassetto "
...
(cat "\n\nMolti file usati dalle versioni precedenti "
     "non sono più richiesti nella Versione 3.1. "
...
(cat "\nLe versioni precedenti hanno numerosi file che non sono "
     ...
     "Il pulsante \"Cancella automaticamente\" cancellerà tutti i file di"
     "sistema obsoleti senza chiedere la vostra conferma su nessuno di essi. "
     "Il pulsante \"Conferma cancellazione\" forzerà il processo di "
...
(set #confirm-delete
(cat "\n\nIl file \"%s\" può essere cancellato?"
))

(set #confirm-delete-help
(cat "\nIl file \"%s\" non è più usato nella Versione 3.1. Potete "
     "cancellare il file col pulsante \"Sì\", o conservarlo selezionando il "
     "pulsante \"No\". "
))

(set #confirm-delete-reason-1
(cat "Questo file è incorporato nella ROM Versione 3.1"
))

(set #confirm-delete-reason-2
(cat "Le funzionalità di questo programma sono state integrate in altri "
     "programmi del Workbench."
))

(set #confirm-delete-reason-3
(cat "Questo programma è stato copiato in una diversa zona del disco "
     "e lì verrà aggiornato."
))

(set #confirm-delete-reason-4
(cat "Questo programma è obsoleto e non dovrebbe essere usato con la "
     "Versione 3.1"
))
...
(set #ask-workbench
(cat "\n\nInserire il disco \"Amiga Workbench\" Versione 3.1 in una unità a dischi."
))

(set #ask-extras
(cat "\n\nInserire il disco \"Amiga Extras\" Versione 3.1 in una unità a dischi."
))

(set #ask-storage
(cat "\n\nInserire il disco \"Amiga Storage\" Versione 3.1 in una unità a dischi."
))
...
(set #which-printer-help
(cat "\nAmiga può usare varie stampanti. "
...
(set #which-keymap-help
(cat "Amiga può operare con tipi differenti di tastiera. Un file di mappa "
...
(set #ask-locale
(cat "\n\nInserire il disco \"Amiga Locale\" Versione 3.1 in una unità a dischi. "
))

(set #no-font-space
(cat "\nNon c'è abbastanza spazio su \"%s\" per inserire tutte le serie "
...
(set #ask-fonts
(cat "\n\nInserire il disco \"Amiga Fonts\" Versione 3.1 in una unità a dischi. "
))

(set #ask-install
(cat "\n\nInserire il disco \"Amiga Install\" Versione 3.1 in una unità a dischi. "
))
...
(set #move-old
(cat "\n\nMå de følgende filer flyttes til \"%s\" skuffen?\n\n%s"
))

(set #move-old-help
(cat "\nInstallationen af Workbench 3.1 vil erstatte nogle af de "
     "nuværende Workbench filer, inklusive Startup-Sequence "
     "og Shell-Startup filer. "
     "For at bibeholde tidligere ændringer De måtte have lavet på disse "
...
(set #checking-target
(cat "\n\nUndersøger \"%s\" for overflødige filer..."
))

(set #delete-old
(cat "\n\nMange af de filer, der benyttes af systemet under Workbench 1.3 "
     "og Workbench 2.0, er ikke længere nødvendige i Workbench 3.1. "
     "Disse filer kan slettes fra Deres system.\n\n"
     "Ønsker De at blive stillet spørgsmål omkring sletning af hver fil, "
     "eller må de slettes automatisk?"
))

(set #delete-old-help
(cat "\nWorkbench 1.3 og Workbench 2.0 indeholder adskillige filer som "
     "ikke benyttes i Workbench 3.1. Disse filer bør slettes for at undgå "
     "rod, og spare plads på Deres harddisk.\n\n"
     "\"Spørge før sletning\" knappen tvinger systemet til at spørge Dem "
     "om tilladelse før sletning af hver enkelt fil. Derved har De mulighed "
     "for at beholde bestemte filer, som De ikke ønsker at slette.\n\n"
     "\"Slette automatisk\" knappen giver systemet lov til at slette filerne, "
     "uden først at bede om Deres tilladelse på nogen af filerne."
))
...
(set #delete-old-2
(cat "Spørge før sletning"
))

(set #deleting-obsolete
(cat "\n\nSletter overflødige filer"
))
...
(cat "\nInstallations programmet har fundet ud af, at filen \"%s\" ikke "
     ...
     "vælge \"Ja\" knappen, eller gemme filen ved at vælge \"Nej\" knappen."
))
...
(set #confirm-delete-reason-4
(cat "Dette program er overflødigt og må ikke bruges sammen med "
     "Workbench 3.1."
))
...
(set #ask-workbench
(cat "\n\nIndsæt venligst \"Amiga Workbench\" disken Version 3.1 i et floppy drev."
))

(set #ask-extras
(cat "\n\nIndsæt venligst \"Amiga Extras\" disken Version 3.1 i et floppy drev."
))

(set #ask-storage
(cat "\n\nIndsæt venligst \"Amiga Storage\" disken Version 3.1 i et floppy drev."
))
...
(cat "\nDeres Amiga kan kommunikere med mange forskellige printere. "
     ...
     "printer De ønsker i anvendelse.\n\n"
     "For at reducere pladsen, der optages af printer-drivere, "
     "kan De vælge kun at kopiere printer-drivere til de printere, "
     "De ønsker at benytte.\n\n"
     "Klik på knapperne ud for hver af de printere De ønsker at have til rådighed "
     "på Deres system. Et 'flueben' indikerer en valgt printer.\n\n"
     @askoptions-help
))
...
(cat "\nDeres Amiga computer kan arbejde med mange forskellige tastatur typer. "
     "En tastaturtype fil beskriver Deres tastatur for Amiga computeren, så "      "den kan arbejde korekt med det. Vælg den tastaturtype De har udfra "      "listens valgmuligheder.\n\n"
     "Klik på knapperne ud for den ønskede tastaturtype. Et ´flueben´ "
...
(set #ask-locale
(cat "\n\nIndsæt venligst \"Amiga Locale\" disken Version 3.1 i et floppy drev."
))

(set #no-font-space
(cat "\nDer er i øjeblikket ikke plads nok på \"%s\" til standard Amiga "
     "fontene.\n\n"
     "Amiga fontene vil derfor ikke blive kopieret.\n\n"
     "De kan kopiere fontene senere ved at trække ikonerne fra "
     "\"Amiga Fonts\" disken Workbench 3.1 ovenpå \"%s\" ikonet."
))

(set #ask-fonts
(cat "\n\nIndsæt venligst \"Amiga Fonts\" disken Version 3.1 i et floppy drev."
))

(set #ask-install
(cat "\n\nIndsæt venligst \"Amiga Install\" disken Version 3.1 i et floppy drev."
))
...
(cat "Hvis De benytter en A2090 harddisk controller sammen med en SCSI "
     ...
     "Startup-A2090SCSI fra Update skuffen på denne disk, som Deres "
     "Startup-Sequence fil på boot disketten. "
     "Hvis De benytter A2090 harddisk controlleren sammen med en ST-506 "
     "harddisk og booter fra en diskette, skal De installerer filen "
     "Startup-S2090ST506 fra Update skuffen på denne disk, som Deres "
     "Startup-Sequence fil på boot disketten. "
))
...
(set #move-old
(cat "\n\nKan följande filer flyttas till lådan \"%s\"?\n\n%s"
))

(set #move-old-help
(cat "Installation av Release 3.1 skriver över de flesta av dina "
     "nuvarande Workbench-filer, bl.a Startup-Sequence. "
     "För att bevara alla ändringar du tidigare gjort av dessa filer "
     "så kopierar Installeraren dem till lådan \"%s\" där du senare kan "
     "hämta dem."
))
...
(set #checking-target
(cat "Kontrollerar \"%s\" efter föråldrade filer..."
))

(set #delete-old
(cat "\n\nMånga filer som används av systemet under tidigare versioner "
     "av operativsystemet behövs inte längre under Release 3.1. "
     "Dessa filer kommer nu att raderas; vill du tillfrågas innan filerna "
     "raderas eller vill du att de skall raderas automatiskt?"
))

(set #delete-old-help
(cat "\nTidigare versioner av operativsystemet har innehållit ett flertal "
     "filer som inte längre behövs under Release 3.1. Dessa filer bör "
     "raderas för att undvika förvirring och spara utrymme på hårddisken.\n\n"
     "Knappen \"Radera automatiskt\" gör så att Installeraren automatiskt "
     "raderar de gamla filerna, utan att be om tillstånd först."
     "Knappen \"Fråga innan radering\" tvingar Installeraren att be om "
     "tillåtelse innan den raderar varje fil. Detta ger dig möjlighet att "
...
(set #delete-old-2
(cat "Fråga innan radering"
))

(set #deleting-obsolete
(cat "\n\nRaderar föråldrade filer"
))
...
(set #confirm-delete-help
(cat "\nInstalleraren har funnit att filen \"%s\" inte längre behövs "
     "under Release 3.1. Du kan radera filen genom att välja \"Ja\" "
     "eller spara filen genom välja \"Nej\"."
))

(set #confirm-delete-reason-1
(cat "Denna fil är numer en del av Release 3.1 ROM (Kickstart)."
))

(set #confirm-delete-reason-2
(cat "Funktionen hos detta program är numer en del av andra Workbench-"
     "program."
))

(set #confirm-delete-reason-3
(cat "Detta program har flyttats till en annan position, och den "
     "nya versionen kommer att placeras där."
))

(set #confirm-delete-reason-4
(cat "Det här programmet är föråldrat och skall inte användas med Release "
     "3.1"
))
...
(set #keeping-old
(cat "\n\nGör backup-kopior av viktiga filer..."
))

(set #ask-workbench
(cat "\n\nSätt i disketten \"Amiga Workbench\" version 3.1 i valfri diskettstation."
))

(set #ask-extras
(cat "\n\nSätt i disketten \"Amiga Extras\" version 3.1 i valfri diskettstation."
))

(set #ask-storage
(cat "\n\nSätt i disketten \"Amiga Storage\" version 3.1 i valfri diskettstation."
))

(set #which-printer
(cat "Vilka drivrutiner för skrivare skall installeras?"
))

(set #which-printer-help
(cat "\nAmiga kan använda många olika modeller av skrivare. "
     "Skrivardrivrutiner är filer som låter Amigan anpassa sig till "
     "en viss skrivare. Man måste kopiera en skrivardrivrutin till "
     "hårddisken för varje skrivare som skall stödjas.\n\n"
     "För att minimera använt hårddiskutrymme kan du välja vilka "
     "drivrutiner du vill kopiera.\n\n"
     "Markera bara rutorna framför skrivarna du kan komma att "
     "vilja använda.\n\n"
     @askoptions-help
))

(set #which-keymap
(cat "Vilka tangentbordsuppsättningar skall installeras?"
))

(set #which-keymap-help
(cat "\nAmigan kan användas med flera olika typer av tangentbord. En "
     "tangentbordsuppsättningsfil (keymap-fil) beskriver tangentbordets "
     "utseende för operativsystemet. Välj typen av tangentbord du har "
     "från listan.\n\n"
     "Markera bara rutorna framför de tangentbord du kan komma att "
     "vilja använda.\n\n"
     @askoptions-help
))
...
(set #ask-locale
(cat "\n\nSätt i disketten \"Amiga Locale\" version 3.1 i valfri diskettstation."
))

(set #no-font-space
(cat "\nDet finns inte utrymme nog på \"%s\" för att installera "
     "standarduppsättningen med Amiga typsnitt.\n\n"
     "Amiga typsnittsuppsättningen kommer inte att kopieras.\n\n"
     "Du kan kopiera typsnitten senare genom att släppa symbolen för "
     "\"Amiga Fonts\" disketten Version 3.1 ovanpå symbolen \"%s\"."
))

(set #ask-fonts
(cat "\n\nSätt in disketten \"Amiga Fonts\" version 3.1 i valfri diskettstation."
))

(set #ask-install
(cat "\n\nSätt in disketten \"Amiga Install\" Version 3.1 i valfri diskettstation."
))

(set #reorganizing
(cat "Justerar filer för att passa din tidigare konfiguration..."
))

(set #a2090
(cat "Om du använder en A2090 hårddisk-controller med en SCSI hårddisk "
     "och startar datorn från en diskett skall du installer filen "
     "Startup-A2090SCSI från Update-lådan på denna diskett som "
     "Startup-Sequence på din uppstartningsdiskett.\n"
     "Om du använder en A2090 hårddisk-controller med en ST-506 hårddisk "
     "och startar datorn från en diskett skall du installer filen "
     "Startup-A2090ST506 från Update-lådan på denna diskett som "
     "Startup-Sequence på din uppstartningsdiskett.\n"
))
...
(set #checking-target
(cat "\n\nGjennomsøker \"%s\" for unødvendige filer..."
))

(set #delete-old
(cat "\nMange av filene systemet benyttet under tidligere "
     "utgaver, er ikke lenger nødvendige for Workbench 3.1. "
     "Disse filene kan slettes fra systemet.\n\n"
     "Ønsker du å få spørsmål om sletting for hver enkelt fil, "
     "eller skal de slettes automatisk?"
))

(set #delete-old-help
(cat "Tidligere versjoner har mange filer som det ikke lenger er bruk for "
     "under Workbench 3.1. Disse bør slettes for å ungå rot, "
     "og for å spare plass på harddisken.\n\n"
     "Knappen \"Slett automatisk\" sørger for å slette disse filene uten "
     "å spørre deg om lov først."
     "Knappen \"Spør før sletting\" vil sørge for at installasjonsprogrammet "
     "spør deg om lov hver gang en fil skal slettes. Dette gjør at "
     "du kan beholde spesielle filer som du ikke ønsker å slette.\n\n"
))
...
(set #delete-old-2
(cat "Spør før sletting"
))

(set #deleting-obsolete
(cat "\n\nSletter unødvendige filer"
))
...
(set #confirm-delete-help
(cat "\nFilen \"%s\" er ikke nødvendig for Workbench 3.1 . Du kan slette "
     "denne filen ved å "
     "velge \"Ja\"-knappen, eller beholde den ved å velge \"Nei\"."
))

(set #confirm-delete-reason-1
(cat "Denne filen er nå integrert i versjon 3.1 ROM-brikken."
))

(set #confirm-delete-reason-2
(cat "Dette programmets funksjon er nå integrert i andre Workbench-programmer."
))
...
(set #confirm-delete-reason-4
(cat "Dette programmet er unødvendig, og skal ikke benyttes med versjon 3.1"
))
...
(cat "Amiga kan benytte mange forskjellige skrivere. "
     ...
     "en skriver. Det må finnes en skriverdefinisjon på "
     "harddisken for hver skrivertype som skal benyttes.\n\n"
     "For å redusere plassen som skriverdefinisjonene opptar, "
...
(cat "Amiga kan opereres fra mange forskjellige tastaturer. "
     "En tastaturdefinisjon forklarer maskinen hvordan den skal reagere på "
...
(set #no-font-space
(cat "\nDet er for øyeblikket ikke plass på \"%s\" til standard Amiga "
     "skrifttyper.\n\n"
     "Amiga skrifttyper vil ikke bli kopiert inn.\n\n"
     "Du kan kopiere skrifttypene senere ved først å lage mer plass, "
     "for deretter å trekke ikonet til "
...
(set #reorganizing
(cat "\n\nJusterer filene så de passer til din tidligere konfigurasjon..."
))

(set #a2090
(cat "Hvis du bruker en A2090 harddisk kontroller med SCSI harddisk og "
     "starter maskinen fra diskett, må du installere filen Startup-A2090SCSI "
     "fra Update-katalogen på denne disketten som Startup-Sequence "
     "på disketten du starter maskinen fra."
     "Hvis du bruker en A2090 harddisk kontroller med en ST-506 harddisk og "
     "starter maskinen fra diskett, må du installere filen Startup-A2090ST506 "
     "fra Update-katalogen på denne disketten som Startup-Sequence "
     "på disketten du starter maskinen fra."
))
