...
APTR               udata;
    ...
    else
    {
        ...
        if (ed)
        {
            ...
            ed->ed_TextAttr.ta_Name  = "coral.font";
            ed->ed_TextAttr.ta_YSize = 16;
            ed->ed_TextAttr.ta_Style = FS_NORMAL;
            ed->ed_TextAttr.ta_Flags = FPF_DISKFONT;
            ...
            OpenLib(ed,&ed->ed_DiskfontBase, "diskfont.library",  MSG_NOTHING);
#ifdef LOCKLAYERINFO
		    ...
		    if (!ed->ed_InitFail)
		    {
		        CopyPrefs(ed,&ed->ed_PrefsWork,&ed->ed_PrefsDefaults);
                        ...
                        else
                        {
                            if (InitDisp(ed))
                            {
                                if (ed->ed_Font = OpenDiskFont(&ed->ed_TextAttr))
                                {
	    ...
	    while (msg = GetMsg(&ed->ed_AppPort))
            ...
            CloseLibrary(ed->ed_DiskfontBase);
