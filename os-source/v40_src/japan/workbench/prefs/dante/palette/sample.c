...
STRPTR           text;
    ...
    && depthGadget && upGadget && downGadget && checkBox && mxKnob)
    {
        /* background */
        ...
        if (complete)
        {
            ...
            RectFill(rp, wp->BorderLeft, top + MENU_TOP - 16, wp->Width - wp->BorderRight - 1, wp->Height - wp->BorderBottom-1);
        }
    ...
    if (amigaKey)	DisposeObject(amigaKey);
    if (closeGadget)	DisposeObject(closeGadget);
    if (sizeGadget)	DisposeObject(sizeGadget);
    if (depthGadget)	DisposeObject(depthGadget);
    if (zoomGadget)	DisposeObject(zoomGadget);
    if (upGadget)	DisposeObject(upGadget);
    if (downGadget)	DisposeObject(downGadget);
    if (checkBox)	DisposeObject(checkBox);
    if (mxKnob)		DisposeObject(mxKnob);
}
