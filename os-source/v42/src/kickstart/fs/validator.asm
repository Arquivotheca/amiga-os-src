...
;		DEBUGENABLE
		...
		XREF	_LVOFault,DoButtons,Request,FlushBitmapBlock
		XREF	CopyOverlap
...
Validator
	IFD DISK_LOADED
		sub.l	a3,a3			so VExit won't try to free it
...
; fact and exit the validator now.  Will need much slower special validate.
		...
		move.l	#STR_USED_TWICE,d1	this error message
...
; A dirlist block.  Add next block on chain to list to scan.
		...
		beq.s	30$
20$		move.l	#STR_BAD_HEADER,d1	bad header type
		bsr	ValidatorError
		bra	CantVal
...
; check that the data key we got was within the range of the partition.
		...
		bge.s	3$
2$		move.l	#STR_OUT_OF_RANGE,d1	error, key out of range
...
;		movea.l	d0,a4			save buffer pointer
		move.l	BitmapCount(a5),d2	get total bitmap keys needed
...
WriteBMBlocks	move.l	a3,-(sp)		save pointer to bitmap
		...
		move.l	BitmapCount(a5),d2	get count of blocks to do
		bra.s	ll40
...
ll40		subq.l	#1,d2			must handle >16 bits (31 here)
		bpl	ll10			and do next bitmap block
		move.l	(sp)+,a3
...
; the header we have is bad, put up a requester to this effect.
		...
		move.l	#STR_BAD_HEADER,d1	put up requester
...
;==============================================================================
; ValidatorError( key, error)
;		  d0	d1
;
; Puts up a requester containing the key number where the error occurred and
; what was wrong with that key.  Text of the error message fetched from DOS
; with the Fault() command.  Fault number is in d1 on entry.
;==============================================================================
ValidatorError	movem.l	d2-d4/a2/a6,-(sp)

		move.l	d1,-(sp)		stash error text
		movea.l	ReqArgs(a5),a0		
		move.l	d0,4(a0)		setup arguments (block # last)
		movea.l	CurrentVolume(a5),a1	using BSTR volume name
		adda.l	a1,a1
		adda.l	a1,a1
		move.l	dl_Name(a1),(a0)	volume name for "error validating"

		movea.l	DosLib(a5),a6
		movea.l	ReqString(a5),a2
		move.l	#STR_ERROR_VALIDATING,d1
		...
		move.l	(sp)+,d1		get main error message
		moveq.l	#0,d2
		move.l	a2,d3			buffer address
		moveq.l	#80,d4			maximum length
		jsr	_LVOFault(a6)
		...
		bsr	DoButtons
		bra	Request			retry means nothing, ignore it
...
goodentry:	; free any fhb entry grabbed (d5 is block, a0 is fhb ptr)
...
;	printf <'type is %ld, hashchain is %ld'>,d0,d5
		move.l	a3,a0
		move.l	d4,d0
		bsr	LooseBlock
