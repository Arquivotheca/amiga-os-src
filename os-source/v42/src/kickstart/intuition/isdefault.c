 ...
 *
 *  $Id: isdefault.c,v 40.0 94/02/15 17:36:45 davidj Exp $
 *
...
void doDefault (void)
{
	...
	case itCLOSEWIN:	/* Delink, delete layer, BorderPatrol() */
	    ...
	    /* delink from screen */
	    delinkThing ((struct Thing *)ITWINDOW, (struct Thing *)&ITWINDOW->WScreen->FirstWindow);
	    UNLOCKIBASE ();
	...
	case itMOVESCREEN:	/* Do it */
	    IPreMoveScreen (ITSCREEN, (struct Rectangle *) object2, subcommand);
	    return;
	...
	case itOPENWIN:	/* Do it */
	    D (printf ("itOPENWIN\n"));
	    *IT_error = IOpenWindow (ITWINDOW, (struct BitMap *) object2, (struct Hook *) subcommand);
	    return;
	...
	case itCLOSESCREEN:	/* Do it */
	    ...
	    *IT_error = ICloseScreen (ITSCREEN);
	    ...
	    /* WHY is this thing around?  Why, just to single-thread ADDEVENT processing. */
	...
	case itMODIFYPROP:	/* Do it */
	    IModifyProp ((struct Gadget *) object2, ITWINDOW, (struct PropPacket *) subcommand);
	    return;
	...
	DERROR (default:
printf ("BAD TOKEN: IT %lx command %ld\n",IT, IT->it_Command);
Debug ());
    }
}
...
void sendWindowMouse (void)
{
     ...
     */
	     ...
	     (*((ULONG *) & AWindow->MouseY) != *((ULONG *) & XWINDOW (AWindow)->LastMouseY)))
    {
