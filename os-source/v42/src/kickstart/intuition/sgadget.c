 ...
 *
 *  $Id: sgadget.c,v 40.0 94/02/15 17:58:28 davidj Exp $
 *
...
static void newActiveGadget (struct Gadget *current, UWORD ghreturn)
{
...
static void turnOffGadget (LONG abort)
{
    struct IntuitionBase *IBase = fetchIBase ();
 ...
 */
static ghResult (UWORD ghreturn, LONG upcode)
{
    struct IntuitionBase *IBase = fetchIBase ();
    struct Gadget *g;
    LONG retval = 1;		/* done, nothing special */
    UWORD sysgadtype;
    struct Window *AWindow = IBase->ActiveWindow;
    struct Screen *screenFamily ();

    if (ghreturn == GMR_MEACTIVE)
	return (0);
	...
	if (sysgadtype = (g->GadgetType & GTYP_SYSTYPEMASK))
	{
		...
		/*** Change token into command, and reuse ***/
	    {
...
/* return function from size/drag gadget state changes turn off active gadget */
static void returnSysGadget (void)
{
    turnOffGadget (FALSE);
 ...
 */
static LONG deferIf (LONG expression)
{
    struct IntuitionBase *IBase = fetchIBase ();
	...
	if (IBase->Flags & GAD_SELECTDOWN)
	{
	    ...
	    {
		fixWindowBorders ();
		ActivateWindow ((struct Window *) IT->it_Object1);
	    }
	    else
	    {
		deferToken ();
 ...
 *	2 - ENDGADGET for requesters
 */
void startGadget (void (*gadgetreturn)(LONG))
{
    struct IntuitionBase *IBase = fetchIBase ();
    UWORD ghreturn;
    LONG gadgetupcode;
