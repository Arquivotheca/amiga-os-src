 ...
 *
 *  $Id: sscreendrag.c,v 40.0 94/02/15 17:55:31 davidj Exp $
 *
...
void startScreenDrag (void (*sdreturn)(void))
{
    ...
    if ((IT->it_Command == itMETADRAG) && !(hitScreen ()))
    {
     ...
     * dragging the parent.
     */
    if ((!TESTFLAG (XSC (dragscreen)->PrivateFlags, PSF_DRAGGABLE)) && (XSC (dragscreen)->ParentScreen))
    {
    ...
    DSD (dumpRect ("GSDRAG: legal position (screen coords):", &IBase->MouseLimits));

    /* account for offset of mouse from screen origin	*/
    offsetRect (&IBase->MouseLimits, dragscreen->MouseX, dragscreen->MouseY);
