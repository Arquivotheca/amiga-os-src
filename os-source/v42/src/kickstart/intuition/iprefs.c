 ...
 *
 *  $Id: iprefs.c,v 40.0 94/02/15 17:37:10 davidj Exp $
 *
 ...
 * as rom default font.
 */
struct IFontPrefs topazprefs = {{{topazprefs.ifp_TABuff.tab_NameBuff, 8, 0, 0,}, KARLA}, NULL, IPF_SYSFONT,};
...
static LONG setIPrefs (CPTR pp, ULONG psize, ULONG ptype, BOOL backsync)
{
...
/* This code won't work any more... */
	 ...
	 */
	    return (LONG) setDefaultMousePointer (
	 ...
	 */
	    ...
	    if ((drecord = (ULONG) FindDisplayInfo (IOVERSCAN (pp)->ios_DisplayID))
	 ...
	 * GetDisplayInfoData( DTAG_MNTR )works, so will GetDisplayInfoData( DTAG_DIMS ) unless
	 * somebody has trashed the database:
	 * && GetDisplayInfoData( drecord, &info.dim, sizeof( struct QueryHeader ), DTAG_DIMS )
	 */
		&& GetDisplayInfoData ((DisplayInfoHandle)drecord, (UBYTE *)&info.mon, sizeof info.mon, DTAG_MNTR, NULL))
	    {
		...
		/* Set the display info (I don't know where this is defined) */
		SetDisplayInfoData (drecord, (UBYTE *)&info.mon, sizeof (info.mon), DTAG_MNTR);

		/* check what I get back */
		D (GetDisplayInfoData (drecord, &info.mon, sizeof (info.mon), DTAG_MNTR, NULL);
		   dumpPt ("viewpos reprise", info.mon.ViewPosition);)
		GetDisplayInfoData ((DisplayInfoHandle)drecord, (UBYTE *)&info.dim, sizeof info.dim, DTAG_DIMS, NULL);
		...
		SetDisplayInfoData (drecord, (UBYTE *)&info.dim, sizeof info.dim, DTAG_DIMS);
		{
		    ULONG default_id;
		 ...
		 */
		    ...
		    if ((drecord = (ULONG) FindDisplayInfo (default_id))
#if 0
			&& GetDisplayInfoData (drecord, &info.mon, sizeof(struct QueryHeader), DTAG_MNTR, NULL)
#else
			&& GetDisplayInfoData ((DisplayInfoHandle)drecord, (UBYTE *)&info.dim, sizeof info.dim, DTAG_DIMS, NULL)
#endif
			)
...
void syncIPrefs (LONG size)
{
	...
	setIPrefs ((CPTR) &p.font, sizeof (p.font), IP_FONT, FALSE);
	...
	setIPrefs ((CPTR) &p.screenmode, sizeof (p.screenmode), IP_SCREENMODE, FALSE);
    }
    DSYNC (
	      else
	      {
	...
	setIPrefs ((CPTR) &p.pointer, sizeof (p.pointer), IP_NEWPOINTER, FALSE);
    }
    DSYNC (
	      else
	      {
...
void trackViewPos (void)
{
	...
	&& (drecord = (ULONG) FindDisplayInfo (default_id))
	&& GetDisplayInfoData ((DisplayInfoHandle)drecord, (UBYTE *)&diminfo, sizeof diminfo, DTAG_DIMS, NULL))
    {
	...
	setIPrefs ((CPTR) &ios, sizeof (ios), IP_OVERSCAN, FALSE);
...
/* Given an array of UWORD 0x0RGB colors, Set the IBase->WBColors[] array in 8 bits-per-gun. */
