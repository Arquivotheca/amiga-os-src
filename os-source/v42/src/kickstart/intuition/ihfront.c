 ...
 *
 *  $Id: ihfront.c,v 40.0 94/02/15 17:41:01 davidj Exp $
 *
 ...
 */
static struct InputToken *newIEToken (struct InputEvent *ie, enum ITCommand command)
{
...
static void createMouseToken (struct InputEvent *ie, UWORD code, enum ITCommand command)
{
    struct InputEvent *cloneie;

    /* for buttonup: get the extra token and insert it */
...
static BOOL rawkeyButtonUpClone (struct InputEvent *ie)
{
 ...
 * it had meaning here...
 */
static BOOL rawkeyButtonDown (struct InputEvent *ie)
{
    struct IntuitionBase *IBase = fetchIBase ();
    ...
    if (TESTFLAG (ie->ie_Qualifier, ALTLEFT) && !TESTFLAG (IBase->Flags, COM_SELECT))
    {
    ...
    else if (TESTFLAG (ie->ie_Qualifier, ALTRIGHT) && !TESTFLAG (IBase->Flags, COM_MENU))
    {
 ...
 */
static BOOL rawkeyCommandKeys (struct InputEvent *ie)
{
    BOOL doit = FALSE;
    UBYTE code;
    struct IntuitionBase *IBase = fetchIBase ();
    ...
    if (TESTFLAG (ie->ie_Qualifier, AMIGALEFT) && (MapRawKey (ie, &code, 1, NULL) == 1))
    {
	struct Screen *sc;
	ULONG flags;
...
static void initCursorInc (void)
{
    struct IntuitionBase *IBase = fetchIBase ();
...
static UWORD repeatCursorInc (void)
{
    struct IntuitionBase *IBase = fetchIBase ();
 ...
 */
static LONG kbdVMouse (struct InputEvent *ie)
{
 ...
 * motion from keyboard.  Uses qualifier only.
 */
static LONG kbdHMouse (struct InputEvent *ie)
{
    UWORD qual = ie->ie_Qualifier;
    LONG retval;
    struct IntuitionBase *IBase = fetchIBase ();
 ...
 * it had meaning here...
 */
static BOOL rawkeyCursorKeys (struct InputEvent *ie)
{
    struct InputToken *it;
    BOOL doit = TRUE;
    WORD xmove;
     ...
     */
    if (doit &&	(cloneie = cloneIEvent (ie)) && (it = newIEToken (cloneie, itMOUSEMOVE)))
    {
...
static void convertRawKey (struct InputEvent *ie)
{
    struct InputToken *it;
...
static UWORD metaDrag (struct InputEvent *ie)
{
    struct IntuitionBase *IBase = fetchIBase ();
     ...
     */
    return (UWORD)((IBase->MetaDragQual) && (ie->ie_Qualifier & METAQUALS) == (IBase->MetaDragQual & METAQUALS));
}
...
static void addButtonEvent (struct InputToken *it, struct TabletData *tablet)
{
    BOOL add_a_button;
 ...
 */
static void convertPointerPos (struct InputEvent *ie, struct LongPoint *currpoint)
{
    struct IntuitionBase *IBase = fetchIBase ();
	 ...
	 */
	if ((!ient->ient_CallBack) || callHook (ient->ient_CallBack, ie, (ULONG)ascreen, rangeX, rangeY, new))
	{
	    ULONG remainder;
 ...
 *	would suck.
 */
static void convertMouse (struct InputEvent *ie)
{
    struct InputToken *it;
    struct IntuitionBase *IBase = fetchIBase ();
...
void createInputTokens (struct InputEvent *ie)
{
    ...
    /* need to track anticipated changes to IBase->LongMouse in case I get multiple
