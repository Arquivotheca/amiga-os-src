 ...
 *
 *  $Id: pools.c,v 40.0 94/02/15 17:31:49 davidj Exp $
 *
...
static struct Node *createPoolNode (struct List *lh)
{
    struct Node *ln;
    ...
    {
	/* Free list back pointer stored in ln_Name */
	ln->ln_Name = (char *) lh;	/* back link */
...
void poolReturn (struct Node *ln)
{
    ...
    D (
	  ...
	  {
	  kprintf ("poolReturn NULL passed\n");
	  Debug ();
	  }
    )
}
