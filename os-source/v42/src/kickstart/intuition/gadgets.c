 ...
 *
 *  $Id: gadgets.c,v 40.0 94/02/15 18:04:50 davidj Exp $
 *
 ...
 */
{
    ...
    g = (struct Gadget *) nthThing ((struct Thing *) g, pos, &realpos);

    /* find last gadget in list passed in */
    lastg = (struct Gadget *) nthThing ((struct Thing *) gadget, numgad - 1, NULL);
...
UWORD RemoveGList (struct Window * window, struct Gadget * gadget, LONG numgad)
{
    ...
    /* find the requester this gadget is hooked up in, if any. */
    if (gadget->GadgetType & REQGADGET)
    {
    ...
    pos = doISM (itREMOVEGAD, (CPTR) gadget, (CPTR) firstgadget, numgad);
...
LONG delinkGadgetList (struct Gadget *gadget, struct Gadget *firstgadget, LONG numgad)
{
    ...
    prevgadget = (struct Gadget *) previousThing ((struct Thing *) gadget, (struct Thing *) firstgadget, &pos);
...
void NewModifyProp (gad, win, req, flags, hpot, vpot, hbody, vbody, numgad)
struct Gadget *gad;
struct Window *win;
struct Requester *req;
ULONG flags, hpot, vpot, hbody, vbody;
LONG numgad;			/* passed as -2 from ModifyProp interface	*/
{
    ...
    doISM (itMODIFYPROP, (CPTR) win, (CPTR) gad, (ULONG) &pp);
}

/*****************************************************************************/

void IModifyProp (gad, win, pp)
struct Gadget *gad;
struct Window *win;
struct PropPacket *pp;
{
