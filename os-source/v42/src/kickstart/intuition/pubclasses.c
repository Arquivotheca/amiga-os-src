 ...
 *
 *  $Id: pubclasses.c,v 40.0 94/02/15 17:30:31 davidj Exp $
 *
...
struct IClass *MakeClass (UBYTE *classid, UBYTE *superid, struct IClass *superclass, ULONG instsize, ULONG flags)
{
    ...
    if (classid && (cl = FindClass (classid)))
	return (NULL);
    ...
    /* if I've got a superclass pointer by now, it's safe; not going to go away on me */
	...
	/* count on 0 init values for: cl_SubclassCount, cl_ObjectCount, cl_Flags */

	/* initialize Hook to no-op initial value */
	cl->cl_Dispatcher.h_Entry = (ULONG (*)()) stubReturn;
