 ...
 *  File
 *
 *  $Id: prop.c,v 40.0 94/02/15 17:30:57 davidj Exp $
 *
...
/* Shouldn't need to assign the result of this flying cast, but SAS 6.3 requires it!!! */
#define KLUDGEupperLeft(A) ( kludge = *((struct Point *) (A)))

/*****************************************************************************/

static UWORD potInRange (LONG potval)
{
    if (potval < 0)
	potval = 0;
    if (potval > 0xFFFF)
	potval = 0xFFFF;
    return (UWORD) (potval & 0xFFFF);
}
 ...
 */
static UWORD containerIncrement (UWORD body)
{
    UWORD inc = 0xFFFF;
...
void containerBumpPots (struct Gadget *g, struct GadgetInfo *gi, struct Point coord, struct IBox *knobbox)
{
	...
	pi->HorizPot = potInRange ((long) pi->HorizPot + mul * containerIncrement (pi->HorizBody));
    }
	...
	pi->VertPot = potInRange ((long) pi->VertPot + mul * containerIncrement (pi->VertBody));
    }
}
 ...
 */
static void dimensionNewKnobForBodys (struct Gadget *g, struct GadgetInfo *gi, struct PGX *pgx)
{
    struct PropInfo *pi;
 ...
 */
static void setPotsForKnobPosition (struct Gadget *g, struct GadgetInfo *gi, struct Point pt, struct PGX *pgx)
{
    struct IBox imbox;
    LONG freedom;
    UWORD pot;
 ...
 */
/* returns false if knob doesn't move */
BOOL dragNewKnob (struct Gadget *g, struct GadgetInfo *gi, struct Point mousept, struct IBox *gbox)
{
    ...
    gadgetPoint (g, gi, mousept, (struct Point *) &newknob.Left, gbox);
    transfPoint ((struct Point *)&newknob.Left, *((struct Point *) &pi->LeftBorder));

    /* get new proposed image left/top from this */
    transfPoint ((struct Point *)&newknob.Left, IBase->KnobOffset);
    ...
    {
	struct Point kludge;	/* ZZZ to spoof SAS 6.3 */
	setPotsForKnobPosition (g, gi, KLUDGEupperLeft (&newknob), &IBase->ActivePGX);
    }
...
{
    return (BOOL)(TESTFLAG (((struct PropInfo *) g->SpecialInfo)->Flags, PROPBORDERLESS));
}
