 ...
 *
 *  $Id: newlook.c,v 40.0 94/02/15 17:33:22 davidj Exp $
 *
...
static struct Image *getScreenSysImage (struct Screen *s, LONG gindex)
{
    ...
    if (*sysimg == NULL)
    {
	/* sysiclass uses this BarHeight, if relevant.  SYSIA_Size default comes from DrawInfo */
	if (!(*sysimg = (struct Image *) NewObject (NULL, SysIClassName,
				   IA_HEIGHT,		(ULONG)(s->BarHeight + 1),
				   SYSIA_DrawInfo,	(ULONG)&(XSC(s)->SDrawInfo),
				   SYSIA_Which,		(ULONG)gindex,
				   TAG_END)))
	{
...
struct Gadget *createNewSysGadget (struct XScreen *s, USHORT res, USHORT gindex, BOOL gimme)
{
...
void embossedBoxTrim
    (struct RastPort *rp, struct IBox *b, UBYTE hthick, UBYTE vthick, UBYTE ulpen, UBYTE lrpen, UBYTE meeting_type)
{
    LONG left, top, bottom, right;
    LONG cnt;
    LONG scum1, scum2, scum3, scum4;	/*dave is scum */
