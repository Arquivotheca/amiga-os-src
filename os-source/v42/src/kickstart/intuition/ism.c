 ...
 *
 *  $Id: ism.c,v 40.0 94/02/15 17:36:17 davidj Exp $
 *
...
void initTokens (void)
{
    ...
    /* Initialize the pool of Intuition messages */
    poolInit (&IBase->ITFreeList, sizeof (struct InputToken), IT_ALLOC);

    /* Initialize the other message lists */
...
/* doIO front end: waits until done returns error code from InputToken */
 ...
 * assumes STATELOCK */
static void dispatchToken (struct InputToken *it)
{
    struct IntuitionBase *IBase = fetchIBase ();
...
static void disposeToken (struct InputToken *it)
{
...
static void notifyToken (struct InputToken *it)
{
