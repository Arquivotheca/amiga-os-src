/*** drawgadgets.c ***********************************************************
 *
 *  $Id: drawgadgets.c,v 40.0 94/02/15 17:52:19 davidj Exp $
 *
 ...
 */
static void drawGGrunt (struct RastPort *rp, struct Gadget *firstg, struct GListEnv *ge, LONG numgad, LONG filter_flags)
{
	...
	while (g)
	{
	    ...
	    if ((filter_flags == DRAWGADGETS_ALL) || ((filter_flags == DRAWGADGETS_BORDER) &&
	    ...
	    g = (struct Gadget *) previousLink ((struct Thing *) firstg, (struct Thing *) g);
	}
    }
}
...
/* gets pens to use for proportional gadgets container. */
static void getContainerPens (struct Gadget *g, struct GadgetInfo *gi, UBYTE cpens[NUMCONTAINERPENS])
{
    ...
    prop_in_border = (TESTFLAG (g->Activation, ALLBORDERS) || TESTFLAG (g->GadgetType, GZZGADGET));

    if (newlookProp (g))
    {
	if ((!TESTFLAG (drinfo->dri_Flags, DRIF_NEWLOOK)) || (drinfo->dri_Depth < 2))
	{
	    /* mono look */
	    cpens[CP_FILL] = penspec[BACKGROUNDPEN];
	    cpens[CP_PATTERN] = penspec[SHADOWPEN];

	    if (prop_in_border && TESTFLAG (gi->gi_Window->Flags, WINDOWACTIVE))
	    {
	    ...
	    {
		/* the "inactive look", also used inside window interiors */
...
{
    void (*render)(struct RastPort *,APTR,LONG,LONG,ULONG,struct DrawInfo *);
    APTR visuals;
    ...
    render = (void (*)(struct RastPort *,APTR,LONG,LONG,ULONG,struct DrawInfo *)) DrawBorder;
    if (TESTFLAG (g->Flags, GADGIMAGE))
	render = DrawImageState;

    /* do it */
    /* note that we pass extra ignored parameters to drawborder */
    /* the idea here is that you can use a boopsi image for BOTH
     * the normal and select render. */
    (*render) (rp, visuals, gbox->Left, gbox->Top, gadgetDrawState (g, gi, suppress_selected), gi->gi_DrInfo);
}

/*****************************************************************************/

/* performs the standard ghosting of the rectangle of a disabled gadget. */
void commonGadgetTextAndGhost (struct RastPort *rp, struct Gadget *g, struct GadgetInfo *gi, struct IBox *gbox)
{
    ...
    void (*renderfunc) (struct RastPort *,APTR,WORD,WORD);

    renderfunc = (void (*)(struct RastPort *,APTR,WORD,WORD)) PrintIText;
    if ((g->Flags & GFLG_LABELMASK) == GFLG_LABELIMAGE)
	renderfunc = (void (*)(struct RastPort *,APTR,WORD,WORD)) DrawImage;
    ...
    if ((g->Flags & (GFLG_DISABLED | GFLG_IMAGEDISABLE)) == GFLG_DISABLED)
...
ULONG gadgetDrawState (struct Gadget *g, struct GadgetInfo *gi, LONG suppress_selected)
{
     ...
     * disabled rendering? */
    else if ((g->Flags & (GFLG_DISABLED | GFLG_IMAGEDISABLE)) == (GFLG_DISABLED | GFLG_IMAGEDISABLE))
    {
	if (selected)
 ...
 * ptr is window or screen
 * NULL gadget means abort
 * numgad: -1 means all, -2 means from top of list if REQGADGET
 */
void drawGadgets (APTR ptr, struct Gadget *gadget, LONG numgad, LONG filter_flags)
{
    ...
    /* jimm: 8/26/86: found out that 1.1 RefreshGadgets() did ALL gadgets in requester list */
...
     * by example (and now by decree).
	...
	void (*beginfunc) (APTR);
	void (*endfunc) (APTR, UWORD);
	APTR pane;			/* Window or layer */

#if 0
	void BeginRefresh (APTR);
	void EndRefresh (APTR, UWORD);
	void BeginUpdate (APTR);
	void EndUpdate (APTR, UWORD);
#endif
	...
	beginfunc = (void (*)(APTR)) BeginRefresh;
	endfunc   = (void (*)(APTR,UWORD)) EndRefresh;
	...
	if (ginfo->gi_Requester)
	{
	    ...
	    beginfunc = (void (*)(APTR)) BeginUpdate;
	    endfunc   = (void (*)(APTR,UWORD)) EndUpdate;
	}
...
/* sync up knob position with NewKnob, draw in new place, erase remnants */
...
void drawKnob (struct Gadget *g, struct GadgetInfo *gi, struct RastPort *rp, struct PGX *pgx)
{
	...
	if (newlookProp (g))
	{
	    ...
	    if ((freedom == 0) || (freedom == (FREEVERT | FREEHORIZ)) || borderlessProp (g))
	    {
	...
	/* ~DNJ~ 7-Feb-94 : Wasn't passing DrawInfo */
	DrawImageState (rp, gimage, left, top, state, gi->gi_DrInfo);

	/* See if we need to XOR anything */
