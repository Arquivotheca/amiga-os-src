 ...
 *
 *  $Id: init.c,v 40.0 94/02/15 17:40:47 davidj Exp $
 *
...
static void initIntuition (void)
{
    LONG i;
    ...
    kprintf ("Intuition %ld.%ld\n", (LONG) IBase->LibNode.lib_Version, (LONG) IBase->LibNode.lib_Revision);
    ...
    IBase->BlankSprite = extsprite = (struct ExtSprite *) AllocSpriteData (&sprite_bm, TAG_DONE);
    IBase->TetrisSprite = IBase->SimpleSprite = (struct SimpleSprite *) extsprite;
	 ...
	 */
	clearWords ((UWORD *) & dri, sizeof (struct DrawInfo) / 2);
...
void InitRequester (struct Requester *req)
{
    ...
    cptr = (BYTE *) req;
    for (i = sizeof (struct Requester); i > 0; i--)
	*cptr++ = 0;
}
