...
#include "zoom_protos.h"

/*---------------------------------------------------------------------------*/

static int windowSizeLimits(struct Window * w,
                            struct Rectangle * rect);

static int windowPositionLimits(struct Window * w,
                                struct Rectangle * rect);

static int legalWindowBox(struct Window * window,
                          struct IBox * box);

static int windowLayerList(struct Window * w,
                           struct LayerEntry * list,
                           BOOL countnulllayers);

static int innerWindowBox(struct Window * w,
                          struct IBox * windowbox,
                          struct IBox * innerbox);

static int doWindowLayer(struct LayerEntry * le,
                         struct IBox * windowbox,
                         struct IBox * innerbox);

static int changeLayer(struct Layer * l,
                       struct IBox * box);

static struct Layer * createReqLayer(struct Layer_Info * linfo,
                                     struct BitMap * bmap,
                                     struct IBox * box,
                                     ULONG flags);

static struct Layer * windowFrontLayer(struct Window * window,
                                       struct Requester * req);

static int damageWholeLayer(struct Layer * l);
