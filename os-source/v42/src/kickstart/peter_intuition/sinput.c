...
#include "sinput_protos.h"

/*---------------------------------------------------------------------------*/

static int setupSGWork(struct SGWork * sgw,
                       struct Gadget * g,
                       struct GadgetInfo * gi,
                       struct StringExtend * sex,
                       struct InputEvent * ie);

static int sgAction(struct SGWork * sgw,
                    BOOL * redisplay,
                    LONG * gadgetupcode);

static int delWord(struct SGWork * sgw);

static int clearSGW(struct SGWork * sgw);

static int stringDelete(struct SGWork * sgw,
                        BOOL del_to_end);

static int stringBackspace(struct SGWork * sgw,
                           int numkill);

static int iHandleMouse(struct SGWork * sgw,
                        BOOL * redisplay,
                        struct IBox * gbox);

static int stringClick(struct Gadget * g,
                       struct GadgetInfo * gi,
                       int clickx);

static BOOL LONGTest(UBYTE * buffer,
                     LONG * longvar);
