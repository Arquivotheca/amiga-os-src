...
#include "vectorclass_protos.h"

/*---------------------------------------------------------------------------*/

static LONG Scale(LONG sourcesize,
                  LONG destsize,
                  LONG coord);

static ULONG dispatchSysI(Class * cl,
                          Object * o,
                          Msg msg);

static int drawSysI(Class * cl,
                    Object * o,
                    struct impDraw * msg);

static BOOL setState(struct Image * im,
                     struct SysIData * si,
                     LONG state,
                     struct SysIClassData * scd);

static BOOL createStateImagery(struct Image * im,
                               struct SysIData * si,
                               struct SysIClassData * scd,
...
static BOOL allocRenderStuff(struct Image * im,
                             struct SysIData * si,
                             struct SysIClassData * scd);

static void freeRenderStuff(struct SysIClassData * scd);

static BOOL allocImageMem(struct Image * im,
                          struct SysIData * si);

static void freeImageMem(struct SysIData * si);

static void drawVectorImage(struct RastPort * rport,
                            struct VectorInfo * vi,
                            LONG Width,
                            LONG Height,
                            ULONG state,
...
static void Draw1P3DBorder(UWORD * Pens,
                           struct RastPort * rp,
                           LONG dir,
                           LONG tx,
                           LONG w,
...
static void myRectangle(struct RastPort * rport,
                        LONG x1,
                        LONG y1,
                        LONG x2,
                        LONG y2);

static int WritePixelPen(int rport,
                         int pen,
                         int x,
                         int y);

static int MoveDrawPen(int rport,
                       int pen,
                       int x,
                       int y,
                       int dx,
...
static LONG
Scale(sourcesize,destsize,coord)
LONG sourcesize;
LONG destsize;
LONG coord;
{
...
static BOOL
createStateImagery( im, si, scd, vi, width, height, dri )
struct Image *im;
struct SysIData *si;
struct SysIClassData *scd;
struct VectorInfo *vi;
