...
* Amount of stack space needed by ReadArgs for temp processing
* no items needed, but we use one longword at RA_STACK(sp) for storage
* of the result of ReadItem
RA_STACK	equ	4 ;MAX_TEMPLATE_ITEMS+4*MAX_MULTIARGS+4
...
AA_Argimage	equr	A5	; Multiarg/template array pointer
...
;	UBYTE template_item_flags[MAX_TEMPLATE_ITEMS]
;	ULONG multiarg_size;  // in bytes for speed
;	UBYTE *multiarg_ptr_arry [num_multiargs - initially MAX_MULTIARGS]
;
;AA_Argimage	equr	SP
MULTIARG_SIZE	EQU	MAX_TEMPLATE_ITEMS
MULTIARG_ARRAY	EQU	MAX_TEMPLATE_ITEMS+4
AA_ARG_SIZE	EQU	MAX_TEMPLATE_ITEMS+4+MAX_MULTIARGS*4
...
* the shared code.
*
* NOTE! D3 is a parameter to ReadArgs.  It's also the D_IsBCPL value!
*
**********************************************************************
_ReadArgs:
...
have_buffer:				; comes here if buffer exists or after
	...
	move.l	#AA_ARG_SIZE,d1
	bsr	GetVec
	move.l	d0,AA_Argimage		; do before test in case of failure!
	tst.l	d0
	 beq	ra_errmem
	; remember how much space we have
	move.l	#MAX_MULTIARGS*4,MULTIARG_SIZE(AA_Argimage)
...
ra_initkeys:
	...
	move.l	AA_Argimage,A3		; Byte array to initialize
...
* A_Keys points to the current template item during post-processing for return
...
; Copy over the temporary MultiArg array into the buffered allocation space
	move.l	D0,A1
	move.l	D_Multi,D0	; don't kill D_Multi, so I can test it (REJ)
	lea	MULTIARG_ARRAY(AA_Argimage),A0 ; array

; If we use multiple /M's, this must be FIXED!
...
rdargs_rts:
	move.l	d0,-(sp)	; return code (save)
	move.l	AA_Argimage,A1
	EXECCALL FreeVec	; FreeVec(NULL) is ok - nukes a6
	move.l	(sp)+,d0
	add.w	#RA_STACK+4,sp
	movem.l	(sp)+,D2-D7/A2-A6 ; restores a6
	rts
...
ra_case2:
	...
	move.l	AA_Argimage,A0	; Was not a keyword - put it in next free.
...
5$:	;-- have number, now store it, update stuff, and loop
	...
	lea	MULTIARG_ARRAY(AA_Argimage),A0
...
ra_StoreMulti:
	move.l	MULTIARG_SIZE(AA_Argimage),d1 ; size of multiarg array (bytes)
	cmp.l	d1,D_Multi
	 bge.s	ra_expand_multi		; Too many multiargs.  Expand array
ra_continue:
...
ra_expand_multi: 			; d1 is old size in bytes
	; add space for MAX_MULTIARGS more - add the fixed requirement also
	add.l	#MAX_MULTIARGS*4+MAX_TEMPLATE_ITEMS+4,d1
	movem.l	d0-d1/a6,-(sp)		; save flags and size
	bsr	GetVec
	move.l	d0,-(sp)		; since we need to save it anyways
	 bne.s	10$			; allocation failed, give error msg
	lea	16(sp),sp		; drop NULL, old d0/d1 and a6 off stack
	bra	ra_errmem

10$	move.l	AA_Argimage,a0		; copy old array to new array
	move.l	d0,a1			; new array
; FIX! can save a few bytes here by snagging size off the stack
	move.l	MULTIARG_SIZE(AA_Argimage),d0  ; size of old allocation
	add.l	#MAX_TEMPLATE_ITEMS+4,d0       ; including fixed part
	EXECCALL CopyMem		; copy template and multiarg arrays
	move.l	AA_Argimage,a1		; old buffer (a6 = execbase)
	jsr	_LVOFreeVec(a6)
	move.l	(sp)+,AA_Argimage	; new buffer installed
	movem.l	(sp)+,d0-d1/a6
	sub.l	#MAX_TEMPLATE_ITEMS+4,d1 ; these aren't part of the array
	move.l	d1,MULTIARG_SIZE(AA_Argimage) ; set the new size
	bra.s	ra_continue
