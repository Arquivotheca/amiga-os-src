 ...
 *	
 *
 * Prototypes for functions defined in files.c
 *
 *	WORD 				get_entry_type			(
 *	LONG 				get_dos_type			(
 *	LONG 				get_entry_size			(
 *	int 				astcsma					(
 *	struct String ** 	match_files				(
 *	void 				free_matches			(
 *	struct DosList * 	LockDevList				(
 *	struct DosList * 	FindDevEntry			(
 *	struct DosList * 	NextDevEntry			(
 *	void 				UnLockDevList			(
 *	BOOL 				is_disk					(
 *	BOOL 				check_node				(
 *	struct String ** 	get_drives				(
 *	long 				GetDiskSpace			(
 *	struct String * 	find_largest_partition	(
 *	struct DeviceNode * find_device_for_task	(
 *	BPTR 				boot_volume				(
 *	
 *
 *	Revision History:
 ...
 *	lwilton 07/19/93:
 *		Changed find_largest_partition to skip ram disks (by name!)
 *	lwilton 10/25/93:
 *		Commented out code that was specifically ignoring ".info" files
 *		in the match_files routine.  Solves lpoma's bug.
 */
...
/*	NOTES:
	...
	the calling interface should be the same as the SAS/C function astcsma().
*/
...
struct StringNode 
{							/* structure to store pathnames & dos types */
	struct MinNode	node;
	LONG			size;
	LONG			dostype;
	WORD			type;
...
{	
	...
	/*	This appears to be a copy of a SAS library routine!  */
	
int astcsma(char *str, char *pat)
{	
#ifdef ONLY2_0
	...
	if (pattern_buf = AllocMem(length,0L))
	{
		...
		{
			if (MatchPatternNoCase(pattern_buf,str)) 
				result = strlen(str);
		}
#else
		if (PatternParse(pat,pattern_buf,length - 4) != -1)
		{
			if (PatternMatch(pattern_buf,str)) 
				result = strlen(str);
		}
#endif
...
{	
	...
	if (Examine(lock,fib))
	{
		...
		{	
			...
			if (fib->fib_EntryType < 0 && mode == MATCH_DIRS_ONLY) 
				continue;
			if (fib->fib_EntryType > 0 && mode == MATCH_FILES_ONLY) 
				continue;
#if 0
			/* what about ".info/.icon" files */
			/* 	Failiing to include .info files in a 'foreach' statement
			 *	causes problems.  We will try removing this and see if
			 *	the removal hopefully doesn't caise worse problems.
			 *
			 *	Normally nobody should be doing a copy to a root directory
			 *	(and certainly NOT a "#?" copy!), so this shouldn't be a
			 *	problem.  We hope.
			 */

			if (!stricmp(fib->fib_FileName, dot_info)) 
				continue;
#endif
			len = strlen(fib->fib_FileName);

			if (no_infos != 0 && astcsma(fib->fib_FileName,"#?.info") == len) 
				continue;
			if (no_fonts != 0 && astcsma(fib->fib_FileName,"#?.font") == len) 
				continue;
...
fail:
	while (n = (struct StringNode *)RemHead((struct List *)&list)) 
		MemFree(n);
	return NULL;
}
...
{	
	...
	for (dnode = (struct DosList *)BADDR(dnode->dol_Next);
		 dnode;
...
{
	/*	Note that use of this code can result in preventing installation
	 *	from a disk on a network server!
	 */
	 
	long	type = DiskType(dnode);

	return ((BOOL)(type != ID_NO_DISK_PRESENT &&
...
{	
	...
	else
	{
		...
		{
			while (dnode = (struct DeviceNode *)
						NextDevEntry((void *)dnode,LDF_DEVICES | LDF_ASSIGNS))
			{
				if (!check_node(&list,dnode,&count))
				{
					UnLockDevList();
...
/*	The following code trys to find the largest partition (with some
 *	exceptions) as the default place to install an application.		
 *
 *	There are several special cases here.  If we find a partition named
 *	"Work" we pretend it has twice as much space as it really does,
 *	giving it precidence over other partitions of similar size.  If we
 *	find partitions named "Ram", "Rad", or "Ramb0", we will ignore them.
 *	installing an application to RAM: by default is not cool.
 *
 *	(Note:  This code could potentially fail, since we are doing name
 *	comparisons.  If the user has named the ramdisk "fred" we still can
 *	install to it.  Also, if the user named the work partition "Ram" we
 *	will ignore it.
 */

/* =======BUG=====  Note it is reported this code can return "RAM:"  */


#define MAX_DEVICES			64

struct DevData 
{									/* for keeping a copy of a dos node */
	struct DeviceNode	dnode;
	char				name[32];
};
...
{	
	...
	unless (id = AllocMem(sizeof *id + MAX_DEVICES * sizeof *dd_list, MEMF_CLEAR)) 
		return NULL;
...
#endif
	{
		...
		{	
			while ((dnode = (struct DeviceNode *)NextDosEntry((void *)dnode,
																LDF_DEVICES)) 
	...
	else
	{
		if (dnode = (struct DeviceNode *)LockDevList())
		{
			while ( dn_count < MAX_DEVICES &&
				(dnode = (struct DeviceNode *)NextDevEntry((void *)dnode,
															LDF_DEVICES)) )
			{
				dd_list[dn_count].dnode = *dnode;
				CpyBstrCstr((char *)dnode->dn_Name,dd_list[dn_count++].name);
	...
	{	
		...
		{	
			...
			if (!stricmp(dd_node->name,"Work:")) 
				size = 2;
			if (!stricmp(dd_node->name, "RAM:")
			||  !stricmp(dd_node->name, "RAD:")
			||	!stricmp(dd_node->name, "RAMB0:"))
				continue;							/* ignore it! */
	...
	if (bname) 
		str = MakeString(bname,strlen(bname));
	else 
		str = MakeString("SYS:",4);
...
{	
	...
	else
	{
		if (dnode = (struct DeviceNode *)LockDevList())
		{
			while (dnode = (struct DeviceNode *)NextDevEntry((void *)dnode,
																LDF_DEVICES))
			{
				if (dnode->dn_Task == mp) 
				{ 
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					if (!strcmp(buffer,"trackdisk.device")) 
						goto askdisk;
