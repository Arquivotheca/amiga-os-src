...
struct GSymbol *FindGSymbol 	(struct GSymbolScope *scope, 
								 char 				 *name, 
								 WORD 				  namelength,
								 struct GSymbol    ***headptr );
	
struct GSymbol *AddGSymbol 		(struct GSymbolScope *scope, 
								 char 				 *name, 
								 WORD 				  namelength,
								 struct GSymbol 	**headptr );
	
BOOL 			DeleteGSymbol 	(struct GSymbolScope *scope, 
								 struct GSymbol 	 *symbol );
							
void 			DeleteGScope 	(struct GSymbolScope *scope );

int				AddGHash 		(struct GSymbolScope *scope, 
								 WORD 				  hsize );
							
WORD 			HashVal 		(char 				 *name, 
								 WORD 				  length, 
								 WORD 				  hashsize );
