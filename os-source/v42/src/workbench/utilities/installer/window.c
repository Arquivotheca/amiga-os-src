 ...
 * ========================================================================= 
 *	
 *	This is the main driver routine for the Installer.
 *	
 *
 ...
 *  extern WORD                     right_edge;
 *  extern WORD                     top_edge;
 *  extern WORD                     bottom_edge;
 *	extern struct 					TreeNode * root;
 *	extern UWORD 					pretend_flag;
 *	extern UWORD 					user_level;
 *	extern UWORD 					minuser_level;
 *	extern UWORD 					transcript_mode;
 *	extern UWORD 					no_pretend;
 *	extern unsigned char * 			icon_name;
 *	extern unsigned char * 			templatefile;
 *	extern unsigned char 			app_name[40];
 ...
 *	extern void * 					language_save;
 *	extern void ** 					app_ptr;
 *	extern void * 					app_save;
 *	extern void ** 					icon_ptr;
 *	extern void * 					icon_save;
 *	extern void ** 					defpart_ptr;
 *	extern void * 					defpart_save;
 *	extern BPTR 					transcript_file;
 *	extern BOOL 					must_log;
 *	extern BOOL 					no_print;
 *	extern BOOL 					exit_quiet;
 *	extern long 					percent_complete;
 *	extern BOOL 					did_final;
 *	extern USHORT * 				chip_busy;
 *	extern BYTE 					inited_busy;
 *	extern BYTE 					inited_fail;
 *	extern BYTE 					is_busy;
 *	extern unsigned char 			transcript_name			[128];
 *	extern unsigned char 			xtra_trans_name			[128];
 *	extern unsigned char * 			temp_transcript_name;
 *	extern unsigned char 			allpat_tx				[3];
 *	extern unsigned char * 			user_level_names		[3];
 *	extern unsigned char * 			pretend_mode_names		[2];
 *	extern unsigned char 			VersionString			[58];
 *	extern WORD 					paramflags;
 *	extern unsigned char * 			params					[9];
 *
 * 
 *	void 				calc_window_size	(struct TextFont * );
 *	void 				assign_text			(void);
 *	WORD 				GetDepthGadgetWidth	(struct Window * );
 *	void 				main				(int , unsigned char ** );
 *	BOOL 				welcome_page		(struct InstallationRecord * );
 *	BOOL 				copylogfile			(unsigned char * , unsigned char * );
 *	unsigned char * 	close_transcript	(void);
 *	void 				set_gadgets			(struct Gadget * );
 *	void 				disable_gadgets		(struct Gadget * );
 *	void 				remove_gadgets		(struct Gadget * );
 *	void 				clear_gadgets		(struct Gadget * );
 *	struct Gadget * 	HandleAnyEvent		(struct ListControl * );
 *	struct Gadget * 	HandleEvents		(struct ListControl * );
 *	BOOL 				check_escape		(void);
 *	int 				save_page			(struct InstallationRecord * , unsigned char * , int (* )(struct InstallationRecord * , unsigned char * ));
 *	int 				do_insert_disk		(struct InstallationRecord * , unsigned char * );
 *	int 				do_nothing			(struct InstallationRecord * , unsigned char * , LONG );
 *	BOOL 				setup_list			(struct ListControl * , unsigned char * , BOOL );
 *	int 				do_drawers			(struct InstallationRecord * , unsigned char * );
 *	int 				show_filelist		(struct InstallationRecord * , unsigned char * );
 *	BOOL 				abortcopy_page		(WORD * );
 *	struct Gadget * 	abortcopy_display	(unsigned char * , unsigned char * );
 *	void 				abortcopy_clear		(WORD * );
 *	int 				makedir_page		(struct InstallationRecord * , unsigned char * );
 *	int 				askdelete_page		(struct InstallationRecord * , unsigned char * );
 *	BOOL 				copy_extra_file		(struct InstallationRecord * , unsigned char * , unsigned char * , unsigned char * , unsigned char * );
 *	struct String * 	copy_page			(struct InstallationRecord * );
 *	int 				radio_page			(struct InstallationRecord * );
 *	int 				checkbox_page		(struct InstallationRecord * );
 *	unsigned char * 	askstring_page		(struct InstallationRecord * );
 *	long 				asknumber_page		(struct InstallationRecord * );
 *	long 				askdisk_page		(struct InstallationRecord * );
 *	long 				yesno_page			(struct InstallationRecord * );
 *	void 				display_help_page	(unsigned char * , WORD );
 *	void 				erase_help_page		(void);
 *	int 				help_page			(struct InstallationRecord * , unsigned char * );
 *	int 				about_page			(struct InstallationRecord * , unsigned char * );
 *	int 				error_page			(struct InstallationRecord * , unsigned char * );
 *
 *	extern BOOL 		dir_special;
 *
 *	unsigned char * 	select_dir			(struct InstallationRecord * );
 *	unsigned char * 	select_file			(struct InstallationRecord * );
 *	void 				message_page		(struct InstallationRecord * );
 *	void 				final_page			(void);
 *	int 				working_page		(struct InstallationRecord * );
 *	int 				quick_working_page	(void);
 *	int 				confirm_page		(struct InstallationRecord * );
 *	int 				version_page		(struct InstallationRecord * , long , long );
 *	struct String * 	make_user_help		(struct InstallationRecord * , struct InstallationRecord * , WORD );
 *	int 				get_user			(struct InstallationRecord * );
 *	int 				get_options			(struct InstallationRecord * );
 *	int  __stdargs 		report_message		(WORD , unsigned char * , ...);
 *	void __stdargs 		simple_message		(unsigned char * , ...);
 *	unsigned char * 	has_special			(void);
 *	int 				fatal_page			(struct InstallationRecord * , unsigned char * );
 *	void 				show_error			(WORD , unsigned char * );
 *	int 				ask_abort			(void);
 *	void 				completion			(long );
 *
 *	extern struct 		IntuiText MsgIText;
 *	extern struct 		IntuiText OkIText;
 *
 *	void 				auto_err			(unsigned char * );
 *
 *	extern USHORT 		Busy1_3		[48];
 *	extern UWORD		Busy2_0		[36];
 *
 *	void 				set_busy			(void);
 ...
 *	lwilton 10/24/93:
 *		Changed transcript_wrap parameters.
 *		Modified askchoice and askoptions to handle all possible options.
 *		Fixed a bug in copyfiles when you had a pattern and the source
 *		specified a file instead of a directory.  This caused a crash
 *		from a bad freemem.
 *		Changed copy_extra_file and copy_file (in action.c) to permit
 *		checking if the .info file was really copied or not.
 *		Changed get_user to put user welcome text in the main body of the
 *		welcome page rather than prepending it to the help text where
 *		nobody would expect to look.
 */
...
{	
	100,60,									/* leftedge, topedge	*/
	300,120, 								/* width, height		*/
	(UBYTE)~0,(UBYTE)~0,					/* detailpen, blockpen	*/
	STD_FLAGS,								/* idcmpflags			*/
	...
	NULL,									/* screen				*/
	NULL,									/* bitmap				*/
	100,120,								/* minwidth, minheight	*/
	(USHORT)~0,(USHORT)~0,					/* maxwidth, maxheight	*/
...
UWORD					pretend_flag 	= FALSE,
						user_level 		= 0,
						minuser_level 	= 0,
						transcript_mode = 2,	/* 0=prn, 1=disk, 2=none */
						no_pretend 		= FALSE;
char					*icon_name		= NULL;
char					*templatefile;
...
extern char	*dot_info, 
			*dot_font;

extern char *no_find_icon;

extern char *trans_made_drawer, 
			*trans_copy_file, 
			*trans_copy_drawer,
			*trans_ask_choice, 
			*trans_no_options, 
...
extern char *bad_params_copy, 
			*invalid_drawer_name;

char		*user_level_names[3],
			*pretend_mode_names[2];

char 		VersionString[] = VSTRING VERSTAG;		/* created by "bumprev" */
...
void calc_window_size(struct TextFont *tf)
{
	...
	window_height = (tf->tf_YSize + 1) * 16 +
		theScreen->RastPort.Font->tf_YSize + 1 + 6;	/* + 6 button compensation */
...
WORD GetDepthGadgetWidth(struct Window *window)
{
	...
	while (gad != NULL)
	{
		if ((gad->GadgetType & 0x00f0) == GTYP_WUPFRONT) 
			width += gad->Width;
		if ((gad->GadgetType & 0x00f0) == GTYP_WDOWNBACK) 
			width += gad->Width;
		gad = gad->NextGadget;
	}
...
{	
	...
	/*	This library_name varaible is for improved error messages.  */
	
	library_name = "intuition.library";
	if (!(IntuitionBase = 
		(struct IntuitionBase*)OpenLibrary(library_name, 0L))) LEAVE;

#ifdef ONLY2_0
	library_name = "gadtools.library";
	if (!(GadToolsBase = OpenLibrary(library_name,0))) LEAVE;
#endif

	library_name = "graphics.library";
	if (!(GfxBase = (struct GfxBase*)OpenLibrary(library_name,0))) LEAVE;

	library_name = "layers.library";
	if (!(LayersBase = OpenLibrary(library_name,0))) LEAVE;

	library_name = "icon.library";
	if (!(IconBase = OpenLibrary(library_name,0))) LEAVE;

	library_name = "console.device";
	...
	if (argc == 0)							/* WorkBench startup */
	{
		...
		if (wbmsg->sm_NumArgs == 0) 
			LEAVE;							/* ok, who sent this!! */

		k = (wbmsg->sm_NumArgs > 1 ? 1 : 0);

		name = (char *)wbmsg->sm_ArgList[k].wa_Name;
		if (name[0] == '\0') 
			LEAVE;							/* if it's not a project or tool */
		...
		if (CheckToolValue(dobj,minuser_tt,average_tv)) 
			minuser_level = 1;
		if (CheckToolValue(dobj,minuser_tt,expert_tv)) 
			minuser_level = 2;

			/* DEFUSER=NOVICE|AVERAGE|EXPERT */
		if (CheckToolValue(dobj,defuser_tt,average_tv)) 
			user_level = 1;
		if (CheckToolValue(dobj,defuser_tt,expert_tv)) 
			user_level = 2;
		...
		if (name = LocateToolType(dobj,logfile_tt))
			...
			/* LOG */
		if (CheckToolValue(dobj,log_tt,false_tv)) 
			must_log = FALSE;

			/* APPNAME=NAME */
		if (name = LocateToolType(dobj,appname_tt)) 
			strcpy(app_name,name);

			/* PRETEND=FALSE */
		if (CheckToolValue(dobj,pretend_tt,false_tv)) 
			no_pretend = TRUE;

			/* PRINT=FALSE */
		if (CheckToolValue(dobj,print_tt,false_tv)) 
			no_print = TRUE;
	...
	else									/* CLI startup */
	{
		...
		for (i=1;i<argc;i++)				/* run through CLI parameters */
		{
			...
			case 2:
				if (!stricmp(argv[i],expert_tv)) 
					minuser_level = 2;
				else 
				if (!stricmp(argv[i],average_tv)) 
					minuser_level = 1;
				break;
	...
	if (user_level < minuser_level) 
		user_level = minuser_level;
	...
	{
		if (!GetScreenData((char *)&wbScreenData,
							sizeof wbScreenData,
							WBENCHSCREEN,
							NULL)) 
		{
	...
	if (init_symbols())							/* init parser */
	{
		...
		if (root = compile(install_script))		/* compile script */
		{
			...
			/* ===BUG=== This routine can find RAM: as the place to use! */
			...
			if (welcome_state == 0 && !welcome_page(NULL)) /* no (welcome "")? */
				goto quit;
...
quit:		de_compile(root);					/* return working space		*/
		...
		report_message(1,GetLocalText(MSG_NO_INIT));
...
exitit:
	...
	if (main_bits & MAINBIT_ASSIGN) 
		assign_text();

	switch (err_mode)
	{
	case ERR_LIBS:
			/*	We need to use library_name to be more explicit here! */
			 
			auto_err(GetLocalText(MSG_NO_LIBRARIES)); 
			break;
	
	case ERR_ICON: 
			auto_err(no_find_icon); 
			break;
	
	case ERR_SCREEN: 
			auto_err(GetLocalText(MSG_NO_WORKBENCH)); 
			break;
	
	case ERR_ARGS: 
			Puts(GetLocalText(MSG_BAD_ARGS)); 
			break;
...
{
	if (welcome_state == 2) 					/* been here before? */
		return TRUE;							/* yes, only welcome once! */
	welcome_state = 2;							/* remember progress */

		/* put up the first options screen */
		
	if (get_user(ir) == FALSE) 					/* user-level select screen */
		return FALSE;							/* user aborted 			*/

		/* put up the second options screen if not in NOVICE mode */
		
repeat:
	if (user_level != USER_NOVICE && get_options(ir) == FALSE) 
		return FALSE;							/* user aborted options		*/
	...
	*ulevel_ptr  = (void *)user_level;
	...
	if (user_level == USER_NOVICE && must_log) 
		transcript_mode = 1;						/* create a disk log file */

	if (transcript_mode < 2)						/* creating a log? 		  */
	{
		transcript_file =
			Open(transcript_mode ? temp_transcript_name : "PRT:", MODE_NEWFILE);

		if (transcript_file == NULL)				/* did the open work? 	  */
		{
		...
		if (Fprintf((void *)transcript_file,		/* try a write! 		  */
					transcript_header,
		...
		if (transcript_mode == 1)
		{
			...
			if (name[0] != 0)
			{
				...
				if (FileOnly(transcript_name) == transcript_name)
				{
					...
					strcpy(format_string,transcript_name);

				if (!strcmp(temp_transcript_name,format_string))
					name = temp_transcript_name;
				else 
				if (copylogfile(temp_transcript_name,format_string))
					name = format_string;
				else 
					name = temp_transcript_name;
			}
			...
			write_log_file_icon(name);				/* write an icon */

			return name;			/* return where the log file ended up... */
		}
	}
...
void set_gadgets(struct Gadget *gad)
{
	...
	if (tz_activate) 
		ActivateGadget(&tz_activate->Gadget,window,NULL);
}
	...
	while (TRUE)
	{
		...
		while (imsg = (struct IntuiMessage *)GetMsg(window->UserPort))
		{
			...
			case RAWKEY:
				if (code == RAW_HELP)
				{
					if (gad = FindGadget(window,HELP_ID)) 
						return gad;
				}
				if (code == RAW_ESC)
				{
					if (gad = FindGadget(window,ABORT_ID)) 
						return gad;
				}
			...
			case GADGETDOWN:
				...
				if (gad->GadgetID == UP_ID || gad->GadgetID == DN_ID)
				{
					if (ctrl != NULL)
					{
						ModifyIDCMP(window,STD_FLAGS|INTUITICKS);	
as_arrow:				arrow = (gad->GadgetID == UP_ID ? -1 : 1);
						...
						if (i >= 0 && i <= ctrl->Count - ctrl->Visible)
						{
							...
							if (ctrl->Files)
								...
								show_text(FindGadget(window,LIST_ID),ctrl,0);
							set_slider(FindGadget(window,SLIDER_ID),ctrl);
						}
					}
				}
				else 
				if (gad->GadgetID < FIRSTRESV_ID &&
...
BOOL check_escape(void)
{
	...
	while (imsg = (struct IntuiMessage *)GetMsg(window->UserPort))
	{
		...
		if (class == RAWKEY && code == RAW_ESC) 
			return TRUE;

		if (class == GADGETUP && gad->GadgetID == COPYABORT_ID) 
			return TRUE;
	...
	int (*func)(struct InstallationRecord *,char *))
{
	...
	if (MakeBitMap(&sbm,window->WScreen->BitMap.Depth,
		...
		window_height - top_edge - bottom_edge) == 0) 
	{
		return 0;
	}

	if (sgad) 
		remove_gadgets(sgad);					/* remove old gadget list */
...
{	BPTR 			lock1, lock2;
	...
	unless ( (str = AllocMem(MAX_STRING,0L)) ) 
		return FALSE;
	...
	while (TRUE)
	{
		...
		if (gad->GadgetID == PROCEED_ID || gad->GadgetID == ABORT_ID)
		{
			...
			if (gad->GadgetID == ABORT_ID)
			{
				...
				if (!dir_special && !aborterr(FUNC_ASKDIR)) 
					continue;
			}
...
int show_filelist(struct InstallationRecord *ir,char *title)
{
	...
	while (TRUE)
	{
		...
		if (gad->GadgetID == PROCEED_ID || gad->GadgetID == ABORT_ID)
		{
			if (gad->GadgetID == ABORT_ID && !aborterr(FUNC_ASKFILE)) 
				continue;
...
struct Gadget *abortcopy_display(char *src,char *dest)
{
	static struct GadgetDef abortcopy_def2[] = 
	{
		...
		{ "",	GTYPE_TEXT | GPUT_UPPER | GTEXT_CENTER, 				},
		{ "",	GTYPE_TEXT | GPUT_UPPER | GTEXT_CENTER | GTEXT_SKIP, 	},
		{ NULL,	GTYPE_TEXT | GPUT_UPPER | GTEXT_CENTER | GTEXT_PATH 
				| GFLAG_LAST, 											}
	};
	...
	char *ext)
{
	...
	if (!P_PRETEND_MODE)
	{
		r = copy_file  (filename, destdir, newname ? xname : NULL,
						ir->bits & IBITS_NOPOSITION);
		if (ierror) 
			return FALSE;
	}
	else 
	...
	/*	copy_file will return 2 if the file doesn't exist and this is
	 *	an allowed error.  It will return 0 if the error is not allowed,
	 *	and 1 if it really copied the file.
	 */
	 
	if (r == 1) 
		transcript(trans_copy_file,filename,destdir);
...
struct String *copy_page(struct InstallationRecord *ir)
{
	...
	{
		{ &gir_prompt,	
				GTYPE_WRAP   | GPUT_UPPER | GTEXT_CENTER | GFLAG_PTR,			},
		{ (void *)TX_HELP, 
				GTYPE_BUTTON | GPUT_LOWER | GWIDTH_HALF | GFLAG_LOCAL,	HELP_ID },
		{ (void *)TX_CHANGE_DEST, 
				GTYPE_BUTTON |GPUT_LOWER |GWIDTH_2 | GFLAG_LOCAL,	4	 		},
		{ (void *)TX_ABORT,	
				GTYPE_BUTTON | GPUT_LOWER | GWIDTH_2 | GFLAG_LOCAL,	ABORT_ID	},
		{ (void *)TX_PROCEED_COPY,
		...
		{ (void *)TX_COPY_DEST,
				GTYPE_TEXT   | GPUT_LOWER | GTEXT_CENTER
				| GTEXT_BOLD | GFLAG_LOCAL,										},
	...
	if (!strchr(destdir,':'))		/* no colon, must be a relative path */
	{
		...
		if (r == 0)
		{
			err_msg (ehead, ERR_DOS, FUNC_COPYFILES,
					 GetLocalText (MSG_NOEXIST_OBJECT));
			return 0;
		}
	}
	...
	{
		err_msg(ehead, ERR_DOS, FUNC_COPYFILES,
				prob_with_source, STR_VAL(ir->source_val.v));
		MemFree(fib);
		return 0;
	}
	...
	if (what > 0)									/* directory 			*/
	{
		if (ir->choices == NULL)					/* no choices given?	*/
		{
			if (!(ir->flags & IFLAG_ALL) 
			&&  !(ir->pattern_val.type == TYPE_STRING))
			{
				/*	It shouldn't be possible to get here, since we did
				 *	some syntax checking before.  But if we do, it means
				 *	that we didn't have any choices, didn't specify "all",
				 *	and don't have a match pattern.  This is bad.
				 */
			...
			if (!(ir->flags & IFLAG_CONFIRM) 		/* don't want confirm	*/
			&&   (ir->flags & IFLAG_ALL) )			/* want all files		*/
			{
				if (!P_PRETEND_MODE)
				{
					...
					if (r != 0 && ir->flags & IFLAG_INFOS)	/* found some	*/
						create_drawer_icon(destdir);		/* icon for dir	*/
				}
			...
			pattern = ((ir->flags & IFLAG_ALL) 
						? allpat_tx 
						: STR_VAL(ir->pattern_val.v));
		...
		else
		{
			...
			unless (count = ir->numchoices) 			/* find some files?	*/
				goto bye;								/* no, give up		*/
		}
	}
	else												/* just a file	*/
	{
		...
		choices = &source;
		count   = 1;
		if (ir->name_val.type == TYPE_STRING) 
			newname = STR_VAL(ir->name_val.v);
	}
	...
	{												/* no confirmation needed */
		...
		while (count--)
		{
			...
			{
				local_delopts = ir->delopts;
				if (copy_object(STR_VAL(choices[first]), destdir,newname,0) == 0) 
					break;
			...
			transcript (trans_copy_file, STR_VAL(choices[first]), destdir);

			if (ir->flags & IFLAG_INFOS)
			{
				local_delopts = ir->delopts | DELOPT_OKNOSRC;
				if (!copy_extra_file(ir,
									 destdir,
									 STR_VAL(choices[first]),
									 newname,
									 dot_info)) 
				{
					break;
	...
	while (first < count)							/* build confirmation page */
	{
redisplay:
		gad = NULL;
...
input:
		...
		if (r == 4)
		{
			extern BOOL	dir_special;				/* icky global variable */
		...
		while (num--)
		{
			...
			if (agad->Flags & SELECTED)
			{
				...
				{
					local_delopts = ir->delopts;
					if (copy_object(STR_VAL(choices[first]),
									destdir,
									newname,0) == 0)
					{
						num = 1;
...
bye:
	...
	if ((ir->flags & IFLAG_ALL) || ir->pattern_val.type == TYPE_STRING)
		if (choices != &source)				/* wasn't single file source	*/
...
{
	{ &gir_prompt,			GTYPE_WRAP   | GPUT_UPPER | GTEXT_CENTER 
							| GFLAG_PTR,										},
	{ (void *)TX_HELP,		GTYPE_BUTTON | GPUT_LOWER | GWIDTH_HALF  
							| GFLAG_LOCAL,							HELP_ID		},
	{ (void *)TX_PROCEED,	GTYPE_BUTTON | GPUT_LOWER | GWIDTH_2 
							| GFLAG_LOCAL,							PROCEED_ID	},
	{ (void *)TX_ABORT,		GTYPE_BUTTON | GPUT_LOWER | GWIDTH_2 
							| GFLAG_LOCAL,							ABORT_ID	},
	{ NULL,					GTYPE_NONE,									2		},
	{ NULL,					GTYPE_BEVEL | GBEVEL_STD | GFLAG_LAST,		0		}
};
...
{
	/*	This procedure handles the "askchoice" mutual exclusion gadgets.  */
	...
	while (first < ir->numchoices && bits == -1)	/* build confirmation page */
	{
		gad = NULL;

        data.Choices	  = (struct String **)&ir->choices[first];
        data.NumChoices   = ir->numchoices - first;
		data.DefChoice	  = (long)ir->default_val.v - first;
        ...
        while (TRUE)
        {
            ...
            if (r == ABORT_ID && !aborterr(FUNC_ASKCHOICE)) 
                continue;
            break;
        }
        if (r == ABORT_ID)
        	break;
		i = final_radio (gad, num);
        clear_gadgets(gad);
	...
	if (r != ABORT_ID)
	{
		...
		{
			transcript_wrap	(ir->prompt_val.v);
			transcript		(trans_ask_choice, ir->choices[bits] + 1);
		}
	}
...
{
	/*	This procedure handles the "askoptions" multiple selection gadgets.  */
	...
	if (ir->numchoices > 32)
	{
		...
		err_msg(ehead,ERR_SCRIPT,FUNC_ASKCHOICE,"More than 32 choices offered");		
	}		

	while (first < ir->numchoices)				/* build confirmation page */
	{
		gad = NULL;

        data.Choices	  = (struct String **)&ir->choices[first];
        data.NumChoices   = ir->numchoices - first;
		data.DefChoice	  = (ULONG)ir->default_val.v >> first;
        many_def[4].Text  = &data;
        ...
        while (TRUE)
        {
            ...
            if (r == ABORT_ID && !aborterr(FUNC_ASKOPTIONS)) 
                continue;
            break;
        }
        if (r == ABORT_ID)
		...
		bits |= final_checkboxes (gad, num) << first;
        clear_gadgets(gad);
		first += num;
    }
...
char *askstring_page(struct InstallationRecord *ir)
{
	...
	while (TRUE)
	{
		...
		if (r == ABORT_ID && !aborterr(FUNC_ASKSTRING)) 
			continue;
		break;
	}
...
long asknumber_page(struct InstallationRecord *ir)
{
	...
	if (ir->maxval != (LONG)0x7fffffff)
	{
		...
		place_bottom += rp->TxHeight + 1;			/*====== KLUDGE! ==========*/
		layout_text (text, 0, TEXT_CENTER, 0);
	}
	...
	while (TRUE)
	{
		...
		if (r == ABORT_ID && !aborterr(FUNC_ASKNUMBER)) 
			continue;
...
long askdisk_page(struct InstallationRecord *ir)
{
	...
	while (TRUE)
	{
		...
		if (r == ABORT_ID && !aborterr(FUNC_ASKDISK)) 
			continue;
...
long yesno_page(struct InstallationRecord *ir)
{
	...
	while (TRUE)
	{
		...
		if (r == ABORT_ID && !aborterr(FUNC_ASKBOOL)) 
			continue;
		break;
	}
...
int about_page(struct InstallationRecord *ir,char *title)
{
	...
	while (TRUE)
	{
		...
		egad = HandleEvents(NULL);
		if (egad && egad->GadgetID == 1) 
			break;
	}
...
char *select_file(struct InstallationRecord *ir)
{
	...
	if (str == NULL)
	{
		...
		{
			err_msg(ehead,ERR_DATA,FUNC_ASKFILE,invalid_drawer_name, str); 
			return NULL;
		}
	}
...
void message_page(struct InstallationRecord *ir)
{
	static struct GadgetDef msg_def[] = 
	{
		{ &gir_prompt,			GTYPE_WRAP 	| GPUT_UPPER | GTEXT_CENTER 
								| GFLAG_PTR,						0			},
		{ NULL,					GTYPE_BUTTON| GPUT_LOWER | GWIDTH_HALF,
																	HELP_ID		},
		{ (void *)TX_PROCEED,	GTYPE_BUTTON| GPUT_LOWER |GWIDTH_2	
								| GFLAG_LOCAL,						PROCEED_ID	},
		{ (void *)TX_ABORT,		GTYPE_BUTTON| GPUT_LOWER |GWIDTH_2		
								| GFLAG_LOCAL|GFLAG_LAST, 			ABORT_ID 	}
	};
	struct Gadget	*gad = NULL;
	WORD			i;
	...
	while (TRUE)
	{
		...
		if (i == ABORT_ID && !aborterr(FUNC_MESSAGE)) 
			continue;
		break;
	}
...
void final_page(void)
{
	/*	This routine prints the "installation complete" page.  The way
	 *	it does it is rather doctored from the original plan, since the
	 *	original plan didn't at all do what the documentation said it
	 *	did.
	 *
	 *	This now works as follows:
	 *
	 *	If the extra_exit_text field is not null, we bring up a page
	 *	with just this text.  If the extra_exit_text is null, we skip
	 *	this step.
	 *
	 *	We then check the "quiet" flag.  If it is off, we display the
	 *	normal exit page, else we skip it.
	 */
	...
	static struct GadgetDef final_def[] = 
	{
		...
		{ (void *)TX_PROCEED,	GTYPE_BUTTON | GPUT_LOWER | 
							GWIDTH_HALF | GFLAG_LOCAL | GFLAG_LAST, PROCEED_ID }
	};
	struct Gadget	*gad = NULL;
	char			buffer1[256],
					buffer2[256],
	...
	if (extra_exit_text)
	{
		transcript_wrap (extra_exit_text);		/* log user exit text	*/
		
		final_def[0].Text = STR_VAL (extra_exit_text);
		make_gadgets (&gad, final_def, NULL,	/* build exit display	*/
			GERASE_PAGE|GSTART_LAYOUT|GSET_GADGETS|GCLEAR_BUSY);

		do_nothing(NULL,NULL,0);				/* wait for a click		*/
	...
	if (exit_quiet == FALSE)					/* want a noisy exit?	*/
	{
		def_dest = STR_VAL(*defpart_ptr);

		if (def_dest[0] == 0) 
			Sprintf(buffer1,inst_complete1,app_name);
		else 
			Sprintf(buffer1,inst_complete2,app_name,def_dest);
		...
		final_def[0].Text = buffer1;			/* app name complete	*/
		final_def[1].Text = buffer2;			/* log file name		*/
#if 0
		if (extra_exit_text) 					/* user-specified text	*/
			final_def[3].Text = STR_VAL(extra_exit_text);
#endif
		make_gadgets (&gad, final_def, NULL,	/* build exit display	*/
			GERASE_PAGE|GSTART_LAYOUT|GSET_GADGETS|GCLEAR_BUSY);
...
int confirm_page(struct InstallationRecord *ir)
{
	...
	while (TRUE)
	{
		...
		if (r == ABORT_ID && !aborterr(FUNC_CONFIRM)) 
			continue;
		break;
	}
...
int version_page(struct InstallationRecord *ir,long old,long new)
{
	...
	if (old != 0) 
		Sprintf(text2,version_msg2,old >> 16,old & 0x0ffff);
	...
	while (TRUE)
	{
		...
		if (r == ABORT_ID && !aborterr(FUNC_COPYLIB)) 
			continue;
		break;
	}
...
{	
	...
	help_temp = make_user_help(&ir, NULL, HELP_USER_LEVEL);
	...
	if (xir && xir->help_val.v)
		user_def1[0].Text = STR_VAL(xir->help_val.v);
	else
	{
	...
	if (minuser_level > 0) 
		disable_radio(gad1,0,3);
	if (minuser_level > 1) 
		disable_radio(gad1,1,3);
...
int get_options(struct InstallationRecord *xir)
{
	...
	if (no_pretend == TRUE) 
		disable_radio(FindGadget(window,1+1),1,2);
	if (no_print == TRUE) 
		disable_radio(FindGadget(window,3+2),0,3);
...
int report_message(WORD mode,char *text,...)
{
	...
	result = save_page(&ir,mode == 1 ? error_msg : NULL,error_page);
...
int fatal_page(struct InstallationRecord *ir,char *title)
{
	static struct GadgetDef fatal_def[] = {
		...
		{ (void *)TX_PROCEED,	GTYPE_BUTTON | GPUT_LOWER | GWIDTH_HALF 
								   | GFLAG_LOCAL | GFLAG_LAST, PROCEED_ID	}
	};
	struct Gadget	*gad 	= NULL;
	WORD			left 	= left_edge,
					right 	= right_edge,
	...
	while (TRUE)
	{
		...
		egad = HandleEvents(NULL);
		if (egad && egad->GadgetID == PROCEED_ID) 
			break;
	}
...
void set_busy(void)								/* set the busy pointer */
{
	UWORD	*busy;
	long	size;
	...
	if (inited_busy == FALSE)
	{
		...
		size = sizeof Busy2_0;
#else
		busy = (IntuitionBase->LibNode.lib_Version >= 36 
					? Busy2_0 : Busy1_3);
		size = (IntuitionBase->LibNode.lib_Version >= 36 
					? sizeof Busy2_0 : sizeof Busy1_3);
#endif

		if ( TypeOfMem((void *)busy) & MEMF_CHIP ) 
			chip_busy = busy;
		else
		{
	...
	if (IntuitionBase->LibNode.lib_Version >= 36)
		...
		SetPointer (window,(void *)chip_busy,BUSY1_3_HEIGHT,15,-7,-10);
#endif
	is_busy = 1;
	completion(percent_complete);
}

void clear_busy(void)							/* clear the busy pointer */
{
	if (is_busy == 0) 
		return;
