*
* $Id: round.asm,v 0.3 91/07/03 10:26:56 mks Exp $
*
* $Log:	round.asm,v $
* Revision 0.3  91/07/03  10:26:56  mks
* First pass at doing branch optimizations
*
* Revision 0.2  91/07/02  15:55:21  mks
* Did my fix and major optimization to the dnrm_lp routine
* I fixed the fact that a lsl.l #29,d2 was being done along
* with a bunch of useless other code.  I was able to reduce
* this to two instructions, both of which are fast.
*
* Revision 0.1  91/06/26  09:00:57  mks
* Initial check in...
*
*
...
*					;the appropriate g-r-s bits.
	...
	bne.s	rnd_cont		;lower bits to zero for size
...
nrm_zero:
	...
	bmi.b	d0_less
	bsr.s	nrm_set		;exp > 64 so exp won't exceed 0
	rts
d0_less:
...
*				;it is just shifted
	bsr.s	nrm_set		;else exp won't go past 0
...
no_sgn_ext:
	...
	bsr.s	dnrm_lp		;denormalize the number
...
;	clr.l	d0			;guard,round,sticky init.
;	bfextu	WBTEMP_GRS(a6){6:3},d2	;extract guard,round, sticky  bit
;	lsl.l	#29,d2			;shift g,r,s to their postions
;	or.l	d2,d0			;in d0

	bfextu	WBTEMP_GRS(a6){6:3},d0	;extract guard,round, sticky  bit
;
; We could have used an lsl.l #29,d0 but no such thing exists...
;
; So, next is a nasty trick.  bfextu will give me the 3 bits at the bottom
; of d0 but I want them at the top.  (31,30,29)  Since I want only those
; bits and since bfextu leaves ZERO bits elsewhere in d0, we will rotate
; the three lower bits around the back (to the right) of the end of d0
; and back into the top.  This makes it one instruction and removes all
; need for d2 and the save/restore of d2.
;
	ror.l	#3,d0			; This is a nasty trick...
