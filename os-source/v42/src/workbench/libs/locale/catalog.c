                        ...
                        if (!StopChunks(iff,IFFPrefChunks,IFFPrefChunkCnt))
                        {
                            while (TRUE)
                            {
                                ...
                                if (cn->cn_ID == ID_STRS && cn->cn_Type == ID_CTLG)
                                {
                                    ...
                                    {
                                        if (!(catalog->ec_Strings = AllocVecFlush(bufSize,MEMF_ANY,LocaleBase)))
                                            break;
    ...
    for (i=0; i<maxLoop; i++)
    {
        if (lang)
        {
            ...
            else
            {
                ...
                if (!(result = AllocVecFlush(sizeof(struct ExtCatalog)+strlen(name)+1,MEMF_CLEAR,LocaleBase)))
                    break;
                ...
                while (--cnt)
                {
                    if (doProgDir)
                    {
                        ...
                        if (GetProgramDir() && LoadCatalog(result,path,lang,version,LocaleBase))
                            break;
                    }
                    else
                    {
                        ...
                        if (LoadCatalog(result,path,lang,version,LocaleBase))
                            break;
                    }
                }
