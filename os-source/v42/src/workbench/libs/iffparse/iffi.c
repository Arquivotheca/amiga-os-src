...
LONG streamaction(struct IFFHandleP *iffp, LONG cmd, LONG clienterr,
                  char *buf, LONG size, struct IFFParseLib *IFFParseBase)
{
struct IFFStreamCmd sc;
    ...
    if (CallAHook(iffp->iffp_StreamHook, iffp, (LONG *) &sc, IFFParseBase))
        return (clienterr);
...
LONG	error;
	 ...
	 */
	return (streamaction (iffp, IFFSCC_SEEK, IFFERR_SEEK, NULL, nbytes, IFFParseBase));
}
...
VOID ASM InitIFFasDOSLVO(REG(a0) struct IFFHandleP *iffp,
			 REG(a6) struct IFFParseLib *IFFParseBase)
{
static const struct Hook InternalDOSHook =
{
    { NULL },	/*  MinNode	*/
    DOS_Stream,	/*  h_Entry	*/
    NULL,		/*  h_SubEntry	*/
...
LONG			signal1, signal2;
    ...
    if (clip = AllocVec(sizeof(struct ClipboardHandle),MEMF_ANY))
    {
        ...
        if ((signal1 >= 0) && (signal2 >= 0))
        {
            ...
            if (OpenDevice("clipboard.device", unit, (APTR)&clip->cbh_Req,0L) == 0)
                return (clip);
        }
    }
...
struct ClipboardHandle	*clip;
	...
	case IFFSCC_CLEANUP:
		...
		return ((LONG) DoIO ((APTR)&clip->cbh_Req));
	...
	case IFFSCC_SEEK:
	 {
		LONG	curoff = clip->cbh_Req.io_Offset;
...
VOID ASM InitIFFasClipLVO(REG(a0) struct IFFHandleP *iffp,
			  REG(a6) struct IFFParseLib *IFFParseBase)
{
static const struct Hook InternalCBHook =
{
    { NULL },	/*  MinNode	*/
    CB_Stream,	/*  h_Entry	*/
    NULL,	/*  h_SubEntry	*/
