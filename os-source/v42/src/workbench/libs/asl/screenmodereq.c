...
static VOID FreeDisplayModes(struct ASLLib *AslBase, struct ExtSMReq *smr)
{
...
static BOOL GetDisplayModes(struct ASLLib *AslBase, struct ExtSMReq *smr)
{
...
DisplayInfoHandle     dh;
    ...
    if (smr->sm_CustomSMList != NULL)
        while (node = (struct DisplayMode *)RemHead(smr->sm_CustomSMList))
            EnqueueAlpha(AslBase,list,(struct Node *)node);
                        ...
                        if ((dispInfo.PropertyFlags & smr->sm_PropertyMask) == (smr->sm_PropertyFlags & smr->sm_PropertyMask))
                        {
                                ...
                                && (smr->sm_MinDepth  <= dimInfo.MaxDepth))
                            {
                                if ((!smr->sm_FilterFunc) || (CallHookPkt(smr->sm_FilterFunc,PUBLIC_SMR(smr),(APTR)modeID)))
                                {
                                    ...
                                    {
                                        node = AllocNamedNode(AslBase,sizeof(struct DisplayMode),nameInfo.Name);
                                    }
                                    else
                                    {
                                        name[0] = 0;
                                        if (GetDisplayInfoData(NULL,(APTR)&monInfo,sizeof(struct MonitorInfo),DTAG_MNTR,modeID))
                                        {
                                            if ((monInfo.Mspc) && (monInfo.Mspc->ms_Node.xln_Name))
                                            {
                                                ...
                                                StripExtension(AslBase,name,".monitor");
                                                strcat(name,":");
                                                len = strlen(name);
                                                while (len>0)
                                        ...
                                        node = AllocNamedNode(AslBase,sizeof(struct DisplayMode),buffer);
                                    }

                                    if (!node)
                                    {
                                        FreeDisplayModes(AslBase,smr);
                                        return(FALSE);
                                    }
...
static VOID UpdatePropList(struct ASLLib *AslBase, struct ExtSMReq *smr)
{
...
DisplayInfoHandle   dh;
    ...
    {
        GT_SetGadgetAttrs(smr->sm_PropertyListGad,smr->sm_InfoWindow,NULL,
                          GTLV_Labels, ~0,
                          TAG_DONE);
        ...
        decPoint = ".";
        if (smr->sm_ReqInfo.ri_Locale)
            decPoint = smr->sm_ReqInfo.ri_Locale->loc_DecimalPoint;
...
static VOID CloseInfoWindow(struct ASLLib *AslBase, struct ExtSMReq *smr)
{
    if (smr->sm_InfoWindow)
    {
        ...
        AslCloseWindow(AslBase,smr->sm_InfoWindow,TRUE);
...
static VOID OpenInfoWindow(struct ASLLib *AslBase, struct ExtSMReq *smr)
{
struct Gadget    *gad;
struct NewGadget  ng;
struct ReqInfo   *ri;
    ...
    {
        ri = &smr->sm_ReqInfo;
        if (smr->sm_InfoWindow = OpenWindowTags(NULL,
                                               WA_Left,         smr->sm_ReqInfo.ri_Window->LeftEdge + smr->sm_InfoCoords.Left,
                                               ...
                                               TAG_DONE))
        {
            ...
            if (smr->sm_PropertyListGad = CreateGadget(LISTVIEW_KIND,gad,&ng,
                                                       GTLV_ReadOnly,   TRUE,
                                                       LAYOUTA_SPACING, 1,
                                                       TAG_DONE))
            {
                ...
                UpdatePropList(AslBase,smr);
                return;
            }
        }

        CloseInfoWindow(AslBase,smr);
...
static VOID RenderSMDisplay(struct ASLLib *AslBase, struct ExtSMReq *smr)
{
...
static VOID DoSMGadgets(struct ASLLib *AslBase, struct ExtSMReq *smr)
{
struct ReqInfo       *ri;
struct DisplayMode   *node;
struct DimensionInfo *dimInfo;
...
ULONG                 nodeNum;
    ...
    if (!smr->sm_ReqInfo.ri_Template[MODELISTGAD].ag_Gadget)
    {
        ...
        SetGadgetAttr(AslBase,ri,MODELISTGAD,
    ...
    {
        SetGadgetAttr(AslBase,ri,WIDTHGAD,GTIN_Number,   smr->sm_DisplayWidth,
                                  GTIN_MaxChars, 4,
                                  TAG_DONE);
    }
    ...
    {
        SetGadgetAttr(AslBase,ri,HEIGHTGAD,GTIN_Number,   smr->sm_DisplayHeight,
                                   GTIN_MaxChars, 4,
                                   TAG_DONE);
    }
    ...
    if (SMF_DODEPTH & smr->sm_Flags)
    {
        ...
        {
            SetGadgetAttr(AslBase,ri,COLORSGAD,GTSL_Level,   (smr->sm_DisplayDepth / 2) - 3,   /* 6 becomes 0, 8 becomes 1 */
                                       GTSL_Min,     0,
        ...
        {
            SetGadgetAttr(AslBase,ri,COLORSGAD,GTSL_Level,   smr->sm_DisplayDepth,
                                       GTSL_Min,     minDepth,
                                       GTSL_Max,     maxDepth,
                                       GA_RelVerify, TRUE,
                                       GA_Immediate, TRUE,
        ...
        SetGadgetAttr(AslBase,ri,NUMCOLORSGAD,GTIN_MaxChars, MaxDigits(smr,ri->ri_Template[COLORSGAD].ag_Gadget,smr->sm_MaxDepth),
                                      /* Above tag is for special use by layout engine */
                                      TAG_DONE);

        if (ri->ri_Template[COLORSGAD].ag_Gadget)
            RenderSMDisplay(AslBase,smr);
    }

    if (SMF_DOAUTOSCROLL & smr->sm_Flags)
    {
        SetGadgetAttr(AslBase,ri,AUTOSCROLLGAD,GTCB_Checked, smr->sm_AutoScroll,
                                       GTCB_Scaled,  TRUE,
                                       TAG_DONE);
    }
    ...
    {
        SetGadgetAttr(AslBase,ri,OVERSCANTYPEGAD,GTCY_Labels, (ULONG)smr->sm_OverscanLabels,
...
static VOID SetDefSizes(struct ASLLib *AslBase, struct ExtSMReq *smr)
{
struct Rectangle *rect;
...
static BOOL CreateSMGadgets(struct ASLLib *AslBase, struct ExtSMReq *smr)
{
struct ReqInfo *ri = &smr->sm_ReqInfo;
...
static VOID NewMode(struct ASLLib *AslBase, struct ExtSMReq *smr, struct DisplayMode *new,
...
ULONG nodeNum;
    ...
    if ((new != smr->sm_CurrentMode) || force)
    {
        if (shuffle)
        {
            ...
            SetGadgetAttr(AslBase,&smr->sm_ReqInfo,MODELISTGAD,
                          GTLV_Selected,    nodeNum,
                          GTLV_MakeVisible, nodeNum,
                          TAG_DONE);
        }
...
static BOOL HandleSMIDCMP(struct ASLLib *AslBase, struct ExtSMReq *smr)
{
struct ReqInfo       *ri = &smr->sm_ReqInfo;
...
ULONG                 seconds, oldSeconds;
    ...
    while (TRUE)
    {
        intuiMsg = GetReqMsg(AslBase,&smr->sm_ReqInfo,PUBLIC_SMR(smr),smr->sm_InfoWindow,0);
        ...
        switch (class)
        {
            ...
            case IDCMP_MENUPICK     : while (icode != MENUNULL)
                                      {
                                          ...
                                          switch ((UWORD)MENU_USERDATA(menuItem))
                                          {
                                              ...
                                              case SM_INFO    : OpenInfoWindow(AslBase,smr);
                                                                break;

                                              case SM_NEXTMODE: if (smr->sm_CurrentMode != (struct DisplayMode *)smr->sm_DisplayModes.lh_TailPred)
                                                                    NewMode(AslBase,smr,(struct DisplayMode *)smr->sm_CurrentMode->dm_Node.ln_Succ,TRUE,FALSE);
                                                                break;

                                              case SM_PREVMODE: if (smr->sm_CurrentMode != (struct DisplayMode *)smr->sm_DisplayModes.lh_Head)
                                                                    NewMode(AslBase,smr,(struct DisplayMode *)smr->sm_CurrentMode->dm_Node.ln_Pred,TRUE,FALSE);
                                                                break;

                                              case SM_RESTORE : SMRestore(smr);
                                                                NewMode(AslBase,smr,GetCurrentMode(smr,smr->sm_DisplayID),TRUE,TRUE);
                                                                break;
                                          }
            ...
            case IDCMP_GADGETUP     : switch ((UWORD)gadget->GadgetID)
                                      {
                                          ...
                                          case SM_DMLIST  : if ((icode != oldICode) || !DoubleClick(oldSeconds,oldMicros,seconds,micros))
                                                            {
                                                                NewMode(AslBase,smr,(struct DisplayMode *)FindNum((struct List *)&smr->sm_DisplayModes,icode),FALSE,TRUE);
                                                                SetDefSizes(AslBase,smr);
            ...
            case IDCMP_NEWSIZE      : if (!CreateSMGadgets(AslBase,smr))
                                          return(FALSE);
                                      break;
        }
    }
...
static VOID ProcessSMTags(struct ASLLib *AslBase, struct ExtSMReq *smr, struct TagItem *tagList)
{
struct TagItem *tag;
...
ULONG           data;
    ...
    while (tag = NextTagItem(&tags))
    {
        ...
        switch (tag->ti_Tag)
        {
            ...
            default                        : ProcessStdTag(AslBase,&smr->sm_ReqInfo,tag);
                                             break;
        }
    }

    smr->sm_Flags = AslPackBoolTags(AslBase,smr->sm_Flags,tagList,
...
struct ExtSMReq *AllocSMRequest(struct ASLLib *AslBase, struct TagItem *tagList)
{
struct ExtSMReq *smr;
    ...
    if (smr = AllocVec(sizeof(struct ExtSMReq), MEMF_CLEAR | MEMF_ANY))
    {
        ...
        ProcessSMTags(AslBase,smr,tagList);
...
APTR SMRequest(struct ASLLib *AslBase, struct ExtSMReq *smr, struct TagItem *tagList)
{
struct ReqInfo *ri     = &smr->sm_ReqInfo;
...
ULONG           round;
    ...
    {
        ProcessSMTags(AslBase,smr,tagList);
        PrepareLocale(AslBase,&smr->sm_ReqInfo);
        ...
        if (SMF_DOOVERSCAN & smr->sm_Flags)
        {
            ...
            SetGadgetAttr(AslBase,&smr->sm_ReqInfo,OVERSCANTYPEGAD,GTCY_Labels, (ULONG)smr->sm_OverscanLabels,
        ...
        if (GetDisplayModes(AslBase,smr))
        {
            error = ERROR_NO_MORE_ENTRIES;
            if (!IsListEmpty(&smr->sm_DisplayModes))
            {
                error = ERROR_NO_FREE_STORE;
                if (OpenRequester(AslBase,&smr->sm_ReqInfo,AM))
                {
                    for (i = 0; i<12; i++)
                    ...
                    if (CreateSMGadgets(AslBase,smr))
                    {
                        RenderSMDisplay(AslBase,smr);
                        error = 0;

                        if (smr->sm_InfoOpened)
                            OpenInfoWindow(AslBase,smr);

                        if (result = HandleSMIDCMP(AslBase,smr))
                        {
                    ...
                    CloseInfoWindow(AslBase,smr);
                    CloseRequester(AslBase,&smr->sm_ReqInfo);
                }
            }
            FreeDisplayModes(AslBase,smr);
        }
...
VOID FreeSMRequest(struct ASLLib *AslBase, struct ExtSMReq *smr)
{
    FreeVec(smr);
}
