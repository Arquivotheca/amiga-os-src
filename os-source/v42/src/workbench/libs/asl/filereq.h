...
struct TreeEntry
{
    struct Node       te_Link;
    UWORD             te_Pad;
    struct TreeEntry *te_Left;
    struct TreeEntry *te_Right;
};
...
struct ExtFileReq
{
    ...
    /* for custom listview */
    struct TreeEntry *fr_BinaryTree;
...
APTR FileRequest(struct ASLLib *AslBase, struct ExtFileReq *requester, struct TagItem *tagList);
VOID FreeFileRequest(struct ASLLib *AslBase, struct ExtFileReq *requester);
