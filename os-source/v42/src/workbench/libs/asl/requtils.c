...
static VOID SaveCoords(struct ReqInfo *ri)
{
...
static VOID __asm BackFillHook(register __a0 struct Hook *hook,
...
struct ReqInfo  *ri;
    ...
    SetABPenDrMd(&crp,ri->ri_DrawInfo->dri_Pens[BACKGROUNDPEN],0,JAM2);
...
APTR PrepareRequester(struct ASLLib *AslBase,
                      struct ReqInfo *ri, APTR templates, ULONG templateSize,
                      ULONG memorySize)
{
APTR memory;
...
VOID CleanupRequester(struct ASLLib *AslBase, struct ReqInfo *ri, APTR memory)
{
    if (LocaleBase)
    {
        if (!ri->ri_UserLocale)
            CloseLocale(ri->ri_Locale);
...
VOID PrepareLocale(struct ASLLib *AslBase, struct ReqInfo *ri)
{
    if (LocaleBase = (APTR)OpenLibrary("locale.library",38))
    {
        if (ri->ri_UserLocale)
            ri->ri_Locale = ri->ri_UserLocale;
        else
            ri->ri_Locale = OpenLocale(NULL);

        catalog = OpenCatalogA(ri->ri_Locale,"sys/libs.catalog",NULL);
    }
    else
    {
        ri->ri_Locale = NULL;
...
static struct Menu *CreateASLMenus(struct ASLLib *AslBase, struct ReqInfo *ri, struct ASLMenu *am)
{
UWORD           i;
struct NewMenu *nm;
struct Menu    *menus;
    ...
    while (i--)
    {
        ...
        else if (am[i].am_Label == (ULONG)MSG_BARLABEL)
        {
    ...
    {
        if (!(LayoutMenus(menus,ri->ri_VisualInfo,GTMN_NewLookMenus,TRUE,
                                                  TAG_DONE)))
        {
            FreeMenus(menus);
	    menus = NULL;
...
static VOID ClearReqInfo(struct ReqInfo *ri)
{
    ri->ri_Window     = NULL;
    ri->ri_Screen     = NULL;
    ri->ri_Menus      = NULL;
...
VOID SleepRequester(struct ASLLib *AslBase, struct ReqInfo *ri)
{
    InitRequester(&ri->ri_LocalSleepReq);
    ri->ri_LocalASleep = Request(&ri->ri_LocalSleepReq,ri->ri_Window);

    SetWindowPointer(ri->ri_Window,WA_BusyPointer,  TRUE,
...
VOID WakeupRequester(struct ASLLib *AslBase, struct ReqInfo *ri)
{
...
BOOL OpenRequester(struct ASLLib *AslBase, struct ReqInfo *ri, struct ASLMenu *menus)
{
    ...
    if (screen)
    {
        ...
        if (ri->ri_DFont = OpenFont(ri->ri_DTextAttr))
        {
            if (ri->ri_VisualInfo = GetVisualInfoA(screen,NULL))
            {
                ...
                {
                    if (ri->ri_Menus = CreateASLMenus(AslBase,ri,menus))
                    {
			...
			LayoutGadgets(AslBase,ri,LGM_SETMIN);

                        if (ri->ri_Window = OpenWindowTags(NULL,
			    ...
			    CalculatePaneSize(AslBase,ri);
                            ...
                            if (ri->ri_MakeSleep && ri->ri_UserWindow)
                            {
                                ...
                                SetWindowPointer(ri->ri_UserWindow,WA_BusyPointer, TRUE,
                                                                   TAG_DONE);
                            }
			    ...
			    SaveCoords(ri);
                            ...
                            AslCloseWindow(AslBase,ri->ri_Window,ri->ri_SharedIDCMP);
                        }
                        FreeLayoutGadgets(AslBase,ri,FALSE);
...
VOID CloseRequester(struct ASLLib *AslBase, struct ReqInfo *ri)
{
    if (ri->ri_ASleep)
    {
...
struct IntuiMessage *GetReqMsg(struct ASLLib *AslBase,
                               struct ReqInfo *ri, APTR object,
...
struct Window       *parent;
    ...
    while (TRUE)
    {
        ...
        {
            if ((struct IntuiMessage *)CallFunc(AslBase,ri->ri_OldHook,FRF_INTUIFUNC,(ULONG)intuiMsg,(ULONG)object,ri->ri_A4) == intuiMsg)
...
VOID ProcessStdTag(struct ASLLib *AslBase, struct ReqInfo *ri, struct TagItem *tag)
{
ULONG data;
...
ULONG CallFunc(struct ASLLib *AslBase,
               ULONG __stdargs  (*oldHook)(ULONG,ULONG,ULONG),
