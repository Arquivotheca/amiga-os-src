...
static VOID GadgetBox(struct ReqInfo * ri,
                      struct IBox * src, struct IBox * dst,
...
static WORD LabelsWidth (struct ASLLib *AslBase, struct ReqInfo * ri, struct RastPort * rp, ULONG tagt, struct TagItem * tags)
{
struct TagItem *tag;
WORD            result = 0;
STRPTR         *labels;
...
static STRPTR GetGadgetLabel (struct ReqInfo * ri, AppStringsID stringID)
{
    STRPTR label;
...
static VOID SetGadgetSize (struct ASLLib *AslBase, struct ReqInfo * ri, struct RastPort * rp, WORD i,
		     ...
		     WORD * gwidth, WORD * gheight)
{
	...
	case CYCLE_KIND:	/* label : margin : glyph : margin : labels : margin */
	    m = LabelsWidth (AslBase, ri, rp, GTCY_Labels, tgad->ag_CreationTags);
...
VOID AslFreeGadgets (struct ASLLib *AslBase, struct ReqInfo * ri)
{
    UWORD i;
...
VOID FreeLayoutGadgets (struct ASLLib *AslBase, struct ReqInfo * ri, BOOL cleanup)
{
struct Window *wp;
...
BOOL LayoutGadgets (struct ASLLib *AslBase, struct ReqInfo * ri, WORD mode)
{
	     ...
	     i++)
	{
		    ...
		    (tgad[i].ag_GadgetKind == DGROUP_KIND))
		{
			...
			if (tgad[k].ag_LOGroup)
			{
			    ...
			    SetGadgetSize (AslBase, ri, rp, k, mspace, fwidth, fheight, &twidth, &mwidth, &gwidth, &gheight);
		...
		else
		{
			...
			SetGadgetSize (AslBase, ri, rp, gstart, mspace, fwidth, fheight, &twidth, &mwidth, &gwidth, &gheight);
...
*/
    ...
    return ((BOOL)(ri->ri_LastAdded != NULL));
}
...
VOID CalculatePaneSize (struct ASLLib *AslBase, struct ReqInfo * ri)
{
...
VOID SetGadgetAttr (struct ASLLib *AslBase, struct ReqInfo * ri, WORD id, ULONG tags,...)
{
