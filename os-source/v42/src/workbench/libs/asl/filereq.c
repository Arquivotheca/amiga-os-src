...
/* stored in ln_Type field, and used as primary sorting key */
#define FILETYPE   1
#define DIRTYPE    2
#define VOLUMETYPE 3
#define ASSIGNTYPE 4
...
struct FileEntry
{
    struct Node       fe_Link;
    UWORD             fe_Pad;
    struct TreeEntry *fe_Left;
    struct TreeEntry *fe_Right;
...
struct DirEntry
{
    struct Node       de_Link;
    UWORD	      de_Pad;
    struct TreeEntry *de_Left;
    struct TreeEntry *de_Right;
...
struct AssignEntry
{
    struct Node       ae_Link;
    UWORD             ae_Pad;
    struct TreeEntry *ae_Left;
    struct TreeEntry *ae_Right;
};
...
struct VolumeEntry
{
    struct Node       ve_Link;
    UWORD             ve_Pad;
    struct TreeEntry *ve_Left;
    struct TreeEntry *ve_Right;
...
static VOID InitFileList(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
struct Node *node;
    ...
    FreeList(AslBase,(struct List *)&fr->fr_TempFileList);

    fr->fr_BinaryTree   = NULL;
    fr->fr_MaxNameLen   = 10;
...
static VOID InsertListEntry(struct ASLLib *AslBase, struct ExtFileReq *fr, struct Node *newEntry)
{
UWORD               nameLen;
struct FileEntry   *file;
struct VolumeEntry *volume;
STRPTR              name;
struct TreeEntry   *node;
struct TreeEntry   *last;
    ...
    if (node = fr->fr_BinaryTree)
    {
        name = &newEntry->ln_Name[-1];

        while (TRUE)
        {
            last = node;
            if (Stricmp(&node->te_Link.ln_Name[-1],name) < 0)
            {
                node = node->te_Right;
                if (!node)
                {
                    last->te_Right = (struct TreeEntry *)newEntry;
                    Insert(&fr->fr_FileList,newEntry,last);
                    return;
                }
            }
            else
            {
                ...
                if (!node)
                {
                    last->te_Left = (struct TreeEntry *)newEntry;
                    Insert(&fr->fr_FileList,newEntry,last->te_Link.ln_Pred);
                    return;
                }
            }
        }
    ...
    fr->fr_BinaryTree = (struct TreeEntry *)newEntry;
    AddHead(&fr->fr_FileList,newEntry);
}
...
static VOID RemoveListEntry(struct ASLLib *AslBase, struct ExtFileReq *fr, STRPTR name)
{
struct Node *node;

    if (node = FindNameNC(AslBase,&fr->fr_FileList,FilePart(name)))
    {
        Remove(node);
        fr->fr_ItemCnt--;
        fr->fr_DisplayValid = FALSE;
...
static VOID ConvDate(struct ASLLib *AslBase, struct DateStamp *ds, STRPTR string)
{
struct DateTime dt;
...
static VOID RenderEntry(struct ASLLib *AslBase, struct ExtFileReq *fr, struct Node *entry, struct Rectangle *rect)
{
struct RastPort    *rp;
struct FileEntry   *file;
struct DirEntry    *dir;
...
STRPTR              drawer;
    ...
    switch (entry->ln_Type)
    {
        case FILETYPE  : file = (struct FileEntry *)entry;
                         ConvDate(AslBase,&file->fe_Date,date);
        ...
        case DIRTYPE   : dir = (struct DirEntry *)entry;
                         ConvDate(AslBase,&dir->de_Date,date);
...
static VOID CreateVolumeList(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
struct DosList            *dl;
struct VolumeEntry        *volume;
...
struct InfoData __aligned  id;
    ...
    while (dl = NextDosEntry(dl,LDF_VOLUMES|LDF_ASSIGNS|LDF_READ))
    {
        ...
        if (dl->dol_Type == DLT_VOLUME)
        {
            ...
            {
                if (volume = AllocNamedNode2(AslBase,sizeof(struct VolumeEntry),VOLUMETYPE,&name[1]))
                {
        ...
        {
            if (assign = AllocNode(AslBase,sizeof(struct AssignEntry) + (ULONG)name[0] + 2))
            {
                ...
                InsertListEntry(AslBase,fr,(struct Node *)assign);
            }
        }
    }
    ...
    while (volume = (struct VolumeEntry *)RemHead((struct List *)&hold))
        InsertListEntry(AslBase,fr,(struct Node *)volume);
}
...
ULONG GetMaxPen(struct ASLLib *AslBase, struct RastPort *rp)
{
ULONG result;

    GetRPAttrs(rp,RPTAG_MaxPen, &result, TAG_DONE);
...
static VOID RenderLV(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
UWORD             yPos;
UWORD             i,start,end;
LONG              scrCnt,dy;
...
ULONG             oldMaxPen;
    ...
    else if (fr->fr_TopItem != fr->fr_OldTopItem)
    {
        ...
        if (abs(scrCnt)+1 < fr->fr_VisibleItems)
        {
            ...
            oldMaxPen = GetMaxPen(AslBase,fr->fr_ReqInfo.ri_Window->RPort);
            SetMaxPen(fr->fr_ReqInfo.ri_Window->RPort,GetMaxPen(AslBase,&fr->fr_Store->rs_LVRPort));
    ...
    while (start < end)
    {
        ...
        RenderEntry(AslBase,fr,node,&rect);
        node = node->ln_Succ;
...
static VOID DoFRGadgets(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
struct ReqInfo *ri;
    ...
    {
       SetGadgetAttr(AslBase,ri,PATTERNGAD,GTST_MaxChars, sizeof(fr->fr_PatternString),
                                   GTST_String,   fr->fr_PatternString,
                                   TAG_DONE);
    }

    SetGadgetAttr(AslBase,ri,DRAWERGAD,GTST_MaxChars, sizeof(fr->fr_DrawerString),
                               GTST_String,   fr->fr_DrawerString,
                               TAG_DONE);

    if (!(FRF_DRAWERSONLY & fr->fr_Options2))
    {
        SetGadgetAttr(AslBase,ri,FILEGAD,GTST_EditHook, &fr->fr_FileHook,
                                 GTST_MaxChars, sizeof(fr->fr_FileString),
                                 GTST_String,   fr->fr_FileString,
                                 TAG_DONE);
    }

    SetGadgetAttr(AslBase,ri,SCROLLERGAD,GTSC_Visible, fr->fr_VisibleItems,
...
static BOOL CreateFRGadgets(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
...
static VOID RenderFRDisplay(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
struct Gadget *gad;
struct Gadget *dgad;
    ...
    AslDrawBevelBox(AslBase,fr->fr_ReqInfo.ri_Window,
    ...
    {
        AslDrawBevelBox(AslBase,fr->fr_ReqInfo.ri_Window,fr->fr_LightLeft-fr->fr_ReqInfo.ri_Window->BorderLeft,
...
static VOID SetStrValue(struct ASLLib *AslBase, struct ExtFileReq *fr, WORD id, STRPTR text)
{
    SetGadgetAttr(AslBase,&fr->fr_ReqInfo,id,GTST_String, text,
                                             TAG_DONE);
}
...
static VOID ActivateGad(struct ASLLib *AslBase, struct ExtFileReq *fr, UWORD currentGad, UWORD quals)
{
UWORD          next, prev;
struct Gadget *gad;
...
LONG FixedParsePattern(struct ASLLib *AslBase, STRPTR str, APTR buffer, ULONG bufSize)
{
LONG result;
...
static VOID ParsePat(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
    FixedParsePattern(AslBase,fr->fr_Pattern,fr->fr_Store->rs_ParsedPattern,sizeof(fr->fr_Store->rs_ParsedPattern));
}
...
static VOID FRRestore(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
...
static VOID InsertList(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
struct Node *node;

    if (fr->fr_TempItemCnt)
    {
        SetGadgetAttr(AslBase,&fr->fr_ReqInfo,SCROLLERGAD,GTSC_Total,fr->fr_ItemCnt+fr->fr_TempItemCnt,
                                                  TAG_DONE);
	...
	InsertListEntry(AslBase,fr,node);
}
...
static BOOL MakeDrawer(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
BOOL               created = FALSE;
...
static BOOL DeleteFiles(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
BOOL              getDir = FALSE;
STRPTR            buffer;
LONG              len;
struct EasyStruct est;
	...
	AddPart(buffer,fr->fr_FileString,len);
	    ...
	    {
		RemoveListEntry(AslBase,fr,buffer);
                ...
                if (strlen(fr->fr_FileString))
		{
		    fr->fr_FileString[0] = 0;
                    SetStrValue(AslBase,fr,FILEGAD,"");
		    fr->fr_DisplayValid = FALSE;
		    fr->fr_FullRender   = TRUE;
		}
		else
                {
                    ...
                    SetStrValue(AslBase,fr,DRAWERGAD,fr->fr_DrawerString);
                    getDir = TRUE;
                }
	    }
	    else
    ...
    LimitTopItem(fr);
    RenderLV(AslBase,fr);
    SetGadgetAttr(AslBase,&fr->fr_ReqInfo,SCROLLERGAD,GTSC_Total, fr->fr_ItemCnt,
                                              GTSC_Top,   fr->fr_TopItem,
                                              TAG_DONE);
...
static BOOL SelectEntry(struct ASLLib *AslBase, struct ExtFileReq *fr, struct Node *entry, UWORD quals,
                        BOOL clickedOn)
{
struct Node *node;
BOOL         getDir = FALSE;
    ...
    if (entry->ln_Type == FILETYPE)
    {
        ...
        SetStrValue(AslBase,fr,FILEGAD,fr->fr_FileString);
    }
    else if (clickedOn)
    {
...
static VOID FreeMultiArgs(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
    if (fr->fr_ArgList)
    {
...
static BOOL QuitFR(struct ASLLib *AslBase, struct ExtFileReq *fr, BOOL cancelled, BPTR dirLock)
{
struct Node *node;
ULONG        cnt;
	...
	if (!dirLock)
        ...
        if (fr->fr_ArgList = AllocVec(sizeof(struct WBArg)*(cnt+1),MEMF_CLEAR|MEMF_ANY))
        {
            ...
            {
                node = FindNameNC(AslBase,&fr->fr_FileList,fr->fr_FileString);
...
static BOOL UserFilters(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
BOOL result;
    ...
    {
         result = (CallFunc(AslBase,fr->fr_ReqInfo.ri_OldHook,FRF_FILTERFUNC,(ULONG)&fr->fr_Store->rs_AP,(ULONG)PUBLIC_FR(fr),fr->fr_ReqInfo.ri_A4) == 0);
    }
...
static VOID CursorSelect(struct ASLLib *AslBase, struct ExtFileReq *fr, struct Gadget *gadget,
...
BOOL            stringBased;
    ...
    if (!IsListEmpty(&fr->fr_FileList))
    {
        ...
        if (quals & ALTKEYS)
        {
            if (icode == CURSORUP)
            {
                SelectEntry(AslBase,fr,fr->fr_FileList.lh_Head,0,FALSE);
                top = 0;
            }
            else
            {
                SelectEntry(AslBase,fr,fr->fr_FileList.lh_TailPred,0,FALSE);
                if (fr->fr_ItemCnt > fr->fr_VisibleItems)
	    ...
	    while (node->ln_Succ && !node->ln_Selected)
            ...
            if (!node->ln_Succ)
            {
                ...
                if (!(node = FindClosest(AslBase,&fr->fr_FileList,fr->fr_FileString)))
            ...
            if (found)
            {
                SelectEntry(AslBase,fr,node,0,FALSE);
        ...
        fr->fr_TopItem = top;
        RenderLV(AslBase,fr);
        SetGadgetAttr(AslBase,&fr->fr_ReqInfo,SCROLLERGAD,GTSC_Top, top,
                                                  TAG_DONE);
    }
...
static VOID DriveLight(struct ASLLib *AslBase, struct ExtFileReq *fr, UWORD pen)
{
    if (fr->fr_LightHeight >= MINLIGHTHEIGHT)
    {
...
static VOID ActivateMajorGuy(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
    if (FRF_DRAWERSONLY & fr->fr_Options2)
...
static BOOL DoParent(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
UWORD len;
BPTR  lock;
BPTR  parent;
...
static BOOL HandleFRIDCMP(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
...
struct Node           *node;
struct Node           *entry;
BOOLEAN                getDir, getVolumes, volumes;
STRPTR                 ptr;
BPTR                   lock;
BOOLEAN                accept;
BOOLEAN                newsize;
...
BOOLEAN                clearTempPat;
    ...
    FixedParsePattern(AslBase,"#?.(INFO|BACKDROP)",infoPat,sizeof(infoPat));
    ParsePat(AslBase,fr);

    ptr = FilePart(fr->fr_DrawerString);
    if (FixedParsePattern(AslBase,ptr,store->rs_TempPattern,sizeof(store->rs_TempPattern)) > 0)
    {
        *ptr         = 0;
        clearTempPat = FALSE;
        SetStrValue(AslBase,fr,DRAWERGAD,fr->fr_DrawerString);
    ...
    while (TRUE)
    {
        if (getDir)
        {
            ...
            InitFileList(AslBase,fr);

            if (!getVolumes)
            {
                RenderLV(AslBase,fr);
                SetGadgetAttr(AslBase,ri,SCROLLERGAD,
                              GTSC_Visible, fr->fr_VisibleItems,
                              GTSC_Total,   0,
                              GTSC_Top,     0,
                              TAG_DONE);
            }
            DriveLight(AslBase,fr,FILLPEN);
            ...
            if (!getVolumes)
            {
                ...
                {
                    DriveLight(AslBase,fr,BACKGROUNDPEN);
                    if (FRF_DOSAVEMODE & fr->fr_Options)
                    {
                        getVolumes = !(MakeDrawer(AslBase,fr));
                    }
                    else
                    {
                        DisplayBeep(ri->ri_Screen);
        ...
        intuiMsg = GetReqMsg(AslBase,ri,PUBLIC_FR(fr),NULL,mask);
        ...
        if (GetMsg(&store->rs_PacketPort))
        {
            if (packet->sp_Pkt.dp_Res1)
            {
                ...
                BtoC(AslBase,store->rs_AP.ap_Info.fib_FileName,store->rs_AP.ap_Info.fib_FileName);
                ...
                if (accept)
                {
                    ...
                    {
                        if (dir = AllocNamedNode2(AslBase,sizeof(struct DirEntry),DIRTYPE,store->rs_AP.ap_Info.fib_FileName))
                        {
                    ...
                    {
                        if (file = AllocNamedNode2(AslBase,sizeof(struct FileEntry),FILETYPE,store->rs_AP.ap_Info.fib_FileName))
                        {
            ...
            else
            {
                ...
                SetGadgetAttr(AslBase,ri,SCROLLERGAD,GTSC_Total,fr->fr_ItemCnt+fr->fr_TempItemCnt,
                                             TAG_DONE);
        ...
        if (intuiMsg)
        {
            ...
            switch (class)
            {
                ...
                case IDCMP_ACTIVEWINDOW : ActivateMajorGuy(AslBase,fr);
                                          break;

                case IDCMP_CLOSEWINDOW  : return(QuitFR(AslBase,fr,TRUE,lock));
                ...
                case IDCMP_MENUPICK     : while (icode != MENUNULL)
                                          {
                                              ...
                                              {
                                                  case FR_CANCEL  : return(QuitFR(AslBase,fr,TRUE,lock));
                                                  case FR_OK      : return(QuitFR(AslBase,fr,FALSE,lock));

                                                  case FR_RESTORE : FRRestore(AslBase,fr);
                                                                    SetStrValue(AslBase,fr,FILEGAD,fr->fr_FileString);
                                                                    SetStrValue(AslBase,fr,DRAWERGAD,fr->fr_DrawerString);
                                                                    SetStrValue(AslBase,fr,PATTERNGAD,fr->fr_PatternString);
                                                                    getDir = TRUE;
                                                                    break;

                                                  case FR_PARENT  : getDir     = TRUE;
                                                                    getVolumes = DoParent(AslBase,fr);
                                                                    break;
                                                  ...
                                                  case FR_DELETE  : getDir = DeleteFiles(AslBase,fr);
                                                                    break;

                                                  case FR_PREVNAME: CursorSelect(AslBase,fr,NULL,CURSORUP,quals);
								    break;

                                                  case FR_NEXTNAME: CursorSelect(AslBase,fr,NULL,CURSORDOWN,quals);
                                                                    break;
                ...
                case IDCMP_GADGETDOWN   : switch ((UWORD)gadget->GadgetID)
                                          {
                                              ...
                                              case FR_LISTCLICK : window->Flags |= WFLG_RMBTRAP;
                                                                  ...
                                                                  if (icode < fr->fr_VisibleItems)
                                                                  {
                                                                      ...
                                                                      if (entry->ln_Succ)
                                                                      {
                                                                          getDir = SelectEntry(AslBase,fr,entry,quals,TRUE);

                                                                          if ((entry == oldEntry)
                                                                          && (!(FRF_DOSAVEMODE & fr->fr_Options))
                                                                          && (DoubleClick(oldSeconds,oldMicros,seconds,micros)))
                                                                              return(QuitFR(AslBase,fr,FALSE,lock));
                                              ...
                                              case FR_PATTERN   : ActivateGad(AslBase,fr,PATTERNGAD,quals);
                                                                  getDir = TRUE;
                                                                  ParsePat(AslBase,fr);
                                                                  break;
                                              ...
                                              case FR_DRAWER    : ReducePath(fr->fr_DrawerString);
                                                                  ...
                                                                  SetStrValue(AslBase,fr,DRAWERGAD,fr->fr_DrawerString);
                                                                  ActivateGad(AslBase,fr,DRAWERGAD,quals);
                                                                  getDir = TRUE;
                                                                  break;

                                              case FR_FILE      : if ((icode == CURSORUP) || (icode == CURSORDOWN))
                                                                  {
								      CursorSelect(AslBase,fr,gadget,icode,quals);
                                                                  }
                                                                  else
                                                                  {
                                                                      ...
                                                                      {
                                                                          if ((FRF_DOPATTERNS & fr->fr_Options)
                                                                          &&  (FixedParsePattern(AslBase,fr->fr_FileString,store->rs_ParsedPattern,sizeof(store->rs_ParsedPattern)) > 0))
                                                                          {
                                                                          ...
                                                                          {
                                                                              ActivateGad(AslBase,fr,FILEGAD,quals);
                                                                          }
                                                                      ...
                                                                      else
                                                                      {
                                                                          ...
                                                                          si->BufferPos -= (ULONG)ptr - (ULONG)fr->fr_FileString;
                                                                          if (si->BufferPos < 0)
                                                                              si->BufferPos = 0;
                                                                          ...
                                                                          &&  (FixedParsePattern(AslBase,fr->fr_FileString,store->rs_ParsedPattern,sizeof(store->rs_ParsedPattern)) > 0))
                                                                          {
                                                                              SetStrValue(AslBase,fr,PATTERNGAD,fr->fr_FileString);
                                                                              strcpy(fr->fr_PatternString,fr->fr_FileString);
                                                                              SetStrValue(AslBase,fr,FILEGAD,"");
                                                                          }
                                                                          else
                ...
                case IDCMP_NEWSIZE      : if (!CreateFRGadgets(AslBase,fr))
                                              return(QuitFR(AslBase,fr,TRUE,lock));
                                          newsize = TRUE;
                                          InsertList(AslBase,fr);
                                          break;
                                          ...
                                          if (fr->fr_ZoomOnFileSig >= 0)
                                          {
                                              if (SetSignal(0,1L << fr->fr_ZoomOnFileSig) & (1L << fr->fr_ZoomOnFileSig))
                                              {
                                                  ...
                                                  if (!IsListEmpty(&fr->fr_FileList))
                                                  {
                                                      ...
                                                      SetGadgetAttr(AslBase,ri,SCROLLERGAD,GTSC_Top, fr->fr_TopItem,
                                                                                   TAG_DONE);
                                                      RenderLV(AslBase,fr);
                                                  }
                                              }
                ...
                case IDCMP_RAWKEY       : if ((icode == CURSORUP) || (icode == CURSORDOWN))
                			      CursorSelect(AslBase,fr,NULL,icode,quals);
...
static VOID ProcessFRTags(struct ASLLib *AslBase, struct ExtFileReq *fr, struct TagItem *tagList)
{
struct TagItem *tag;
...
ULONG           data;
    ...
    while (tag = NextTagItem(&tags))
    {
        ...
        switch(tag->ti_Tag)
        {
            ...
            default                    : ProcessStdTag(AslBase,&fr->fr_ReqInfo,tag);
                                         break;
        }
    }

    fr->fr_Options = AslPackBoolTags(AslBase,fr->fr_Options,tagList,
                                     ASLFR_DoPatterns,    FRF_DOPATTERNS,
                                     ASLFR_DoMultiSelect, FRF_DOMULTISELECT,
                                     ASLFR_DoSaveMode,    FRF_DOSAVEMODE,
                                     TAG_DONE);

    fr->fr_Options2 = AslPackBoolTags(AslBase,fr->fr_Options2,tagList,
...
struct ExtFileReq *AllocFileRequest(struct ASLLib *AslBase, struct TagItem *tagList)
{
struct ExtFileReq *fr;
	...
	NewList((struct List *)&fr->fr_TempFileList);
        ...
        ProcessFRTags(AslBase,fr,tagList);

        fr->fr_Coords = fr->fr_ReqInfo.ri_Coords;

	fr->fr_FileHook.h_Entry = (HOOKFUNC)EditHook;
...
APTR FileRequest(struct ASLLib *AslBase, struct ExtFileReq *fr, struct TagItem *tagList)
{
struct ReqInfo *ri     = &fr->fr_ReqInfo;
...
struct FRStore *store;
    ...
    FreeMultiArgs(AslBase,fr);
    if (store = fr->fr_Store = PrepareRequester(AslBase,ri,AG,sizeof(AG),sizeof(struct FRStore)))
    {
        ProcessFRTags(AslBase,fr,tagList);
        PrepareLocale(AslBase,&fr->fr_ReqInfo);
        ...
        if (OpenRequester(AslBase,ri,AM))
        {
            ...
            if (CreateFRGadgets(AslBase,fr))
            {
                ...
                result = HandleFRIDCMP(AslBase,fr);
            ...
            CloseRequester(AslBase,ri);
        }

        fr->fr_Coords = ri->ri_Coords;

        if (!result)
            FRRestore(AslBase,fr);

        CleanupRequester(AslBase,ri,store);
    }
...
VOID FreeFileRequest(struct ASLLib *AslBase, struct ExtFileReq *fr)
{
    FreeMultiArgs(AslBase,fr);
    FreeVec(fr);
}
