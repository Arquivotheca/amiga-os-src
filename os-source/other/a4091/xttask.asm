...
XTSelect	move.b	XTPORT0(a4),d0		all $ff means nothing connected
	...
	printf <'Select: RunningUnit $%lx'>,a2
		movea.l	a2,a0	fetch unit pointer
...
XTBadCmd:
	...
	ENDC
		...
		movea.l	a2,a0	get unit pointer back
...
XT_READ:
XT_WRITE:
XT_SEEK		movea.l	a2,a1	fetch the active unit
		move.b	#TOTAL_SECTORS,SASR(a4)	fill in geometry info
		move.b	hu_TotalBlocks+1(a1),SCMD(a4)  sectors per track
		move.b	hu_TotalHeads(a1),SCMD(a4)   tracks per cylinder
		move.b	hu_TotalCyls+2(a1),SCMD(a4)    total cylinders
		move.b	hu_TotalCyls+3(a1),SCMD(a4)
...
;==============================================================================
XTAddressBad	movea.l	a2,a0	fetch the unit pointer
...
;==============================================================================
XT_RQS		movea.l	a2,a1	fetch the sense data from unit
...
; I'm just going to wait for a data request now and send it immediately
		clr.l	-(sp)
		clr.l	-(sp)
		movea.l	a2,a0	need unit parameters
		move.w	hu_TotalCyls+2(a0),(sp)	number of cylinders
...
; we've issued the command, now read back 4 bytes of sense info from the drive
		movea.l	a2,a0	refetch the unit pointer
