...
*	Source Control
*	--------------
*	$Header: format.asm,v 34.13 88/04/06 13:05:25 bart Exp $
*
*	$Locker:  $
*
*	$Log:	format.asm,v $
*   Revision 34.13  88/04/06  13:05:25  bart
*   format performs badtrans in HDTrkRead and HDTrkWrite
*   
*   Revision 34.12  88/04/04  20:37:22  bart
*   pop d2 off stack if hdsectorize error
*   
*   Revision 34.11  88/02/18  12:45:25  bart
*   format must make iorequest current -- bart
*   
*   Revision 34.10  88/01/21  18:04:58  bart
*   compatible with disk based / binddriver useage
*   
*   Revision 34.9  87/12/04  19:14:00  bart
*   checkpoint
*   
*   Revision 34.8  87/12/04  12:08:30  bart
*   checkpoint before adding check for existing dosname on eb_mountlist
*   
*   Revision 34.7  87/10/14  14:55:33  bart
*   10-13 rev 1
*   
*   Revision 34.6  87/10/14  14:15:59  bart
*   beginning update to cbm-source.10.13.87
*   
*   Revision 34.5  87/07/08  14:00:59  bart
*   y
*   
*   Revision 34.4  87/06/11  15:48:03  bart
*   working autoboot 06.11.87 bart
*   
*   Revision 34.3  87/06/03  10:59:04  bart
*   checkpoint
*   
*   Revision 34.2  87/05/31  16:35:49  bart
*   chickpoint
*   
*   Revision 34.1  87/05/29  19:39:26  bart
*   checkpoint
*   
*   Revision 34.0  87/05/29  17:39:44  bart
...
*       must make iorequest current -- bart         
		MOVE.L  A4,HD_IO_REQUEST(A6)
...
*	Get number of reserved cylinders (= # of first cyl. in 1st partition)
		MOVE.L	HDB_ENVIRONMENT+DE_LOWCYL*4(A5),D1
...
NotToBig:	AND.L	#$0000FFFF,D0	; D0 = sectors
		...
		CLR.L	D1
		MOVE.B	HDU_HEADS(A3),D1
		MULU	D3,D1		; D1 = number sectors / cylinder
...
HDTrkComon:	;the rest of this routine is identical for reads and writes!
		...
		MOVE.L	D0,D1				; has the first sector in this track been
		BSR		BadTrans			; previously found as bad?
		CMP.L	D0,D1				; if remapped
		BNE		WrtSects			; try out each sector in track
...
WrtSectLp1:	
		...
		MOVE.L	D3,D1
		BSR		BadTrans			; previously found as bad?
		CMP.L	D3,D1				; if remapped
		BNE		WrtSectELp1			; just ignore
		MOVE.L	HD_CMDPTR(A6),A1	; Point to device command block
		MOVE.W	D1,CMD_MIDADDR(A1)	; Set low bytes block #
		SWAP	D1			; Get hi byte
		OR.B	HDU_UNITNUM(A3),D1	; Add in Unit number
		MOVE.B	D1,CMD_LUNHIADDR(A1)
		SWAP	D1			; Restore D1
