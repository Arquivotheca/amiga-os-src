...
GetBadLp1:	CMP.B	#$FF,BAO_HEAD(A1) ; If end of table,
		BEQ	WrtBoot		;	exit loop
                ...
                IFGE    INFO_LEVEL-60
                INFOMSG 60,<'%s/Format is adding Bad Block\n'>
                ENDC
...
AddBadBlock:				; Add bad block to unit's table
		...
		BEQ.S	IsBad		; If already in table, exit
                ...
                IFGE    INFO_LEVEL-60
		MOVE.L D0,-(SP)
                INFOMSG 60,<'%s/AddBadBlock: Block %ld not in table'>
		ADDQ.L #4,SP
                ENDC
...
dummy1:
                IFGE    INFO_LEVEL-60
                MOVEM.L  D0/D2,-(SP)
                INFOMSG 60,<'%s/Bad Block is %ld mapped to %ld'>
                ADDQ.L  #8,SP
		RTS
                ENDC
