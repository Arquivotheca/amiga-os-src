...
; use the normal scsi-like setup.
	...
	printf <'initing SCSITask as subroutine...\n'>
		...
		move.l	iot_SysLib(a5),-(sp)	    and exec base pointer
...
NotifyChange	movea.l	cb_IORequest(a0),a1
		...
		bne.s	NotifyDone		nope, so no Cause
...
ec0c		printf <'spindle failure\n'>
		...
		moveq	#0,d0		; no error
		bra.s	ErrorQuit	; returns d0 as error code
1$:		bra.s	ErrorRetry
...
;	printf <'send cb=%lx...'>,a0
	...
	IFD IS_IDE
		...
		bsr	HandleCommand		message should be replied now
