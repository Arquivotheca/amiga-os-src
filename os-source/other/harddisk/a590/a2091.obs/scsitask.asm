...
FlushDMA	tst.b	st_DMAGoing(a5)		was DMA started
	...
	IFD DMA_FLUSH_OK
		...
		bclr.b	#DMAB_TCE,d0		turn off terminal count
		move.b	d0,CNTR(a4)
...
NoFlushNeeded	move.w	#0,SRST(a4)		come to a full stop
		rts
...
; must use terminal count if the DMA doesn't support the Flush trickery
	...
	move.b	#DMAF_TCE!DMAF_PMODE!DMAF_INTENA!DMAF_DDIR!DMAF_XTEOP,CNTR(a4)
	ENDC DMA_FLUSH_OK
		clr.b	cb_DidReadDMA(a0)	we don't flush 030 data cache
...
; must use terminal count if the DMA doesn't support the Flush trickery
	IFND DMA_FLUSH_OK
		move.b	#DMAF_TCE!DMAF_PMODE!DMAF_INTENA!DMAF_XTEOP,CNTR(a4)
	ENDC !DMA_FLUSH_OK
		move.b	#1,cb_DidReadDMA(a0)	So we flush 030 data cache
...
;New addition for the 68030.  If we just did a read operation and the system
;has a 68030 installed, we flush the data cache because we may have invalidated
;it with the DMA data (to the same place).  This is pretty inefficient, but
;what else can you do when the 030 doesn't have bus snooping ?
30$		tst.b	cb_DidReadDMA(a1)	did we do a read ?
		beq.s	NoDataFlush		nope, skip the tests
		btst.b	#2,AttnFlags+1(a6)	test for an 030
		beq.s	NoDataFlush		nope, so no test
	printf <'SCSI: Flushing 030 data cache\n'>
		lea.l	FlushCode(pc),a0	must execute this code
		exg	a0,a5			code to a5, globals to a1
		jsr	_LVOSupervisor(a6)	preserves a1
		exg	a0,a5			restore globals
		clr.b	cb_DidReadDMA(a1)	and fix read flag

NoDataFlush	printf <'SCSI: replying command block\n\n'>
...
;==============================================================================
; This code is called in supervisor mode to flush the 68030's data cache.
;==============================================================================
FlushCode	dc.w	$4e7a,$0002		;movec.l CACR,d0
		ori.w	#$800,d0		;flush data cache (inst is OK)
