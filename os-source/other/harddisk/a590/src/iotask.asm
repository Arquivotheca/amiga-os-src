...
IOEventLoop	move.l	iot_CmdRetMask(a5),d0	wait for returning commands
		...
		jsr	_LVOWait(a6)
		and.l	iot_CmdRetMask(a5),d0	did we get returning cmd ?
...
;	printf <'CommandBlock at 0x%lx returned\n'>,d0
		movea.l	d0,a0
...
2$		move.l	IO_DATA(a1),cb_Data(a0)	copy relevant IORequest fields
	...
	ENDC
		...
		bra	RWLeft			start the read/write op
...
; if there was a check condition and SCSIF_AUTOSENSE is 1, do request sense now
		...
		beq	DirectComplete		don't need to do request sense
...
ErrHandlers	DC.W	ec00-ErrHandlers	no sense information
		...
		DC.W	0	ec0a-ErrHandlers
		DC.W	0	ec0b-ErrHandlers
		DC.W	0	ec0c-ErrHandlers
		DC.W	0	ec0d-ErrHandlers
		DC.W	0	ec0e-ErrHandlers
...
;	printf <'Sending command block at 0x%lx\n'>,a0
...
;==============================================================================
CmdComplete	move.l	a0,-(sp)		save command block ptr
		movea.l	cb_IORequest(a0),a1
