...
NoXTFix		move.l	#2048,d0		enough for 256 mapped blocks
		...
		beq	RDBExit			didn't get the memory
...
ReadBadBlocks	move.l	d2,d0			we want this block
		move.l	#IDNAME_BADLIST,d1	its one of these
		...
		subi.w	#bbb_BadData,d0
		lsr.w	#3,d0			d0 holds count
		lea.l	bbb_BadData(a3),a0	a0 points to first entry
		bra.s	NextBad
...
; we're ready to rock 'n roll now.  Add the node to DOS's list or eb_MountList
		...
		beq	NormalAdd		nope, just mount it
...
MUGetFlags	move.l	hu_RDB(a2),d0		is there a RigidDiskBlock ?
...
MUGotFlags	tst.b	hu_Type(a2)		is this an XT drive
...
20$		move.l	d2,d0			we want this DOSType
		movea.l	a2,a0			from this unit
		bsr	LoadFS			go load the filing system
		tst.l	d0			did we get an fse struct
		beq	FSPatched		nope, so no patch done
...
LoadFS		movem.l	d2/a2-a4/a6,-(sp)
		...
		bmi	NoFSLoaded		no file systems here
...
FoundFS		moveq.l	#fse_SIZEOF,d0		get the structure
		bsr	GetPubMem
		tst.l	d0
		beq	NoFSLoaded		failed the allocation
		movea.l	d0,a4
