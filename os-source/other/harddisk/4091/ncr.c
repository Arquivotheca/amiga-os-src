...
// Quick ints are NOT safe on 4091's - a PAL is wrong.
...
{
//kprintf("%ldus\n",micros);
	WaitAWhile(0,micros,*((struct ExecBase **) 4));
}
...
//kprintf("rejecting msg $%lx, error %ld\n",msg,msg->cb_ErrorCode);
			ReplyMsg(msg);
			continue;
		}
...
//kprintf("queued msg $%lx (ior $%lx) for unit $%lx(%ld:%ld)\n",
//msg,msg->cb_IORequest,unit,unit->hu_Unit,unit->hu_LUN);
...
//kprintf("scsi_Length = $%lx, io_Length = $%lx, io_Offset = $%lx\n",
//scsicmd->scsi_Length,msg->cb_IORequest->io_Length,msg->cb_IORequest->io_Offset);
	...
	// CAREFUL! memory below the chipmem memheader doesn't show as
	// MEMF_CHIP!
	if (!msg->cb_Programmed &&
	    !(((ULONG) scsicmd->scsi_Data) & 0xff000000) &&
// FIX!!!! REMOVE!!!! TEMP for kludge a4000t!
#ifdef IS_A4000T_KLUDGE
	    1)
#else
	    !(TypeOfMem(scsicmd->scsi_Data)&MEMF_CHIP) &&
	    ((ULONG) scsicmd->scsi_Data) > 0x1000)
#endif
	{
//kprintf("DMA from Z-3 to Z-2 space ($%lx size $%lx)\n",scsicmd->scsi_Data,
//scsicmd->scsi_Length);
...
//kprintf("CachePreDMA: $%lx, $%lx\n",data,msg->cb_CurrentLength);
		...
		// remember the original length/pointer
		msg->cb_CacheLength = msg->cb_CurrentLength;
		msg->cb_CacheData   = data;
...
//kprintf("scheduled...");
	...
	// If running is false, we were called from Unschedule().  We know
	// the chip isn't running since we won't actually restart the chip
	// until we've handled all the ints.  We know the ncr chip is going
	// to end up going to the scheduler, so there's no need to wake it up.
	if (running)
	{
//		Disable();
		...
		// MUST check ISTATF_CON.  The reason is that the reselect
		// during select code checks the con bit, and if set it knows
		// that is was either selected or reselected.  It then waits
		// for reselect, knowing that if it fails (and it goes to
		// alt_sig_p3) it MUST have been selected.  If we were to set
		// the sigp bit while it was in this area, it might break the
		// wait reselect, theoretically (maybe not possible in practice
		// or the chip may guarantee it is safe, but better to be safe.
...
static void
UnSchedule (struct SCSIGlobals *g, struct CommandBlock *msg, LONG success)
{
...
//msg->cb_SCSICmd->scsi_Length);
	...
	// 'success' is for paranoia about where Unschedule may be called from
	if (msg->cb_Programmed && success)
	{
	    if ((msg->cb_SCSICmd->scsi_Flags &
		 (1 << SCSIB_READ_WRITE)))
	    {
		// read
		// careful - io_complete already bumped actual
		// move_data.len will be 0 if all the data xferred.
//kprintf("Z2 io complete, copying $%lx bytes to $%lx\n",
//msg->cb_CurrentLength,
//((UBYTE *)
// msg->cb_SCSICmd->scsi_Data) +
...
init_structs (struct SCSIGlobals *g, struct IOTask_Globals *IOGlobals)
{
	...
	script = AllocMem(sizeof(SCRIPT) + 4,0);
	// FIX! errors

	g->st_Script = script;
	CopyMem(SCRIPT,script,sizeof(SCRIPT));

	// the jump instruction must be in fast ram!
	jump = &(script[sizeof(SCRIPT)/sizeof(script[0])]);
	*jump = jump_rel_inst;
...
//kprintf("Script is at $%lx\n",script);
	...
	for (i = 0; i < CMD_BLKS; i++)
	{
		...
		g->st_Scheduler.array[i].make_jump.source = ((ULONG) jump) + 1;
...
#define SBCL_DEFAULT	(3000/NCR_CLOCK_FREQ)	// clock/3
#define DCNTL_VAL	(DCNTLF_CF1 | DCNTLF_CF0)
#else
#if (NCR_CLOCK_FREQ > 37)	// actually 37.5
#define SBCL_DEFAULT	(2000/NCR_CLOCK_FREQ)	// clock/2
#define DCNTL_VAL	(0)
#else
#if (NCR_CLOCK_FREQ > 25)	// actually 25.01
#define SBCL_DEFAULT	(1500/NCR_CLOCK_FREQ)	// clock/1.5
#define DCNTL_VAL	(DCNTLF_CF0)
#else
#define SBCL_DEFAULT	(1000/NCR_CLOCK_FREQ)	// clock/1
#define DCNTL_VAL	(DCNTLF_CF1)
	...
	if (!g->st_UseFast)
	{
		...
		g->st_SBCL = 2000/NCR_CLOCK_FREQ;
	} else {
// FIX! change dynamically
		// default to SCLK/1 for fast sync
		b->sbcl = SBCLF_SSCF0;
		g->st_SBCL = 1000/NCR_CLOCK_FREQ;
	}
...
// set sxfer values for an affected unit.

void
set_sxfer (struct SCSIGlobals *g,
	   struct HDUnit *unit,
	   struct CommandBlock *msg,
	   UBYTE  sxfer)
{
	...
	((struct Find_addr_entry *)
	 LABEL(find_addr_table))[unit->hu_Unit].set_sxfer.imm = sxfer;

	g->st_BoardAddress->sxfer = sxfer;
	...
	for (i = 0; i < CMD_BLKS; i++)
	{
	  // if active and same ID (hu_Unit is SCSI address)
	  if ((g->st_CmdBlocks[i].iocb_CmdBlock.cb_StateSave == STATE_ACTIVE)&&
	      (((struct HDUnit *)
	        (g->st_CmdBlocks[i].iocb_CmdBlock.cb_IORequest->io_Unit))->
		hu_Unit == unit->hu_Unit))
	  {
//kprintf("updating cmd $%lx for sync\n",&(g->st_CmdBlocks[i].iocb_CmdBlock));
	    // same SCSI ID - update sxfer vals
	    // set up select value
	    g->st_CmdBlocks[i].iocb_CmdBlock.cb_DSA.select_data.sync = sxfer;
	  }
	}
}
...
//kprintf("-%ld",b->dsps);
		    ...
		    case io_complete:
			// make chip goto scheduler - won't happen until
			// we're done handling ints.
...
// FIX? problems with NEC cdrom drive - status is 1, analyzer says 2...
//CacheClearE(0,0x7fffffff,CACRF_ClearD);
//kprintf("status %ld (%ld)\n",msg->cb_DSA.status_buf[0],msg->cb_SCSICmd->scsi_Status);
...
//kprintf("scatter: scsi_Data = $%lx, scsi_Actual = $%lx, currentlength = $%lx\n",
...
// FIX! doesn't handle broken-up DMA to the Z2 buffer!!!
// FIX? do we need to see if we did CachePreDMA??
			    CachePostDMA(msg->cb_CacheData,
					 &(msg->cb_CacheLength),
					 msg->cb_DidCacheStuff & ~DMA_Continue);
...
//kprintf("Z2: copying $%lx bytes from $%lx\n",size,original_pos+original_len);
				CopyMem(original_pos + original_len,
					g->st_Z2Buffer,
					size);
			    } else {
				// read
//kprintf("Z2: copying $%lx bytes to $%lx\n",original_len,original_pos);
				CopyMem(g->st_Z2Buffer,
					original_pos,
					original_len);
			    }
			    ...
			    // reset cache pointers for next predma
...
// I've factored out the freq in g->st_SBCL, so we have:
// xferp = (TP)/(2000/Freq) - 4
// this allows me to be certain it fits in a signed uword to save a divide routine
				// make sure it rounds up
				xferp = ((WORD) (g->st_SyncBuf[0]*4 +
					 g->st_SBCL-1)) /
					((WORD) (g->st_SBCL));
...
//kprintf("replying with sync message\n");
...
//kprintf("NOPing outgoing message\n");
				// we initiated.  all is good.
				// Since we've sent the identify and sync,
				// reset the outgoing message buffer.
				// Not 100% needed, but some stupid drives like
				// the Matsushita 128 MO drive go Msg Out when
				// they shouldn't.
				msg->cb_DSA.send_msg.len = 1;
				msg->cb_DSA.send_buf[0]  = NOP; // do nothing
...
//kprintf("target rejected sync message\n");
				...
				// undo sync negotiation!!!!!!
				set_sxfer(g,unit,msg,0);
			} else {
				// what the hell did he reject? identify?
				// ignore...  I guess...
				// we don't do tags unless he accepts them...
...
//#define SCRIPT_DEBUGGING_ON
#ifdef SCRIPT_DEBUGGING_ON
...
//kprintf("status $%08lx, istat $%lx, sfbr $%lx\n",status,istat,b->sfbr);
...
//kprintf("Illegal instruction: at $%lx ($%08lx ->$%08lx $%08lx %08lx)\n",b->dsp,
...
//b->dsp,len,addr);
//kprintf("status = $%08lx, istat = $%lx, msg = $%lx\n",status,istat,msg);
//kprintf("move size = $%lx, move addr = $%lx\n",msg->cb_DSA.move_data.len,
//msg->cb_DSA.move_data.addr);
//kprintf("cmd size = $%lx, cmd addr = $%lx\n",msg->cb_DSA.command_data.len,
//msg->cb_DSA.command_data.addr);
				abort_all(g,HFERR_Phase);
				return;
			} else {
//kprintf("ctest8 = $%lx\n",b->ctest8);
...
kprintf("$%lx bytes in fifo, len = $%lx, addr = $%lx\n",offset,len,addr);
kprintf("---D: $%lx of $%lx addr=$%lx\n",
msg->cb_SCSICmd->scsi_Actual,msg->cb_SCSICmd->scsi_Actual+len,addr);
kprintf("status (sstat2|sstat1|sstat0|dstat) = $%08lx, istat = $%lx\n",status,istat);
kprintf("dsp = $%08lx, scratch = $%08lx, dsps = $%08lx, dnad = $%08lx\n",
b->dsp,b->scratch,b->dsps,b->dnad);
kprintf("dbc = $%06lx, dcmd = $%02lx\n",b->dbc & 0x00ffffff,b->dnad);
kprintf("scntl0 = $%02lx, scntl1 = $%02lx, sdid = $%02lx, sien = $%02lx\n",
b->scntl0,b->scntl1,b->sdid,b->sien);
kprintf("scid = $%02lx, sxfer = $%02lx, sodl = $%02lx, socl = $%02lx\n",
b->scid,b->sxfer,b->sodl,b->socl);
kprintf("sfbr = $%02lx, sidl = $%02lx, sbdl = $%02lx, sbcl = $%02lx\n",
b->sfbr,b->sidl,b->sbdl,b->sbcl);
kprintf("ctest0 = $%02lx, ctest1 = $%02lx, ctest2/ctest3 not read\n",
b->ctest0,b->ctest1);
kprintf("ctest4 = $%02lx, ctest5 = $%02lx, ctest6 (not read), ctest7 = $%02lx\n",
b->ctest4,b->ctest5,b->ctest7);
kprintf("temp = $%08lx, dfifo = $%02lx, ctest8 = $%02lx\n",
b->temp,b->dfifo,b->ctest8);
kprintf("lcrc = $%02lx, dmode = $%02lx, dien = $%02lx, dwt = $%02lx\n",
b->lcrc,b->dmode,b->dien,b->dwt);
kprintf("dcntl = $%02lx, adder = $%08lx\n\n",b->dcntl,b->adder);
#endif
			    // reset the internal buffers/fifos...
			    // then wait for it to finish (resets when done)
			    if (offset)
#ifdef WRONG
// WRONG!!!! That should be if (!(dstat & DSTATF_DFE) || ....) !!!!
// Also, ORF and ILF are backwards (see above).  Ignore, use offset, which tells us
// if we need a flush.
// FIX! make simpler
			    if ((dstat & DSTATF_DFE) ||
			    ...
			    // force mods to move data to memory...
			    CacheClearE(&(msg->cb_DSA.move_data),
				        sizeof(msg->cb_DSA.move_data),
					CACRF_ClearD);
...
//kprintf("dsp = $%lx, status = $%08lx, istat = $%lx\n",b->dsp,status,istat);
//if (!msg)
...
#ifdef SINGLE_STEP
if (restart!=DO_RESTART && b->dsp != new_dsp)
 kprintf("IMPOSSIBLE! $%lx != $%lx\n",b->dsp,new_dsp);
...
//kprintf("turned on quick int %ld\n",g->st_QuickIntNum);
	...
	if (BattMemBase)
	{
		...
		ReadBattMem(&(g->st_UseFast),BATTMEM_SCSI_FAST_SYNC_ADDR,
			    BATTMEM_SCSI_FAST_SYNC_LEN);
		// UseFast defaults to TRUE, invert sense
		g->st_UseFast = !g->st_UseFast;

		ReadBattMem(&(g->st_UseTags),BATTMEM_SCSI_TAG_QUEUES_ADDR,
			    BATTMEM_SCSI_TAG_QUEUES_LEN);
	}
#endif

#ifdef USE_JUMPERS
	// read jumpers - override battmem settings
	{
#ifdef IS_A3090
		UBYTE jumpers = ((UBYTE *) boardaddr)[A3090_JUMPERS];
#endif
#ifdef IS_A4000T
		UBYTE jumpers = ((ULONG *) boardaddr)[A3090_JUMPERS/
						      sizeof(ULONG)];
//kprintf("jumper reg = $%08lx, addr = $%lx\n",
//((ULONG *) boardaddr)[A3090_JUMPERS/sizeof(ULONG)],
//&(((ULONG *) boardaddr)[A3090_JUMPERS/sizeof(ULONG)]));
#endif
...
#define J_SLOW	0x10
#define J_SYNC	0x20
//#define J_TAGS 0x40	termination, can't be used
#define J_LUNS	0x80
