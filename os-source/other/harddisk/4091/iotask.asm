...
; can read the appropriate IO_UNIT addresses as needed.
		...
		move.l	d0,a1
		clr.l	IO_OFFSET(a1)		not absolutely needed
...
;	printf <'new io = $%lx, command %ld, cb=$%lx, offset $%lx'>,a1,IO_COMMAND(a1),a0,IO_OFFSET(a1)
...
;
; Checks that the unit in the IORequest has removeable media and adds it to
; the list (array) of polled drives if it does (and isn't already there).  If
; the first timer request hasn't been sent, then it is sent at this time too.
;
...
;	printf <'sending TUR to $%lx'>,d0
		...
		clr.l	scsi_Data(a1)		make sure pointer isn't random
		clr.b	scsi_Flags(a1)
...
; IFD DEBUG_CODE
;		move.l	#100000,IOTV_TIME+TV_MICRO(a1)
...
GotCode:	printf <'sense key = %ld, code = $%lx'>,d0,d1
		cmpi.w	#$4f,d1			is it in range ?
		bgt.s	BadCode
		lsl.w	#1,d1			accessing table of word offsets
		move.w	ErrHandlers(pc,d1.w),d1	get the offset
		beq.s	BadCode			0 offset = not handled
		jmp	ErrHandlers(pc,d1.w)	handle this error

; the code is out of the normal defined range, or we don't handle it
BadCode:
		cmpi.w	#$7f,d1			is it in vendor unique?
		ble	MaybeRetryAll		no, maybe do a retry
		cmpi.b	#$04,d0			is code Not Ready?
		bne	MaybeRetryAll		no, something else
		move.l	d0,d1			let's go to ec04 (LUN not ready)
		bra.s	GotCode
