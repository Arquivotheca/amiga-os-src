...
M68060 Software Package BETA Release B1 --

M68060 Software Package Copyright (C) 1993, 1994 Motorola Inc.
All rights reserved.
...
set EXC_SR,		STOFF + 0x4 + 0x0 	# stack status register
set EXC_PC,		STOFF + 0x6 + 0x0 	# stack pc
set EXC_VOFF,	STOFF + 0xa + 0x0 	# stacked vector offset
set EXC_EA,		STOFF + 0xc + 0x0	   # stacked <ea>

set EXC_FP,		0x0			   # frame pointer

set EXC_AREGS,	   -68			# full
set EXC_DREGS,	   -100			# full
set EXC_FPREGS,   -36			# full

set EXC_A7,		EXC_AREGS+(7*4)
set OLD_A7,		EXC_AREGS+(6*4)
set EXC_A6,		EXC_AREGS+(6*4)
...
set STORE_FLG,	LV+19
set SPOP_FLG,	LV+18
set DY_MO_FLG,	LV+17			      # DYADIC/MONADIC flag
set SGLOP_FLG,	LV+16
...
set SPCOND_FLG,		LV+10

set EXC_CC,		LV+8
set EXC_EXTWPTR,	LV+4
set EXC_EXTWORD,	LV+2
set EXC_CMDREG,		LV+2
set EXC_OPWORD,		LV+0
...
# FSAVE status bits #
#####################
set fsave_exc,		0xf			# fsave "exception present" bit
set fsave_null,		0xe			# fsave "frame not null" bit

###########################
# SPecial CONDition FLaGs #
###########################
set ftrapcc_flg,	0x01
set fbsun_flg,		0x02
set mia7_flg,		0x04
set mda7_flg,		0x08
set fmovm_flg,		0x40
...
#########################################################################
# _fpsp_unimp(): unimplemented fp instruction exception handler 	#
#									#
...
funimp_gen_op:
	...
	mov.l		(tbl_trans.w,%pc,%d1.w*4),%d1
	jsr		(tbl_trans.w,%pc,%d1.l*1) # emulate
...
#	xref		t_dz2,t_operr	
#	xref		serror,sone,src_zero,src_inf,snzrinx	
...
#	xref		slog10,slog2,slogn,slognp1	
#	xref		slog10d,slog2d,slognd,slognp1d	
#	xref		stentox,stentoxd,stwotox,stwotoxd	

	global		tbl_trans
	swbeg		&0x1c0
tbl_trans:
	long 		serror	 - tbl_trans	# $00-0 fmovecr all
	long 		serror	 - tbl_trans	# $00-1 fmovecr all
	long 		serror	 - tbl_trans	# $00-2 fmovecr all
	long 		serror	 - tbl_trans	# $00-3 fmovecr all
	long 		serror	 - tbl_trans	# $00-4 fmovecr all
	long 		serror	 - tbl_trans	# $00-5 fmovecr all
	long 		serror	 - tbl_trans	# $00-6 fmovecr all
	long 		serror	 - tbl_trans	# $00-7 fmovecr all

	long 		serror	 - tbl_trans	# $01-0 fint norm
	long		serror	 - tbl_trans	# $01-1 fint zero
	long		serror	 - tbl_trans	# $01-2 fint inf
	long		serror	 - tbl_trans	# $01-3 fint qnan
	long		serror	 - tbl_trans	# $01-5 fint denorm
	long		serror	 - tbl_trans	# $01-4 fint snan
	long		serror	 - tbl_trans	# $01-6 fint unnorm
	long		serror	 - tbl_trans	# $01-7 ERROR

	long		ssinh	 - tbl_trans	# $02-0 fsinh norm
	long		src_zero - tbl_trans	# $02-1 fsinh zero
	long		src_inf	 - tbl_trans	# $02-2 fsinh inf
	long		src_qnan - tbl_trans	# $02-3 fsinh qnan
	long		ssinhd	 - tbl_trans	# $02-5 fsinh denorm
	long		src_snan - tbl_trans	# $02-4 fsinh snan
	long		serror	 - tbl_trans	# $02-6 fsinh unnorm
	long		serror	 - tbl_trans	# $02-7 ERROR

	long		serror	 - tbl_trans	# $03-0 fintrz norm
	long		serror	 - tbl_trans	# $03-1 fintrz zero
	long		serror	 - tbl_trans	# $03-2 fintrz inf
	long		serror	 - tbl_trans	# $03-3 fintrz qnan
	long		serror	 - tbl_trans	# $03-5 fintrz denorm
	long		serror	 - tbl_trans	# $03-4 fintrz snan
	long		serror	 - tbl_trans	# $03-6 fintrz unnorm
	long		serror	 - tbl_trans	# $03-7 ERROR

	long		serror	 - tbl_trans	# $04-0 fsqrt norm
	long		serror	 - tbl_trans	# $04-1 fsqrt zero
	long		serror	 - tbl_trans	# $04-2 fsqrt inf
	long		serror	 - tbl_trans	# $04-3 fsqrt qnan
	long		serror	 - tbl_trans	# $04-5 fsqrt denorm
	long		serror	 - tbl_trans	# $04-4 fsqrt snan
	long		serror	 - tbl_trans	# $04-6 fsqrt unnorm
	long		serror	 - tbl_trans	# $04-7 ERROR

	long		serror	 - tbl_trans	# $05-0 ERROR
	long		serror	 - tbl_trans	# $05-1 ERROR
	long		serror	 - tbl_trans	# $05-2 ERROR
	long		serror	 - tbl_trans	# $05-3 ERROR
	long		serror	 - tbl_trans	# $05-4 ERROR
	long		serror	 - tbl_trans	# $05-5 ERROR
	long		serror	 - tbl_trans	# $05-6 ERROR
	long		serror	 - tbl_trans	# $05-7 ERROR

	long		slognp1	 - tbl_trans	# $06-0 flognp1 norm
	long		src_zero - tbl_trans	# $06-1 flognp1 zero
	long		sopr_inf - tbl_trans	# $06-2 flognp1 inf
	long		src_qnan - tbl_trans	# $06-3 flognp1 qnan
	long		slognp1d - tbl_trans	# $06-5 flognp1 denorm
	long		src_snan - tbl_trans	# $06-4 flognp1 snan
	long		serror	 - tbl_trans	# $06-6 flognp1 unnorm
	long		serror	 - tbl_trans	# $06-7 ERROR

	long		serror	 - tbl_trans	# $07-0 ERROR
	long		serror	 - tbl_trans	# $07-1 ERROR
	long		serror	 - tbl_trans	# $07-2 ERROR
	long		serror	 - tbl_trans	# $07-3 ERROR
	long		serror	 - tbl_trans	# $07-4 ERROR
	long		serror	 - tbl_trans	# $07-5 ERROR
	long		serror	 - tbl_trans	# $07-6 ERROR
	long		serror	 - tbl_trans	# $07-7 ERROR

	long		setoxm1	 - tbl_trans	# $08-0 fetoxm1 norm
	long		src_zero - tbl_trans	# $08-1 fetoxm1 zero
	long		setoxm1i - tbl_trans	# $08-2 fetoxm1 inf
	long		src_qnan - tbl_trans	# $08-3 fetoxm1 qnan
	long		setoxm1d - tbl_trans	# $08-5 fetoxm1 denorm
	long		src_snan - tbl_trans	# $08-4 fetoxm1 snan
	long		serror	 - tbl_trans	# $08-6 fetoxm1 unnorm
	long		serror	 - tbl_trans	# $08-7 ERROR

	long		stanh	 - tbl_trans	# $09-0 ftanh norm
	long		src_zero - tbl_trans	# $09-1 ftanh zero
	long		src_one	 - tbl_trans	# $09-2 ftanh inf
	long		src_qnan - tbl_trans	# $09-3 ftanh qnan
	long		stanhd	 - tbl_trans	# $09-5 ftanh denorm
	long		src_snan - tbl_trans	# $09-4 ftanh snan
	long		serror	 - tbl_trans	# $09-6 ftanh unnorm
	long		serror	 - tbl_trans	# $09-7 ERROR

	long		satan	 - tbl_trans	# $0a-0 fatan norm
	long		src_zero - tbl_trans	# $0a-1 fatan zero
	long		spi_2	 - tbl_trans	# $0a-2 fatan inf
	long		src_qnan - tbl_trans	# $0a-3 fatan qnan
	long		satand	 - tbl_trans	# $0a-5 fatan denorm
	long		src_snan - tbl_trans	# $0a-4 fatan snan
	long		serror	 - tbl_trans	# $0a-6 fatan unnorm
	long		serror	 - tbl_trans	# $0a-7 ERROR

	long		serror	 - tbl_trans	# $0b-0 ERROR
	long		serror	 - tbl_trans	# $0b-1 ERROR
	long		serror	 - tbl_trans	# $0b-2 ERROR
	long		serror	 - tbl_trans	# $0b-3 ERROR
	long		serror	 - tbl_trans	# $0b-4 ERROR
	long		serror	 - tbl_trans	# $0b-5 ERROR
	long		serror	 - tbl_trans	# $0b-6 ERROR
	long		serror	 - tbl_trans	# $0b-7 ERROR

	long		sasin	 - tbl_trans	# $0c-0 fasin norm
	long		src_zero - tbl_trans	# $0c-1 fasin zero
	long		t_operr	 - tbl_trans	# $0c-2 fasin inf
	long		src_qnan - tbl_trans	# $0c-3 fasin qnan
	long		sasind	 - tbl_trans	# $0c-5 fasin denorm
	long		src_snan - tbl_trans	# $0c-4 fasin snan
	long		serror	 - tbl_trans	# $0c-6 fasin unnorm
	long		serror	 - tbl_trans	# $0c-7 ERROR

	long		satanh	 - tbl_trans	# $0d-0 fatanh norm
	long		src_zero - tbl_trans	# $0d-1 fatanh zero
	long		t_operr	 - tbl_trans	# $0d-2 fatanh inf
	long		src_qnan - tbl_trans	# $0d-3 fatanh qnan
	long		satanhd	 - tbl_trans	# $0d-5 fatanh denorm
	long		src_snan - tbl_trans	# $0d-4 fatanh snan
	long		serror	 - tbl_trans	# $0d-6 fatanh unnorm
	long		serror	 - tbl_trans	# $0d-7 ERROR

	long		ssin	 - tbl_trans	# $0e-0 fsin norm
	long		src_zero - tbl_trans	# $0e-1 fsin zero
	long		t_operr	 - tbl_trans	# $0e-2 fsin inf
	long		src_qnan - tbl_trans	# $0e-3 fsin qnan
	long		ssind	 - tbl_trans	# $0e-5 fsin denorm
	long		src_snan - tbl_trans	# $0e-4 fsin snan
	long		serror	 - tbl_trans	# $0e-6 fsin unnorm
	long		serror	 - tbl_trans	# $0e-7 ERROR
	
	long		stan	 - tbl_trans	# $0f-0 ftan norm
	long		src_zero - tbl_trans	# $0f-1 ftan zero
	long		t_operr	 - tbl_trans	# $0f-2 ftan inf
	long		src_qnan - tbl_trans	# $0f-3 ftan qnan
	long		stand	 - tbl_trans	# $0f-5 ftan denorm
	long		src_snan - tbl_trans	# $0f-4 ftan snan
	long		serror	 - tbl_trans	# $0f-6 ftan unnorm
	long		serror	 - tbl_trans	# $0f-7 ERROR

	long		setox	 - tbl_trans	# $10-0 fetox norm
	long		ld_pone	 - tbl_trans	# $10-1 fetox zero
	long		szr_inf	 - tbl_trans	# $10-2 fetox inf
	long		src_qnan - tbl_trans	# $10-3 fetox qnan
	long		setoxd	 - tbl_trans	# $10-5 fetox denorm
	long		src_snan - tbl_trans	# $10-4 fetox snan
	long		serror	 - tbl_trans	# $10-6 fetox unnorm
	long		serror	 - tbl_trans	# $10-7 ERROR

	long		stwotox	 - tbl_trans	# $11-0 ftwotox norm
	long		ld_pone	 - tbl_trans	# $11-1 ftwotox zero
	long		szr_inf	 - tbl_trans	# $11-2 ftwotox inf
	long		src_qnan - tbl_trans	# $11-3 ftwotox qnan
	long		stwotoxd - tbl_trans	# $11-5 ftwotox denorm
	long		src_snan - tbl_trans	# $11-4 ftwotox snan
	long		serror	 - tbl_trans	# $11-6 ftwotox unnorm
	long		serror	 - tbl_trans	# $11-7 ERROR

	long		stentox	 - tbl_trans	# $12-0 ftentox norm
	long		ld_pone	 - tbl_trans	# $12-1 ftentox zero
	long		szr_inf	 - tbl_trans	# $12-2 ftentox inf
	long		src_qnan - tbl_trans	# $12-3 ftentox qnan
	long		stentoxd - tbl_trans	# $12-5 ftentox denorm
	long		src_snan - tbl_trans	# $12-4 ftentox snan
	long		serror	 - tbl_trans	# $12-6 ftentox unnorm
	long		serror	 - tbl_trans	# $12-7 ERROR

	long		serror	 - tbl_trans	# $13-0 ERROR
	long		serror	 - tbl_trans	# $13-1 ERROR
	long		serror	 - tbl_trans	# $13-2 ERROR
	long		serror	 - tbl_trans	# $13-3 ERROR
	long		serror	 - tbl_trans	# $13-4 ERROR
	long		serror	 - tbl_trans	# $13-5 ERROR
	long		serror	 - tbl_trans	# $13-6 ERROR
	long		serror	 - tbl_trans	# $13-7 ERROR

	long		slogn	 - tbl_trans	# $14-0 flogn norm
	long		t_dz2	 - tbl_trans	# $14-1 flogn zero
	long		sopr_inf - tbl_trans	# $14-2 flogn inf
	long		src_qnan - tbl_trans	# $14-3 flogn qnan
	long		slognd	 - tbl_trans	# $14-5 flogn denorm
	long		src_snan - tbl_trans	# $14-4 flogn snan
	long		serror	 - tbl_trans	# $14-6 flogn unnorm
	long		serror	 - tbl_trans	# $14-7 ERROR

	long		slog10	 - tbl_trans	# $15-0 flog10 norm
	long		t_dz2	 - tbl_trans	# $15-1 flog10 zero
	long		sopr_inf - tbl_trans	# $15-2 flog10 inf
	long		src_qnan - tbl_trans	# $15-3 flog10 qnan
	long		slog10d	 - tbl_trans	# $15-5 flog10 denorm
	long		src_snan - tbl_trans	# $15-4 flog10 snan
	long		serror	 - tbl_trans	# $15-6 flog10 unnorm
	long		serror	 - tbl_trans	# $15-7 ERROR

	long		slog2	 - tbl_trans	# $16-0 flog2 norm
	long		t_dz2	 - tbl_trans	# $16-1 flog2 zero
	long		sopr_inf - tbl_trans	# $16-2 flog2 inf
	long		src_qnan - tbl_trans	# $16-3 flog2 qnan
	long		slog2d	 - tbl_trans	# $16-5 flog2 denorm
	long		src_snan - tbl_trans	# $16-4 flog2 snan
	long		serror	 - tbl_trans	# $16-6 flog2 unnorm
	long		serror	 - tbl_trans	# $16-7 ERROR

	long		serror	 - tbl_trans	# $17-0 ERROR
	long		serror	 - tbl_trans	# $17-1 ERROR
	long		serror	 - tbl_trans	# $17-2 ERROR
	long		serror	 - tbl_trans	# $17-3 ERROR
	long		serror	 - tbl_trans	# $17-4 ERROR
	long		serror	 - tbl_trans	# $17-5 ERROR
	long		serror	 - tbl_trans	# $17-6 ERROR
	long		serror	 - tbl_trans	# $17-7 ERROR

	long		serror	 - tbl_trans	# $18-0 fabs norm
	long		serror	 - tbl_trans	# $18-1 fabs zero
	long		serror	 - tbl_trans	# $18-2 fabs inf
	long		serror	 - tbl_trans	# $18-3 fabs qnan
	long		serror	 - tbl_trans	# $18-5 fabs denorm
	long		serror	 - tbl_trans	# $18-4 fabs snan
	long		serror	 - tbl_trans	# $18-6 fabs unnorm
	long		serror	 - tbl_trans	# $18-7 ERROR

	long		scosh	 - tbl_trans	# $19-0 fcosh norm
	long		ld_pone	 - tbl_trans	# $19-1 fcosh zero
	long		ld_pinf	 - tbl_trans	# $19-2 fcosh inf
	long		src_qnan - tbl_trans	# $19-3 fcosh qnan
	long		scoshd	 - tbl_trans	# $19-5 fcosh denorm
	long		src_snan - tbl_trans	# $19-4 fcosh snan
	long		serror	 - tbl_trans	# $19-6 fcosh unnorm
	long		serror	 - tbl_trans	# $19-7 ERROR

	long		serror	 - tbl_trans	# $1a-0 fneg norm
	long		serror	 - tbl_trans	# $1a-1 fneg zero
	long		serror	 - tbl_trans	# $1a-2 fneg inf
	long		serror	 - tbl_trans	# $1a-3 fneg qnan
	long		serror	 - tbl_trans	# $1a-5 fneg denorm
	long		serror	 - tbl_trans	# $1a-4 fneg snan
	long		serror	 - tbl_trans	# $1a-6 fneg unnorm
	long		serror	 - tbl_trans	# $1a-7 ERROR

	long		serror	 - tbl_trans	# $1b-0 ERROR
	long		serror	 - tbl_trans	# $1b-1 ERROR
	long		serror	 - tbl_trans	# $1b-2 ERROR
	long		serror	 - tbl_trans	# $1b-3 ERROR
	long		serror	 - tbl_trans	# $1b-4 ERROR
	long		serror	 - tbl_trans	# $1b-5 ERROR
	long		serror	 - tbl_trans	# $1b-6 ERROR
	long		serror	 - tbl_trans	# $1b-7 ERROR

	long		sacos	 - tbl_trans	# $1c-0 facos norm
	long		ld_ppi2	 - tbl_trans	# $1c-1 facos zero
	long		t_operr	 - tbl_trans	# $1c-2 facos inf
	long		src_qnan - tbl_trans	# $1c-3 facos qnan
	long		sacosd	 - tbl_trans	# $1c-5 facos denorm
	long		src_snan - tbl_trans	# $1c-4 facos snan
	long		serror	 - tbl_trans	# $1c-6 facos unnorm
	long		serror	 - tbl_trans	# $1c-7 ERROR

	long		scos	 - tbl_trans	# $1d-0 fcos norm
	long		ld_pone	 - tbl_trans	# $1d-1 fcos zero
	long		t_operr	 - tbl_trans	# $1d-2 fcos inf
	long		src_qnan - tbl_trans	# $1d-3 fcos qnan
	long		scosd	 - tbl_trans	# $1d-5 fcos denorm
	long		src_snan - tbl_trans	# $1d-4 fcos snan
	long		serror	 - tbl_trans	# $1d-6 fcos unnorm
	long		serror	 - tbl_trans	# $1d-7 ERROR

	long		sgetexp	 - tbl_trans	# $1e-0 fgetexp norm
	long		src_zero - tbl_trans	# $1e-1 fgetexp zero
	long		t_operr	 - tbl_trans	# $1e-2 fgetexp inf
	long		src_qnan - tbl_trans	# $1e-3 fgetexp qnan
	long		sgetexpd - tbl_trans	# $1e-5 fgetexp denorm
	long		src_snan - tbl_trans	# $1e-4 fgetexp snan
	long		serror	 - tbl_trans	# $1e-6 fgetexp unnorm
	long		serror	 - tbl_trans	# $1e-7 ERROR

	long		sgetman	 - tbl_trans	# $1f-0 fgetman norm
	long		src_zero - tbl_trans	# $1f-1 fgetman zero
	long		t_operr	 - tbl_trans	# $1f-2 fgetman inf
	long		src_qnan - tbl_trans	# $1f-3 fgetman qnan
	long		sgetmand - tbl_trans	# $1f-5 fgetman denorm
	long		src_snan - tbl_trans	# $1f-4 fgetman snan
	long		serror	 - tbl_trans	# $1f-6 fgetman unnorm
	long		serror	 - tbl_trans	# $1f-7 ERROR

	long		serror	 - tbl_trans	# $20-0 fdiv norm
	long		serror	 - tbl_trans	# $20-1 fdiv zero
	long		serror	 - tbl_trans	# $20-2 fdiv inf
	long		serror	 - tbl_trans	# $20-3 fdiv qnan
	long		serror	 - tbl_trans	# $20-5 fdiv denorm
	long		serror	 - tbl_trans	# $20-4 fdiv snan
	long		serror	 - tbl_trans	# $20-6 fdiv unnorm
	long		serror	 - tbl_trans	# $20-7 ERROR

	long		smod_snorm - tbl_trans	# $21-0 fmod norm
	long		smod_szero - tbl_trans	# $21-1 fmod zero
	long		smod_sinf - tbl_trans	# $21-2 fmod inf
	long		sop_sqnan - tbl_trans	# $21-3 fmod qnan
	long		smod_sdnrm - tbl_trans	# $21-5 fmod denorm
	long		sop_ssnan - tbl_trans	# $21-4 fmod snan
	long		serror	 - tbl_trans	# $21-6 fmod unnorm
	long		serror	 - tbl_trans	# $21-7 ERROR

	long		serror	 - tbl_trans	# $22-0 fadd norm
	long		serror	 - tbl_trans	# $22-1 fadd zero
	long		serror	 - tbl_trans	# $22-2 fadd inf
	long		serror	 - tbl_trans	# $22-3 fadd qnan
	long		serror	 - tbl_trans	# $22-5 fadd denorm
	long		serror	 - tbl_trans	# $22-4 fadd snan
	long		serror	 - tbl_trans	# $22-6 fadd unnorm
	long		serror	 - tbl_trans	# $22-7 ERROR

	long		serror	 - tbl_trans	# $23-0 fmul norm
	long		serror	 - tbl_trans	# $23-1 fmul zero
	long		serror	 - tbl_trans	# $23-2 fmul inf
	long		serror	 - tbl_trans	# $23-3 fmul qnan
	long		serror	 - tbl_trans	# $23-5 fmul denorm
	long		serror	 - tbl_trans	# $23-4 fmul snan
	long		serror	 - tbl_trans	# $23-6 fmul unnorm
	long		serror	 - tbl_trans	# $23-7 ERROR

	long		serror	 - tbl_trans	# $24-0 fsgldiv norm
	long		serror	 - tbl_trans	# $24-1 fsgldiv zero
	long		serror	 - tbl_trans	# $24-2 fsgldiv inf
	long		serror	 - tbl_trans	# $24-3 fsgldiv qnan
	long		serror	 - tbl_trans	# $24-5 fsgldiv denorm
	long		serror	 - tbl_trans	# $24-4 fsgldiv snan
	long		serror	 - tbl_trans	# $24-6 fsgldiv unnorm
	long		serror	 - tbl_trans	# $24-7 ERROR

	long		srem_snorm - tbl_trans	# $25-0 frem norm
	long		srem_szero - tbl_trans	# $25-1 frem zero
	long		srem_sinf - tbl_trans	# $25-2 frem inf
	long		sop_sqnan - tbl_trans	# $25-3 frem qnan
	long		srem_sdnrm - tbl_trans	# $25-5 frem denorm
	long		sop_ssnan - tbl_trans	# $25-4 frem snan
	long		serror	 - tbl_trans	# $25-6 frem unnorm
	long		serror	 - tbl_trans	# $25-7 ERROR

	long		sscale_snorm - tbl_trans # $26-0 fscale norm
	long		sscale_szero - tbl_trans # $26-1 fscale zero
	long		sscale_sinf - tbl_trans	# $26-2 fscale inf
	long		sop_sqnan - tbl_trans	# $26-3 fscale qnan
	long		sscale_sdnrm - tbl_trans # $26-5 fscale denorm
	long		sop_ssnan - tbl_trans	# $26-4 fscale snan
	long		serror	 - tbl_trans	# $26-6 fscale unnorm
	long		serror	 - tbl_trans	# $26-7 ERROR

	long		serror	 - tbl_trans	# $27-0 fsglmul norm
	long		serror	 - tbl_trans	# $27-1 fsglmul zero
	long		serror	 - tbl_trans	# $27-2 fsglmul inf
	long		serror	 - tbl_trans	# $27-3 fsglmul qnan
	long		serror	 - tbl_trans	# $27-5 fsglmul denorm
	long		serror	 - tbl_trans	# $27-4 fsglmul snan
	long		serror	 - tbl_trans	# $27-6 fsglmul unnorm
	long		serror	 - tbl_trans	# $27-7 ERROR

	long		serror	 - tbl_trans	# $28-0 fsub norm
	long		serror	 - tbl_trans	# $28-1 fsub zero
	long		serror	 - tbl_trans	# $28-2 fsub inf
	long		serror	 - tbl_trans	# $28-3 fsub qnan
	long		serror	 - tbl_trans	# $28-5 fsub denorm
	long		serror	 - tbl_trans	# $28-4 fsub snan
	long		serror	 - tbl_trans	# $28-6 fsub unnorm
	long		serror	 - tbl_trans	# $28-7 ERROR

	long		serror	 - tbl_trans	# $29-0 ERROR
	long		serror	 - tbl_trans	# $29-1 ERROR
	long		serror	 - tbl_trans	# $29-2 ERROR
	long		serror	 - tbl_trans	# $29-3 ERROR
	long		serror	 - tbl_trans	# $29-4 ERROR
	long		serror	 - tbl_trans	# $29-5 ERROR
	long		serror	 - tbl_trans	# $29-6 ERROR
	long		serror	 - tbl_trans	# $29-7 ERROR

	long		serror	 - tbl_trans	# $2a-0 ERROR
	long		serror	 - tbl_trans	# $2a-1 ERROR
	long		serror	 - tbl_trans	# $2a-2 ERROR
	long		serror	 - tbl_trans	# $2a-3 ERROR
	long		serror	 - tbl_trans	# $2a-4 ERROR
	long		serror	 - tbl_trans	# $2a-5 ERROR
	long		serror	 - tbl_trans	# $2a-6 ERROR
	long		serror	 - tbl_trans	# $2a-7 ERROR

	long		serror	 - tbl_trans	# $2b-0 ERROR
	long		serror	 - tbl_trans	# $2b-1 ERROR
	long		serror	 - tbl_trans	# $2b-2 ERROR
	long		serror	 - tbl_trans	# $2b-3 ERROR
	long		serror	 - tbl_trans	# $2b-4 ERROR
	long		serror	 - tbl_trans	# $2b-5 ERROR
	long		serror	 - tbl_trans	# $2b-6 ERROR
	long		serror	 - tbl_trans	# $2b-7 ERROR

	long		serror	 - tbl_trans	# $2c-0 ERROR
	long		serror	 - tbl_trans	# $2c-1 ERROR
	long		serror	 - tbl_trans	# $2c-2 ERROR
	long		serror	 - tbl_trans	# $2c-3 ERROR
	long		serror	 - tbl_trans	# $2c-4 ERROR
	long		serror	 - tbl_trans	# $2c-5 ERROR
	long		serror	 - tbl_trans	# $2c-6 ERROR
	long		serror	 - tbl_trans	# $2c-7 ERROR

	long		serror	 - tbl_trans	# $2d-0 ERROR
	long		serror	 - tbl_trans	# $2d-1 ERROR
	long		serror	 - tbl_trans	# $2d-2 ERROR
	long		serror	 - tbl_trans	# $2d-3 ERROR
	long		serror	 - tbl_trans	# $2d-4 ERROR
	long		serror	 - tbl_trans	# $2d-5 ERROR
	long		serror	 - tbl_trans	# $2d-6 ERROR
	long		serror	 - tbl_trans	# $2d-7 ERROR

	long		serror	 - tbl_trans	# $2e-0 ERROR
	long		serror	 - tbl_trans	# $2e-1 ERROR
	long		serror	 - tbl_trans	# $2e-2 ERROR
	long		serror	 - tbl_trans	# $2e-3 ERROR
	long		serror	 - tbl_trans	# $2e-4 ERROR
	long		serror	 - tbl_trans	# $2e-5 ERROR
	long		serror	 - tbl_trans	# $2e-6 ERROR
	long		serror	 - tbl_trans	# $2e-7 ERROR

	long		serror	 - tbl_trans	# $2f-0 ERROR
	long		serror	 - tbl_trans	# $2f-1 ERROR
	long		serror	 - tbl_trans	# $2f-2 ERROR
	long		serror	 - tbl_trans	# $2f-3 ERROR
	long		serror	 - tbl_trans	# $2f-4 ERROR
	long		serror	 - tbl_trans	# $2f-5 ERROR
	long		serror	 - tbl_trans	# $2f-6 ERROR
	long		serror	 - tbl_trans	# $2f-7 ERROR

	long		ssincos	 - tbl_trans	# $30-0 fsincos norm
	long		ssincosz - tbl_trans	# $30-1 fsincos zero
	long		ssincosi - tbl_trans	# $30-2 fsincos inf
	long		ssincosqnan - tbl_trans	# $30-3 fsincos qnan
	long		ssincosd - tbl_trans	# $30-5 fsincos denorm
	long		ssincossnan - tbl_trans	# $30-4 fsincos snan
	long		serror	 - tbl_trans	# $30-6 fsincos unnorm
	long		serror	 - tbl_trans	# $30-7 ERROR

	long		ssincos	 - tbl_trans	# $31-0 fsincos norm
	long		ssincosz - tbl_trans	# $31-1 fsincos zero
	long		ssincosi - tbl_trans	# $31-2 fsincos inf
	long		ssincosqnan - tbl_trans	# $31-3 fsincos qnan
	long		ssincosd - tbl_trans	# $31-5 fsincos denorm
	long		ssincossnan - tbl_trans	# $31-4 fsincos snan
	long		serror	 - tbl_trans	# $31-6 fsincos unnorm
	long		serror	 - tbl_trans	# $31-7 ERROR

	long		ssincos	 - tbl_trans	# $32-0 fsincos norm
	long		ssincosz - tbl_trans	# $32-1 fsincos zero
	long		ssincosi - tbl_trans	# $32-2 fsincos inf
	long		ssincosqnan - tbl_trans	# $32-3 fsincos qnan
	long		ssincosd - tbl_trans	# $32-5 fsincos denorm
	long		ssincossnan - tbl_trans	# $32-4 fsincos snan
	long		serror	 - tbl_trans	# $32-6 fsincos unnorm
	long		serror	 - tbl_trans	# $32-7 ERROR

	long		ssincos	 - tbl_trans	# $33-0 fsincos norm
	long		ssincosz - tbl_trans	# $33-1 fsincos zero
	long		ssincosi - tbl_trans	# $33-2 fsincos inf
	long		ssincosqnan - tbl_trans	# $33-3 fsincos qnan
	long		ssincosd - tbl_trans	# $33-5 fsincos denorm
	long		ssincossnan - tbl_trans	# $33-4 fsincos snan
	long		serror	 - tbl_trans	# $33-6 fsincos unnorm
	long		serror	 - tbl_trans	# $33-7 ERROR

	long		ssincos	 - tbl_trans	# $34-0 fsincos norm
	long		ssincosz - tbl_trans	# $34-1 fsincos zero
	long		ssincosi - tbl_trans	# $34-2 fsincos inf
	long		ssincosqnan - tbl_trans	# $34-3 fsincos qnan
	long		ssincosd - tbl_trans	# $34-5 fsincos denorm
	long		ssincossnan - tbl_trans	# $34-4 fsincos snan
	long		serror	 - tbl_trans	# $34-6 fsincos unnorm
	long		serror	 - tbl_trans	# $34-7 ERROR

	long		ssincos	 - tbl_trans	# $35-0 fsincos norm
	long		ssincosz - tbl_trans	# $35-1 fsincos zero
	long		ssincosi - tbl_trans	# $35-2 fsincos inf
	long		ssincosqnan - tbl_trans	# $35-3 fsincos qnan
	long		ssincosd - tbl_trans	# $35-5 fsincos denorm
	long		ssincossnan - tbl_trans	# $35-4 fsincos snan
	long		serror	 - tbl_trans	# $35-6 fsincos unnorm
	long		serror	 - tbl_trans	# $35-7 ERROR

	long		ssincos	 - tbl_trans	# $36-0 fsincos norm
	long		ssincosz - tbl_trans	# $36-1 fsincos zero
	long		ssincosi - tbl_trans	# $36-2 fsincos inf
	long		ssincosqnan - tbl_trans	# $36-3 fsincos qnan
	long		ssincosd - tbl_trans	# $36-5 fsincos denorm
	long		ssincossnan - tbl_trans	# $36-4 fsincos snan
	long		serror	 - tbl_trans	# $36-6 fsincos unnorm
	long		serror	 - tbl_trans	# $36-7 ERROR

	long		ssincos	 - tbl_trans	# $37-0 fsincos norm
	long		ssincosz - tbl_trans	# $37-1 fsincos zero
	long		ssincosi - tbl_trans	# $37-2 fsincos inf
	long		ssincosqnan - tbl_trans	# $37-3 fsincos qnan
	long		ssincosd - tbl_trans	# $37-5 fsincos denorm
	long		ssincossnan - tbl_trans	# $37-4 fsincos snan
	long		serror	 - tbl_trans	# $37-6 fsincos unnorm
	long		serror	 - tbl_trans	# $37-7 ERROR
...
#########################################
# fpsp_ovfl(): Q FP Overflow Handler	#
#########################################
	global		_fpsp_ovfl
_fpsp_ovfl:
...
#########################################
# fpsp_unfl(): Q FP Underflow Handler	#
#########################################
	global		_fpsp_unfl
_fpsp_unfl:
...
#################################################################
# fpsp_unsupp(): Q FP Unsupported Data Type Handler
#
...
#	*****************
#
# -only instructions that can come this way are:
#	fabs,fadd,fdiv,fint,fintrz,fmul,fneg,fsgldiv,
#	fsglmul,fsqrt,fsub,ftst,fmov
#################################################################
	global		_fpsp_unsupp
_fpsp_unsupp:
...
# jump to the "_real_{}()" handler...
	...
	swbeg		&0x8
tbl_fu_out:
	short		fu_out_error	- tbl_fu_out	# BSUN can't happen
	short		fu_out_error 	- tbl_fu_out	# SNAN can't happen
	short		fu_operr	- tbl_fu_out	# OPERR
	short		fu_ovfl 	- tbl_fu_out	# OVFL
	short		fu_unfl 	- tbl_fu_out	# UNFL
	short		fu_out_error	- tbl_fu_out	# DZ can't happen
	short		fu_inex 	- tbl_fu_out	# INEX2
	short		fu_out_error	- tbl_fu_out	# INEX1 won't make it here
...
#############################################################
# _fpsp_effadd(): Q Unimplemented Effective Address Handler #
#############################################################
	global		_fpsp_effadd
_fpsp_effadd:
...
iea_op:
	...
	bsr.l		_mem_read		# read extended immediate
...
iea_op_pack:
	...
	mov.l		EXC_EXTWPTR(%a6),%a0	# pass: ptr to #<data>
	bsr.l		get_packed2		# get packed operand
...
############################################
# fpsp_operr(): Q FP Operand Error Handler #
############################################
	global		_fpsp_operr
_fpsp_operr:
...
tbl_operr:
	...
	short		foperr_error - tbl_operr # sgl prec shouldn't happen
	short		foperr_error - tbl_operr # ext prec shouldn't happen
	short		foperr_exit  - tbl_operr # packed won't enter here
	short		foperr_out_w - tbl_operr # word integer
	short		foperr_error - tbl_operr # dbl prec shouldn't happen
	short		foperr_out_b - tbl_operr # byte integer
	short		foperr_error - tbl_operr # packed won't enter here
...
#####################################################
# fpsp_snan(): Q FP Signalling Not-a-number Handler #
#####################################################
	global		_fpsp_snan
_fpsp_snan:
...
tbl_snan:
	...
	short		fsnan_error - tbl_snan # packed needs no help
...
fsnan_out_d:
	...
	bsr.l		_mem_write		# write the default result
	bra.w		fsnan_exit
...
#####################################
# fpsp_inex(): Q FP Inexact Handler #
#####################################
	global		_fpsp_inex
_fpsp_inex:
...
#		     Also, for extended precision and packed, the stacked
#		     <ea> value is 8 bytes too big
#	2) (An)+   : The register is not updated.
#	3) #<data> : The upper longword of the immediate operand is stacked.
#		     b,w,l and s sizes are completely stacked. d,x, and p
#		     are not.
#		     
	global		_dcalc_ea
_dcalc_ea:
...
dcea_imm:
	cmp.w		%a0,&0x1		# is it a byte size?
	beq.b		dcea_imm_byte		# yes
	lea		([USER_FPIAR,%a6],0x4),%a0 # no; return <ea>
	rts
dcea_imm_byte:
	lea		([USER_FPIAR,%a6],0x5),%a0 # return <ea> + 1
	rts
...
ceaf_pi0:
	addi.l		&0xc,EXC_A0(%a6)
	rts
ceaf_pi1:
	addi.l		&0xc,EXC_A1(%a6)
	rts
ceaf_pi2:
	add.l		&0xc,%a2
	rts
...
# the emulation routine set bsun and BSUN was enabled. eventually have to
# fix stack and jump to the bsun handler.
# for now, we will let the caller of this routine shift the stack frame up to
...
ftrapcc_trap:
#	mov.l		USER_FPIAR(%a6),EXC_EA(%a6) # Address = Current PC
#	mov.w		&0x201c,EXC_VOFF(%a6)	# Fmt = 0x2; Vector Offset = 0x01c
...
# eliminate the effective address field.
ftrapcc_bsun:
#	mov.l		USER_FPIAR(%a6),EXC_PC(%a6) # PC = Current PC
#	mov.w		&0x00c0,EXC_VOFF(%a6)	# Fmt = 0x0; Vector Offset = 0x0c0
	mov.b		&fbsun_flg,SPCOND_FLG(%a6)
	rts
...
fmovm_out_ctrl_done:
	...
	mov.l		%d0,-(%sp)		# save size
	bsr.l		_mem_write		# copy data to user mem
	add.l		(%sp)+,%sp		# clear fpreg data from stack
	rts
...
fmovm_data_in:
	...
	bsr.l		_mem_read		# copy data from user mem
...
# jump to the corresponding function for each {MODE,REG} pair.
	...
	swbeg		&64
tbl_fea_mode:
	short		fmovm_error	-	tbl_fea_mode
	short		fmovm_error	-	tbl_fea_mode
	short		fmovm_error	-	tbl_fea_mode
	short		fmovm_error	-	tbl_fea_mode
	short		fmovm_error	-	tbl_fea_mode
...
faipa7_byte:
	add.w		&0x1,%a0		# stack post-inc byte does word
	bra.b		faipa7_cont
...
fmovm_ctrl:
	...
	beq.b		fctrl_in_7		# yes
	cmpi.b		%d0,&0x98		# fpcr & fpsr ?
	beq.b		fctrl_in_6		# yes
...
#									#
# DESCRIPTION ********************************************************* #
#	sSIN() computes the sine of an input argument			#
#	sCOS() computes the cosine, and sSINCOS computes both.		#
#	The corresponding entry points with a "d" computes the same	#
#	corresponding function values for denormalized inputs.		#
#									#
# INPUT *************************************************************** #
#	a0  = pointer to double-extended input "X"			#
#	d1  = control register input (rnd mode,prec)			#
#									#
# OUTPUT ************************************************************** #
#	fp0 = sin(X) or cos(X) 						#
...
#									#
# MODIFIES ************************************************************	#
#	Fp0 for SIN or COS; both Fp0 and Fp1 for SINCOS.		#
#									#
...
#									#
# SPEED *************************************************************** #
#	The programs sSIN and sCOS take approximately 150 cycles for	#
...
# ...LEAST SIG. BIT OF D0 IN SIGN POSITION
	...
	fmov.l		%d0,%control		# restore users exceptions
...
#									#
# INPUT *************************************************************** #
#	Double-extended number X in location pointed to	by address 	#
#	register a0.							#
#									#
# OUTPUT ************************************************************** #
#	The value tan(X) returned in floating-point register Fp0.	#
#									#
# ACCURACY and MONOTONICITY ******************************************* #
...
#									#
# SPEED *************************************************************** #
#	The program sTAN takes approximately 170 cycles for input 	#
#	argument X such that |X| < 15Pi, which is the the usual 	#
#	situation.							#
#									#
# ALGORITHM *********************************************************** #
#									#
...
#									#
# OUTPUT ************************************************************** #
#	Arctan(X) returned in floating-point register Fp0.		#
#									#
# ACCURACY and MONOTONICITY ******************************************* #
...
#									#
# SPEED *************************************************************** #
#	The program satan takes approximately 160 cycles for input	#
...
#									#
# DESCRIPTION *********************************************************	#
#	The entry point sAsin computes the inverse sine of an input	#
#	argument; sAsind does the same except for denormalized input.	#
#									#
# INPUT ***************************************************************	#
#	Double-extended number X in location pointed to by address	#
#	register a0.							#
#									#
# OUTPUT **************************************************************	# 
#	The value arcsin(X) returned in floating-point register Fp0.	#
#									#
# ACCURACY and MONOTONICITY *******************************************	#
...
#									#
# SPEED ***************************************************************	#
#	The program sASIN takes approximately 310 cycles.		#
#									#
# ALGORITHM ***********************************************************	#
#									#
#	ASIN								#
...
#									#
# DESCRIPTION ********************************************************* #
#	The entry point sAcos computes the inverse cosine of an input	#
#	argument; sAcosd does the same except for denormalized input.	#
#									#
# INPUT ***************************************************************	#
#	Double-extended number X in location pointed to			#
#	by address register a0.						#
#									#
# OUTPUT ************************************************************** #
#	The value arccos(X) returned in floating-point register Fp0.	#
#									#
# ACCURACY and MONOTONICITY *******************************************	#
#	The returned result is within 3 ulps in	64 significant bit,	#
#	i.e. within 0.5001 ulp to 53 bits if the result is subsequently	#
...
#									#
# SPEED *************************************************************** #
#	The program sCOS takes approximately 310 cycles.		#
#									#
# ALGORITHM *********************************************************** #
#									#
#	ACOS								#
...
sacosd:
	fmov.l		%d0,%control		# load user's rnd mode/prec
	fmov.x		PIBY2(%pc),%fp0
	bra		t_pinx2
...
#	first one is measured when the function is invoked the first	#
#	time (so the instructions and data are not in cache), and the	#
#	second one is measured when the function is reinvoked at the	#
#	same input argument.						#
#									#
...
#									#
#	The program setoxm1 takes approximately ???/??? cycles for 	#
#	input argument X, 0.25 <= |X| < 70log2. For |X| < 0.25, it 	#
# 	takes approximately ???/??? cycles. For the less common 	#
#	arguments, depending on their values, the program may run 	#
#	faster or slower -- but no worse than 10% slower even in the 	#
#	extreme cases.							#
#									#
...
NORMAL:
	fmov.l		%d0,%control		# restore user FPCR
...
#	The exponent bias is removed and the exponent value is returned	#
#	as an extended precision number in fp0.				#
#	sGETEXPD() handles denormalized numbers. 			#
#									#
# 	sGETMAN() extracts the mantissa of the input argument. The 	#
#	mantissa is converted to an extended precision number w/ an	#
#	exponent of $3fff and is returned in fp0. The range of the 	#
#	result is [1.0 - 2.0).						#
...
#									#
# XDEF **************************************************************** #
#	sgetexp(), sgetexpd(), sgetman(), sgetmand()			#
#									#
# XREF **************************************************************** #
#	norm()								#
#									#
...
#									#
# OUTPUT **************************************************************	#
#	The value cosh(X) returned in floating-point register Fp0.	#
#									#
# ACCURACY and MONOTONICITY *******************************************	#
#	The returned result is within 3 ulps in 64 significant bit, 	#
#	i.e. within 0.5001 ulp to 53 bits if the result is subsequently	#
...
#									#
# SPEED ***************************************************************	#
#	The program sCOSH takes approximately 250 cycles.		#
#									#
# ALGORITHM ***********************************************************	#
#									#
#	COSH								#
...
#									#
# DESCRIPTION *********************************************************	#
#	sSINH() computes the hyperbolic sine of an input argument.	#
#	sSINHD() does the same except for denormalized input.		#
#									#
# INPUT *************************************************************** #
#	a0  = pointer to double-extended input				#
#	d1  = control register input (rnd mode,prec)			#
#									#
# OUTPUT ************************************************************** #
#	fp0 = sinh(x)							#
...
#									#
# XREF ****************************************************************	#
#	setox(), setoxm1(), t_frcinx(), t_extdnrm(), t_ovfl()		#
#									#
# ACCURACY and MONOTONICITY *******************************************	#
#	The returned result is within 3 ulps in 64 significant bit, 	#
#	i.e. within 0.5001 ulp to 53 bits if the result is subsequently #
...
#									#
# SPEED	*************************************************************** #
#	The program sSINH takes approximately 280 cycles.		#
#									#
# ALGORITHM *********************************************************** #
#									#
#       SINH								#
...
#									#
# DESCRIPTION ********************************************************* #
#	sTANH() computes the hyperbolic tangent of an input argument.	#
#	sTANHD() does the same except for denormalized input.		#
#									#
# INPUT ***************************************************************	#
#	a0  = pointer to double-extended input				#
#	d1  = control register input (rnd mode,prec)			#
...
# 									#
# XREF **************************************************************** #
#	setox(), setoxm1(), t_extdnrm(), t_frcinx()			#
#									#
# ACCURACY and MONOTONICITY *******************************************	#
#	The returned result is within 3 ulps in 64 significant bit, 	#
#	i.e. within 0.5001 ulp to 53 bits if the result is subsequently #
...
#									#
# SPEED ***************************************************************	#
#	The program stanh takes approximately 270 cycles.		#
#									#
# ALGORITHM ***********************************************************	#
#									#
#	TANH								#
...
#									#
# OUTPUT **************************************************************	#
#	log(X) or log(1+X) returned in floating-point register Fp0.	#
#									#
# ACCURACY and MONOTONICITY *******************************************	#
#	The returned result is within 2 ulps in 64 significant bit, 	#
#	i.e. within 0.5001 ulp to 53 bits if the result is subsequently	#
...
#									#
# SPEED ***************************************************************	#
#	The program slogn takes approximately 190 cycles for input 	#
...
#									#
# DESCRIPTION ********************************************************* #
#	The entry point satanh computes the inverse hyperbolic tangent	#
#	of an input argument; satanhd does the same except for 		#
#	denormalized input.						#
#									#
# INPUT ***************************************************************	#
#	Double-extended number X in location pointed to	by address	#
...
#									#
# OUTPUT **************************************************************	# 
#	The value arctanh(X) returned in floating-point register Fp0.	#
#									#
# ACCURACY and MONOTONICITY *******************************************	#
#	The returned result is within 3 ulps in	64 significant bit,	#
#	i.e. within 0.5001 ulp to 53 bits if the result is subsequently	#
...
#									#
# SPEED ***************************************************************	#
#	The program satanh takes approximately 270 cycles.		#
#									#
# ALGORITHM ***********************************************************	#
#									#
#	ATANH								#
...
#									#
# DESCRIPTION ********************************************************* #
#       The entry point slog10 computes the base-10 logarithm of an 	#
#	input argument X. slog10d does the same except the input value	#
#	is a denormalized number. sLog2 and sLog2d are the base-2 	#
#	analogues.							#
#									#
# INPUT *************************************************************** #
#	Double-extended value in memory location pointed to by address	#
...
#									#
# OUTPUT **************************************************************	#
#	log_10(X) or log_2(X) returned in floating-point register fp0.	#
#									#
# ACCURACY and MONOTONICITY *******************************************	#
...
#									#
# SPEED ***************************************************************	#
#	Two timings are measured, both in the copy-back mode. The first	#
#	one is measured when the function is invoked the first time (so	#
#	the instructions and data are not in cache), and the second one	#
#	is measured when the function is reinvoked at the same input 	#
#	argument.							#
#									#
# ALGORITHM ***********************************************************	#
#									#
...
#									#
# OUTPUT **************************************************************	#
#	The function values are returned in Fp0.			#
#									#
# ACCURACY and MONOTONICITY *******************************************	#
#	The returned result is within 2 ulps in 64 significant bit, 	#
#	i.e. within 0.5001 ulp to 53 bits if the result is subsequently	#
...
#									#
# SPEED ***************************************************************	#
#	The program stwotox takes approximately 190 cycles and the	#
...
#--ENTRY POINT FOR 2**(X) FOR DENORMALIZED ARGUMENT
	...
	fmov.l		%d0,%control		# set user's rounding mode/precision
...
#########################################################################
# fmovcr.s 3.1 12/10/90							#
#									#
# DESCRIPTION *********************************************************	#
# 	_060fpsp_smovcr() returns the ROM constant at the offset	#
#	specified in d1 rounded to the mode and precision specified 	#
#	in d0. 								#
#									#
# INPUT	***************************************************************	#
# 	d0 = rnd prec,mode						#
...
# XDEF **************************************************************** #
#	_060fpsp_smovcr()						#
#	_060fpsp_PTENRN		table of power of ten values (RN)	#
#	_060fpsp_PTENRM		table of power of ten values (RM)	#
#	_060fpsp_PTENRP		table of power of ten values (RP)	#
#									#
# XREF **************************************************************** #
#	_060fpsp_round()						#
#									#
...
#									#
# XREF **************************************************************** #
#	t_ovfl_sc(), t_resdnrm(), t_unfl(), t_catch2()			#
#									#
# *********************************************************************	#
#		Copyright (C) Motorola, Inc. 1990			#
#			All Rights Reserved				#
...
#									#
# DESCRIPTION ********************************************************* #
#      	sMOD() computes the floating point MOD of the input values X,Y.	#
#	sREM() computes the floating point (IEEE) REM of the input 	#
#	values X,Y.							#
#									#
# INPUT *************************************************************** #
...
#	- t_dz sets N bit according to sign of src operand	#
#	- t_dz2 sets N bit always				#
#################################################################
	global		t_dz
t_dz:
...
# transcendental emulation. hopefully, this won't be happening
# much because it will be VERY slow.
# The only exceptions capable of passing through here are
# Overflow and Underflow.
# 	- the fsave frame holds the src operand,
# 	- fp0 holds the dst operand,
# 	- d0 holds the type of instruction that took and exception.
#
# Now, be VEWWY VEWWY QUIE-E-E-ET...
#
	global		fgen_except
fgen_except:
...
#########################################################################
# addsub_scaler(): scale inputs to add/sub such that no UNFL/OVFL 	#
#		   exceptions would result.				#
# INPUT:								#
#	FP_SRC : fp op1(src)						#
#	FP_DST : fp op2(dst)						#
# OUTPUT:								#
#	FP_SRC : fp op1 scaled(src)					#
#	FP_DST : fp op2 scaled(dst)					#
#	%d0     : scale amount						#
#########################################################################
...
quick_scale12:
	andi.w		&0x8000,FP_SCR0_EX(%a6)	# zero src mantissa
...
quick_scale22:
	andi.w		&0x8000,FP_SCR1_EX(%a6)	# zero dst mantissa
...
# - If either operand to an operation is a signalling NAN (SNAN), then 
# the SNAN bit is set in the FPSR EXC byte. If the SNAN trap enable
# bit is set in the floating-point control register (FPCR) ENABLE byte,
# then the trap is taken and the destination is not modified. If the
# SNAN trap enable bit is not set, then the SNAN is converted to a
# nonsignalling NAN (by setting the SNAN bit in the operand to one), and
# the operation continues as described in the preceding paragraph, for
# nonsignalling NANs. (p.1-38;040 Ref.)
#
	global		res_qnan
	global		res_snan
res_qnan:
...
#
# BUG FIX:
# need to set the sticky flag if the inex2 exception bit was set
# as a result of instruction emulation AND if the rounding mode
# is RN.
#
#	tst.b		3+L_SCR3(%a6)		# is rnd mode = RN?
#	bne.b		denorm_call		# no; don't change anything
...
#################################################
# _norm(): - normalize the input operand	#
#	   - operand should not be zero		#
#	   - output exponent is unaltered	#
# INPUT:					#
#	%a0 : fp operand to normalize		#
# OUTPUT:					#
#	%a0 : normalized fp operand		#
# 	%d0 : amt. shifted			#
# CHANGED:					#
#	%d0, %d1				#
# XREF:						#
...
#									#
# INPUT:								#
#	%a0 	: ptr to number to denormalize				#
#	%d0 	: scale factor						#
#	hi(%d1) : rnd prec						#
#	lo(%d1) : rnd mode						#
# OUTPUT:								#
# 	%d0.b	: ccode ('Z' bit may be set)				#
#########################################################################
	global		unf_res
unf_res:
...
#################################################################
# ovf_res2(): -overflow result calculation			#
#	      -the ccodes are returned instead of set here 	#
#	       since the calling routine may be fmove out which #
# 	       doesn't want them set at all.			#
# INPUT:							#
#	hi(%d0) : rnd prec					#
#	lo(%d0)	: rnd mode					#
#	%d1.b 	: '-1' => (-); '0' => (+)			#
...
#########################################################
# _load_fop():						#
#	- load the operands for the current operation	#
#							#
# OUTPUT:						#
#	FP_SRC : src					#
# 	FP_DST : dst					#
...
load_dbl:
	...
	bsr.l		_mem_read		# fetch src operand from memory
...
#############################
op011:
	bfextu		EXC_CMDREG(%a6){&6:&3}, %d0 # extract src field
	bsr.l		load_fpn1		# fetch src fpreg ptr

	bsr.l		set_tag_x		# get src type tag

	cmpi.b		%d0, &UNNORM		# is src fpreg an UNNORM?
	beq.b		op011_unnorm		# yes
...
#########################################################################
# store_fpreg(): store an fp value to the fpreg designated by the	#
#		 CMDREG dst field. 					#
#		 currently, the value may be DENORM or SNAN and 	#
#		 will not take an exception.				#
# INPUT:								#
#	%fp0 : fp value to store					#
#	%d0  : index of fpreg to store into				#
# MODIFIED:								#
#	%d0								#
#########################################################################
	global		store_fpreg
...
############################################################################
#
# "local" error. needed because pc rel tables can't reference functions
...
#								#
# INPUT:							#
#	a0 : ptr to src operand					#
#	d0 : rnd prec,mode					#
# OUTPUT:							#
#	fp0 : intermediate underflow or overflow result IF	#
#	      OVFL/UNFL occurred for a sgl or dbl operand	#
#	USER_FPSR : exc,accrued bits for fp move out		#
# CHANGED:							#
#	d0, d1, a0, fp0, fp1, L_SCR[1-3], FP_SCR[1,2]		#
# XREF:								#
#	_round(), _denorm(), ovf_res(), unf_res()		#
#################################################################
	global		fout
fout:
...
# the stack frame. so, leave it in FP_SRC for now and deal with it later...
	...
	bsr.l		_mem_write		# write ext prec number to memory
...
fout_dbl_exg:
	...
	bsr.l		_mem_write		# store dbl fop to memory
...
#########################################################################
# dst_dbl(): create double precision value from extended precision	#
#									#
# INPUT:								#
#	a0 : source in extended precision				#
# OUTPUT:								#
...
#########################################################################
# dst_sgl(): create single precision value from extended precision	#
#									#
# INPUT:								#
#	a0 : source in extended precision				#
# OUTPUT:								#
...
#################################
# fmul(): fp multiply routine  	#
#				#
# INPUT:			#
# 	a0 : ptr to src	operand	#
# 	a1 : ptr to dst operand #
#	d0 : rnd prec,mode	#
# OUTPUT:			#
#	fp0 : default result	#
#	fp1 : EXOP		#
...
#########################################
# fintrz(): fp force int (RZ) routine	#
#					#
# INPUT:				#
#	a0 : ptr to src operand		#
#	d0 : rnd prec,mode		#
...
#########################################
# fabs(): fp absolute value routine	#
#					#
# INPUT:				#
#	a0 : ptr to src	operand		#
# OUTPUT:				#
...
#########################################
# fsglmul(): fp sgl multiply routine  	#
#					#
# INPUT:				#
# 	a0 : ptr to src	operand		#
# 	a1 : ptr to dst	operand		#
...
tbl_fsglmul_op:
	...
	short		fsglmul_error		- tbl_fsglmul_op #
	short		fsglmul_error		- tbl_fsglmul_op #
...
tbl_fsgldiv_op:
	...
	short		fsgldiv_error		- tbl_fsgldiv_op #
	short		fsgldiv_error		- tbl_fsgldiv_op #
...
# inc_areg(): increment an address register by a given amount
#
# INPUT: d1 = index of areg
# 	 d0 = amt to increment by
	global		inc_areg
inc_areg:
...
# dec_areg(): decrement an address register by a given amount
#
# INPUT: d1 = index of areg
# 	 d0 = amt to decrement by
	global		dec_areg
dec_areg:
...
get_packed2:
	...
	mov.l		&0xc,%d0		# pass: 12 bytes
	bsr.l		_mem_read		# read packed operand
...
#									#
# DESCRIPTION *********************************************************	#
#	Converts normalized packed bcd value pointed to by register a0	#
#	to extended-precision value in FP0.				#
#									#
# INPUT ***************************************************************	#
#	Normalized packed bcd value in pointed to by a0.		#
#									#
# OUTPUT **************************************************************	#
...
#
#	Output:	result in the FP_SCR0 space on the stack.
#
...
#
	fmov.l		&rm_mode*0x10,%control	# set RM and ext
...
#	Input/Output
	...
	fmov.l		&rz_mode*0x10,%control	# set RZ rounding mode
...
A13_con:
	...
	mov.w		&1,%d5		# set ICTR
	fmov.l		&rm_mode*0x10,%control	# set rmode to RM
...
A14_st:
	fmov.l		&rz_mode*0x10,%control	# force rz for conversion
