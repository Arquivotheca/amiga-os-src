...
set EXC_ISR,		STOFF + 0x4		# stack status register
set EXC_IPC,		STOFF + 0x6		# stack pc
set EXC_IVOFF,		STOFF + 0xa		# stacked vector offset

set EXC_FP,		0x0			# frame pointer

set EXC_AREGS,		-32			# full
set EXC_DREGS,		-64			# full
...
#############################################################
# UIEH: Unimplemented Integer instruction Exception Handler #
#############################################################
	global		_isp_unimp
_isp_unimp:
...
addr_ind_p_a0:
	...
	mov.l		%d1,EXC_A0(%a6)		# Save incr value
	mov.l		%d0,%a0
	rts
...
addr_ind_p_a1:
	...
	mov.l		%d0,%d1
	add.l		%a0,%d1			# Increment
	mov.l		%d1,EXC_A1(%a6)		# Save incr value
	mov.l		%d0,%a0
	rts
...
addr_ind_p_a2:
	...
	mov.l		%d0,%d1
	add.l		%a0,%d1			# Increment
	mov.l		%d1,EXC_A2(%a6)		# Save incr value
	mov.l		%d0,%a0
	rts
...
addr_ind_p_a3:
	...
	mov.l		%d0,%d1
	add.l		%a0,%d1			# Increment
	mov.l		%d1,EXC_A3(%a6)		# Save incr value
	mov.l		%d0,%a0
	rts
...
addr_ind_p_a4:
	...
	mov.l		%d0,%d1
	add.l		%a0,%d1			# Increment
	mov.l		%d1,EXC_A4(%a6)		# Save incr value
	mov.l		%d0,%a0
	rts
...
addr_ind_p_a5:
	...
	mov.l		%d0,%d1
	add.l		%a0,%d1			# Increment
	mov.l		%d1,EXC_A5(%a6)		# Save incr value
	mov.l		%d0,%a0
	rts
...
addr_ind_p_a6:
	...
	mov.l		%d0,%d1
	add.l		%a0,%d1			# Increment
	mov.l		%d1,EXC_A6(%a6)		# Save incr value
	mov.l		%d0,%a0
	rts
...
aipa7_byte:
	add.w		&0x1,%a0		# stack post-inc byte does 2 bytes
	bra.b		aipa7_cont
...
# Immediate: #<data> #
#########################################################################
# word, long, extended: <ea> of the data is the current extension word	#
# 	pointer value. new extension word pointer is simply the old	#
# 	plus the number of bytes in the data type(2,4,or 12).		#
# byte: <ea> of the data is the current extension word pointer value	#
# 	plus one(immediate byte data is the lo byte of a word).		#
# 	the new extension word pointer is the old plus 2 for the word	#
# 	that contained the byte.					#
#########################################################################
immediate:
	cmp.w		%a0,&0x1		# is size == BYTE?
	beq.b		immediate_byte		# yes

immediate_other:
	mov.l		EXC_EXTWPTR(%a6),%d0	# fetch extension word ptr
	mov.l		%d0,%d1			# make a copy
	add.l		%a0,%d1			# new ptr = extwptr + size
	mov.l		%d1,EXC_EXTWPTR(%a6)	# save new extwptr
...
immediate_byte:
	mov.l		EXC_EXTWPTR(%a6),%d0	# fetch extension word ptr
	mov.l		%d0,%d1			# make a copy
	addq.l		&0x2,%d1		# new ptr = extwptr + 0x2
	mov.l		%d1,EXC_EXTWPTR(%a6)	# save new extwptr
	addq.l		&0x1,%d0		# <ea> is of byte within word
	mov.l		%d0,%a0			# return <ea> in a0
	rts
...
# fetch %ay. if it's all on the stack, then grab it in one instruction,
# otherwise, br to subroutine that will read it. This sub. may eventually
# be inlined. read_an_long() returns the result in %a0 and destroys %d0
	mov.l		(EXC_AREGS,%a6,%d0.w*4), %a0 # fetch %ay

	mov.w		EXC_EXTWORD(%a6), %d0	# fetch 16-bit disp
	add.w		%d0, %a0		# add: %an + sgn_ext(disp)

# separate movep to memory and movep from memory.
...
# fetch %dx. if it's all on the stack, then grab it in one instruction,
# otherwise, br to subroutine that will read it. This sub. may eventually
# be inlined. read_dn_long() returns the result in %d0 and destroys no
# other registers for now...
	mov.l		(EXC_DREGS,%a6,%d0.w*4), %d0 # fetch %dx

# separate word movep from long movep
	btst		&0x6, %d1		# word or long operation?
	beq		r2mwtrans

# this version of r2mltrans is for the case that it is desired to
# keep _mem_write() as the hook to reading/writing data.
# the check for user/supervisor space is done within _mem_write().
# %a0 = dst addr
# %d0 = Dx
r2mltrans:
	mov.l		%d0, EXC_TEMP(%a6)	# store %dn on stack

	mov.l		%a2, -(%sp)		# if (all_regs_not_saved)
	mov.l		%a0, %a2		# then save %a2

	mov.l		&1, %d0			# pass: count
	lea		(%a2), %a1		# pass: dst addr + 0x0
	lea		EXC_TEMP(%a6), %a0	# pass: src addr + 0x0
	bsr.l		_mem_write

	mov.l		&1, %d0			# pass: count
	lea		0x2(%a2), %a1		# pass: dst addr + 0x2
	lea		EXC_TEMP+1(%a6), %a0	# pass: src addr + 0x1
	bsr.l		_mem_write

	mov.l		&1, %d0			# pass: count
	lea		0x4(%a2), %a1		# pass: dst addr + 0x4
	lea		EXC_TEMP+2(%a6), %a0	# pass: src addr + 0x2
	bsr.l		_mem_write

	mov.l		&1, %d0			# pass: count
	lea		0x6(%a2), %a1		# pass: dst addr + 0x6
	lea		EXC_TEMP+3(%a6), %a0	# pass: src addr + 0x3
	bsr.l		_mem_write		# 
...
# this version of r2mwtrans is for the case that it is desired to
# keep _mem_write() as the hook to reading/writing data.
# the check for user/supervisor space is done within _mem_write().
# %a0 = dst addr
# %d0 = Dx
r2mwtrans:
	...
	mov.l		&1, %d0			# pass: count
	lea		(%a2), %a1		# pass: dst addr + 0x0
	lea		0x2+EXC_TEMP(%a6), %a0	# pass: src addr + 0x0
	bsr.l		_mem_write		# os  : write hi

	mov.l		&1, %d0			# pass: count
	lea		0x2(%a2), %a1		# pass: dst addr + 0x2
	lea		0x2+EXC_TEMP+1(%a6), %a0 # pass: src addr + 0x1
	bsr.l		_mem_write		# os  : write lo

	mov.l		(%sp)+, %a2		# restore %a2

	rts

# mem2reg: read bytes from memory(even the ones between the required bytes);
...
# this version of m2rltrans is for the case that it is desired to
# keep _mem_read() as the hook to reading/writing data.
# the check for user/supervisor space is done within _mem_read().
# %a0 = dst addr
m2rltrans:
	mov.l		&0x7, %d0		# pass: cnt (read all 7 bytes)
	lea		EXC_TEMP(%a6), %a1	# pass: dst addr
	bsr.l		_mem_read		# os  : read all

	lea		EXC_TEMP(%a6), %a0	# move bytes from stack to reg.

	mov.b		(%a0), %d0		# fetch hi-hi order byte
	lsl.l		&0x8, %d0
	mov.b		0x2(%a0), %d0		# fetch lo-hi order byte
	lsl.l		&0x8, %d0
	mov.b		0x4(%a0), %d0		# fetch hi-lo order byte
	lsl.l		&0x8, %d0
	mov.b		0x6(%a0), %d0		# fetch lo-lo order byte

	mov.b		EXC_OPWORD(%a6), %d1
	lsr.b		&0x1, %d1
	and.w		&0x7, %d1		# extract Dx from opcode word
	
# store %dx. if it's all on the stack, then store it in one instruction,
# otherwise, br to subroutine that will write it. This sub. may eventually
# be inlined. %d0 = dst value, %d1 = Dx; no regs are destroyed.
	mov.l		%d0, (EXC_DREGS,%a6,%d1.w*4) # store %dx	

	rts

# this version of m2rwtrans is for the case that it is desired to
# keep _mem_read() as the hook to reading/writing data.
# the check for user/supervisor space is done within _mem_read().
# %a0 = dst addr
m2rwtrans:
	mov.l		&0x3, %d0		# pass: cnt (read all 3 bytes)
	lea		EXC_TEMP(%a6), %a1	# pass: dst addr
	bsr.l		_mem_read		# os  : read all

	lea		EXC_TEMP(%a6), %a0	# move bytes from stack to reg.
	
	mov.b		(%a0), %d0		# fetch hi order byte
	lsl.l		&0x8, %d0
	mov.b		0x2(%a0), %d0		# fetch lo order byte
...
# otherwise, br to subroutine that will write it. This sub. may eventually
# be inlined. %d0 = dst value, %d1 = Dx; no regs are destroyed.
	mov.w		%d0, (EXC_DREGS+2,%a6,%d1.w*4) # store %dx
...
#########################################################################
# chk2_cmp2(): 								#
# In the interest of simplicity, all operands are converted to longword	#
# size whether the operation is byte, word, or long. The bnds are	#
# sign extended accordingly. If Rn is a data regsiter, Rn is also	#
# sign extended. If Rn is an address register, it need not be sign 	#
...
_div64:
	...
	bne.b		dcontrolmodel_s		# %dn dest or control mode?
...
_mul64:
	...
	bgt.b		mul64_memop		# src is in memory
...
mul64_regop:
	...
	bra.b		mul64_multiplicand	# now, fetch multiplicand
...
_cas_terminate2:
	...
	mov.l		&51,%d1			# want to move 51 longwords
...
# mask interrupt levels 0-6. save old mask value.
	mov.w		%sr,%d7			# save current SR
	ori.w		&0x0700,%d7		# inhibit interrupts
