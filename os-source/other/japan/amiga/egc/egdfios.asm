;************************************************************************/
;*                                                                      */
;* EGDFIOS.ASM : EGDrive file I/O routine    for small Model            */
;*                                                      MS-C            */
;*                   Version 3.41                                       */
;*                                                                      */
...
;   write or create    : ah = 3ch  al = 1

    mov     dx,word ptr [bp+@ab]    ;get fname pointer/segment
    mov     ax,word ptr [bp+@ab+2]  ;get mode
    cmp     ax,01h
    jz      bcreat
...
;        ds/dx : I/O buffer ptr
    ...
    mov     ax,word ptr [bp+@ab+2]
    mul     word ptr [bp+@ab+4]
    mov     cx,ax                   ;get I/O length    
    mov     bx,word ptr [bp+@ab+6]  ;get files handle
    mov     dx,word ptr [bp+@ab]    ;get I/O buffer ptr offset/seg
...
;        ds/dx : I/O 
    ...
    mul        word ptr [bp+@ab+4]    
    mov        cx,ax                ;get I/O length    
    mov        bx,ss:[bp+@ab+6]     ;get files handle
...
;        cx/dx : set I/O location
    mov     dx,word ptr [bp+@ab+2]    ;get I/O position lower value
    mov     cx,word ptr [bp+@ab+4]    ;get I/O position upper value
    mov     bx,word ptr [bp+@ab]      ;get files handle
    mov     ax,word ptr [bp+@ab+6]    ;get seek orign
...
_bsftell proc  near
    ...
    mov     bx,word ptr[bp+@ab] ;get file handle
